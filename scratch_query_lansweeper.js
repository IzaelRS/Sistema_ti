...
async function run() {
    try {
        ...
        const data = await fetchLansweeperCustomReport(env.LANSWEEPER_URL, cookies, 'web50get1IPlocation', '@iplocation=DR%20-%20Mibos');
        
        if (data && data.AddedRows && data.AddedRows.length > 0) {
            console.log('Columns count:', data.AddedRows[0].length);
            console.log('First row raw data:', data.AddedRows[0]);
            console.log('Second row raw data:', data.AddedRows[1]);
        } else {
            console.log('No rows returned or data structure is unexpected:', data);
        }
    } catch (err) {
