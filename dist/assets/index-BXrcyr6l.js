(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))o(a);new MutationObserver(a=>{for(const s of a)if(s.type==="childList")for(const i of s.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&o(i)}).observe(document,{childList:!0,subtree:!0});function n(a){const s={};return a.integrity&&(s.integrity=a.integrity),a.referrerPolicy&&(s.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?s.credentials="include":a.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function o(a){if(a.ep)return;a.ep=!0;const s=n(a);fetch(a.href,s)}})();const ve="/api",I={async get(t){const e=await fetch(`${ve}${t}`);if(!e.ok){const n=await e.json().catch(()=>({}));throw new Error(n.error||`HTTP error! status: ${e.status}`)}return await e.json()},async post(t,e){const n=await fetch(`${ve}${t}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)});if(!n.ok){const o=await n.json().catch(()=>({}));throw new Error(o.error||`HTTP error! status: ${n.status}`)}return await n.json()},async put(t,e){const n=await fetch(`${ve}${t}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)});if(!n.ok){const o=await n.json().catch(()=>({}));throw new Error(o.error||`HTTP error! status: ${n.status}`)}return await n.json()},async delete(t){const e=await fetch(`${ve}${t}`,{method:"DELETE"});if(!e.ok){const n=await e.json().catch(()=>({}));throw new Error(n.error||`HTTP error! status: ${e.status}`)}return await e.json()},async upload(t,e){const n=await fetch(`${ve}${t}`,{method:"POST",body:e});if(!n.ok){const o=await n.json().catch(()=>({}));throw new Error(o.error||`HTTP error! status: ${n.status}`)}return await n.json()}};let me=null;const z={init(){const t=localStorage.getItem("user");if(t)try{return me=JSON.parse(t),!0}catch{return this.logout(),!1}return!1},getUser(){return me},isAdmin(){return me&&me.role==="Administrador"},async login(t,e){try{const n=await I.post("/login",{email:t,password:e});return me=n,localStorage.setItem("user",JSON.stringify(n)),{success:!0,user:n}}catch(n){return{success:!1,error:n.message}}},logout(){me=null,localStorage.removeItem("user")}},l={show(t){const e=document.getElementById(t);e&&e.classList.remove("hidden")},hide(t){const e=document.getElementById(t);e&&e.classList.add("hidden")},toggle(t,e){const n=document.getElementById(t);n&&n.classList.toggle("hidden",e)},setText(t,e){const n=document.getElementById(t);n&&(n.innerText=e)},setValue(t,e){const n=document.getElementById(t);n&&(n.value=e)},getValue(t){const e=document.getElementById(t);return e?e.value:null},on(t,e,n){const o=document.getElementById(t);o&&o.addEventListener(e,n)}},lt={canvas:null,ctx:null,particles:[],animationFrameId:null,isActive:!1,init(){if(this.canvas=document.getElementById("account-network-bg"),!this.canvas)return;this.ctx=this.canvas.getContext("2d"),this.resize(),window.addEventListener("resize",()=>{this.isActive&&this.resize()});const t=window.innerWidth<=768;this.particleCount=t?30:60,this.connectDistance=150,this.particleColor="rgba(34, 211, 238, 0.5)",this.particles=[];for(let e=0;e<this.particleCount;e++)this.particles.push({x:Math.random()*this.canvas.width,y:Math.random()*this.canvas.height,vx:(Math.random()-.5)*1.5,vy:(Math.random()-.5)*1.5,radius:Math.random()*2+1})},resize(){if(!this.canvas)return;const t=document.getElementById("account-section");t&&(this.canvas.width=t.clientWidth,this.canvas.height=t.clientHeight)},updateAndDraw(){if(!(!this.isActive||!this.canvas)){this.ctx.clearRect(0,0,this.canvas.width,this.canvas.height);for(let t=0;t<this.particles.length;t++){const e=this.particles[t];e.x+=e.vx,e.y+=e.vy,(e.x<0||e.x>this.canvas.width)&&(e.vx*=-1),(e.y<0||e.y>this.canvas.height)&&(e.vy*=-1),this.ctx.beginPath(),this.ctx.arc(e.x,e.y,e.radius,0,Math.PI*2),this.ctx.fillStyle=this.particleColor,this.ctx.fill();for(let n=t+1;n<this.particles.length;n++){const o=this.particles[n],a=e.x-o.x,s=e.y-o.y,i=Math.sqrt(a*a+s*s);if(i<this.connectDistance){this.ctx.beginPath(),this.ctx.lineWidth=1;const r=1-i/this.connectDistance;this.ctx.strokeStyle=`rgba(34, 211, 238, ${r*.4})`,this.ctx.moveTo(e.x,e.y),this.ctx.lineTo(o.x,o.y),this.ctx.stroke()}}}this.animationFrameId=requestAnimationFrame(()=>this.updateAndDraw())}},start(){this.canvas||this.init(),this.isActive||(this.isActive=!0,this.resize(),this.updateAndDraw())},stop(){this.isActive=!1,this.animationFrameId&&(cancelAnimationFrame(this.animationFrameId),this.animationFrameId=null)}};let be=[];const Ze={async fetch(){try{be=await I.get("/users"),this.render(be)}catch(t){console.error("Error fetching Users:",t)}},getUsers(){return be},render(t){const e=document.getElementById("user-table-body");e&&(e.innerHTML=t.map(n=>{const o=n.role==="Administrador",a=z.isAdmin()?`
            ...
            `}).join("")},render(t){const e=document.getElementById("doc-list-body");if(!e)return;const n=document.getElementById("doc-list-thead"),o=ie.toLowerCase()==="contratos"||ie.toLowerCase()==="termo de uso",a=window.auth&&window.auth.isAdmin(),s=a?"":'class="role-hidden"';ct=t;const i=t.length,r=Math.ceil(i/we);F>r&&(F=Math.max(1,r)),F<1&&(F=1);const c=(F-1)*we,u=t.slice(c,c+we);if(n&&(o?n.innerHTML=`
                    <tr>
                        <th>Nome</th>
                        <th>Tamanho</th>
                        <th>Tipo</th>
            ...
            `,this.renderPaginationControls("doc-pagination",0,0);return}e.innerHTML=u.map(d=>{const p=d.mimetype==="application/pdf"?"📕":"🖼️",m=(d.size/1024).toFixed(1)+" KB",f=d.created_at?new Date(d.created_at).toLocaleDateString("pt-BR"):"-",g=d.mimetype==="application/pdf"?"PDF":"Imagem",h=a?`<button class="btn-delete" onclick="window.DocsHandler.delete(${d.id})" style="padding: 4px 10px; font-size: 0.85rem;">Deletar</button>`:"",v=d.start_date?new Date(d.start_date+"T00:00:00").toLocaleDateString("pt-BR"):"-",T=d.end_date?d.end_date==="Indefinido"?"Indefinido":new Date(d.end_date+"T00:00:00").toLocaleDateString("pt-BR"):"-";return o?`
                ...
                `}).join(""),this.renderPaginationControls("doc-pagination",r,i)},async handleUpload(t){t.preventDefault();const e=document.getElementById("doc-file"),n=document.getElementById("doc-category"),o=document.getElementById("doc-display-name");if(!e.files.length){alert("Selecione um arquivo.");return}const a=new FormData,s=n?n.value:"Geral";a.append("category",s),a.append("customName",o?o.value:""),a.append("document",e.files[0]);const i=s.toLowerCase();if(i==="contratos"||i==="termo de uso"){const r=document.getElementById("doc-start-date"),c=document.getElementById("doc-end-date"),u=document.getElementById("doc-indefinite");r&&r.value&&a.append("startDate",r.value),u&&u.checked?a.append("endDate","Indefinido"):c&&c.value&&a.append("endDate",c.value)}try{await I.upload("/documents",a),l.hide("modal-upload"),document.getElementById("doc-form").reset();const r=document.getElementById("doc-dates-container");r&&(r.style.display="none");const c=document.getElementById("doc-end-date");c&&(c.disabled=!1),l.setText("file-name-display","Respeite o formato .png ou .pdf"),this.fetch(),alert("Documento adicionado com sucesso!")}catch(r){console.error(r),alert("Erro ao subir arquivo.")}},async delete(t){if(confirm("Deletar este documento?"))try{await I.delete(`/documents/${t}`),this.fetch()}catch{alert("Erro ao excluir documento.")}},search(t){if(ie.toLowerCase()==="dashboard")this.renderDashboard();else{F=1;const e=Ae.filter(n=>(n.category||"Geral").toLowerCase()===ie.toLowerCase()&&n.original_name.toLowerCase().includes(t));this.render(e)}},changePage(t){F=t,this.render(ct)},renderPaginationControls(t,e,n){const o=document.getElementById(t);if(!o)return;if(e===0){o.innerHTML="";return}let a="";a+=`
        ...
        `,o.innerHTML=a}};let te=[],L={summaries:[]},dt=null,M=null,Ne="list",xe=null,Z=null,ut=null,R=1;const Ee=10;let He=[];const j={getPendingProcId(){return dt},async fetch(){try{R=1,te=await I.get("/procedures"),this.renderTable(te)}catch(t){console.error("Error fetching FAQs:",t)}},getFaqs(){return te},setListingMode(t){Ne=t,R=1,this.renderTable(He.length?He:te)},renderTable(t){const e=document.getElementById("list-table-container"),n=document.getElementById("list-cards-container"),o=document.getElementById("proc-table-body");if(!e||!n||!o)return;He=t;const a=t.length,s=Math.ceil(a/Ee);R>s&&(R=Math.max(1,s)),R<1&&(R=1);const i=(R-1)*Ee,r=t.slice(i,i+Ee);Ne==="list"?(l.show("list-table-container"),l.hide("list-cards-container"),r.length===0?o.innerHTML=`
                    <tr>
            ...
            `).join("");const o=L.summaries.find(a=>a.id===M);o?(n.textContent=o.title,n.style.color="var(--text-main)",e.classList.remove("hidden")):(n.textContent="Nenhum sumário selecionado",n.style.color="var(--accent)",e.classList.add("hidden"))},selectSummary(t){M=t,this.renderProcedureBuilderSidebar(),this.renderProcedureBuilder()},updateSummaryTitle(t,e){const n=L.summaries.find(a=>a.id===t);n&&(n.title=e||"Sem título"),this.renderProcedureBuilderSidebar();const o=L.summaries.find(a=>a.id===M);o&&(document.getElementById("current-summary-name").textContent=o.title)},addSummary(){const t="sum_"+Date.now();L.summaries.push({id:t,title:`Sumário ${L.summaries.length+1}`,sections:[]}),M=t,this.renderProcedureBuilderSidebar(),this.renderProcedureBuilder()},removeSummary(t){confirm("Excluir este sumário apagará todos os campos dentro dele. Deseja continuar?")&&(L.summaries=L.summaries.filter(e=>e.id!==t),M===t&&(M=L.summaries.length>0?L.summaries[0].id:null),this.renderProcedureBuilderSidebar(),this.renderProcedureBuilder())},renderProcedureBuilder(){const t=document.getElementById("procedure-edit-container");if(!t)return;if(!M){t.innerHTML='<p class="empty-state">Crie um novo sumário na barra lateral para adicionar conteúdo.</p>';return}const e=L.summaries.find(o=>o.id===M);if(!e)return;const n=e.sections;if(n.length===0){t.innerHTML=`<p class="empty-state">Nenhum campo em "${e.title}". Clique em "+ Novo Container" para começar.</p>`;return}t.innerHTML=n.map((o,a)=>`
             ...
             </div>`).join("")},handleSumDragStart(t,e){xe="summary",Z=e,t.dataTransfer.effectAllowed="move",setTimeout(()=>{t.target&&t.target.classList.add("dragging")},0)},handleSumDrop(t,e){if(t.preventDefault(),xe!=="summary"||Z===null||Z===e)return;const n=L.summaries.splice(Z,1)[0];L.summaries.splice(e,0,n),this.renderProcedureBuilderSidebar()},handleSecDragStart(t,e,n){xe="container",Z=e,ut=n,t.dataTransfer.effectAllowed="move",setTimeout(()=>{const o=t.target.nodeType===1?t.target.closest(".editable-section"):null;o&&o.classList.add("dragging")},0)},handleDragOver(t){t.preventDefault(),t.dataTransfer.dropEffect="move"},handleSecDrop(t,e,n){if(t.preventDefault(),xe!=="container"||Z===null||ut!==n)return;const o=L.summaries.find(s=>s.id===n);if(!o||Z===e)return;const a=o.sections.splice(Z,1)[0];o.sections.splice(e,0,a),this.renderProcedureBuilder()},handleDragEnd(t){document.querySelectorAll(".editable-section.dragging").forEach(e=>e.classList.remove("dragging")),t&&t.target&&t.target.setAttribute&&t.target.setAttribute("draggable","false"),xe=null,Z=null},updateSectionTitle(t,e){const n=L.summaries.find(o=>o.id===M);n&&(n.sections[t].title=e)},updateSectionData(t,e){const n=L.summaries.find(o=>o.id===M);n&&(n.sections[t].data=e)},removeSection(t){const e=L.summaries.find(n=>n.id===M);e&&e.sections.splice(t,1),this.renderProcedureBuilder()},getRteToolbarHTML(){return`
        ...
        `},addFaqItem(t){const e=L.summaries.find(n=>n.id===M);e&&(e.sections[t].data=e.sections[t].data||[],e.sections[t].data.push({q:"",a:""}),this.renderProcedureBuilder())},updateFaqItem(t,e,n,o){const a=L.summaries.find(s=>s.id===M);a&&(a.sections[t].data[e][n]=o)},removeFaqItem(t,e){const n=L.summaries.find(o=>o.id===M);n&&n.sections[t].data.splice(e,1),this.renderProcedureBuilder()},addSection(t,e){if(!M){alert("Selecione primeiro um sumário na barra lateral.");return}const n=L.summaries.find(o=>o.id===M);n&&(n.sections.push({id:Date.now(),title:t,type:e,data:e==="FAQ"?[]:e==="TEXTO"?"":null}),this.renderProcedureBuilder())},async handleSectionFileDrop(t,e){e.dataTransfer.files&&e.dataTransfer.files.length>0&&await this.uploadSectionFile(t,e.dataTransfer.files[0])},async handleSectionFileUpload(t,e){const n=e.files[0];n&&await this.uploadSectionFile(t,n)},async uploadSectionFile(t,e){const n=new FormData;n.append("file",e);try{const o=await I.upload("/upload",n),a=L.summaries.find(s=>s.id===M);a&&(a.sections[t].data={name:e.name,path:o.path,mimetype:e.type},this.renderProcedureBuilder())}catch{alert("Erro no upload")}},async handleSaveProcedure(){const t=parseInt(l.getValue("proc-id"));if(!t)return;const n={...te.find(o=>o.id===t),content:JSON.stringify(L)};try{await I.put(`/procedures/${t}`,n),alert("Salvo com sucesso!"),this.toggleEditMode(!1),this.openDetail(t),this.fetch()}catch{alert("Erro ao salvar")}},search(t){R=1;const e=te.filter(n=>(n.name||n.title||"").toLowerCase().includes(t)||(n.responsible||"").toLowerCase().includes(t)||(n.group_name||"").toLowerCase().includes(t));this.renderTable(e)},changePage(t){R=t,this.renderTable(He)},renderPaginationControls(t,e,n){const o=document.getElementById(t);if(!o)return;if(e===0){o.innerHTML="";return}let a="";a+=`
            <button class="pagination-btn" 
                    ${R===1?"disabled":""} 
        ...
        `,o.innerHTML=a}};let ce=[],Q={},oe=null,Ue=null,Ge=null,U=[],ze=[],et={},X={},ye,nt,qe,je,yt,Oe;const vt={init(){ye=document.getElementById("timeline-event-form"),nt=document.getElementById("view-visualizacao"),qe=document.getElementById("view-attention"),je=document.getElementById("view-anexo"),yt=document.getElementById("view-relatorio"),Oe=document.getElementById("view-config"),window.timelineHandler=vt,window.applyFilters=kt,window.clearFilters=Ct,window.toggleFilters=Dt,window.handleDelete=Bt,window.resetForm=it,window.toggleAccordion=wt,window.handleFormSubmit=gt,window.editEvent=at,window.deleteTopic=Pt,window.deleteSubtopic=Ft,window.handleTrackDragStart=Rt,window.handleTrackDragOver=Vt,window.handleTrackDragEnd=zt;const t=document.getElementById("timeline-topic-form");t&&t.addEventListener("submit",Ht);const e=document.getElementById("timeline-subtopic-form");e&&e.addEventListener("submit",_t);const n=document.getElementById("topico");n&&n.addEventListener("change",u=>{ot(u.target.value)});const o=document.getElementById("em-ocorrencia");o&&o.addEventListener("change",u=>{const d=document.getElementById("fim"),p=document.getElementById("inicio");if(u.target.checked){if(!p.value){const m=new Date;m.setMinutes(m.getMinutes()-m.getTimezoneOffset()),p.value=m.toISOString().slice(0,16)}d.required=!1}else{const m=new Date;m.setMinutes(m.getMinutes()-m.getTimezoneOffset()),d.value=m.toISOString().slice(0,16),d.required=!0}});const a=document.getElementById("auto-refresh-toggle");a&&a.addEventListener("change",u=>{bt(u.target.checked)}),document.querySelectorAll("[data-timeline-tab]").forEach(u=>{u.addEventListener("click",d=>{const p=d.currentTarget.getAttribute("data-timeline-tab");Me(p)})}),ye&&ye.addEventListener("submit",gt);const s=document.getElementById("rep-filter-start"),i=document.getElementById("rep-filter-end"),r=document.getElementById("rep-filter-topic"),c=document.getElementById("rep-filter-subtopic");s&&s.addEventListener("change",()=>ke()),i&&i.addEventListener("change",()=>ke()),r&&r.addEventListener("change",u=>{At(u.target.value),ke()}),c&&c.addEventListener("change",()=>ke()),window.addEventListener("SectionChange",u=>{u.detail&&u.detail.section==="timeline"&&de().then(()=>{ee(),pt()})}),de().then(()=>{ee(),pt()})}};window.addEventListener("focus",()=>{nt&&ee()});function ot(t,e=null){const n=document.getElementById("sub-topico");if(!n)return;const o=t?t.toLowerCase().trim():"";if(!o||!X[o]){n.innerHTML='<option value="">Selecione o tópico primeiro...</option>',n.classList.remove("has-options");return}n.innerHTML='<option value="" disabled selected>Escolha o evento...</option>',X[o].forEach(a=>{const s=document.createElement("option");s.value=a.toLowerCase(),s.textContent=a,e&&s.value===e.toLowerCase()&&(s.selected=!0),n.appendChild(s)}),e||(n.selectedIndex=1),n.classList.add("has-options")}async function de(){try{const t=await fetch("/api/timeline/config");if(!t.ok)throw new Error("Falha ao buscar configurações");const e=await t.json();U=e.topics||[],ze=e.subtopics||[],et={},X={},U.forEach(o=>{et[o.id]=o.color,X[o.id]=[]}),ze.forEach(o=>{const a=o.topic_id;X[a]&&X[a].push(o.name)}),It();const n=document.getElementById("view-config");n&&n.classList.contains("active")&&Et()}catch(t){console.error("Error loading config:",t)}}function It(){const t=document.getElementById("topico");if(t){const o=t.value;t.innerHTML='<option value="" disabled selected>Selecione um tópico...</option>',U.forEach(a=>{const s=document.createElement("option");s.value=a.id,s.textContent=a.name,t.appendChild(s)}),t.value=o}const e=document.getElementById("rep-filter-topic");if(e){const o=e.value;e.innerHTML='<option value="Todos">Todos</option>',U.forEach(a=>{const s=document.createElement("option");s.value=a.id,s.textContent=a.name,e.appendChild(s)}),o&&[...e.options].some(a=>a.value===o)?e.value=o:e.value="Todos"}const n=document.getElementById("subtopic-topic-id");n&&(n.innerHTML='<option value="" disabled selected>Selecione um tópico...</option>',U.forEach(o=>{const a=document.createElement("option");a.value=o.id,a.textContent=o.name,n.appendChild(a)}))}function ee(){fetch("/api/timeline/events").then(t=>{if(!t.ok)throw new Error("Failed to fetch");return t.json()}).then(t=>{ce=t,st(),qe&&qe.classList.contains("active")&&xt()}).catch(t=>{console.error("Error loading events:",t)})}function pt(){const t=document.getElementById("timeline-tab-anexo"),e=document.getElementById("timeline-tab-config");if(window.auth&&window.auth.isAdmin())t&&t.classList.remove("role-hidden"),e&&e.classList.remove("role-hidden");else{t&&t.classList.add("role-hidden"),e&&e.classList.add("role-hidden");const o=je&&je.classList.contains("active"),a=Oe&&Oe.classList.contains("active");(o||a)&&Me("visualizacao")}}function Me(t){const e={visualizacao:{section:nt,button:document.querySelector('[data-timeline-tab="visualizacao"]')},attention:{section:qe,button:document.querySelector('[data-timeline-tab="attention"]')},anexo:{section:je,button:document.querySelector('[data-timeline-tab="anexo"]')},relatorio:{section:yt,button:document.querySelector('[data-timeline-tab="relatorio"]')},config:{section:Oe,button:document.querySelector('[data-timeline-tab="config"]')}};Object.values(e).forEach(n=>{n.section&&n.section.classList.remove("active"),n.button&&n.button.classList.remove("active")}),e[t]&&(e[t].section&&e[t].section.classList.add("active"),e[t].button&&e[t].button.classList.add("active")),t==="visualizacao"?(ee(),Te(!0)):t==="attention"?(xt(),Te(!0)):t==="relatorio"?(ke(),Te(!1)):(t==="config"&&Et(),Te(!1))}function Te(t){const e=document.getElementById("floating-refresh-control");if(e)if(t){e.classList.remove("hidden");const n=document.getElementById("auto-refresh-toggle");n&&n.checked&&!oe&&bt(!0)}else e.classList.add("hidden"),oe&&(clearInterval(oe),oe=null)}function bt(t){oe&&(clearInterval(oe),oe=null),t&&(ee(),oe=setInterval(ee,6e4))}function gt(t){t.preventDefault();const e=ye.querySelector('button[type="submit"]');e&&(e.textContent="Salvando...",e.disabled=!0);const o={id:document.getElementById("event-id").value||Date.now().toString(),nome:document.getElementById("nome").value,topico:document.getElementById("topico").value,sub_topico:document.getElementById("sub-topico").value,em_ocorrencia:document.getElementById("em-ocorrencia").checked?1:0,inicio:document.getElementById("inicio").value,fim:document.getElementById("fim").value,descricao:document.getElementById("descricao").value,anotacao:document.getElementById("anotacao").value,cor:document.getElementById("cor").value};fetch("/api/timeline/events",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)}).then(async a=>{const s=await a.text();if(!a.ok)throw new Error(`Server error (${a.status}): ${s}`);return JSON.parse(s)}).then(()=>{alert("Evento salvo com sucesso!"),it(),Me("visualizacao")}).catch(a=>{console.error("Error saving event:",a),alert("Erro ao salvar evento: "+a.message)}).finally(()=>{e&&(e.textContent="Salvar Evento",e.disabled=!1)})}function at(t){const e=ce.find(s=>s.id===t);if(!e)return;document.getElementById("event-id").value=e.id,document.getElementById("nome").value=e.nome;const n=Ce(e.topico);document.getElementById("topico").value=n,ot(n,e.sub_topico);const o=document.getElementById("em-ocorrencia");o.checked=e.em_ocorrencia==1||e.em_ocorrencia==="true"||!e.fim,o.dispatchEvent(new Event("change")),document.getElementById("inicio").value=e.inicio,document.getElementById("fim").value=e.fim||"",document.getElementById("descricao").value=e.descricao||"",document.getElementById("anotacao").value=e.anotacao||"",document.getElementById("cor").value=e.cor||"#000000",Me("anexo");const a=document.getElementById("btn-delete");a&&(a.style.display="block")}function it(){ye&&ye.reset();const t=document.getElementById("event-id");t&&(t.value=""),ot("");const e=document.getElementById("fim");e&&(e.required=!0);const n=document.getElementById("cor");n&&(n.value="#000000");const o=document.getElementById("btn-delete");o&&(o.style.display="none")}function Bt(){const t=document.getElementById("event-id").value;t&&confirm("Tem certeza que deseja excluir este evento?")&&fetch(`/api/timeline/events/${t}`,{method:"DELETE"}).then(e=>{if(!e.ok)throw new Error("Failed to delete");return e.json()}).then(()=>{alert("Evento excluído!"),it(),Me("visualizacao")}).catch(e=>{console.error("Error deleting:",e),alert("Erro ao excluir: "+e.message)})}function kt(t){const e=document.getElementById(`filter-start-${t}`),n=document.getElementById(`filter-end-${t}`),o=document.getElementById(`filter-sub-topic-${t}`),a=e&&e.value?new Date(e.value).getTime():null,s=n&&n.value?new Date(n.value).getTime():null,i=o?o.value:"";Q[t]={start:a,end:s,subTopic:i},st()}function Ct(t){const e=document.getElementById(`filter-start-${t}`),n=document.getElementById(`filter-end-${t}`),o=document.getElementById(`filter-sub-topic-${t}`);e&&(e.value=""),n&&(n.value=""),o&&(o.value=""),Q[t]=null,st()}function Dt(t){const e=document.getElementById(`filters-panel-${t}`),n=document.getElementById(`btn-toggle-${t}`);e&&n&&(e.classList.toggle("hidden"),n.classList.toggle("active"))}function wt(t){const e=document.getElementById(t);e&&e.classList.toggle("active")}function st(){const t=document.getElementById("timeline-tracks-container");if(!t)return;const e=Array.from(t.querySelectorAll(".timeline-container")).map(a=>a.dataset.topicId),n=U.map(a=>a.id);if(e.length!==n.length||!n.every(a=>e.includes(a))){t.innerHTML="";const a=window.auth&&window.auth.isAdmin(),s=a?'style="cursor: grab;"':"";U.forEach(i=>{const r=`
            ...
            `;t.insertAdjacentHTML("beforeend",r);const c=document.getElementById(`filter-sub-topic-${i.id}`);c&&X[i.id]&&X[i.id].forEach(u=>{const d=document.createElement("option");d.value=u.toLowerCase(),d.textContent=u,c.appendChild(d)})})}U.forEach(a=>{const s=document.getElementById(`track-${a.id}`),i=document.getElementById(`min-date-${a.id}`),r=document.getElementById(`max-date-${a.id}`);s&&(s.innerHTML=""),i&&(i.textContent=""),r&&(r.textContent="")}),ce.length!==0&&U.forEach(a=>{const s=a.id,i=ce.filter(v=>Ce(v.topico)===s);let r=i;Q[s]&&Q[s].subTopic&&(r=i.filter(v=>(v.sub_topico?v.sub_topico.toLowerCase():"")===Q[s].subTopic.toLowerCase()));const c=Q[s]&&Q[s].start?Q[s].start:new Date("2026-01-01T00:00:00").getTime(),u=Q[s]&&Q[s].end?Q[s].end:Date.now();St(s,r,c,u);const d=c,p=u,m=p-d,f=document.getElementById(`min-date-${s}`),g=document.getElementById(`max-date-${s}`);f&&(f.textContent=new Date(d).toLocaleDateString()+" "+new Date(d).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})),g&&(g.textContent=new Date(p).toLocaleDateString()+" "+new Date(p).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}));const h=document.getElementById(`track-${s}`);h&&r.forEach(v=>{const T=new Date(v.inicio).getTime(),E=v.fim?new Date(v.fim).getTime():Date.now();if(E<d||T>p)return;const $=Math.max(T,d),y=Math.min(E,p),x=($-d)/m*100,b=(y-$)/m*100;if(b<=0)return;const C=document.createElement("div");C.className="timeline-bar",C.style.left=`${x}%`,C.style.width=`${b}%`,C.style.color=v.cor&&v.cor!=="#000000"?v.cor:et[s]||"#6b7280";const w=document.createElement("div");w.className="timeline-bar-visual",C.appendChild(w);const D=document.createElement("div");D.className="timeline-identifier-point";const H=new Date(v.inicio).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),N=v.fim?new Date(v.fim).toLocaleString([],{dateStyle:"short",timeStyle:"short"}):"Em andamento",Y=a.name,A=v.sub_topico?v.sub_topico.charAt(0).toUpperCase()+v.sub_topico.slice(1):"-";D.setAttribute("data-tooltip",`Tópico: ${Y}
Eventos: ${A}
Início: ${H} - Fim: ${N}
Descrição: ${v.descricao||"-"}`),!v.fim&&D.classList.add("pulsing"),window.auth&&window.auth.isAdmin()?(D.style.cursor="pointer",D.onclick=ue=>{ue.stopPropagation(),at(v.id)}):D.style.cursor="default",C.appendChild(D),h.appendChild(C)})})}function Ce(t){return t?t.toLowerCase().trim():""}function St(t,e,n,o){const a=document.getElementById(`sla-${t}`);if(!a)return;const s=o-n;if(s<=0){a.textContent="N/A";return}const r=e.filter(m=>{const f=new Date(m.inicio).getTime();return(m.fim?new Date(m.fim).getTime():Date.now())>n&&f<o}).map(m=>({start:Math.max(new Date(m.inicio).getTime(),n),end:Math.min(m.fim?new Date(m.fim).getTime():Date.now(),o)}));r.sort((m,f)=>m.start-f.start);const c=[];if(r.length>0){let m=r[0];for(let f=1;f<r.length;f++){const g=r[f];g.start<m.end?m.end=Math.max(m.end,g.end):(c.push(m),m=g)}c.push(m)}let u=0;c.forEach(m=>{u+=m.end-m.start});const d=(s-u)/s*100;let p="#10b981";d<50?p="#ef4444":d<90&&(p="#f97316"),a.style.color=p,a.textContent=d.toFixed(4)+"%"}function xt(){const t=document.getElementById("attention-topics-container");if(!t)return;t.innerHTML="";const e=ce.filter(n=>!n.fim);U.forEach(n=>{const o=n.id,a=e.filter(h=>Ce(h.topico)===o),s=document.createElement("div");s.className=a.length>0?"accordion-item active":"accordion-item",s.id=`attn-acc-${o}`;const i=document.createElement("div");i.className="accordion-header",i.onclick=()=>wt(`attn-acc-${o}`);const r=document.createElement("div");r.className="accordion-title-group";const c=document.createElement("div");c.className="topic-indicator",c.style.backgroundColor=n.color;const u=document.createElement("h3");u.textContent=n.name;const d=document.createElement("span");d.style.cssText="background: #f1f5f9; padding: 2px 8px; border-radius: 12px; font-size: 0.95rem; font-weight: 900; color: #0f172a; margin-left: 0.5rem; border: 1px solid #cbd5e1;",d.textContent=`${a.length}`,r.appendChild(c),r.appendChild(u),r.appendChild(d);const p=document.createElement("span");p.className="accordion-chevron",p.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>',i.appendChild(r),i.appendChild(p);const m=document.createElement("div");m.className="accordion-content";const f=document.createElement("div");f.className="accordion-body";const g=document.createElement("div");if(g.className="attention-carousel",a.length===0){const h=document.createElement("div");h.className="empty-state",h.textContent="Nenhum evento em andamento.",g.appendChild(h)}else a.forEach(h=>{const v=document.createElement("div");v.className="attention-card",v.style.borderLeftColor=h.cor&&h.cor!=="#000000"?h.cor:n.color;const T=document.createElement("h3");T.textContent=h.nome;const E=document.createElement("div");E.className="sub-topic",E.textContent=h.sub_topico||"-";const $=document.createElement("div");$.className="card-detail",$.innerHTML=`<strong>Início:</strong> ${new Date(h.inicio).toLocaleString()}`;const y=Date.now()-new Date(h.inicio).getTime(),x=document.createElement("div");x.className="card-duration",x.innerHTML=`<strong>Tempo:</strong> <span>${Mt(y)}</span>`;const b=document.createElement("div");b.className="card-description",b.textContent=h.descricao||"-",v.appendChild(T),v.appendChild(E),v.appendChild($),v.appendChild(x),v.appendChild(b),window.auth&&window.auth.isAdmin()?(v.style.cursor="pointer",v.onclick=()=>at(h.id)):v.style.cursor="default",g.appendChild(v)});f.appendChild(g),m.appendChild(f),s.appendChild(i),s.appendChild(m),t.appendChild(s)})}function Mt(t){if(t<0)return"0s";const e=Math.floor(t/1e3),n=Math.floor(e/60),o=Math.floor(n/60),a=Math.floor(o/24),s=[];return a>0&&s.push(`${a}d`),(o%24>0||a>0)&&s.push(`${o%24}h`),(n%60>0||o>0)&&s.push(`${n%60}m`),s.push(`${e%60}s`),s.join(" ")}function At(t){const e=document.getElementById("rep-filter-subtopic");if(!e)return;e.innerHTML='<option value="Todos">Todos</option>';const n=t?t.toLowerCase().trim():"";n&&X[n]&&X[n].forEach(o=>{const a=document.createElement("option");a.value=o.toLowerCase(),a.textContent=o,e.appendChild(a)})}function ke(){let t=ce;const e=document.getElementById("rep-filter-start")?.value,n=document.getElementById("rep-filter-end")?.value,o=document.getElementById("rep-filter-topic")?.value,a=document.getElementById("rep-filter-subtopic")?.value;if(e){const b=new Date(e+"T00:00:00").getTime();t=t.filter(C=>new Date(C.inicio).getTime()>=b)}if(n){const b=new Date(n+"T23:59:59").getTime();t=t.filter(C=>new Date(C.inicio).getTime()<=b)}o&&o!=="Todos"&&(t=t.filter(b=>Ce(b.topico)===o.toLowerCase())),a&&a!=="Todos"&&(t=t.filter(b=>b.sub_topico&&b.sub_topico.toLowerCase()===a.toLowerCase()));const s=document.getElementById("rep-kpi-total"),i=document.getElementById("rep-kpi-active"),r=document.getElementById("rep-kpi-avg-time");s&&(s.textContent=t.length);const c=t.filter(b=>b.em_ocorrencia==1||b.em_ocorrencia==="true"||!b.fim);i&&(i.textContent=c.length);const u=t.filter(b=>b.fim);let d="0h 0m";if(u.length>0){const C=u.reduce((N,Y)=>N+(new Date(Y.fim).getTime()-new Date(Y.inicio).getTime()),0)/u.length,w=Math.floor(C/6e4),D=Math.floor(w/60),H=w%60;d=`${D}h ${H}m`}if(r&&(r.textContent=d),!window.Chart){console.warn("Chart.js is not loaded.");return}const p=U,m=e?new Date(e+"T00:00:00").getTime():new Date(new Date().getFullYear()+"-01-01T00:00:00").getTime(),f=n?new Date(n+"T23:59:59").getTime():Date.now(),g=p.map(b=>b.name),h=p.map(b=>{const C=b.id,w=ce.filter(_=>Ce(_.topico)===C),D=f-m;if(D<=0)return 100;const N=w.filter(_=>{const K=new Date(_.inicio).getTime();return(_.fim?new Date(_.fim).getTime():Date.now())>m&&K<f}).map(_=>({start:Math.max(new Date(_.inicio).getTime(),m),end:Math.min(_.fim?new Date(_.fim).getTime():Date.now(),f)}));N.sort((_,K)=>_.start-K.start);const Y=[];if(N.length>0){let _=N[0];for(let K=1;K<N.length;K++){const ae=N[K];ae.start<_.end?_.end=Math.max(_.end,ae.end):(Y.push(_),_=ae)}Y.push(_)}const J=(_=>{let K=0;return _.forEach(ae=>{K+=ae.end-ae.start}),K})(Y),ue=(D-J)/D*100;return parseFloat(ue.toFixed(4))}),v=p.map(b=>b.color||"#6b7280"),T=document.getElementById("chart-rep-sla");T&&(Ue&&Ue.destroy(),Ue=new window.Chart(T,{type:"bar",data:{labels:g,datasets:[{label:"Disponibilidade %",data:h,backgroundColor:v,borderRadius:6}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{y:{min:Math.max(0,Math.min(...h)-5),max:100,ticks:{callback:b=>b+"%"}}}}}));const E={};t.forEach(b=>{const C=b.sub_topico?b.sub_topico.charAt(0).toUpperCase()+b.sub_topico.slice(1).toLowerCase():"Não especificado";E[C]=(E[C]||0)+1});const $=Object.keys(E),y=Object.values(E),x=document.getElementById("chart-rep-qty");x&&(Ge&&Ge.destroy(),Ge=new window.Chart(x,{type:"doughnut",data:{labels:$.length>0?$:["Nenhum evento"],datasets:[{data:y.length>0?y:[0],backgroundColor:["#3b82f6","#10b981","#f59e0b","#8b5cf6","#ec4899","#6366f1","#14b8a6","#f43f5e","#a855f7","#06b6d4"]}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{position:"right",labels:{boxWidth:12}}}}}))}function Ht(t){t.preventDefault();const e=document.getElementById("topic-id"),n=document.getElementById("topic-name"),o=document.getElementById("topic-color");if(!e||!n||!o)return;const a={id:e.value.trim().toLowerCase(),name:n.value.trim(),color:o.value};if(!a.id){alert("Por favor, defina um ID para o tópico.");return}fetch("/api/timeline/config/topics",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)}).then(s=>{if(!s.ok)throw new Error("Erro ao salvar tópico");return s.json()}).then(()=>{alert("Tópico salvo com sucesso!"),e.value="",n.value="",o.value="#3b82f6",de().then(()=>{ee()})}).catch(s=>{console.error(s),alert("Erro: "+s.message)})}function _t(t){t.preventDefault();const e=document.getElementById("subtopic-topic-id"),n=document.getElementById("subtopic-name");if(!e||!n)return;const o={topic_id:e.value,name:n.value.trim()};if(!o.topic_id||!o.name){alert("Preencha todos os campos do evento.");return}fetch("/api/timeline/config/subtopics",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)}).then(a=>{if(!a.ok)throw new Error("Erro ao adicionar evento");return a.json()}).then(()=>{alert("Evento adicionado!"),n.value="",de()}).catch(a=>{console.error(a),alert("Erro: "+a.message)})}function Pt(t){confirm("Excluir este tópico também removerá todos os seus eventos associados. Deseja continuar?")&&fetch(`/api/timeline/config/topics/${t}`,{method:"DELETE"}).then(e=>{if(!e.ok)throw new Error("Erro ao excluir tópico");return e.json()}).then(()=>{alert("Tópico excluído!"),de().then(()=>{ee()})}).catch(e=>{console.error(e),alert("Erro: "+e.message)})}function Ft(t){confirm("Deseja realmente excluir este evento?")&&fetch(`/api/timeline/config/subtopics/${t}`,{method:"DELETE"}).then(e=>{if(!e.ok)throw new Error("Erro ao excluir evento");return e.json()}).then(()=>{alert("Evento excluído!"),de()}).catch(e=>{console.error(e),alert("Erro: "+e.message)})}function Et(){const t=document.getElementById("config-topics-list");t&&(t.innerHTML="",U.length===0?t.innerHTML='<div style="color: var(--text-muted); font-size: 0.9rem;">Nenhum tópico cadastrado.</div>':U.forEach(n=>{const o=document.createElement("div");o.style.cssText="display: flex; align-items: center; justify-content: space-between; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 8px; border: 1px solid var(--glass-border); margin-bottom: 6px;",o.innerHTML=`
                ...
                `,t.appendChild(o)}));const e=document.getElementById("config-subtopics-list");e&&(e.innerHTML="",ze.length===0?e.innerHTML='<div style="color: var(--text-muted); font-size: 0.9rem;">Nenhum evento cadastrado.</div>':ze.forEach(n=>{const o=U.find(r=>r.id===n.topic_id),a=o?o.name:n.topic_id,s=o?o.color:"#6b7280",i=document.createElement("div");i.style.cssText="display: flex; align-items: center; justify-content: space-between; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 8px; border: 1px solid var(--glass-border); margin-bottom: 6px;",i.innerHTML=`
                    <div>
                ...
                `,e.appendChild(i)}))}function Rt(t,e){t.currentTarget.classList.add("dragging"),t.dataTransfer.effectAllowed="move"}function Vt(t){t.preventDefault();const e=document.querySelector(".timeline-container.dragging");if(!e)return;const n=document.getElementById("timeline-tracks-container");if(!n)return;const a=[...n.querySelectorAll(".timeline-container:not(.dragging)")].find(s=>{const i=s.getBoundingClientRect();return t.clientY<=i.top+i.height/2});a?n.insertBefore(e,a):n.appendChild(e)}function zt(t){const e=document.querySelector(".timeline-container.dragging");e&&e.classList.remove("dragging"),document.querySelectorAll(".timeline-container").forEach(a=>{a.setAttribute("draggable","false")});const n=document.getElementById("timeline-tracks-container");if(!n)return;const o=Array.from(n.querySelectorAll(".timeline-container")).map(a=>a.dataset.topicId);qt(o)}function qt(t){fetch("/api/timeline/config/topics/reorder",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({order:t})}).then(e=>{if(!e.ok)throw new Error("Erro ao salvar nova ordenação");return e.json()}).then(()=>{console.log("Ordem dos tópicos atualizada com sucesso."),de().then(()=>{ee()})}).catch(e=>{console.error(e),alert("Erro ao salvar ordenação: "+e.message)})}let ge=[],Ye=[],Je=[],We=[],S="extensions",P=1,fe=100,Qe=[];const Le={setActiveTab(t){S=t,P=1;const e=document.getElementById("telephony-search");e&&(e.value="",t==="extensions"?e.placeholder="Pesquisar ramais por número, nome ou usuário...":t==="queues"?e.placeholder="Pesquisar filas por número ou nome...":t==="blf"?e.placeholder="Pesquisar BLF por nome...":t==="users"&&(e.placeholder="Pesquisar usuários por nome ou perfil...")),document.querySelectorAll(".telephony-tabs-nav .acc-tab-btn").forEach(s=>{s.id===`tab-telephony-${t}`?s.classList.add("active"):s.classList.remove("active")}),document.querySelectorAll(".telephony-tab-content").forEach(s=>{s.id===`telephony-view-${t==="users"?"users":t==="queues"?"queues":t}`?s.classList.remove("hidden"):s.classList.add("hidden")});const a=this.getActiveDataList();this.render(a)},getActiveDataList(){return S==="extensions"?ge:S==="queues"?Ye:S==="blf"?Je:S==="users"?We:[]},async fetch(){const t=this.getActiveTableBody();t&&(t.innerHTML='<tr><td colspan="10" style="text-align: center; padding: 2rem; color: var(--text-muted);">Carregando dados...</td></tr>');try{if(P=1,S==="extensions")ge=await I.get("/telephony/extensions"),this.render(ge);else if(S==="queues")Ye=await I.get("/telephony/queues"),this.render(Ye);else if(S==="blf"){if(ge.length===0)try{ge=await I.get("/telephony/extensions")}catch(e){console.warn("Could not pre-fetch extensions for BLF mapping:",e)}Je=await I.get("/telephony/blfs"),this.render(Je)}else S==="users"&&(We=await I.get("/telephony/users"),this.render(We))}catch(e){console.error(`Error fetching telephony ${S}:`,e),t&&(t.innerHTML=`<tr><td colspan="10" style="text-align: center; padding: 2rem; color: #ef4444;">Erro ao carregar dados: ${e.message||"Erro de rede"}</td></tr>`)}},getActiveTableBody(){return S==="extensions"?document.getElementById("telephony-table-body"):S==="queues"?document.getElementById("telephony-queues-table-body"):S==="blf"?document.getElementById("telephony-blf-table-body"):S==="users"?document.getElementById("telephony-users-table-body"):null},render(t){const e=this.getActiveTableBody();if(!e)return;Qe=t;const n=t.length,o=Math.ceil(n/fe);P>o&&(P=Math.max(1,o)),P<1&&(P=1);const a=(P-1)*fe,s=t.slice(a,a+fe);if(s.length===0){const i=S==="extensions"?7:S==="queues"?6:S==="blf"?4:5;e.innerHTML=`
                <tr>
        ...
        `,$=0;h.forEach(y=>{const x=y.status==="active"||y.status_label==="ativo",b=x?"#10b981":"#ef4444",C=x?"Online":y.status_label||y.status||"Offline",w=x?"rgba(16,185,129,0.12)":"rgba(239,68,68,0.12)",D=x?"#6ee7b7":"#fca5a5",H=x?"rgba(16,185,129,0.3)":"rgba(239,68,68,0.3)",N=$%2===0?"transparent":"rgba(255,255,255,0.015)";$++,E+=`
            ...
            </div>`;try{let e=await I.get("/monitoring/events?limit=1000");if(Be&&(e=e.filter(i=>(i.title||"").toLowerCase().includes(Be)||(i.description||"").toLowerCase().includes(Be)||(i.source||"").toLowerCase().includes(Be))),Ke!=="all"&&(e=e.filter(i=>i.severity===Ke)),_e){const i=new Date(_e+"T00:00:00").getTime();e=e.filter(r=>r.created_at?new Date(r.created_at).getTime()>=i:!1)}if(Pe){const i=new Date(Pe+"T23:59:59").getTime();e=e.filter(r=>r.created_at?new Date(r.created_at).getTime()<=i:!1)}he=e.length;const n=Math.max(1,Math.ceil(he/Ie));G>n&&(G=n);const o=document.getElementById("event-history-count");o&&(o.textContent=he>0?he:"",o.style.display=he>0?"inline-flex":"none");const a=(G-1)*Ie,s=e.slice(a,a+Ie);this.renderEvents(s),this.renderPagination(he,n)}catch(e){console.error("Erro ao buscar histórico de eventos:",e),t.innerHTML=`
                <div style="text-align: center; padding: 3rem; color: var(--text-muted);">
...
`);for(let a=1;a<o.length;a++){const s=o[a].trim().split(/\s+/);s.length>=6&&n.push({mountpoint:s[5],total:s[1],used:s[2],free:s[3],percent:parseInt(s[4].replace("%",""),10)||0})}}catch(o){console.warn("Erro ao fazer parse da tabela de disco offline:",o)}else Array.isArray(k.disco)&&(n=k.disco.map(o=>({mountpoint:o.mountpoint,total:typeof o.total_gb=="number"?`${o.total_gb.toFixed(2)} GB`:o.total_gb||"0 GB",used:typeof o.used_gb=="number"?`${o.used_gb.toFixed(2)} GB`:o.used_gb||"0 GB",free:typeof o.free_gb=="number"?`${o.free_gb.toFixed(2)} GB`:o.free_gb||"0 GB",percent:typeof o.percent=="number"?Math.round(o.percent):parseInt(o.percent,10)||0})));n.length>0?t.innerHTML=n.map(o=>{const a=o.percent;return`
        ...
        `;const e=document.getElementById("btn-refresh-apis-status");let n=null;e&&(n=e.querySelector("svg"),e.disabled=!0,e.style.opacity="0.6",e.style.cursor="not-allowed",n&&(n.style.animation="spin 0.8s linear infinite"));try{const o=await I.get("/monitoring/apis-status?refresh=true&t="+Date.now());if(o&&o.success&&Array.isArray(o.apis))Xe=o.apis,W==="alerts"&&this.renderGnewServicesStatus(),this.renderApisGrid(o.apis);else throw new Error("Resposta inválida do servidor.")}catch(o){console.error("Erro ao buscar status das APIs:",o),t.innerHTML=`
            ...
            `}).join("")}},async fetchAndRenderSwitchesStatus(t=!1,e=!1){const n=document.getElementById("switches-auto-refresh"),o=e||n&&n.checked,a=document.getElementById("monitoring-switches-tbody");if(!a)return;const s=document.getElementById("btn-refresh-switches-status");let i=null;s&&(i=s.querySelector("svg"),s.disabled=!0,s.style.opacity="0.6",s.style.cursor="not-allowed",i&&(i.style.animation="spin 0.8s linear infinite"));try{if(o){const r=await I.get(`/monitoring/switches?ping=false&refresh=${t}&t=${Date.now()}`);if(r&&r.success&&Array.isArray(r.switches)){this.renderSwitchesTable(r.switches),r.switches.forEach(c=>{const u=document.getElementById(`switch-row-${c.id}`);if(u){const d=u.querySelector(".switch-sync-indicator");d&&(d.innerHTML=`
                                ...
                                `)}}),ht=!0;for(const c of r.switches){if(W!=="infra")break;const u=document.getElementById(`switch-row-${c.id}`);if(u){const d=u.querySelector(".switch-sync-indicator");d&&(d.innerHTML=`
