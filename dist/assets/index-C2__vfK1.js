(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))o(a);new MutationObserver(a=>{for(const s of a)if(s.type==="childList")for(const i of s.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&o(i)}).observe(document,{childList:!0,subtree:!0});function n(a){const s={};return a.integrity&&(s.integrity=a.integrity),a.referrerPolicy&&(s.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?s.credentials="include":a.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function o(a){if(a.ep)return;a.ep=!0;const s=n(a);fetch(a.href,s)}})();const fe="/api",$={async get(e){const t=await fetch(`${fe}${e}`);if(!t.ok){const n=await t.json().catch(()=>({}));throw new Error(n.error||`HTTP error! status: ${t.status}`)}return await t.json()},async post(e,t){const n=await fetch(`${fe}${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});if(!n.ok){const o=await n.json().catch(()=>({}));throw new Error(o.error||`HTTP error! status: ${n.status}`)}return await n.json()},async put(e,t){const n=await fetch(`${fe}${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});if(!n.ok){const o=await n.json().catch(()=>({}));throw new Error(o.error||`HTTP error! status: ${n.status}`)}return await n.json()},async delete(e){const t=await fetch(`${fe}${e}`,{method:"DELETE"});if(!t.ok){const n=await t.json().catch(()=>({}));throw new Error(n.error||`HTTP error! status: ${t.status}`)}return await t.json()},async upload(e,t){const n=await fetch(`${fe}${e}`,{method:"POST",body:t});if(!n.ok){const o=await n.json().catch(()=>({}));throw new Error(o.error||`HTTP error! status: ${n.status}`)}return await n.json()}};let de=null;const F={init(){const e=localStorage.getItem("user");if(e)try{return de=JSON.parse(e),!0}catch{return this.logout(),!1}return!1},getUser(){return de},isAdmin(){return de&&de.role==="Administrador"},async login(e,t){try{const n=await $.post("/login",{email:e,password:t});return de=n,localStorage.setItem("user",JSON.stringify(n)),{success:!0,user:n}}catch(n){return{success:!1,error:n.message}}},logout(){de=null,localStorage.removeItem("user")}},l={show(e){const t=document.getElementById(e);t&&t.classList.remove("hidden")},hide(e){const t=document.getElementById(e);t&&t.classList.add("hidden")},toggle(e,t){const n=document.getElementById(e);n&&n.classList.toggle("hidden",t)},setText(e,t){const n=document.getElementById(e);n&&(n.innerText=t)},setValue(e,t){const n=document.getElementById(e);n&&(n.value=t)},getValue(e){const t=document.getElementById(e);return t?t.value:null},on(e,t,n){const o=document.getElementById(e);o&&o.addEventListener(t,n)}},et={canvas:null,ctx:null,particles:[],animationFrameId:null,isActive:!1,init(){if(this.canvas=document.getElementById("account-network-bg"),!this.canvas)return;this.ctx=this.canvas.getContext("2d"),this.resize(),window.addEventListener("resize",()=>{this.isActive&&this.resize()});const e=window.innerWidth<=768;this.particleCount=e?30:60,this.connectDistance=150,this.particleColor="rgba(34, 211, 238, 0.5)",this.particles=[];for(let t=0;t<this.particleCount;t++)this.particles.push({x:Math.random()*this.canvas.width,y:Math.random()*this.canvas.height,vx:(Math.random()-.5)*1.5,vy:(Math.random()-.5)*1.5,radius:Math.random()*2+1})},resize(){if(!this.canvas)return;const e=document.getElementById("account-section");e&&(this.canvas.width=e.clientWidth,this.canvas.height=e.clientHeight)},updateAndDraw(){if(!(!this.isActive||!this.canvas)){this.ctx.clearRect(0,0,this.canvas.width,this.canvas.height);for(let e=0;e<this.particles.length;e++){const t=this.particles[e];t.x+=t.vx,t.y+=t.vy,(t.x<0||t.x>this.canvas.width)&&(t.vx*=-1),(t.y<0||t.y>this.canvas.height)&&(t.vy*=-1),this.ctx.beginPath(),this.ctx.arc(t.x,t.y,t.radius,0,Math.PI*2),this.ctx.fillStyle=this.particleColor,this.ctx.fill();for(let n=e+1;n<this.particles.length;n++){const o=this.particles[n],a=t.x-o.x,s=t.y-o.y,i=Math.sqrt(a*a+s*s);if(i<this.connectDistance){this.ctx.beginPath(),this.ctx.lineWidth=1;const r=1-i/this.connectDistance;this.ctx.strokeStyle=`rgba(34, 211, 238, ${r*.4})`,this.ctx.moveTo(t.x,t.y),this.ctx.lineTo(o.x,o.y),this.ctx.stroke()}}}this.animationFrameId=requestAnimationFrame(()=>this.updateAndDraw())}},start(){this.canvas||this.init(),this.isActive||(this.isActive=!0,this.resize(),this.updateAndDraw())},stop(){this.isActive=!1,this.animationFrameId&&(cancelAnimationFrame(this.animationFrameId),this.animationFrameId=null)}};let he=[];const Ue={async fetch(){try{he=await $.get("/users"),this.render(he)}catch(e){console.error("Error fetching Users:",e)}},getUsers(){return he},render(e){const t=document.getElementById("user-table-body");t&&(t.innerHTML=e.map(n=>{const o=n.role==="Administrador",a=F.isAdmin()?`
            ...
            `}).join("")},render(e){const t=document.getElementById("doc-list-body");if(!t)return;const n=document.getElementById("doc-list-thead"),o=oe.toLowerCase()==="contratos"||oe.toLowerCase()==="termo de uso",a=window.auth&&window.auth.isAdmin(),s=a?"":'class="role-hidden"';tt=e;const i=e.length,r=Math.ceil(i/ye);H>r&&(H=Math.max(1,r)),H<1&&(H=1);const c=(H-1)*ye,u=e.slice(c,c+ye);if(n&&(o?n.innerHTML=`
                    <tr>
                        <th>Nome</th>
                        <th>Tamanho</th>
                        <th>Tipo</th>
            ...
            `,this.renderPaginationControls("doc-pagination",0,0);return}t.innerHTML=u.map(d=>{const p=d.mimetype==="application/pdf"?"📕":"🖼️",m=(d.size/1024).toFixed(1)+" KB",f=d.created_at?new Date(d.created_at).toLocaleDateString("pt-BR"):"-",g=d.mimetype==="application/pdf"?"PDF":"Imagem",h=a?`<button class="btn-delete" onclick="window.DocsHandler.delete(${d.id})" style="padding: 4px 10px; font-size: 0.85rem;">Deletar</button>`:"",y=d.start_date?new Date(d.start_date+"T00:00:00").toLocaleDateString("pt-BR"):"-",L=d.end_date?d.end_date==="Indefinido"?"Indefinido":new Date(d.end_date+"T00:00:00").toLocaleDateString("pt-BR"):"-";return o?`
                    <tr>
                        <td>
                            <span style="font-weight: 500; display: flex; align-items: center; gap: 8px;">
                                <span>${p}</span>
                ...
                `}).join(""),this.renderPaginationControls("doc-pagination",r,i)},async handleUpload(e){e.preventDefault();const t=document.getElementById("doc-file"),n=document.getElementById("doc-category"),o=document.getElementById("doc-display-name");if(!t.files.length){alert("Selecione um arquivo.");return}const a=new FormData,s=n?n.value:"Geral";a.append("category",s),a.append("customName",o?o.value:""),a.append("document",t.files[0]);const i=s.toLowerCase();if(i==="contratos"||i==="termo de uso"){const r=document.getElementById("doc-start-date"),c=document.getElementById("doc-end-date"),u=document.getElementById("doc-indefinite");r&&r.value&&a.append("startDate",r.value),u&&u.checked?a.append("endDate","Indefinido"):c&&c.value&&a.append("endDate",c.value)}try{await $.upload("/documents",a),l.hide("modal-upload"),document.getElementById("doc-form").reset();const r=document.getElementById("doc-dates-container");r&&(r.style.display="none");const c=document.getElementById("doc-end-date");c&&(c.disabled=!1),l.setText("file-name-display","Respeite o formato .png ou .pdf"),this.fetch(),alert("Documento adicionado com sucesso!")}catch(r){console.error(r),alert("Erro ao subir arquivo.")}},async delete(e){if(confirm("Deletar este documento?"))try{await $.delete(`/documents/${e}`),this.fetch()}catch{alert("Erro ao excluir documento.")}},search(e){if(oe.toLowerCase()==="dashboard")this.renderDashboard();else{H=1;const t=Be.filter(n=>(n.category||"Geral").toLowerCase()===oe.toLowerCase()&&n.original_name.toLowerCase().includes(e));this.render(t)}},changePage(e){H=e,this.render(tt)},renderPaginationControls(e,t,n){const o=document.getElementById(e);if(!o)return;if(t===0){o.innerHTML="";return}let a="";a+=`
        ...
        `;const i=(H-1)*ye+1,r=Math.min(H*ye,n);a+=`
            <span class="pagination-info">
                Exibindo ${i}-${r} de ${n}
            </span>
        `,o.innerHTML=a}};let Z=[],x={summaries:[]},nt=null,C=null,Pe="list",ve=null,K=null,ot=null,P=1;const be=10;let ke=[];const z={getPendingProcId(){return nt},async fetch(){try{P=1,Z=await $.get("/procedures"),this.renderTable(Z)}catch(e){console.error("Error fetching FAQs:",e)}},getFaqs(){return Z},setListingMode(e){Pe=e,P=1,this.renderTable(ke.length?ke:Z)},renderTable(e){const t=document.getElementById("list-table-container"),n=document.getElementById("list-cards-container"),o=document.getElementById("proc-table-body");if(!t||!n||!o)return;ke=e;const a=e.length,s=Math.ceil(a/be);P>s&&(P=Math.max(1,s)),P<1&&(P=1);const i=(P-1)*be,r=e.slice(i,i+be);Pe==="list"?(l.show("list-table-container"),l.hide("list-cards-container"),r.length===0?o.innerHTML=`
                    <tr>
            ...
            `).join("");const o=x.summaries.find(a=>a.id===C);o?(n.textContent=o.title,n.style.color="var(--text-main)",t.classList.remove("hidden")):(n.textContent="Nenhum sumário selecionado",n.style.color="var(--accent)",t.classList.add("hidden"))},selectSummary(e){C=e,this.renderProcedureBuilderSidebar(),this.renderProcedureBuilder()},updateSummaryTitle(e,t){const n=x.summaries.find(a=>a.id===e);n&&(n.title=t||"Sem título"),this.renderProcedureBuilderSidebar();const o=x.summaries.find(a=>a.id===C);o&&(document.getElementById("current-summary-name").textContent=o.title)},addSummary(){const e="sum_"+Date.now();x.summaries.push({id:e,title:`Sumário ${x.summaries.length+1}`,sections:[]}),C=e,this.renderProcedureBuilderSidebar(),this.renderProcedureBuilder()},removeSummary(e){confirm("Excluir este sumário apagará todos os campos dentro dele. Deseja continuar?")&&(x.summaries=x.summaries.filter(t=>t.id!==e),C===e&&(C=x.summaries.length>0?x.summaries[0].id:null),this.renderProcedureBuilderSidebar(),this.renderProcedureBuilder())},renderProcedureBuilder(){const e=document.getElementById("procedure-edit-container");if(!e)return;if(!C){e.innerHTML='<p class="empty-state">Crie um novo sumário na barra lateral para adicionar conteúdo.</p>';return}const t=x.summaries.find(o=>o.id===C);if(!t)return;const n=t.sections;if(n.length===0){e.innerHTML=`<p class="empty-state">Nenhum campo em "${t.title}". Clique em "+ Novo Container" para começar.</p>`;return}e.innerHTML=n.map((o,a)=>`
             ...
             </div>`).join("")},handleSumDragStart(e,t){ve="summary",K=t,e.dataTransfer.effectAllowed="move",setTimeout(()=>{e.target&&e.target.classList.add("dragging")},0)},handleSumDrop(e,t){if(e.preventDefault(),ve!=="summary"||K===null||K===t)return;const n=x.summaries.splice(K,1)[0];x.summaries.splice(t,0,n),this.renderProcedureBuilderSidebar()},handleSecDragStart(e,t,n){ve="container",K=t,ot=n,e.dataTransfer.effectAllowed="move",setTimeout(()=>{const o=e.target.nodeType===1?e.target.closest(".editable-section"):null;o&&o.classList.add("dragging")},0)},handleDragOver(e){e.preventDefault(),e.dataTransfer.dropEffect="move"},handleSecDrop(e,t,n){if(e.preventDefault(),ve!=="container"||K===null||ot!==n)return;const o=x.summaries.find(s=>s.id===n);if(!o||K===t)return;const a=o.sections.splice(K,1)[0];o.sections.splice(t,0,a),this.renderProcedureBuilder()},handleDragEnd(e){document.querySelectorAll(".editable-section.dragging").forEach(t=>t.classList.remove("dragging")),e&&e.target&&e.target.setAttribute&&e.target.setAttribute("draggable","false"),ve=null,K=null},updateSectionTitle(e,t){const n=x.summaries.find(o=>o.id===C);n&&(n.sections[e].title=t)},updateSectionData(e,t){const n=x.summaries.find(o=>o.id===C);n&&(n.sections[e].data=t)},removeSection(e){const t=x.summaries.find(n=>n.id===C);t&&t.sections.splice(e,1),this.renderProcedureBuilder()},getRteToolbarHTML(){return`
        ...
        `},addFaqItem(e){const t=x.summaries.find(n=>n.id===C);t&&(t.sections[e].data=t.sections[e].data||[],t.sections[e].data.push({q:"",a:""}),this.renderProcedureBuilder())},updateFaqItem(e,t,n,o){const a=x.summaries.find(s=>s.id===C);a&&(a.sections[e].data[t][n]=o)},removeFaqItem(e,t){const n=x.summaries.find(o=>o.id===C);n&&n.sections[e].data.splice(t,1),this.renderProcedureBuilder()},addSection(e,t){if(!C){alert("Selecione primeiro um sumário na barra lateral.");return}const n=x.summaries.find(o=>o.id===C);n&&(n.sections.push({id:Date.now(),title:e,type:t,data:t==="FAQ"?[]:t==="TEXTO"?"":null}),this.renderProcedureBuilder())},async handleSectionFileDrop(e,t){t.dataTransfer.files&&t.dataTransfer.files.length>0&&await this.uploadSectionFile(e,t.dataTransfer.files[0])},async handleSectionFileUpload(e,t){const n=t.files[0];n&&await this.uploadSectionFile(e,n)},async uploadSectionFile(e,t){const n=new FormData;n.append("file",t);try{const o=await $.upload("/upload",n),a=x.summaries.find(s=>s.id===C);a&&(a.sections[e].data={name:t.name,path:o.path,mimetype:t.type},this.renderProcedureBuilder())}catch{alert("Erro no upload")}},async handleSaveProcedure(){const e=parseInt(l.getValue("proc-id"));if(!e)return;const n={...Z.find(o=>o.id===e),content:JSON.stringify(x)};try{await $.put(`/procedures/${e}`,n),alert("Salvo com sucesso!"),this.toggleEditMode(!1),this.openDetail(e),this.fetch()}catch{alert("Erro ao salvar")}},search(e){P=1;const t=Z.filter(n=>(n.name||n.title||"").toLowerCase().includes(e)||(n.responsible||"").toLowerCase().includes(e)||(n.group_name||"").toLowerCase().includes(e));this.renderTable(t)},changePage(e){P=e,this.renderTable(ke)},renderPaginationControls(e,t,n){const o=document.getElementById(e);if(!o)return;if(t===0){o.innerHTML="";return}let a="";a+=`
            <button class="pagination-btn" 
                    ${P===1?"disabled":""} 
        ...
        `,o.innerHTML=a}};window.toggleGhAccordion=function(e){const t=document.getElementById(e);t&&t.classList.toggle("open")};let O=[],ue="list",ae="month",E=new Date,R=1;const we=10;let at=[];const _={async fetch(){try{R=1,O=await $.get("/accounts"),this.initDashboardMultiselects(),this.populateCompanyFilter(),this.handleSearch(),this.checkAccountAlerts()}catch(e){console.error("Falha ao obter contas",e)}},populateCompanyFilter(){const e=document.getElementById("dash-filter-company-dynamic-options");if(e){const t=new Set;e.querySelectorAll('input[type="checkbox"]:checked').forEach(a=>{t.add(a.value)});const n=[...new Set(O.map(a=>a.company_name).filter(Boolean))].sort((a,s)=>a.localeCompare(s));let o="";n.forEach(a=>{const s=t.has(a)?"checked":"";o+=`<label class="multiselect-option"><input type="checkbox" value="${a}" ${s}> <span>${a}</span></label>`}),e.innerHTML=o,this.setupMultiselectListeners("dash-filter-company")}},setupMultiselectListeners(e){if(!document.getElementById(`${e}-container`))return;const n=document.getElementById(`${e}-trigger`),o=document.getElementById(`${e}-dropdown`);if(!n||!o)return;n.dataset.listenerBound||(n.addEventListener("click",r=>{r.stopPropagation(),document.querySelectorAll(".multiselect-dropdown").forEach(c=>{c!==o&&c.classList.add("hidden")}),o.classList.toggle("hidden")}),n.dataset.listenerBound="true");const a=o.querySelector('input[value="Todos"]'),s=Array.from(o.querySelectorAll('input[type="checkbox"]')).filter(r=>r.value!=="Todos"),i=()=>{const r=s.filter(u=>u.checked).map(u=>u.value),c=n.querySelector(".trigger-label");a.checked||s.length>0&&r.length===s.length?(a.checked=!0,c&&(c.innerText="Todos")):r.length===0?c&&(c.innerText="Nenhum"):r.length===1?c&&(c.innerText=r[0]):c&&(c.innerText=`${r.length} selecionados`)};a&&!a.dataset.listenerBound&&(a.addEventListener("change",()=>{s.forEach(r=>{r.checked=a.checked}),i(),this.renderDashboard()}),a.dataset.listenerBound="true"),s.forEach(r=>{r.dataset.listenerBound||(r.addEventListener("change",()=>{s.every(u=>u.checked)?a.checked=!0:a.checked=!1,i(),this.renderDashboard()}),r.dataset.listenerBound="true")}),i()},initDashboardMultiselects(){this.setupMultiselectListeners("dash-filter-category"),window.multiselectOutsideClickListenerBound||(document.addEventListener("click",e=>{e.target.closest(".custom-multiselect-container")||document.querySelectorAll(".multiselect-dropdown").forEach(t=>{t.classList.add("hidden")})}),window.multiselectOutsideClickListenerBound=!0)},getMultiselectValues(e){const t=document.getElementById(`${e}-dropdown`);if(!t)return["Todos"];const n=t.querySelector('input[value="Todos"]');return n&&n.checked?["Todos"]:Array.from(t.querySelectorAll('input[type="checkbox"]:checked')).map(o=>o.value).filter(o=>o!=="Todos")},resetMultiselects(){["dash-filter-category","dash-filter-company"].forEach(e=>{const t=document.getElementById(`${e}-dropdown`);if(t){t.querySelectorAll('input[type="checkbox"]').forEach(a=>{a.checked=a.value==="Todos"});const o=document.getElementById(`${e}-trigger`);if(o){const a=o.querySelector(".trigger-label");a&&(a.innerText="Todos")}}})},getAccounts(){return O},setAccountsViewMode(e){ue=e,this.handleSearch()},setCalendarSubView(e){ae=e,this.handleSearch()},shiftCalendarDate(e){ae==="day"?E.setDate(E.getDate()+e):ae==="month"?E.setMonth(E.getMonth()+e):ae==="year"&&E.setFullYear(E.getFullYear()+e),l.setValue("filter-day",E.getDate()),l.setValue("filter-month",E.getMonth()),l.setValue("filter-year",E.getFullYear()),this.handleSearch()},handleFilterChange(e=!1){if(e){const t=l.getValue("filter-cal-year")?parseInt(l.getValue("filter-cal-year")):E.getFullYear(),n=l.getValue("filter-cal-month")?parseInt(l.getValue("filter-cal-month")):E.getMonth();E=new Date(t,n,1)}else{const t=l.getValue("filter-year")?parseInt(l.getValue("filter-year")):E.getFullYear(),n=l.getValue("filter-month")?parseInt(l.getValue("filter-month")):E.getMonth(),o=l.getValue("filter-day")?parseInt(l.getValue("filter-day")):E.getDate();E=new Date(t,n,o)}l.setValue("filter-month",E.getMonth()),l.setValue("filter-year",E.getFullYear()),this.handleSearch()},handleSearch(){const e=(l.getValue("accounts-search")||"").toLowerCase();let t=O.filter(n=>n.company_name.toLowerCase().includes(e)||n.description&&n.description.toLowerCase().includes(e));if(ue==="list"){R=1;const n=l.getValue("filter-status")||"",o=document.getElementById("filter-date-toggle"),a=o?o.checked:!1,s=E.getFullYear(),i=E.getMonth(),r=E.getDate();t=t.filter(c=>{if(n&&c.status!==n)return!1;if(!a||!c.due_date)return!0;const[u,d,p]=c.due_date.split("-"),m=parseInt(u,10),f=parseInt(d,10)-1,g=parseInt(p,10);return c.type==="Único"?m===s&&f===i&&g===r:c.type==="Recorrente"?g===r:!0}),this.renderAccountsList(t)}else ue==="notificacoes"?this.renderNotifications():ue==="dashboard"?this.renderDashboard():this.renderCalendarWrapper(t)},checkAccountAlerts(){let e=!1;const t=new Date;t.setHours(0,0,0,0),O.forEach(o=>{const a=(o.status||"").trim().toLowerCase(),s=(o.payment_status||"").trim().toLowerCase();if(a==="on"&&s==="pendente"&&o.due_date){const[i,r,c]=o.due_date.split("-");let u=new Date(parseInt(i,10),parseInt(r,10)-1,parseInt(c,10));u.setHours(0,0,0,0),u.getTime()<=t.getTime()&&(e=!0)}});const n=document.getElementById("icon-alert-bell");n&&(e?n.classList.add("alert-pulse"):n.classList.remove("alert-pulse"))},renderNotifications(){const e=document.getElementById("accounts-notifications-body");if(!e)return;e.innerHTML="";const t=new Date;t.setHours(0,0,0,0);let n=O.filter(o=>{const a=(o.status||"").trim().toLowerCase(),s=(o.payment_status||"").trim().toLowerCase();if(a!=="on"||s!=="pendente"||!o.due_date)return!1;const[i,r,c]=o.due_date.split("-");let u=new Date(parseInt(i,10),parseInt(r,10)-1,parseInt(c,10));return u.setHours(0,0,0,0),u.getTime()<=t.getTime()});if(n.length===0){e.innerHTML='<tr><td colspan="6" style="text-align:center; color: var(--text-muted); padding: 20px;">Nenhuma conta urgente ou atrasada.</td></tr>';return}n.forEach(o=>{const a=document.createElement("tr");let s="Sem Data";if(o.due_date){const r=o.due_date.split("-");r.length===3&&(s=`${r[2]}/${r[1]}/${r[0]}`)}const i=F.isAdmin()?`
            ...
            `,e.appendChild(a)})},renderAccountsList(e){const t=document.getElementById("accounts-table-body");if(!t)return;t.innerHTML="",this.renderSidebarMiniCalendar(),at=e;const n=e.length,o=Math.ceil(n/we);R>o&&(R=Math.max(1,o)),R<1&&(R=1);const a=(R-1)*we,s=e.slice(a,a+we);if(s.length===0){t.innerHTML='<tr><td colspan="7" style="text-align:center; color: var(--text-muted); padding: 20px;">Nenhuma conta encontrada.</td></tr>',this.renderPaginationControls("accounts-list-pagination",0,0),this.renderDashboard();return}s.forEach(i=>{const r=document.createElement("tr");let c="Sem Data";if(i.due_date){const p=i.due_date.split("-");p.length===3&&(c=`${p[2]}/${p[1]}/${p[0]}`)}const u=i.status==="Off",d=F.isAdmin()?`
        ...
        `;let s=0;for(let c=1;c<=t;c++)(c===1||c===t||c>=R-1&&c<=R+1)&&(s&&c-s>1&&(a+='<span style="color: var(--text-muted); padding: 0 4px;">...</span>'),a+=`
        ...
        `;const i=(R-1)*we+1,r=Math.min(R*we,n);a+=`
            <span class="pagination-info">
                Exibindo ${i}-${r} de ${n}
            </span>
        `,o.innerHTML=a}};let re=[],J={},te=null,_e=null,Fe=null,q=[],Se=[],Ge={},Q={},ge,Je,Me,Ae,rt,He;const lt={init(){ge=document.getElementById("timeline-event-form"),Je=document.getElementById("view-visualizacao"),Me=document.getElementById("view-attention"),Ae=document.getElementById("view-anexo"),rt=document.getElementById("view-relatorio"),He=document.getElementById("view-config"),window.timelineHandler=lt,window.applyFilters=vt,window.clearFilters=bt,window.toggleFilters=wt,window.handleDelete=yt,window.resetForm=Ke,window.toggleAccordion=dt,window.handleFormSubmit=st,window.editEvent=Qe,window.deleteTopic=It,window.deleteSubtopic=Ct,window.handleTrackDragStart=Bt,window.handleTrackDragOver=kt,window.handleTrackDragEnd=Dt;const e=document.getElementById("timeline-topic-form");e&&e.addEventListener("submit",Tt);const t=document.getElementById("timeline-subtopic-form");t&&t.addEventListener("submit",Lt);const n=document.getElementById("topico");n&&n.addEventListener("change",u=>{We(u.target.value)});const o=document.getElementById("em-ocorrencia");o&&o.addEventListener("change",u=>{const d=document.getElementById("fim"),p=document.getElementById("inicio");if(u.target.checked){if(!p.value){const m=new Date;m.setMinutes(m.getMinutes()-m.getTimezoneOffset()),p.value=m.toISOString().slice(0,16)}d.required=!1}else{const m=new Date;m.setMinutes(m.getMinutes()-m.getTimezoneOffset()),d.value=m.toISOString().slice(0,16),d.required=!0}});const a=document.getElementById("auto-refresh-toggle");a&&a.addEventListener("change",u=>{ct(u.target.checked)}),document.querySelectorAll("[data-timeline-tab]").forEach(u=>{u.addEventListener("click",d=>{const p=d.currentTarget.getAttribute("data-timeline-tab");Ce(p)})}),ge&&ge.addEventListener("submit",st);const s=document.getElementById("rep-filter-start"),i=document.getElementById("rep-filter-end"),r=document.getElementById("rep-filter-topic"),c=document.getElementById("rep-filter-subtopic");s&&s.addEventListener("change",()=>$e()),i&&i.addEventListener("change",()=>$e()),r&&r.addEventListener("change",u=>{$t(u.target.value),$e()}),c&&c.addEventListener("change",()=>$e()),window.addEventListener("SectionChange",u=>{u.detail&&u.detail.section==="timeline"&&le().then(()=>{X(),it()})}),le().then(()=>{X(),it()})}};window.addEventListener("focus",()=>{Je&&X()});function We(e,t=null){const n=document.getElementById("sub-topico");if(!n)return;const o=e?e.toLowerCase().trim():"";if(!o||!Q[o]){n.innerHTML='<option value="">Selecione o tópico primeiro...</option>',n.classList.remove("has-options");return}n.innerHTML='<option value="" disabled selected>Escolha o evento...</option>',Q[o].forEach(a=>{const s=document.createElement("option");s.value=a.toLowerCase(),s.textContent=a,t&&s.value===t.toLowerCase()&&(s.selected=!0),n.appendChild(s)}),t||(n.selectedIndex=1),n.classList.add("has-options")}async function le(){try{const e=await fetch("/api/timeline/config");if(!e.ok)throw new Error("Falha ao buscar configurações");const t=await e.json();q=t.topics||[],Se=t.subtopics||[],Ge={},Q={},q.forEach(o=>{Ge[o.id]=o.color,Q[o.id]=[]}),Se.forEach(o=>{const a=o.topic_id;Q[a]&&Q[a].push(o.name)}),ht();const n=document.getElementById("view-config");n&&n.classList.contains("active")&&mt()}catch(e){console.error("Error loading config:",e)}}function ht(){const e=document.getElementById("topico");if(e){const o=e.value;e.innerHTML='<option value="" disabled selected>Selecione um tópico...</option>',q.forEach(a=>{const s=document.createElement("option");s.value=a.id,s.textContent=a.name,e.appendChild(s)}),e.value=o}const t=document.getElementById("rep-filter-topic");if(t){const o=t.value;t.innerHTML='<option value="Todos">Todos</option>',q.forEach(a=>{const s=document.createElement("option");s.value=a.id,s.textContent=a.name,t.appendChild(s)}),o&&[...t.options].some(a=>a.value===o)?t.value=o:t.value="Todos"}const n=document.getElementById("subtopic-topic-id");n&&(n.innerHTML='<option value="" disabled selected>Selecione um tópico...</option>',q.forEach(o=>{const a=document.createElement("option");a.value=o.id,a.textContent=o.name,n.appendChild(a)}))}function X(){fetch("/api/timeline/events").then(e=>{if(!e.ok)throw new Error("Failed to fetch");return e.json()}).then(e=>{re=e,Xe(),Me&&Me.classList.contains("active")&&ut()}).catch(e=>{console.error("Error loading events:",e)})}function it(){const e=document.getElementById("timeline-tab-anexo"),t=document.getElementById("timeline-tab-config");if(window.auth&&window.auth.isAdmin())e&&e.classList.remove("role-hidden"),t&&t.classList.remove("role-hidden");else{e&&e.classList.add("role-hidden"),t&&t.classList.add("role-hidden");const o=Ae&&Ae.classList.contains("active"),a=He&&He.classList.contains("active");(o||a)&&Ce("visualizacao")}}function Ce(e){const t={visualizacao:{section:Je,button:document.querySelector('[data-timeline-tab="visualizacao"]')},attention:{section:Me,button:document.querySelector('[data-timeline-tab="attention"]')},anexo:{section:Ae,button:document.querySelector('[data-timeline-tab="anexo"]')},relatorio:{section:rt,button:document.querySelector('[data-timeline-tab="relatorio"]')},config:{section:He,button:document.querySelector('[data-timeline-tab="config"]')}};Object.values(t).forEach(n=>{n.section&&n.section.classList.remove("active"),n.button&&n.button.classList.remove("active")}),t[e]&&(t[e].section&&t[e].section.classList.add("active"),t[e].button&&t[e].button.classList.add("active")),e==="visualizacao"?(X(),xe(!0)):e==="attention"?(ut(),xe(!0)):e==="relatorio"?($e(),xe(!1)):(e==="config"&&mt(),xe(!1))}function xe(e){const t=document.getElementById("floating-refresh-control");if(t)if(e){t.classList.remove("hidden");const n=document.getElementById("auto-refresh-toggle");n&&n.checked&&!te&&ct(!0)}else t.classList.add("hidden"),te&&(clearInterval(te),te=null)}function ct(e){te&&(clearInterval(te),te=null),e&&(X(),te=setInterval(X,6e4))}function st(e){e.preventDefault();const t=ge.querySelector('button[type="submit"]');t&&(t.textContent="Salvando...",t.disabled=!0);const o={id:document.getElementById("event-id").value||Date.now().toString(),nome:document.getElementById("nome").value,topico:document.getElementById("topico").value,sub_topico:document.getElementById("sub-topico").value,em_ocorrencia:document.getElementById("em-ocorrencia").checked?1:0,inicio:document.getElementById("inicio").value,fim:document.getElementById("fim").value,descricao:document.getElementById("descricao").value,anotacao:document.getElementById("anotacao").value,cor:document.getElementById("cor").value};fetch("/api/timeline/events",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)}).then(async a=>{const s=await a.text();if(!a.ok)throw new Error(`Server error (${a.status}): ${s}`);return JSON.parse(s)}).then(()=>{alert("Evento salvo com sucesso!"),Ke(),Ce("visualizacao")}).catch(a=>{console.error("Error saving event:",a),alert("Erro ao salvar evento: "+a.message)}).finally(()=>{t&&(t.textContent="Salvar Evento",t.disabled=!1)})}function Qe(e){const t=re.find(s=>s.id===e);if(!t)return;document.getElementById("event-id").value=t.id,document.getElementById("nome").value=t.nome;const n=Te(t.topico);document.getElementById("topico").value=n,We(n,t.sub_topico);const o=document.getElementById("em-ocorrencia");o.checked=t.em_ocorrencia==1||t.em_ocorrencia==="true"||!t.fim,o.dispatchEvent(new Event("change")),document.getElementById("inicio").value=t.inicio,document.getElementById("fim").value=t.fim||"",document.getElementById("descricao").value=t.descricao||"",document.getElementById("anotacao").value=t.anotacao||"",document.getElementById("cor").value=t.cor||"#000000",Ce("anexo");const a=document.getElementById("btn-delete");a&&(a.style.display="block")}function Ke(){ge&&ge.reset();const e=document.getElementById("event-id");e&&(e.value=""),We("");const t=document.getElementById("fim");t&&(t.required=!0);const n=document.getElementById("cor");n&&(n.value="#000000");const o=document.getElementById("btn-delete");o&&(o.style.display="none")}function yt(){const e=document.getElementById("event-id").value;e&&confirm("Tem certeza que deseja excluir este evento?")&&fetch(`/api/timeline/events/${e}`,{method:"DELETE"}).then(t=>{if(!t.ok)throw new Error("Failed to delete");return t.json()}).then(()=>{alert("Evento excluído!"),Ke(),Ce("visualizacao")}).catch(t=>{console.error("Error deleting:",t),alert("Erro ao excluir: "+t.message)})}function vt(e){const t=document.getElementById(`filter-start-${e}`),n=document.getElementById(`filter-end-${e}`),o=document.getElementById(`filter-sub-topic-${e}`),a=t&&t.value?new Date(t.value).getTime():null,s=n&&n.value?new Date(n.value).getTime():null,i=o?o.value:"";J[e]={start:a,end:s,subTopic:i},Xe()}function bt(e){const t=document.getElementById(`filter-start-${e}`),n=document.getElementById(`filter-end-${e}`),o=document.getElementById(`filter-sub-topic-${e}`);t&&(t.value=""),n&&(n.value=""),o&&(o.value=""),J[e]=null,Xe()}function wt(e){const t=document.getElementById(`filters-panel-${e}`),n=document.getElementById(`btn-toggle-${e}`);t&&n&&(t.classList.toggle("hidden"),n.classList.toggle("active"))}function dt(e){const t=document.getElementById(e);t&&t.classList.toggle("active")}function Xe(){const e=document.getElementById("timeline-tracks-container");if(!e)return;const t=Array.from(e.querySelectorAll(".timeline-container")).map(a=>a.dataset.topicId),n=q.map(a=>a.id);if(t.length!==n.length||!n.every(a=>t.includes(a))){e.innerHTML="";const a=window.auth&&window.auth.isAdmin(),s=a?'style="cursor: grab;"':"";q.forEach(i=>{const r=`
            ...
            `;e.insertAdjacentHTML("beforeend",r);const c=document.getElementById(`filter-sub-topic-${i.id}`);c&&Q[i.id]&&Q[i.id].forEach(u=>{const d=document.createElement("option");d.value=u.toLowerCase(),d.textContent=u,c.appendChild(d)})})}q.forEach(a=>{const s=document.getElementById(`track-${a.id}`),i=document.getElementById(`min-date-${a.id}`),r=document.getElementById(`max-date-${a.id}`);s&&(s.innerHTML=""),i&&(i.textContent=""),r&&(r.textContent="")}),re.length!==0&&q.forEach(a=>{const s=a.id,i=re.filter(y=>Te(y.topico)===s);let r=i;J[s]&&J[s].subTopic&&(r=i.filter(y=>(y.sub_topico?y.sub_topico.toLowerCase():"")===J[s].subTopic.toLowerCase()));const c=J[s]&&J[s].start?J[s].start:new Date("2026-01-01T00:00:00").getTime(),u=J[s]&&J[s].end?J[s].end:Date.now();xt(s,r,c,u);const d=c,p=u,m=p-d,f=document.getElementById(`min-date-${s}`),g=document.getElementById(`max-date-${s}`);f&&(f.textContent=new Date(d).toLocaleDateString()+" "+new Date(d).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})),g&&(g.textContent=new Date(p).toLocaleDateString()+" "+new Date(p).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}));const h=document.getElementById(`track-${s}`);h&&r.forEach(y=>{const L=new Date(y.inicio).getTime(),T=y.fim?new Date(y.fim).getTime():Date.now();if(T<d||L>p)return;const S=Math.max(L,d),N=Math.min(T,p),V=(S-d)/m*100,w=(N-S)/m*100;if(w<=0)return;const k=document.createElement("div");k.className="timeline-bar",k.style.left=`${V}%`,k.style.width=`${w}%`,k.style.color=y.cor&&y.cor!=="#000000"?y.cor:Ge[s]||"#6b7280";const b=document.createElement("div");b.className="timeline-bar-visual",k.appendChild(b);const A=document.createElement("div");A.className="timeline-identifier-point";const j=new Date(y.inicio).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),U=y.fim?new Date(y.fim).toLocaleString([],{dateStyle:"short",timeStyle:"short"}):"Em andamento",G=a.name,B=y.sub_topico?y.sub_topico.charAt(0).toUpperCase()+y.sub_topico.slice(1):"-";A.setAttribute("data-tooltip",`Tópico: ${G}
Eventos: ${B}
Início: ${j} - Fim: ${U}
Descrição: ${y.descricao||"-"}`),!y.fim&&A.classList.add("pulsing"),window.auth&&window.auth.isAdmin()?(A.style.cursor="pointer",A.onclick=ce=>{ce.stopPropagation(),Qe(y.id)}):A.style.cursor="default",k.appendChild(A),h.appendChild(k)})})}function Te(e){return e?e.toLowerCase().trim():""}function xt(e,t,n,o){const a=document.getElementById(`sla-${e}`);if(!a)return;const s=o-n;if(s<=0){a.textContent="N/A";return}const r=t.filter(m=>{const f=new Date(m.inicio).getTime();return(m.fim?new Date(m.fim).getTime():Date.now())>n&&f<o}).map(m=>({start:Math.max(new Date(m.inicio).getTime(),n),end:Math.min(m.fim?new Date(m.fim).getTime():Date.now(),o)}));r.sort((m,f)=>m.start-f.start);const c=[];if(r.length>0){let m=r[0];for(let f=1;f<r.length;f++){const g=r[f];g.start<m.end?m.end=Math.max(m.end,g.end):(c.push(m),m=g)}c.push(m)}let u=0;c.forEach(m=>{u+=m.end-m.start});const d=(s-u)/s*100;let p="#10b981";d<50?p="#ef4444":d<90&&(p="#f97316"),a.style.color=p,a.textContent=d.toFixed(4)+"%"}function ut(){const e=document.getElementById("attention-topics-container");if(!e)return;e.innerHTML="";const t=re.filter(n=>!n.fim);q.forEach(n=>{const o=n.id,a=t.filter(h=>Te(h.topico)===o),s=document.createElement("div");s.className=a.length>0?"accordion-item active":"accordion-item",s.id=`attn-acc-${o}`;const i=document.createElement("div");i.className="accordion-header",i.onclick=()=>dt(`attn-acc-${o}`);const r=document.createElement("div");r.className="accordion-title-group";const c=document.createElement("div");c.className="topic-indicator",c.style.backgroundColor=n.color;const u=document.createElement("h3");u.textContent=n.name;const d=document.createElement("span");d.style.cssText="background: #f1f5f9; padding: 2px 8px; border-radius: 12px; font-size: 0.95rem; font-weight: 900; color: #0f172a; margin-left: 0.5rem; border: 1px solid #cbd5e1;",d.textContent=`${a.length}`,r.appendChild(c),r.appendChild(u),r.appendChild(d);const p=document.createElement("span");p.className="accordion-chevron",p.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>',i.appendChild(r),i.appendChild(p);const m=document.createElement("div");m.className="accordion-content";const f=document.createElement("div");f.className="accordion-body";const g=document.createElement("div");if(g.className="attention-carousel",a.length===0){const h=document.createElement("div");h.className="empty-state",h.textContent="Nenhum evento em andamento.",g.appendChild(h)}else a.forEach(h=>{const y=document.createElement("div");y.className="attention-card",y.style.borderLeftColor=h.cor&&h.cor!=="#000000"?h.cor:n.color;const L=document.createElement("h3");L.textContent=h.nome;const T=document.createElement("div");T.className="sub-topic",T.textContent=h.sub_topico||"-";const S=document.createElement("div");S.className="card-detail",S.innerHTML=`<strong>Início:</strong> ${new Date(h.inicio).toLocaleString()}`;const N=Date.now()-new Date(h.inicio).getTime(),V=document.createElement("div");V.className="card-duration",V.innerHTML=`<strong>Tempo:</strong> <span>${Et(N)}</span>`;const w=document.createElement("div");w.className="card-description",w.textContent=h.descricao||"-",y.appendChild(L),y.appendChild(T),y.appendChild(S),y.appendChild(V),y.appendChild(w),window.auth&&window.auth.isAdmin()?(y.style.cursor="pointer",y.onclick=()=>Qe(h.id)):y.style.cursor="default",g.appendChild(y)});f.appendChild(g),m.appendChild(f),s.appendChild(i),s.appendChild(m),e.appendChild(s)})}function Et(e){if(e<0)return"0s";const t=Math.floor(e/1e3),n=Math.floor(t/60),o=Math.floor(n/60),a=Math.floor(o/24),s=[];return a>0&&s.push(`${a}d`),(o%24>0||a>0)&&s.push(`${o%24}h`),(n%60>0||o>0)&&s.push(`${n%60}m`),s.push(`${t%60}s`),s.join(" ")}function $t(e){const t=document.getElementById("rep-filter-subtopic");if(!t)return;t.innerHTML='<option value="Todos">Todos</option>';const n=e?e.toLowerCase().trim():"";n&&Q[n]&&Q[n].forEach(o=>{const a=document.createElement("option");a.value=o.toLowerCase(),a.textContent=o,t.appendChild(a)})}function $e(){let e=re;const t=document.getElementById("rep-filter-start")?.value,n=document.getElementById("rep-filter-end")?.value,o=document.getElementById("rep-filter-topic")?.value,a=document.getElementById("rep-filter-subtopic")?.value;if(t){const w=new Date(t+"T00:00:00").getTime();e=e.filter(k=>new Date(k.inicio).getTime()>=w)}if(n){const w=new Date(n+"T23:59:59").getTime();e=e.filter(k=>new Date(k.inicio).getTime()<=w)}o&&o!=="Todos"&&(e=e.filter(w=>Te(w.topico)===o.toLowerCase())),a&&a!=="Todos"&&(e=e.filter(w=>w.sub_topico&&w.sub_topico.toLowerCase()===a.toLowerCase()));const s=document.getElementById("rep-kpi-total"),i=document.getElementById("rep-kpi-active"),r=document.getElementById("rep-kpi-avg-time");s&&(s.textContent=e.length);const c=e.filter(w=>w.em_ocorrencia==1||w.em_ocorrencia==="true"||!w.fim);i&&(i.textContent=c.length);const u=e.filter(w=>w.fim);let d="0h 0m";if(u.length>0){const k=u.reduce((U,G)=>U+(new Date(G.fim).getTime()-new Date(G.inicio).getTime()),0)/u.length,b=Math.floor(k/6e4),A=Math.floor(b/60),j=b%60;d=`${A}h ${j}m`}if(r&&(r.textContent=d),!window.Chart){console.warn("Chart.js is not loaded.");return}const p=q,m=t?new Date(t+"T00:00:00").getTime():new Date(new Date().getFullYear()+"-01-01T00:00:00").getTime(),f=n?new Date(n+"T23:59:59").getTime():Date.now(),g=p.map(w=>w.name),h=p.map(w=>{const k=w.id,b=re.filter(D=>Te(D.topico)===k),A=f-m;if(A<=0)return 100;const U=b.filter(D=>{const W=new Date(D.inicio).getTime();return(D.fim?new Date(D.fim).getTime():Date.now())>m&&W<f}).map(D=>({start:Math.max(new Date(D.inicio).getTime(),m),end:Math.min(D.fim?new Date(D.fim).getTime():Date.now(),f)}));U.sort((D,W)=>D.start-W.start);const G=[];if(U.length>0){let D=U[0];for(let W=1;W<U.length;W++){const ne=U[W];ne.start<D.end?D.end=Math.max(D.end,ne.end):(G.push(D),D=ne)}G.push(D)}const Y=(D=>{let W=0;return D.forEach(ne=>{W+=ne.end-ne.start}),W})(G),ce=(A-Y)/A*100;return parseFloat(ce.toFixed(4))}),y=p.map(w=>w.color||"#6b7280"),L=document.getElementById("chart-rep-sla");L&&(_e&&_e.destroy(),_e=new window.Chart(L,{type:"bar",data:{labels:g,datasets:[{label:"Disponibilidade %",data:h,backgroundColor:y,borderRadius:6}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{y:{min:Math.max(0,Math.min(...h)-5),max:100,ticks:{callback:w=>w+"%"}}}}}));const T={};e.forEach(w=>{const k=w.sub_topico?w.sub_topico.charAt(0).toUpperCase()+w.sub_topico.slice(1).toLowerCase():"Não especificado";T[k]=(T[k]||0)+1});const S=Object.keys(T),N=Object.values(T),V=document.getElementById("chart-rep-qty");V&&(Fe&&Fe.destroy(),Fe=new window.Chart(V,{type:"doughnut",data:{labels:S.length>0?S:["Nenhum evento"],datasets:[{data:N.length>0?N:[0],backgroundColor:["#3b82f6","#10b981","#f59e0b","#8b5cf6","#ec4899","#6366f1","#14b8a6","#f43f5e","#a855f7","#06b6d4"]}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{position:"right",labels:{boxWidth:12}}}}}))}function Tt(e){e.preventDefault();const t=document.getElementById("topic-id"),n=document.getElementById("topic-name"),o=document.getElementById("topic-color");if(!t||!n||!o)return;const a={id:t.value.trim().toLowerCase(),name:n.value.trim(),color:o.value};if(!a.id){alert("Por favor, defina um ID para o tópico.");return}fetch("/api/timeline/config/topics",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)}).then(s=>{if(!s.ok)throw new Error("Erro ao salvar tópico");return s.json()}).then(()=>{alert("Tópico salvo com sucesso!"),t.value="",n.value="",o.value="#3b82f6",le().then(()=>{X()})}).catch(s=>{console.error(s),alert("Erro: "+s.message)})}function Lt(e){e.preventDefault();const t=document.getElementById("subtopic-topic-id"),n=document.getElementById("subtopic-name");if(!t||!n)return;const o={topic_id:t.value,name:n.value.trim()};if(!o.topic_id||!o.name){alert("Preencha todos os campos do evento.");return}fetch("/api/timeline/config/subtopics",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)}).then(a=>{if(!a.ok)throw new Error("Erro ao adicionar evento");return a.json()}).then(()=>{alert("Evento adicionado!"),n.value="",le()}).catch(a=>{console.error(a),alert("Erro: "+a.message)})}function It(e){confirm("Excluir este tópico também removerá todos os seus eventos associados. Deseja continuar?")&&fetch(`/api/timeline/config/topics/${e}`,{method:"DELETE"}).then(t=>{if(!t.ok)throw new Error("Erro ao excluir tópico");return t.json()}).then(()=>{alert("Tópico excluído!"),le().then(()=>{X()})}).catch(t=>{console.error(t),alert("Erro: "+t.message)})}function Ct(e){confirm("Deseja realmente excluir este evento?")&&fetch(`/api/timeline/config/subtopics/${e}`,{method:"DELETE"}).then(t=>{if(!t.ok)throw new Error("Erro ao excluir evento");return t.json()}).then(()=>{alert("Evento excluído!"),le()}).catch(t=>{console.error(t),alert("Erro: "+t.message)})}function mt(){const e=document.getElementById("config-topics-list");e&&(e.innerHTML="",q.length===0?e.innerHTML='<div style="color: var(--text-muted); font-size: 0.9rem;">Nenhum tópico cadastrado.</div>':q.forEach(n=>{const o=document.createElement("div");o.style.cssText="display: flex; align-items: center; justify-content: space-between; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 8px; border: 1px solid var(--glass-border); margin-bottom: 6px;",o.innerHTML=`
                ...
                `,e.appendChild(o)}));const t=document.getElementById("config-subtopics-list");t&&(t.innerHTML="",Se.length===0?t.innerHTML='<div style="color: var(--text-muted); font-size: 0.9rem;">Nenhum evento cadastrado.</div>':Se.forEach(n=>{const o=q.find(r=>r.id===n.topic_id),a=o?o.name:n.topic_id,s=o?o.color:"#6b7280",i=document.createElement("div");i.style.cssText="display: flex; align-items: center; justify-content: space-between; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 8px; border: 1px solid var(--glass-border); margin-bottom: 6px;",i.innerHTML=`
                    <div>
                ...
                `,t.appendChild(i)}))}function Bt(e,t){e.currentTarget.classList.add("dragging"),e.dataTransfer.effectAllowed="move"}function kt(e){e.preventDefault();const t=document.querySelector(".timeline-container.dragging");if(!t)return;const n=document.getElementById("timeline-tracks-container");if(!n)return;const a=[...n.querySelectorAll(".timeline-container:not(.dragging)")].find(s=>{const i=s.getBoundingClientRect();return e.clientY<=i.top+i.height/2});a?n.insertBefore(t,a):n.appendChild(t)}function Dt(e){const t=document.querySelector(".timeline-container.dragging");t&&t.classList.remove("dragging"),document.querySelectorAll(".timeline-container").forEach(a=>{a.setAttribute("draggable","false")});const n=document.getElementById("timeline-tracks-container");if(!n)return;const o=Array.from(n.querySelectorAll(".timeline-container")).map(a=>a.dataset.topicId);St(o)}function St(e){fetch("/api/timeline/config/topics/reorder",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({order:e})}).then(t=>{if(!t.ok)throw new Error("Erro ao salvar nova ordenação");return t.json()}).then(()=>{console.log("Ordem dos tópicos atualizada com sucesso."),le().then(()=>{X()})}).catch(t=>{console.error(t),alert("Erro ao salvar ordenação: "+t.message)})}let me=[],Ve=[],Re=[],ze=[],I="extensions",M=1,pe=100,Oe=[];const Ee={setActiveTab(e){I=e,M=1;const t=document.getElementById("telephony-search");t&&(t.value="",e==="extensions"?t.placeholder="Pesquisar ramais por número, nome ou usuário...":e==="queues"?t.placeholder="Pesquisar filas por número ou nome...":e==="blf"?t.placeholder="Pesquisar BLF por nome...":e==="users"&&(t.placeholder="Pesquisar usuários por nome ou perfil...")),document.querySelectorAll(".telephony-tabs-nav .acc-tab-btn").forEach(s=>{s.id===`tab-telephony-${e}`?s.classList.add("active"):s.classList.remove("active")}),document.querySelectorAll(".telephony-tab-content").forEach(s=>{s.id===`telephony-view-${e==="users"?"users":e==="queues"?"queues":e}`?s.classList.remove("hidden"):s.classList.add("hidden")});const a=this.getActiveDataList();this.render(a)},getActiveDataList(){return I==="extensions"?me:I==="queues"?Ve:I==="blf"?Re:I==="users"?ze:[]},async fetch(){const e=this.getActiveTableBody();e&&(e.innerHTML='<tr><td colspan="10" style="text-align: center; padding: 2rem; color: var(--text-muted);">Carregando dados...</td></tr>');try{if(M=1,I==="extensions")me=await $.get("/telephony/extensions"),this.render(me);else if(I==="queues")Ve=await $.get("/telephony/queues"),this.render(Ve);else if(I==="blf"){if(me.length===0)try{me=await $.get("/telephony/extensions")}catch(t){console.warn("Could not pre-fetch extensions for BLF mapping:",t)}Re=await $.get("/telephony/blfs"),this.render(Re)}else I==="users"&&(ze=await $.get("/telephony/users"),this.render(ze))}catch(t){console.error(`Error fetching telephony ${I}:`,t),e&&(e.innerHTML=`<tr><td colspan="10" style="text-align: center; padding: 2rem; color: #ef4444;">Erro ao carregar dados: ${t.message||"Erro de rede"}</td></tr>`)}},getActiveTableBody(){return I==="extensions"?document.getElementById("telephony-table-body"):I==="queues"?document.getElementById("telephony-queues-table-body"):I==="blf"?document.getElementById("telephony-blf-table-body"):I==="users"?document.getElementById("telephony-users-table-body"):null},render(e){const t=this.getActiveTableBody();if(!t)return;Oe=e;const n=e.length,o=Math.ceil(n/pe);M>o&&(M=Math.max(1,o)),M<1&&(M=1);const a=(M-1)*pe,s=e.slice(a,a+pe);if(s.length===0){const i=I==="extensions"?7:I==="queues"?6:I==="blf"?4:5;t.innerHTML=`
                <tr>
                    <td colspan="${i}" style="text-align: center; padding: 2rem; color: var(--text-muted);">
                        Nenhum registro encontrado.
                    </td>
                </tr>
            `,this.renderPaginationControls("telephony-pagination",0,0);return}I==="extensions"?this.renderExtensionsList(t,s):I==="queues"?this.renderQueuesList(t,s):I==="blf"?this.renderBlfsList(t,s):I==="users"&&this.renderUsersList(t,s),this.renderPaginationControls("telephony-pagination",o,n)},renderExtensionsList(e,t){e.innerHTML=t.map(n=>{const o=n.exten||"-",a=n.nome||"-",s=n.ddr||"-",i=n.Username||"-",r=n.Secret||"",c=n.regra_saida_nome?`<span class="badge" style="background: rgba(255,255,255,0.05); color: var(--text-main); font-size: 0.8rem; padding: 4px 8px; border-radius: 6px;">${n.regra_saida_nome}</span>`:"-",u=n.observacao||"-",d=r.replace(/'/g,"\\'");return`
                <tr>
                    <td>
            ...
            `}).join("")},toggleQueueRow(e){const t=document.getElementById(`queue-details-${e}`),n=document.getElementById(`queue-arrow-${e}`);t&&(t.classList.toggle("hidden"),n&&(t.classList.contains("hidden")?n.style.transform="rotate(0deg)":n.style.transform="rotate(180deg)"))},toggleBlfRow(e){const t=document.getElementById(`blf-details-${e}`),n=document.getElementById(`blf-arrow-${e}`);t&&(t.classList.toggle("hidden"),n&&(t.classList.contains("hidden")?n.style.transform="rotate(0deg)":n.style.transform="rotate(180deg)"))},toggleUserSecret(e){alert("Por segurança do PABX Gnew, as senhas dos usuários do portal são armazenadas com criptografia unidirecional na base e não podem ser lidas em texto claro.")},search(e){M=1;const n=this.getActiveDataList().filter(o=>I==="extensions"?(o.exten||"").toLowerCase().includes(e)||(o.nome||"").toLowerCase().includes(e)||(o.Username||"").toLowerCase().includes(e)||(o.ddr||"").toLowerCase().includes(e)||(o.observacao||"").toLowerCase().includes(e):I==="queues"?(o.exten||"").toLowerCase().includes(e)||(o.nome||"").toLowerCase().includes(e)||(o.Estrategia||"").toLowerCase().includes(e):I==="blf"?(o.Nome||"").toLowerCase().includes(e):I==="users"?(o.username||"").toLowerCase().includes(e)||(o.email||"").toLowerCase().includes(e)||(o.Tipo||"").toLowerCase().includes(e):!1);this.render(n)},changePage(e){M=e,this.render(Oe)},setPageSize(e){pe=parseInt(e,10),M=1,this.render(Oe)},toggleSecret(e,t){const n=document.getElementById(`secret-txt-${e}`),o=document.getElementById(`secret-icon-${e}`);!n||!o||(n.textContent==="••••••••"?(n.textContent=t,o.innerHTML=`
        ...
        `;const i=(M-1)*pe+1,r=Math.min(M*pe,n);a+=`
            <span class="pagination-info">
                Exibindo ${i}-${r} de ${n}
            </span>
        `,o.innerHTML=a}};let je=[],qe="",Ne="alerts",v=null;const Ze={init(){const e=document.getElementById("tab-monitoring-alerts");e&&e.addEventListener("click",()=>this.setActiveTab("alerts"));const t=document.getElementById("tab-monitoring-events");t&&t.addEventListener("click",()=>this.setActiveTab("events"));const n=document.getElementById("tab-monitoring-gnew");n&&n.addEventListener("click",()=>this.setActiveTab("gnew"));const o=document.getElementById("monitoring-events-search-input");o&&o.addEventListener("input",c=>{qe=c.target.value.toLowerCase(),this.render()});const a=document.getElementById("btn-refresh-monitoring");a&&a.addEventListener("click",()=>this.fetchDiagnostics());const s=document.getElementById("gnew-disk-accordion-header");s&&s.addEventListener("click",()=>{const c=document.getElementById("gnew-disk-accordion-content"),u=document.getElementById("gnew-disk-chevron");c&&u&&(c.style.maxHeight==="0px"?(c.style.maxHeight="1000px",u.style.transform="rotate(0deg)"):(c.style.maxHeight="0px",u.style.transform="rotate(-90deg)"))});const i=document.getElementById("btn-refresh-gnew-disk");i&&i.addEventListener("click",c=>{c.stopPropagation(),this.fetchDiagnostics()});const r=document.getElementById("btn-refresh-gnew-services");r&&r.addEventListener("click",async()=>{const c=r,u=c.querySelector("svg");if(!c.disabled){c.disabled=!0,c.style.opacity="0.6",c.style.cursor="not-allowed",u&&(u.style.animation="spin 0.8s linear infinite");try{await this.fetchDiagnostics()}finally{c.disabled=!1,c.style.opacity="",c.style.cursor="pointer",u&&(u.style.animation="")}}}),window.monitoringHandler=this},setActiveTab(e){Ne=e;const t=document.getElementById("tab-monitoring-alerts"),n=document.getElementById("tab-monitoring-events"),o=document.getElementById("tab-monitoring-gnew");t&&t.classList.toggle("active",e==="alerts"),n&&n.classList.toggle("active",e==="events"),o&&o.classList.toggle("active",e==="gnew");const a=document.getElementById("monitoring-tab-content-alerts"),s=document.getElementById("monitoring-tab-content-events"),i=document.getElementById("monitoring-tab-content-gnew");a&&(a.classList.toggle("hidden",e!=="alerts"),a.classList.toggle("active",e==="alerts")),s&&(s.classList.toggle("hidden",e!=="events"),s.classList.toggle("active",e==="events")),i&&(i.classList.toggle("hidden",e!=="gnew"),i.classList.toggle("active",e==="gnew")),e==="gnew"?this.fetchDiagnostics():this.render()},render(){Ne==="alerts"?this.renderGnewServicesStatus():this.renderEvents(je)},renderGnewServicesStatus(){const e=document.getElementById("monitoring-alerts-grid");if(!e)return;if(e.style.display="flex",e.style.flexDirection="column",e.style.gap="0",!v||!v.servicos||!Array.isArray(v.servicos.servicos)||v.servicos.servicos.length===0){e.innerHTML=`
            ...
            `;return}const t=v.servicos.servicos,n=t.length,o=t.filter(c=>c.status!=="active"&&c.status_label!=="ativo").length,a=n-o,s=document.getElementById("monitor-kpi-total"),i=document.getElementById("monitor-kpi-warning"),r=document.getElementById("monitor-kpi-info");s&&(s.textContent=n),i&&(i.textContent=o),r&&(r.textContent=a),e.innerHTML=`
            <div class="monitor-list">
                ...
                ${t.map((c,u)=>{const d=c.status==="active"||c.status_label==="ativo",p=d?"#10b981":"#ef4444",m=d?"Online":c.status_label||c.status||"Offline",f=d?"rgba(16,185,129,0.12)":"rgba(239,68,68,0.12)",g=d?"#6ee7b7":"#fca5a5",h=d?"rgba(16,185,129,0.3)":"rgba(239,68,68,0.3)";return`
                        <div class="monitor-list-row" style="background: ${u%2===0?"transparent":"rgba(255,255,255,0.015)"};">
                            ...
                            <div class="monitor-list-col-status">
                                <span class="monitor-badge" style="background:${f}; color:${g}; border-color:${h};">${m}</span>
                            </div>
                        </div>`}).join("")}
            </div>`},renderEvents(e){const t=document.getElementById("monitoring-events-grid");if(!t)return;t.style.display="flex",t.style.flexDirection="column",t.style.gap="0";const o=(e||[]).filter(a=>!qe||(a.title||"").toLowerCase().includes(qe));if(o.length===0){t.innerHTML=`
                <div style="text-align: center; padding: 4rem; color: var(--text-muted);">
            ...
            <div class="monitor-list">
                ...
                ${o.map((a,s)=>{const i=a.severity||"info";let r="Info",c="#3b82f6",u="rgba(59,130,246,0.12)",d="#93c5fd",p="rgba(59,130,246,0.3)";i==="critical"?(r="Crítico",c="#ef4444",u="rgba(239,68,68,0.12)",d="#fca5a5",p="rgba(239,68,68,0.3)"):i==="warning"?(r="Alerta",c="#f59e0b",u="rgba(245,158,11,0.12)",d="#fde047",p="rgba(245,158,11,0.3)"):i==="success"&&(r="Ok",c="#10b981",u="rgba(16,185,129,0.12)",d="#6ee7b7",p="rgba(16,185,129,0.3)");const m=a.created_at?new Date(a.created_at).toLocaleString("pt-BR"):"-";return`
                        <div class="monitor-list-row" style="background: ${s%2===0?"transparent":"rgba(255,255,255,0.015)"};">
                            <div class="monitor-list-col-name">
                                <span class="monitor-dot" style="background: ${c};"></span>
                                <div>
                                    <span class="monitor-svc-name">${a.title}</span>
                                    ${a.description?`<span class="monitor-svc-desc">${a.description}</span>`:""}
                                </div>
                            </div>
                            <div class="monitor-list-col-sev">
                                <span class="monitor-badge" style="background:${u}; color:${d}; border-color:${p};">${r}</span>
                            </div>
                            <div class="monitor-list-col-time">${m}</div>
                        </div>`}).join("")}
            </div>`},updateKPIs(e,t){const n=e-t,o=document.getElementById("monitor-kpi-total"),a=document.getElementById("monitor-kpi-warning"),s=document.getElementById("monitor-kpi-info");o&&(o.textContent=e),a&&(a.textContent=t),s&&(s.textContent=n)},async fetchDiagnostics(){try{const e=await $.get("/monitoring/diagnostico?t="+Date.now()),t=e&&e.status==="online";if(this.updateGnewApiStatus(t,t?"Gnew Online":"Gnew Offline (Contingência)",e?e.message:""),e&&e.data)v=e.data,this.renderGnewDiagnostics();else throw new Error("Dados inválidos na resposta da API.")}catch(e){console.error("Erro ao buscar diagnósticos da Gnew:",e),this.updateGnewApiStatus(!1,"Erro de Conexão",e.message)}},updateGnewApiStatus(e,t,n){const o=document.getElementById("gnew-api-status-badge"),a=document.getElementById("gnew-api-message");if(o){o.className=`api-status-badge ${e?"online":"offline"}`,o.style.background=e?"rgba(16, 185, 129, 0.1)":"rgba(239, 68, 68, 0.1)",o.style.color=e?"#6ee7b7":"#fca5a5",o.style.borderColor=e?"#10b981":"#ef4444";const s=o.querySelector(".status-text");s&&(s.textContent=t)}a&&n&&(a.textContent=n)},parseMemoryOutput(e){try{const n=e.split(`
...
`).find(o=>o.trim().endsWith(" /"));if(n){const o=n.trim().split(/\s+/);if(o.length>=5){const a=o[1],s=o[2],i=o[4].replace("%",""),r=parseInt(i,10);if(!isNaN(r))return{percentage:r,detail:`${s} em uso de ${a} (Montagem em /)`}}}}catch(t){console.warn("Erro ao fazer parse do disco:",t)}return{percentage:0,detail:"Erro no parse"}},renderGnewDiagnostics(){if(!v)return;if(v.memoria){let n={percentage:0,detail:"Dados de memória indisponíveis"};if(v.memoria.output)n=this.parseMemoryOutput(v.memoria.output);else if(typeof v.memoria.percent<"u"){const i=(v.memoria.total_mb/1024).toFixed(1),r=(v.memoria.used_mb/1024).toFixed(1);n={percentage:Math.round(v.memoria.percent),detail:`${r}GB em uso de ${i}GB total`}}const o=document.getElementById("gnew-kpi-mem-text"),a=document.getElementById("gnew-kpi-mem-bar"),s=document.getElementById("gnew-kpi-mem-detail");o&&(o.textContent=`${n.percentage}%`),a&&(a.style.width=`${n.percentage}%`),s&&(s.textContent=n.detail)}if(v.disco){let n={percentage:0,detail:"Dados de disco indisponíveis"};if(v.disco.output)n=this.parseDiskOutput(v.disco.output);else if(Array.isArray(v.disco)){const i=v.disco.find(r=>r.mountpoint==="/");i&&(n={percentage:Math.round(i.percent),detail:`${i.used_gb.toFixed(1)}GB em uso de ${i.total_gb.toFixed(1)}GB (Montagem em /)`})}const o=document.getElementById("gnew-kpi-disk-text"),a=document.getElementById("gnew-kpi-disk-bar"),s=document.getElementById("gnew-kpi-disk-detail");o&&(o.textContent=`${n.percentage}%`),a&&(a.style.width=`${n.percentage}%`),s&&(s.textContent=n.detail)}const e=document.getElementById("gnew-disk-table-body");if(e){let n=[];if(v.disco)if(v.disco.output)try{const o=v.disco.output.trim().split(`
`);for(let a=1;a<o.length;a++){const s=o[a].trim().split(/\s+/);s.length>=6&&n.push({mountpoint:s[5],total:s[1],used:s[2],free:s[3],percent:parseInt(s[4].replace("%",""),10)||0})}}catch(o){console.warn("Erro ao fazer parse da tabela de disco offline:",o)}else Array.isArray(v.disco)&&(n=v.disco.map(o=>({mountpoint:o.mountpoint,total:typeof o.total_gb=="number"?`${o.total_gb.toFixed(2)} GB`:o.total_gb||"0 GB",used:typeof o.used_gb=="number"?`${o.used_gb.toFixed(2)} GB`:o.used_gb||"0 GB",free:typeof o.free_gb=="number"?`${o.free_gb.toFixed(2)} GB`:o.free_gb||"0 GB",percent:typeof o.percent=="number"?Math.round(o.percent):parseInt(o.percent,10)||0})));n.length>0?e.innerHTML=n.map(o=>{const a=o.percent;return`
                ...
                `}if(v.servicos&&v.servicos.timestamp)try{const o=new Date(v.servicos.timestamp).toLocaleString("pt-BR"),a=document.getElementById("gnew-services-timestamp");a&&(a.textContent=`Última verificação: ${o}`)}catch(n){console.warn("Erro ao formatar timestamp dos serviços:",n)}const t=document.getElementById("gnew-services-list");if(t){let n=[];v.servicos&&Array.isArray(v.servicos.servicos)&&(n=v.servicos.servicos),n.length>0?(t.innerHTML=n.map(a=>{const s=a.status==="active"||a.status_label==="ativo",i=s?"rgba(16, 185, 129, 0.1)":"rgba(239, 68, 68, 0.1)",r=s?"#6ee7b7":"#fca5a5",c=s?"#10b981":"#ef4444",u=s?"#10b981":"#ef4444";return`
