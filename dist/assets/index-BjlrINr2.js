(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))o(a);new MutationObserver(a=>{for(const s of a)if(s.type==="childList")for(const i of s.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&o(i)}).observe(document,{childList:!0,subtree:!0});function n(a){const s={};return a.integrity&&(s.integrity=a.integrity),a.referrerPolicy&&(s.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?s.credentials="include":a.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function o(a){if(a.ep)return;a.ep=!0;const s=n(a);fetch(a.href,s)}})();const ve="/api",T={async get(t){const e=await fetch(`${ve}${t}`);if(!e.ok){const n=await e.json().catch(()=>({}));throw new Error(n.error||`HTTP error! status: ${e.status}`)}return await e.json()},async post(t,e){const n=await fetch(`${ve}${t}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)});if(!n.ok){const o=await n.json().catch(()=>({}));throw new Error(o.error||`HTTP error! status: ${n.status}`)}return await n.json()},async put(t,e){const n=await fetch(`${ve}${t}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)});if(!n.ok){const o=await n.json().catch(()=>({}));throw new Error(o.error||`HTTP error! status: ${n.status}`)}return await n.json()},async delete(t){const e=await fetch(`${ve}${t}`,{method:"DELETE"});if(!e.ok){const n=await e.json().catch(()=>({}));throw new Error(n.error||`HTTP error! status: ${e.status}`)}return await e.json()},async upload(t,e){const n=await fetch(`${ve}${t}`,{method:"POST",body:e});if(!n.ok){const o=await n.json().catch(()=>({}));throw new Error(o.error||`HTTP error! status: ${n.status}`)}return await n.json()}};let ue=null;const F={init(){const t=localStorage.getItem("user");if(t)try{return ue=JSON.parse(t),!0}catch{return this.logout(),!1}return!1},getUser(){return ue},isAdmin(){return ue&&ue.role==="Administrador"},async login(t,e){try{const n=await T.post("/login",{email:t,password:e});return ue=n,localStorage.setItem("user",JSON.stringify(n)),{success:!0,user:n}}catch(n){return{success:!1,error:n.message}}},logout(){ue=null,localStorage.removeItem("user")}},l={show(t){const e=document.getElementById(t);e&&e.classList.remove("hidden")},hide(t){const e=document.getElementById(t);e&&e.classList.add("hidden")},toggle(t,e){const n=document.getElementById(t);n&&n.classList.toggle("hidden",e)},setText(t,e){const n=document.getElementById(t);n&&(n.innerText=e)},setValue(t,e){const n=document.getElementById(t);n&&(n.value=e)},getValue(t){const e=document.getElementById(t);return e?e.value:null},on(t,e,n){const o=document.getElementById(t);o&&o.addEventListener(e,n)}},st={canvas:null,ctx:null,particles:[],animationFrameId:null,isActive:!1,init(){if(this.canvas=document.getElementById("account-network-bg"),!this.canvas)return;this.ctx=this.canvas.getContext("2d"),this.resize(),window.addEventListener("resize",()=>{this.isActive&&this.resize()});const t=window.innerWidth<=768;this.particleCount=t?30:60,this.connectDistance=150,this.particleColor="rgba(34, 211, 238, 0.5)",this.particles=[];for(let e=0;e<this.particleCount;e++)this.particles.push({x:Math.random()*this.canvas.width,y:Math.random()*this.canvas.height,vx:(Math.random()-.5)*1.5,vy:(Math.random()-.5)*1.5,radius:Math.random()*2+1})},resize(){if(!this.canvas)return;const t=document.getElementById("account-section");t&&(this.canvas.width=t.clientWidth,this.canvas.height=t.clientHeight)},updateAndDraw(){if(!(!this.isActive||!this.canvas)){this.ctx.clearRect(0,0,this.canvas.width,this.canvas.height);for(let t=0;t<this.particles.length;t++){const e=this.particles[t];e.x+=e.vx,e.y+=e.vy,(e.x<0||e.x>this.canvas.width)&&(e.vx*=-1),(e.y<0||e.y>this.canvas.height)&&(e.vy*=-1),this.ctx.beginPath(),this.ctx.arc(e.x,e.y,e.radius,0,Math.PI*2),this.ctx.fillStyle=this.particleColor,this.ctx.fill();for(let n=t+1;n<this.particles.length;n++){const o=this.particles[n],a=e.x-o.x,s=e.y-o.y,i=Math.sqrt(a*a+s*s);if(i<this.connectDistance){this.ctx.beginPath(),this.ctx.lineWidth=1;const r=1-i/this.connectDistance;this.ctx.strokeStyle=`rgba(34, 211, 238, ${r*.4})`,this.ctx.moveTo(e.x,e.y),this.ctx.lineTo(o.x,o.y),this.ctx.stroke()}}}this.animationFrameId=requestAnimationFrame(()=>this.updateAndDraw())}},start(){this.canvas||this.init(),this.isActive||(this.isActive=!0,this.resize(),this.updateAndDraw())},stop(){this.isActive=!1,this.animationFrameId&&(cancelAnimationFrame(this.animationFrameId),this.animationFrameId=null)}};let be=[];const Ke={async fetch(){try{be=await T.get("/users"),this.render(be)}catch(t){console.error("Error fetching Users:",t)}},getUsers(){return be},render(t){const e=document.getElementById("user-table-body");e&&(e.innerHTML=t.map(n=>{const o=n.role==="Administrador",a=F.isAdmin()?`
                ...
                <tr>
                    ...
                    <td>
                        <div style="display: flex; gap: 10px; align-items: center;">
                            <a href="${p.path}" target="_blank" class="btn-secondary" style="padding: 4px 10px; font-size: 0.85rem; text-decoration: none; display: inline-flex; align-items: center; gap: 5px;">
                                Ver
                            </a>
                            ${k}
                        </div>
            ...
            `,this.renderPaginationControls("doc-pagination",0,0);return}e.innerHTML=u.map(d=>{const g=d.mimetype==="application/pdf"?"📕":"🖼️",m=(d.size/1024).toFixed(1)+" KB",h=d.created_at?new Date(d.created_at).toLocaleDateString("pt-BR"):"-",p=d.mimetype==="application/pdf"?"PDF":"Imagem",f=a?`<button class="btn-delete" onclick="window.DocsHandler.delete(${d.id})" style="padding: 4px 10px; font-size: 0.85rem;">Deletar</button>`:"",y=d.start_date?new Date(d.start_date+"T00:00:00").toLocaleDateString("pt-BR"):"-",L=d.end_date?d.end_date==="Indefinido"?"Indefinido":new Date(d.end_date+"T00:00:00").toLocaleDateString("pt-BR"):"-";return o?`
                    <tr>
                        <td>
                            <span style="font-weight: 500; display: flex; align-items: center; gap: 8px;">
                                <span>${g}</span>
        ...
        `,o.innerHTML=a}};let ee=[],x={summaries:[]},lt=null,B=null,qe="list",xe=null,X=null,ct=null,_=1;const Ee=10;let He=[];const z={getPendingProcId(){return lt},async fetch(){try{_=1,ee=await T.get("/procedures"),this.renderTable(ee)}catch(t){console.error("Error fetching FAQs:",t)}},getFaqs(){return ee},setListingMode(t){qe=t,_=1,this.renderTable(He.length?He:ee)},renderTable(t){const e=document.getElementById("list-table-container"),n=document.getElementById("list-cards-container"),o=document.getElementById("proc-table-body");if(!e||!n||!o)return;He=t;const a=t.length,s=Math.ceil(a/Ee);_>s&&(_=Math.max(1,s)),_<1&&(_=1);const i=(_-1)*Ee,r=t.slice(i,i+Ee);qe==="list"?(l.show("list-table-container"),l.hide("list-cards-container"),r.length===0?o.innerHTML=`
            ...
            `).join("");const o=x.summaries.find(a=>a.id===B);o?(n.textContent=o.title,n.style.color="var(--text-main)",e.classList.remove("hidden")):(n.textContent="Nenhum sumário selecionado",n.style.color="var(--accent)",e.classList.add("hidden"))},selectSummary(t){B=t,this.renderProcedureBuilderSidebar(),this.renderProcedureBuilder()},updateSummaryTitle(t,e){const n=x.summaries.find(a=>a.id===t);n&&(n.title=e||"Sem título"),this.renderProcedureBuilderSidebar();const o=x.summaries.find(a=>a.id===B);o&&(document.getElementById("current-summary-name").textContent=o.title)},addSummary(){const t="sum_"+Date.now();x.summaries.push({id:t,title:`Sumário ${x.summaries.length+1}`,sections:[]}),B=t,this.renderProcedureBuilderSidebar(),this.renderProcedureBuilder()},removeSummary(t){confirm("Excluir este sumário apagará todos os campos dentro dele. Deseja continuar?")&&(x.summaries=x.summaries.filter(e=>e.id!==t),B===t&&(B=x.summaries.length>0?x.summaries[0].id:null),this.renderProcedureBuilderSidebar(),this.renderProcedureBuilder())},renderProcedureBuilder(){const t=document.getElementById("procedure-edit-container");if(!t)return;if(!B){t.innerHTML='<p class="empty-state">Crie um novo sumário na barra lateral para adicionar conteúdo.</p>';return}const e=x.summaries.find(o=>o.id===B);if(!e)return;const n=e.sections;if(n.length===0){t.innerHTML=`<p class="empty-state">Nenhum campo em "${e.title}". Clique em "+ Novo Container" para começar.</p>`;return}t.innerHTML=n.map((o,a)=>`
             ...
             </div>`).join("")},handleSumDragStart(t,e){xe="summary",X=e,t.dataTransfer.effectAllowed="move",setTimeout(()=>{t.target&&t.target.classList.add("dragging")},0)},handleSumDrop(t,e){if(t.preventDefault(),xe!=="summary"||X===null||X===e)return;const n=x.summaries.splice(X,1)[0];x.summaries.splice(e,0,n),this.renderProcedureBuilderSidebar()},handleSecDragStart(t,e,n){xe="container",X=e,ct=n,t.dataTransfer.effectAllowed="move",setTimeout(()=>{const o=t.target.nodeType===1?t.target.closest(".editable-section"):null;o&&o.classList.add("dragging")},0)},handleDragOver(t){t.preventDefault(),t.dataTransfer.dropEffect="move"},handleSecDrop(t,e,n){if(t.preventDefault(),xe!=="container"||X===null||ct!==n)return;const o=x.summaries.find(s=>s.id===n);if(!o||X===e)return;const a=o.sections.splice(X,1)[0];o.sections.splice(e,0,a),this.renderProcedureBuilder()},handleDragEnd(t){document.querySelectorAll(".editable-section.dragging").forEach(e=>e.classList.remove("dragging")),t&&t.target&&t.target.setAttribute&&t.target.setAttribute("draggable","false"),xe=null,X=null},updateSectionTitle(t,e){const n=x.summaries.find(o=>o.id===B);n&&(n.sections[t].title=e)},updateSectionData(t,e){const n=x.summaries.find(o=>o.id===B);n&&(n.sections[t].data=e)},removeSection(t){const e=x.summaries.find(n=>n.id===B);e&&e.sections.splice(t,1),this.renderProcedureBuilder()},getRteToolbarHTML(){return`
        ...
        `},addFaqItem(t){const e=x.summaries.find(n=>n.id===B);e&&(e.sections[t].data=e.sections[t].data||[],e.sections[t].data.push({q:"",a:""}),this.renderProcedureBuilder())},updateFaqItem(t,e,n,o){const a=x.summaries.find(s=>s.id===B);a&&(a.sections[t].data[e][n]=o)},removeFaqItem(t,e){const n=x.summaries.find(o=>o.id===B);n&&n.sections[t].data.splice(e,1),this.renderProcedureBuilder()},addSection(t,e){if(!B){alert("Selecione primeiro um sumário na barra lateral.");return}const n=x.summaries.find(o=>o.id===B);n&&(n.sections.push({id:Date.now(),title:t,type:e,data:e==="FAQ"?[]:e==="TEXTO"?"":null}),this.renderProcedureBuilder())},async handleSectionFileDrop(t,e){e.dataTransfer.files&&e.dataTransfer.files.length>0&&await this.uploadSectionFile(t,e.dataTransfer.files[0])},async handleSectionFileUpload(t,e){const n=e.files[0];n&&await this.uploadSectionFile(t,n)},async uploadSectionFile(t,e){const n=new FormData;n.append("file",e);try{const o=await T.upload("/upload",n),a=x.summaries.find(s=>s.id===B);a&&(a.sections[t].data={name:e.name,path:o.path,mimetype:e.type},this.renderProcedureBuilder())}catch{alert("Erro no upload")}},async handleSaveProcedure(){const t=parseInt(l.getValue("proc-id"));if(!t)return;const n={...ee.find(o=>o.id===t),content:JSON.stringify(x)};try{await T.put(`/procedures/${t}`,n),alert("Salvo com sucesso!"),this.toggleEditMode(!1),this.openDetail(t),this.fetch()}catch{alert("Erro ao salvar")}},search(t){_=1;const e=ee.filter(n=>(n.name||n.title||"").toLowerCase().includes(t)||(n.responsible||"").toLowerCase().includes(t)||(n.group_name||"").toLowerCase().includes(t));this.renderTable(e)},changePage(t){_=t,this.renderTable(He)},renderPaginationControls(t,e,n){const o=document.getElementById(t);if(!o)return;if(e===0){o.innerHTML="";return}let a="";a+=`
            <button class="pagination-btn" 
                    ${_===1?"disabled":""} 
        ...
        `,o.innerHTML=a}};window.toggleGhAccordion=function(t){const e=document.getElementById(t);e&&e.classList.toggle("open")};let j=[],me="list",ie="month",E=new Date,R=1;const $e=10;let dt=[];const P={async fetch(){try{R=1,j=await T.get("/accounts"),this.initDashboardMultiselects(),this.populateCompanyFilter(),this.handleSearch(),this.checkAccountAlerts()}catch(t){console.error("Falha ao obter contas",t)}},populateCompanyFilter(){const t=document.getElementById("dash-filter-company-dynamic-options");if(t){const e=new Set;t.querySelectorAll('input[type="checkbox"]:checked').forEach(a=>{e.add(a.value)});const n=[...new Set(j.map(a=>a.company_name).filter(Boolean))].sort((a,s)=>a.localeCompare(s));let o="";n.forEach(a=>{const s=e.has(a)?"checked":"";o+=`<label class="multiselect-option"><input type="checkbox" value="${a}" ${s}> <span>${a}</span></label>`}),t.innerHTML=o,this.setupMultiselectListeners("dash-filter-company")}},setupMultiselectListeners(t){if(!document.getElementById(`${t}-container`))return;const n=document.getElementById(`${t}-trigger`),o=document.getElementById(`${t}-dropdown`);if(!n||!o)return;n.dataset.listenerBound||(n.addEventListener("click",r=>{r.stopPropagation(),document.querySelectorAll(".multiselect-dropdown").forEach(c=>{c!==o&&c.classList.add("hidden")}),o.classList.toggle("hidden")}),n.dataset.listenerBound="true");const a=o.querySelector('input[value="Todos"]'),s=Array.from(o.querySelectorAll('input[type="checkbox"]')).filter(r=>r.value!=="Todos"),i=()=>{const r=s.filter(u=>u.checked).map(u=>u.value),c=n.querySelector(".trigger-label");a.checked||s.length>0&&r.length===s.length?(a.checked=!0,c&&(c.innerText="Todos")):r.length===0?c&&(c.innerText="Nenhum"):r.length===1?c&&(c.innerText=r[0]):c&&(c.innerText=`${r.length} selecionados`)};a&&!a.dataset.listenerBound&&(a.addEventListener("change",()=>{s.forEach(r=>{r.checked=a.checked}),i(),this.renderDashboard()}),a.dataset.listenerBound="true"),s.forEach(r=>{r.dataset.listenerBound||(r.addEventListener("change",()=>{s.every(u=>u.checked)?a.checked=!0:a.checked=!1,i(),this.renderDashboard()}),r.dataset.listenerBound="true")}),i()},initDashboardMultiselects(){this.setupMultiselectListeners("dash-filter-category"),window.multiselectOutsideClickListenerBound||(document.addEventListener("click",t=>{t.target.closest(".custom-multiselect-container")||document.querySelectorAll(".multiselect-dropdown").forEach(e=>{e.classList.add("hidden")})}),window.multiselectOutsideClickListenerBound=!0)},getMultiselectValues(t){const e=document.getElementById(`${t}-dropdown`);if(!e)return["Todos"];const n=e.querySelector('input[value="Todos"]');return n&&n.checked?["Todos"]:Array.from(e.querySelectorAll('input[type="checkbox"]:checked')).map(o=>o.value).filter(o=>o!=="Todos")},resetMultiselects(){["dash-filter-category","dash-filter-company"].forEach(t=>{const e=document.getElementById(`${t}-dropdown`);if(e){e.querySelectorAll('input[type="checkbox"]').forEach(a=>{a.checked=a.value==="Todos"});const o=document.getElementById(`${t}-trigger`);if(o){const a=o.querySelector(".trigger-label");a&&(a.innerText="Todos")}}})},getAccounts(){return j},setAccountsViewMode(t){me=t,this.handleSearch()},setCalendarSubView(t){ie=t,this.handleSearch()},shiftCalendarDate(t){ie==="day"?E.setDate(E.getDate()+t):ie==="month"?E.setMonth(E.getMonth()+t):ie==="year"&&E.setFullYear(E.getFullYear()+t),l.setValue("filter-day",E.getDate()),l.setValue("filter-month",E.getMonth()),l.setValue("filter-year",E.getFullYear()),this.handleSearch()},handleFilterChange(t=!1){if(t){const e=l.getValue("filter-cal-year")?parseInt(l.getValue("filter-cal-year")):E.getFullYear(),n=l.getValue("filter-cal-month")?parseInt(l.getValue("filter-cal-month")):E.getMonth();E=new Date(e,n,1)}else{const e=l.getValue("filter-year")?parseInt(l.getValue("filter-year")):E.getFullYear(),n=l.getValue("filter-month")?parseInt(l.getValue("filter-month")):E.getMonth(),o=l.getValue("filter-day")?parseInt(l.getValue("filter-day")):E.getDate();E=new Date(e,n,o)}l.setValue("filter-month",E.getMonth()),l.setValue("filter-year",E.getFullYear()),this.handleSearch()},handleSearch(){const t=(l.getValue("accounts-search")||"").toLowerCase();let e=j.filter(n=>n.company_name.toLowerCase().includes(t)||n.description&&n.description.toLowerCase().includes(t));if(me==="list"){R=1;const n=l.getValue("filter-status")||"",o=document.getElementById("filter-date-toggle"),a=o?o.checked:!1,s=E.getFullYear(),i=E.getMonth(),r=E.getDate();e=e.filter(c=>{if(n&&c.status!==n)return!1;if(!a||!c.due_date)return!0;const[u,d,g]=c.due_date.split("-"),m=parseInt(u,10),h=parseInt(d,10)-1,p=parseInt(g,10);return c.type==="Único"?m===s&&h===i&&p===r:c.type==="Recorrente"?p===r:!0}),this.renderAccountsList(e)}else me==="notificacoes"?this.renderNotifications():me==="dashboard"?this.renderDashboard():this.renderCalendarWrapper(e)},checkAccountAlerts(){let t=!1;const e=new Date;e.setHours(0,0,0,0),j.forEach(o=>{const a=(o.status||"").trim().toLowerCase(),s=(o.payment_status||"").trim().toLowerCase();if(a==="on"&&s==="pendente"&&o.due_date){const[i,r,c]=o.due_date.split("-");let u=new Date(parseInt(i,10),parseInt(r,10)-1,parseInt(c,10));u.setHours(0,0,0,0),u.getTime()<=e.getTime()&&(t=!0)}});const n=document.getElementById("icon-alert-bell");n&&(t?n.classList.add("alert-pulse"):n.classList.remove("alert-pulse"))},renderNotifications(){const t=document.getElementById("accounts-notifications-body");if(!t)return;t.innerHTML="";const e=new Date;e.setHours(0,0,0,0);let n=j.filter(o=>{const a=(o.status||"").trim().toLowerCase(),s=(o.payment_status||"").trim().toLowerCase();if(a!=="on"||s!=="pendente"||!o.due_date)return!1;const[i,r,c]=o.due_date.split("-");let u=new Date(parseInt(i,10),parseInt(r,10)-1,parseInt(c,10));return u.setHours(0,0,0,0),u.getTime()<=e.getTime()});if(n.length===0){t.innerHTML='<tr><td colspan="6" style="text-align:center; color: var(--text-muted); padding: 20px;">Nenhuma conta urgente ou atrasada.</td></tr>';return}n.forEach(o=>{const a=document.createElement("tr");let s="Sem Data";if(o.due_date){const r=o.due_date.split("-");r.length===3&&(s=`${r[2]}/${r[1]}/${r[0]}`)}const i=F.isAdmin()?`
            ...
            `,e.appendChild(r)}),this.renderPaginationControls("accounts-list-pagination",o,n),this.renderDashboard()},renderDashboard(){if(me!=="dashboard")return;this.initDashboardMultiselects();const t=l.getValue("dash-filter-start"),e=l.getValue("dash-filter-end"),n=l.getValue("dash-filter-type")||"Todos",o=l.getValue("dash-filter-status")||"Todos",a=l.getValue("dash-filter-payment")||"Todos",s=this.getMultiselectValues("dash-filter-category"),i=this.getMultiselectValues("dash-filter-company");let r=t?new Date(t+"T00:00:00"):null,c=e?new Date(e+"T23:59:59"):null;if(!r&&!c){const b=new Date;r=new Date(b.getFullYear(),b.getMonth(),1,0,0,0),c=new Date(b.getFullYear(),b.getMonth()+1,0,23,59,59)}else r?c||(c=new Date(2100,11,31)):r=new Date(2e3,0,1);let u=0,d=0,g=new Set,m=new Set,h=0,p=0,f=0,y="-",L=0,$=0,k={},N={},V={};j.forEach(b=>{if(!b.due_date||n!=="Todos"&&b.type!==n||o!=="Todos"&&b.status!==o||a!=="Todos"&&b.payment_status!==a)return;if(!s.includes("Todos")){if(s.length===0)return;const C=b.category||"Outros";if(!s.includes(C))return}if(!i.includes("Todos")&&(i.length===0||!i.includes(b.company_name)))return;let A=0,O=new Date(r);O.setHours(0,0,0,0);let U=new Date(c);U.setHours(0,0,0,0);let Y=3650;for(;O<=U&&Y>0;){if(this.isEventOnDate(b,O.getFullYear(),O.getMonth(),O.getDate())){A++;const C=`${O.getFullYear()}-${String(O.getMonth()+1).padStart(2,"0")}`;V[C]||(V[C]={total:0,pago:0,pendente:0,fixo:0,variavel:0});const J=parseFloat(b.value||0);V[C].total+=J,b.payment_status==="Pago"&&(V[C].pago+=J),b.payment_status==="Pendente"&&(V[C].pendente+=J),b.type==="Recorrente"&&(V[C].fixo+=J),b.type==="Único"&&(V[C].variavel+=J)}O.setDate(O.getDate()+1),Y--}if(A>0){const C=parseFloat(b.value||0)*A;u+=C,d+=A,g.add(b.category||"Outros"),m.add(b.company_name),b.payment_status==="Pago"&&(h+=C),b.payment_status==="Pendente"&&(p+=C),b.type==="Recorrente"&&(L+=C),b.type==="Único"&&($+=C),C>f&&(f=C,y=b.company_name);const J=b.category||"Outros";N[J]=(N[J]||0)+C;const de=b.company_name||"Sem Empresa";k[de]=(k[de]||0)+C}}),l.setText("dash-metric-valor","R$ "+u.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})),l.setText("dash-metric-contas",d.toString()),l.setText("dash-metric-tipos",g.size.toString()),l.setText("dash-metric-empresas",m.size.toString()),l.setText("dash-metric-pago","R$ "+h.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})),l.setText("dash-metric-pendente","R$ "+p.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})),l.setText("dash-metric-maior-valor","R$ "+f.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})),l.setText("dash-metric-maior-nome",y),l.setText("dash-metric-fixo","R$ "+L.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})),l.setText("dash-metric-variavel","R$ "+$.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2}));const w=l.getValue("dash-sort-empresas")||"desc",D=l.getValue("dash-sort-categorias")||"desc";this.renderTierList("dash-list-empresas",k,w),this.renderTierList("dash-list-categorias",N,D),this.renderTimeChart(V)},renderTimeChart(t){window.timeChartInstance&&window.timeChartInstance.destroy();const e=document.getElementById("chart-dashboard-time");if(!e)return;const n=Object.keys(t).sort(),o=n.map(d=>{const[g,m]=d.split("-");return`${m}/${g}`}),a=n.map(d=>t[d].total),s=n.map(d=>t[d].pago),i=n.map(d=>t[d].pendente),r=n.map(d=>t[d].fixo),c=n.map(d=>t[d].variavel),u={type:"line",data:{labels:o,datasets:[{label:"Valor Total (R$)",data:a,borderColor:"#3b82f6",backgroundColor:"rgba(59, 130, 246, 0.1)",borderWidth:2,pointBackgroundColor:"#3b82f6",pointRadius:4,fill:!0,tension:.3},{label:"Total Pago (R$)",data:s,borderColor:"#4ade80",backgroundColor:"transparent",borderWidth:2,pointBackgroundColor:"#4ade80",pointRadius:4,fill:!1,tension:.3},{label:"Total Pendente (R$)",data:i,borderColor:"#facc15",backgroundColor:"transparent",borderWidth:2,pointBackgroundColor:"#facc15",pointRadius:4,fill:!1,tension:.3},{label:"Custo Fixo (R$)",data:r,borderColor:"#60a5fa",backgroundColor:"transparent",borderWidth:2,pointBackgroundColor:"#60a5fa",pointRadius:4,fill:!1,tension:.3},{label:"Custo Variável (R$)",data:c,borderColor:"#c084fc",backgroundColor:"transparent",borderWidth:2,pointBackgroundColor:"#c084fc",pointRadius:4,fill:!1,tension:.3}]},options:{responsive:!0,maintainAspectRatio:!1,interaction:{mode:"index",intersect:!1},plugins:{legend:{position:"top",labels:{color:getComputedStyle(document.documentElement).getPropertyValue("--text-main").trim()||"#e2e8f0",usePointStyle:!0,boxWidth:8}},tooltip:{callbacks:{label:function(d){let g=d.dataset.label||"";return g&&(g+=": "),d.parsed.y!==null&&(g+=new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(d.parsed.y)),g}}}},scales:{y:{beginAtZero:!0,grid:{color:"rgba(255, 255, 255, 0.05)",drawBorder:!1},ticks:{color:getComputedStyle(document.documentElement).getPropertyValue("--text-muted").trim()||"#94a3b8",callback:function(d,g,m){return new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL",maximumFractionDigits:0}).format(d)}}},x:{grid:{color:"rgba(255, 255, 255, 0.05)",drawBorder:!1},ticks:{color:getComputedStyle(document.documentElement).getPropertyValue("--text-muted").trim()||"#94a3b8"}}}}};window.timeChartInstance=new Chart(e.getContext("2d"),u)},renderTierList(t,e,n){const o=document.getElementById(t);if(!o)return;const a=Object.entries(e);if(a.length===0){o.innerHTML='<div style="color: var(--text-muted); text-align: center; font-size: 0.9rem; padding: 10px;">Nenhum dado encontrado no período</div>';return}a.sort((r,c)=>n==="asc"?r[1]-c[1]:c[1]-r[1]);const s=a.slice(0,10);let i="";s.forEach(([r,c],u)=>{const d=u===0&&n==="desc",g=d?"🏆 ":u+1+". ";i+=`
            ...
            <div class="smc-grid">
        `;for(let p=0;p<a;p++)h+='<div class="smc-day empty"></div>';for(let p=1;p<=s;p++)h+=`<div class="smc-day ${p===o?"active":""}" onclick="window.AccountsHandler.selectDateFromMiniCalendar(${e}, ${n}, ${p})">${p}</div>`;h+="</div>",t.forEach(p=>{p&&(p.innerHTML=h)})},changeMiniCalendarMonthYear(t,e){let n=E.getDate();const o=new Date(t,parseInt(e)+1,0).getDate();n>o&&(n=o),E=new Date(t,e,n);try{l.setValue("filter-cal-year",t),l.setValue("filter-cal-month",e)}catch{}this.handleSearch(),this.renderSidebarMiniCalendar()},selectDateFromMiniCalendar(t,e,n){E=new Date(t,e,n);try{l.setValue("filter-cal-year",t),l.setValue("filter-cal-month",e)}catch{}if(me==="calendar"){const o=document.getElementById("toggle-accounts-cal-day");o&&o.click()}else this.handleSearch(),this.renderSidebarMiniCalendar()},renderCalendarMonth(t,e,n){const o=["Janeiro","Fevereiro","Março","Abril","Maio","Junho","Julho","Agosto","Setembro","Outubro","Novembro","Dezembro"];l.setText("calendar-date-display",`${o[n]} ${e}`);const a=document.getElementById("calendar-month-grid");a.innerHTML="";const s=new Date(e,n,1).getDay(),i=new Date(e,n+1,0).getDate(),r=new Date,c=r.getFullYear()===e&&r.getMonth()===n;new Date(r.getFullYear(),r.getMonth(),1);for(let d=0;d<s;d++)a.innerHTML+='<div class="calendar-day empty"></div>';for(let d=1;d<=i;d++){const g=c&&r.getDate()===d?"today":"";a.innerHTML+=`<div class="calendar-day ${g}" id="cal-day-cell-${d}">
                <div class="calendar-date">${d}</div>
                <div class="calendar-events" id="cal-events-${d}"></div>
            </div>`}this.getLatestRecorrenteAccounts(t).forEach(d=>{if(!d.due_date)return;const g=new Date(e,n,1),m=new Date(r.getFullYear(),r.getMonth(),1);let h=!0;if(d.status==="Off"&&g.getTime()>=m.getTime()&&(h=!1),!!h){for(let p=1;p<=i;p++)if(this.isEventOnDate(d,e,n,p)){const f=document.getElementById(`cal-events-${p}`);if(f){const y=`${e}-${String(n+1).padStart(2,"0")}-${String(p).padStart(2,"0")}`;let L=d.payment_status==="Pago"?"event-paid":d.payment_status==="Pendente"?"event-pending":"event-canceled";d.type==="Recorrente"&&y!==d.due_date&&(L="event-pending");const $=document.createElement("div");$.className=`event-pill event-${d.type.toLowerCase()} ${L}`,$.title=d.company_name,$.innerText=d.company_name,$.onclick=k=>{this.openDedicatedPage(d.id,y)},f.appendChild($)}}}})},renderCalendarDay(t,e,n,o){const a=["Janeiro","Fevereiro","Março","Abril","Maio","Junho","Julho","Agosto","Setembro","Outubro","Novembro","Dezembro"];l.setText("calendar-date-display",`${String(o).padStart(2,"0")} de ${a[n]} de ${e}`);const s=document.getElementById("calendar-day-list");s.innerHTML="";const i=new Date(e,n,o),r=new Date;r.setHours(0,0,0,0),i.setHours(0,0,0,0);let c=0;this.getLatestRecorrenteAccounts(t).forEach(d=>{let g=!0;if(d.status==="Off"&&i.getTime()>=r.getTime()&&(g=!1),!!g&&this.isEventOnDate(d,e,n,o)){c++;const m=`${e}-${String(n+1).padStart(2,"0")}-${String(o).padStart(2,"0")}`;let h=d.payment_status==="Pago"?"#4ade80":d.payment_status==="Pendente"?"#facc15":"#ef4444";d.type==="Recorrente"&&m!==d.due_date&&(h="#facc15"),s.innerHTML+=`
        ...
        `,o.innerHTML=a}};let le=[],W={},ne=null,Ne=null,Ue=null,q=[],Re=[],Xe={},K={},ye,et,ze,je,pt,Oe;const gt={init(){ye=document.getElementById("timeline-event-form"),et=document.getElementById("view-visualizacao"),ze=document.getElementById("view-attention"),je=document.getElementById("view-anexo"),pt=document.getElementById("view-relatorio"),Oe=document.getElementById("view-config"),window.timelineHandler=gt,window.applyFilters=Tt,window.clearFilters=Lt,window.toggleFilters=It,window.handleDelete=$t,window.resetForm=ot,window.toggleAccordion=ht,window.handleFormSubmit=mt,window.editEvent=nt,window.deleteTopic=Mt,window.deleteSubtopic=At,window.handleTrackDragStart=Ht,window.handleTrackDragOver=_t,window.handleTrackDragEnd=Pt;const t=document.getElementById("timeline-topic-form");t&&t.addEventListener("submit",Dt);const e=document.getElementById("timeline-subtopic-form");e&&e.addEventListener("submit",St);const n=document.getElementById("topico");n&&n.addEventListener("change",u=>{tt(u.target.value)});const o=document.getElementById("em-ocorrencia");o&&o.addEventListener("change",u=>{const d=document.getElementById("fim"),g=document.getElementById("inicio");if(u.target.checked){if(!g.value){const m=new Date;m.setMinutes(m.getMinutes()-m.getTimezoneOffset()),g.value=m.toISOString().slice(0,16)}d.required=!1}else{const m=new Date;m.setMinutes(m.getMinutes()-m.getTimezoneOffset()),d.value=m.toISOString().slice(0,16),d.required=!0}});const a=document.getElementById("auto-refresh-toggle");a&&a.addEventListener("change",u=>{ft(u.target.checked)}),document.querySelectorAll("[data-timeline-tab]").forEach(u=>{u.addEventListener("click",d=>{const g=d.currentTarget.getAttribute("data-timeline-tab");Me(g)})}),ye&&ye.addEventListener("submit",mt);const s=document.getElementById("rep-filter-start"),i=document.getElementById("rep-filter-end"),r=document.getElementById("rep-filter-topic"),c=document.getElementById("rep-filter-subtopic");s&&s.addEventListener("change",()=>ke()),i&&i.addEventListener("change",()=>ke()),r&&r.addEventListener("change",u=>{Ct(u.target.value),ke()}),c&&c.addEventListener("change",()=>ke()),window.addEventListener("SectionChange",u=>{u.detail&&u.detail.section==="timeline"&&ce().then(()=>{Z(),ut()})}),ce().then(()=>{Z(),ut()})}};window.addEventListener("focus",()=>{et&&Z()});function tt(t,e=null){const n=document.getElementById("sub-topico");if(!n)return;const o=t?t.toLowerCase().trim():"";if(!o||!K[o]){n.innerHTML='<option value="">Selecione o tópico primeiro...</option>',n.classList.remove("has-options");return}n.innerHTML='<option value="" disabled selected>Escolha o evento...</option>',K[o].forEach(a=>{const s=document.createElement("option");s.value=a.toLowerCase(),s.textContent=a,e&&s.value===e.toLowerCase()&&(s.selected=!0),n.appendChild(s)}),e||(n.selectedIndex=1),n.classList.add("has-options")}async function ce(){try{const t=await fetch("/api/timeline/config");if(!t.ok)throw new Error("Falha ao buscar configurações");const e=await t.json();q=e.topics||[],Re=e.subtopics||[],Xe={},K={},q.forEach(o=>{Xe[o.id]=o.color,K[o.id]=[]}),Re.forEach(o=>{const a=o.topic_id;K[a]&&K[a].push(o.name)}),Et();const n=document.getElementById("view-config");n&&n.classList.contains("active")&&vt()}catch(t){console.error("Error loading config:",t)}}function Et(){const t=document.getElementById("topico");if(t){const o=t.value;t.innerHTML='<option value="" disabled selected>Selecione um tópico...</option>',q.forEach(a=>{const s=document.createElement("option");s.value=a.id,s.textContent=a.name,t.appendChild(s)}),t.value=o}const e=document.getElementById("rep-filter-topic");if(e){const o=e.value;e.innerHTML='<option value="Todos">Todos</option>',q.forEach(a=>{const s=document.createElement("option");s.value=a.id,s.textContent=a.name,e.appendChild(s)}),o&&[...e.options].some(a=>a.value===o)?e.value=o:e.value="Todos"}const n=document.getElementById("subtopic-topic-id");n&&(n.innerHTML='<option value="" disabled selected>Selecione um tópico...</option>',q.forEach(o=>{const a=document.createElement("option");a.value=o.id,a.textContent=o.name,n.appendChild(a)}))}function Z(){fetch("/api/timeline/events").then(t=>{if(!t.ok)throw new Error("Failed to fetch");return t.json()}).then(t=>{le=t,at(),ze&&ze.classList.contains("active")&&yt()}).catch(t=>{console.error("Error loading events:",t)})}function ut(){const t=document.getElementById("timeline-tab-anexo"),e=document.getElementById("timeline-tab-config");if(window.auth&&window.auth.isAdmin())t&&t.classList.remove("role-hidden"),e&&e.classList.remove("role-hidden");else{t&&t.classList.add("role-hidden"),e&&e.classList.add("role-hidden");const o=je&&je.classList.contains("active"),a=Oe&&Oe.classList.contains("active");(o||a)&&Me("visualizacao")}}function Me(t){const e={visualizacao:{section:et,button:document.querySelector('[data-timeline-tab="visualizacao"]')},attention:{section:ze,button:document.querySelector('[data-timeline-tab="attention"]')},anexo:{section:je,button:document.querySelector('[data-timeline-tab="anexo"]')},relatorio:{section:pt,button:document.querySelector('[data-timeline-tab="relatorio"]')},config:{section:Oe,button:document.querySelector('[data-timeline-tab="config"]')}};Object.values(e).forEach(n=>{n.section&&n.section.classList.remove("active"),n.button&&n.button.classList.remove("active")}),e[t]&&(e[t].section&&e[t].section.classList.add("active"),e[t].button&&e[t].button.classList.add("active")),t==="visualizacao"?(Z(),Te(!0)):t==="attention"?(yt(),Te(!0)):t==="relatorio"?(ke(),Te(!1)):(t==="config"&&vt(),Te(!1))}function Te(t){const e=document.getElementById("floating-refresh-control");if(e)if(t){e.classList.remove("hidden");const n=document.getElementById("auto-refresh-toggle");n&&n.checked&&!ne&&ft(!0)}else e.classList.add("hidden"),ne&&(clearInterval(ne),ne=null)}function ft(t){ne&&(clearInterval(ne),ne=null),t&&(Z(),ne=setInterval(Z,6e4))}function mt(t){t.preventDefault();const e=ye.querySelector('button[type="submit"]');e&&(e.textContent="Salvando...",e.disabled=!0);const o={id:document.getElementById("event-id").value||Date.now().toString(),nome:document.getElementById("nome").value,topico:document.getElementById("topico").value,sub_topico:document.getElementById("sub-topico").value,em_ocorrencia:document.getElementById("em-ocorrencia").checked?1:0,inicio:document.getElementById("inicio").value,fim:document.getElementById("fim").value,descricao:document.getElementById("descricao").value,anotacao:document.getElementById("anotacao").value,cor:document.getElementById("cor").value};fetch("/api/timeline/events",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)}).then(async a=>{const s=await a.text();if(!a.ok)throw new Error(`Server error (${a.status}): ${s}`);return JSON.parse(s)}).then(()=>{alert("Evento salvo com sucesso!"),ot(),Me("visualizacao")}).catch(a=>{console.error("Error saving event:",a),alert("Erro ao salvar evento: "+a.message)}).finally(()=>{e&&(e.textContent="Salvar Evento",e.disabled=!1)})}function nt(t){const e=le.find(s=>s.id===t);if(!e)return;document.getElementById("event-id").value=e.id,document.getElementById("nome").value=e.nome;const n=Ce(e.topico);document.getElementById("topico").value=n,tt(n,e.sub_topico);const o=document.getElementById("em-ocorrencia");o.checked=e.em_ocorrencia==1||e.em_ocorrencia==="true"||!e.fim,o.dispatchEvent(new Event("change")),document.getElementById("inicio").value=e.inicio,document.getElementById("fim").value=e.fim||"",document.getElementById("descricao").value=e.descricao||"",document.getElementById("anotacao").value=e.anotacao||"",document.getElementById("cor").value=e.cor||"#000000",Me("anexo");const a=document.getElementById("btn-delete");a&&(a.style.display="block")}function ot(){ye&&ye.reset();const t=document.getElementById("event-id");t&&(t.value=""),tt("");const e=document.getElementById("fim");e&&(e.required=!0);const n=document.getElementById("cor");n&&(n.value="#000000");const o=document.getElementById("btn-delete");o&&(o.style.display="none")}function $t(){const t=document.getElementById("event-id").value;t&&confirm("Tem certeza que deseja excluir este evento?")&&fetch(`/api/timeline/events/${t}`,{method:"DELETE"}).then(e=>{if(!e.ok)throw new Error("Failed to delete");return e.json()}).then(()=>{alert("Evento excluído!"),ot(),Me("visualizacao")}).catch(e=>{console.error("Error deleting:",e),alert("Erro ao excluir: "+e.message)})}function Tt(t){const e=document.getElementById(`filter-start-${t}`),n=document.getElementById(`filter-end-${t}`),o=document.getElementById(`filter-sub-topic-${t}`),a=e&&e.value?new Date(e.value).getTime():null,s=n&&n.value?new Date(n.value).getTime():null,i=o?o.value:"";W[t]={start:a,end:s,subTopic:i},at()}function Lt(t){const e=document.getElementById(`filter-start-${t}`),n=document.getElementById(`filter-end-${t}`),o=document.getElementById(`filter-sub-topic-${t}`);e&&(e.value=""),n&&(n.value=""),o&&(o.value=""),W[t]=null,at()}function It(t){const e=document.getElementById(`filters-panel-${t}`),n=document.getElementById(`btn-toggle-${t}`);e&&n&&(e.classList.toggle("hidden"),n.classList.toggle("active"))}function ht(t){const e=document.getElementById(t);e&&e.classList.toggle("active")}function at(){const t=document.getElementById("timeline-tracks-container");if(!t)return;const e=Array.from(t.querySelectorAll(".timeline-container")).map(a=>a.dataset.topicId),n=q.map(a=>a.id);if(e.length!==n.length||!n.every(a=>e.includes(a))){t.innerHTML="";const a=window.auth&&window.auth.isAdmin(),s=a?'style="cursor: grab;"':"";q.forEach(i=>{const r=`
            ...
            `;t.insertAdjacentHTML("beforeend",r);const c=document.getElementById(`filter-sub-topic-${i.id}`);c&&K[i.id]&&K[i.id].forEach(u=>{const d=document.createElement("option");d.value=u.toLowerCase(),d.textContent=u,c.appendChild(d)})})}q.forEach(a=>{const s=document.getElementById(`track-${a.id}`),i=document.getElementById(`min-date-${a.id}`),r=document.getElementById(`max-date-${a.id}`);s&&(s.innerHTML=""),i&&(i.textContent=""),r&&(r.textContent="")}),le.length!==0&&q.forEach(a=>{const s=a.id,i=le.filter(y=>Ce(y.topico)===s);let r=i;W[s]&&W[s].subTopic&&(r=i.filter(y=>(y.sub_topico?y.sub_topico.toLowerCase():"")===W[s].subTopic.toLowerCase()));const c=W[s]&&W[s].start?W[s].start:new Date("2026-01-01T00:00:00").getTime(),u=W[s]&&W[s].end?W[s].end:Date.now();Bt(s,r,c,u);const d=c,g=u,m=g-d,h=document.getElementById(`min-date-${s}`),p=document.getElementById(`max-date-${s}`);h&&(h.textContent=new Date(d).toLocaleDateString()+" "+new Date(d).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})),p&&(p.textContent=new Date(g).toLocaleDateString()+" "+new Date(g).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}));const f=document.getElementById(`track-${s}`);f&&r.forEach(y=>{const L=new Date(y.inicio).getTime(),$=y.fim?new Date(y.fim).getTime():Date.now();if($<d||L>g)return;const k=Math.max(L,d),N=Math.min($,g),V=(k-d)/m*100,w=(N-k)/m*100;if(w<=0)return;const D=document.createElement("div");D.className="timeline-bar",D.style.left=`${V}%`,D.style.width=`${w}%`,D.style.color=y.cor&&y.cor!=="#000000"?y.cor:Xe[s]||"#6b7280";const b=document.createElement("div");b.className="timeline-bar-visual",D.appendChild(b);const A=document.createElement("div");A.className="timeline-identifier-point";const O=new Date(y.inicio).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),U=y.fim?new Date(y.fim).toLocaleString([],{dateStyle:"short",timeStyle:"short"}):"Em andamento",Y=a.name,C=y.sub_topico?y.sub_topico.charAt(0).toUpperCase()+y.sub_topico.slice(1):"-";A.setAttribute("data-tooltip",`Tópico: ${Y}
Eventos: ${C}
Início: ${O} - Fim: ${U}
Descrição: ${y.descricao||"-"}`),!y.fim&&A.classList.add("pulsing"),window.auth&&window.auth.isAdmin()?(A.style.cursor="pointer",A.onclick=de=>{de.stopPropagation(),nt(y.id)}):A.style.cursor="default",D.appendChild(A),f.appendChild(D)})})}function Ce(t){return t?t.toLowerCase().trim():""}function Bt(t,e,n,o){const a=document.getElementById(`sla-${t}`);if(!a)return;const s=o-n;if(s<=0){a.textContent="N/A";return}const r=e.filter(m=>{const h=new Date(m.inicio).getTime();return(m.fim?new Date(m.fim).getTime():Date.now())>n&&h<o}).map(m=>({start:Math.max(new Date(m.inicio).getTime(),n),end:Math.min(m.fim?new Date(m.fim).getTime():Date.now(),o)}));r.sort((m,h)=>m.start-h.start);const c=[];if(r.length>0){let m=r[0];for(let h=1;h<r.length;h++){const p=r[h];p.start<m.end?m.end=Math.max(m.end,p.end):(c.push(m),m=p)}c.push(m)}let u=0;c.forEach(m=>{u+=m.end-m.start});const d=(s-u)/s*100;let g="#10b981";d<50?g="#ef4444":d<90&&(g="#f97316"),a.style.color=g,a.textContent=d.toFixed(4)+"%"}function yt(){const t=document.getElementById("attention-topics-container");if(!t)return;t.innerHTML="";const e=le.filter(n=>!n.fim);q.forEach(n=>{const o=n.id,a=e.filter(f=>Ce(f.topico)===o),s=document.createElement("div");s.className=a.length>0?"accordion-item active":"accordion-item",s.id=`attn-acc-${o}`;const i=document.createElement("div");i.className="accordion-header",i.onclick=()=>ht(`attn-acc-${o}`);const r=document.createElement("div");r.className="accordion-title-group";const c=document.createElement("div");c.className="topic-indicator",c.style.backgroundColor=n.color;const u=document.createElement("h3");u.textContent=n.name;const d=document.createElement("span");d.style.cssText="background: #f1f5f9; padding: 2px 8px; border-radius: 12px; font-size: 0.95rem; font-weight: 900; color: #0f172a; margin-left: 0.5rem; border: 1px solid #cbd5e1;",d.textContent=`${a.length}`,r.appendChild(c),r.appendChild(u),r.appendChild(d);const g=document.createElement("span");g.className="accordion-chevron",g.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>',i.appendChild(r),i.appendChild(g);const m=document.createElement("div");m.className="accordion-content";const h=document.createElement("div");h.className="accordion-body";const p=document.createElement("div");if(p.className="attention-carousel",a.length===0){const f=document.createElement("div");f.className="empty-state",f.textContent="Nenhum evento em andamento.",p.appendChild(f)}else a.forEach(f=>{const y=document.createElement("div");y.className="attention-card",y.style.borderLeftColor=f.cor&&f.cor!=="#000000"?f.cor:n.color;const L=document.createElement("h3");L.textContent=f.nome;const $=document.createElement("div");$.className="sub-topic",$.textContent=f.sub_topico||"-";const k=document.createElement("div");k.className="card-detail",k.innerHTML=`<strong>Início:</strong> ${new Date(f.inicio).toLocaleString()}`;const N=Date.now()-new Date(f.inicio).getTime(),V=document.createElement("div");V.className="card-duration",V.innerHTML=`<strong>Tempo:</strong> <span>${kt(N)}</span>`;const w=document.createElement("div");w.className="card-description",w.textContent=f.descricao||"-",y.appendChild(L),y.appendChild($),y.appendChild(k),y.appendChild(V),y.appendChild(w),window.auth&&window.auth.isAdmin()?(y.style.cursor="pointer",y.onclick=()=>nt(f.id)):y.style.cursor="default",p.appendChild(y)});h.appendChild(p),m.appendChild(h),s.appendChild(i),s.appendChild(m),t.appendChild(s)})}function kt(t){if(t<0)return"0s";const e=Math.floor(t/1e3),n=Math.floor(e/60),o=Math.floor(n/60),a=Math.floor(o/24),s=[];return a>0&&s.push(`${a}d`),(o%24>0||a>0)&&s.push(`${o%24}h`),(n%60>0||o>0)&&s.push(`${n%60}m`),s.push(`${e%60}s`),s.join(" ")}function Ct(t){const e=document.getElementById("rep-filter-subtopic");if(!e)return;e.innerHTML='<option value="Todos">Todos</option>';const n=t?t.toLowerCase().trim():"";n&&K[n]&&K[n].forEach(o=>{const a=document.createElement("option");a.value=o.toLowerCase(),a.textContent=o,e.appendChild(a)})}function ke(){let t=le;const e=document.getElementById("rep-filter-start")?.value,n=document.getElementById("rep-filter-end")?.value,o=document.getElementById("rep-filter-topic")?.value,a=document.getElementById("rep-filter-subtopic")?.value;if(e){const w=new Date(e+"T00:00:00").getTime();t=t.filter(D=>new Date(D.inicio).getTime()>=w)}if(n){const w=new Date(n+"T23:59:59").getTime();t=t.filter(D=>new Date(D.inicio).getTime()<=w)}o&&o!=="Todos"&&(t=t.filter(w=>Ce(w.topico)===o.toLowerCase())),a&&a!=="Todos"&&(t=t.filter(w=>w.sub_topico&&w.sub_topico.toLowerCase()===a.toLowerCase()));const s=document.getElementById("rep-kpi-total"),i=document.getElementById("rep-kpi-active"),r=document.getElementById("rep-kpi-avg-time");s&&(s.textContent=t.length);const c=t.filter(w=>w.em_ocorrencia==1||w.em_ocorrencia==="true"||!w.fim);i&&(i.textContent=c.length);const u=t.filter(w=>w.fim);let d="0h 0m";if(u.length>0){const D=u.reduce((U,Y)=>U+(new Date(Y.fim).getTime()-new Date(Y.inicio).getTime()),0)/u.length,b=Math.floor(D/6e4),A=Math.floor(b/60),O=b%60;d=`${A}h ${O}m`}if(r&&(r.textContent=d),!window.Chart){console.warn("Chart.js is not loaded.");return}const g=q,m=e?new Date(e+"T00:00:00").getTime():new Date(new Date().getFullYear()+"-01-01T00:00:00").getTime(),h=n?new Date(n+"T23:59:59").getTime():Date.now(),p=g.map(w=>w.name),f=g.map(w=>{const D=w.id,b=le.filter(S=>Ce(S.topico)===D),A=h-m;if(A<=0)return 100;const U=b.filter(S=>{const Q=new Date(S.inicio).getTime();return(S.fim?new Date(S.fim).getTime():Date.now())>m&&Q<h}).map(S=>({start:Math.max(new Date(S.inicio).getTime(),m),end:Math.min(S.fim?new Date(S.fim).getTime():Date.now(),h)}));U.sort((S,Q)=>S.start-Q.start);const Y=[];if(U.length>0){let S=U[0];for(let Q=1;Q<U.length;Q++){const oe=U[Q];oe.start<S.end?S.end=Math.max(S.end,oe.end):(Y.push(S),S=oe)}Y.push(S)}const J=(S=>{let Q=0;return S.forEach(oe=>{Q+=oe.end-oe.start}),Q})(Y),de=(A-J)/A*100;return parseFloat(de.toFixed(4))}),y=g.map(w=>w.color||"#6b7280"),L=document.getElementById("chart-rep-sla");L&&(Ne&&Ne.destroy(),Ne=new window.Chart(L,{type:"bar",data:{labels:p,datasets:[{label:"Disponibilidade %",data:f,backgroundColor:y,borderRadius:6}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{y:{min:Math.max(0,Math.min(...f)-5),max:100,ticks:{callback:w=>w+"%"}}}}}));const $={};t.forEach(w=>{const D=w.sub_topico?w.sub_topico.charAt(0).toUpperCase()+w.sub_topico.slice(1).toLowerCase():"Não especificado";$[D]=($[D]||0)+1});const k=Object.keys($),N=Object.values($),V=document.getElementById("chart-rep-qty");V&&(Ue&&Ue.destroy(),Ue=new window.Chart(V,{type:"doughnut",data:{labels:k.length>0?k:["Nenhum evento"],datasets:[{data:N.length>0?N:[0],backgroundColor:["#3b82f6","#10b981","#f59e0b","#8b5cf6","#ec4899","#6366f1","#14b8a6","#f43f5e","#a855f7","#06b6d4"]}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{position:"right",labels:{boxWidth:12}}}}}))}function Dt(t){t.preventDefault();const e=document.getElementById("topic-id"),n=document.getElementById("topic-name"),o=document.getElementById("topic-color");if(!e||!n||!o)return;const a={id:e.value.trim().toLowerCase(),name:n.value.trim(),color:o.value};if(!a.id){alert("Por favor, defina um ID para o tópico.");return}fetch("/api/timeline/config/topics",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)}).then(s=>{if(!s.ok)throw new Error("Erro ao salvar tópico");return s.json()}).then(()=>{alert("Tópico salvo com sucesso!"),e.value="",n.value="",o.value="#3b82f6",ce().then(()=>{Z()})}).catch(s=>{console.error(s),alert("Erro: "+s.message)})}function St(t){t.preventDefault();const e=document.getElementById("subtopic-topic-id"),n=document.getElementById("subtopic-name");if(!e||!n)return;const o={topic_id:e.value,name:n.value.trim()};if(!o.topic_id||!o.name){alert("Preencha todos os campos do evento.");return}fetch("/api/timeline/config/subtopics",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)}).then(a=>{if(!a.ok)throw new Error("Erro ao adicionar evento");return a.json()}).then(()=>{alert("Evento adicionado!"),n.value="",ce()}).catch(a=>{console.error(a),alert("Erro: "+a.message)})}function Mt(t){confirm("Excluir este tópico também removerá todos os seus eventos associados. Deseja continuar?")&&fetch(`/api/timeline/config/topics/${t}`,{method:"DELETE"}).then(e=>{if(!e.ok)throw new Error("Erro ao excluir tópico");return e.json()}).then(()=>{alert("Tópico excluído!"),ce().then(()=>{Z()})}).catch(e=>{console.error(e),alert("Erro: "+e.message)})}function At(t){confirm("Deseja realmente excluir este evento?")&&fetch(`/api/timeline/config/subtopics/${t}`,{method:"DELETE"}).then(e=>{if(!e.ok)throw new Error("Erro ao excluir evento");return e.json()}).then(()=>{alert("Evento excluído!"),ce()}).catch(e=>{console.error(e),alert("Erro: "+e.message)})}function vt(){const t=document.getElementById("config-topics-list");t&&(t.innerHTML="",q.length===0?t.innerHTML='<div style="color: var(--text-muted); font-size: 0.9rem;">Nenhum tópico cadastrado.</div>':q.forEach(n=>{const o=document.createElement("div");o.style.cssText="display: flex; align-items: center; justify-content: space-between; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 8px; border: 1px solid var(--glass-border); margin-bottom: 6px;",o.innerHTML=`
        ...
        `,o.innerHTML=a}},Ie=30;let Be="",Qe="all",_e="",Pe="",G=1,fe=0,he="alerts",v=null,Fe=null;const it={init(){const t=document.getElementById("tab-monitoring-alerts");t&&t.addEventListener("click",()=>this.setActiveTab("alerts"));const e=document.getElementById("tab-monitoring-events");e&&e.addEventListener("click",()=>this.setActiveTab("events"));const n=document.getElementById("tab-monitoring-gnew");n&&n.addEventListener("click",()=>this.setActiveTab("gnew"));const o=document.getElementById("monitoring-events-search-input");o&&o.addEventListener("input",p=>{Be=p.target.value.toLowerCase(),G=1,this.fetchAndRenderEventHistory()});const a=document.getElementById("monitoring-events-severity-filter");a&&a.addEventListener("change",p=>{Qe=p.target.value,G=1,this.fetchAndRenderEventHistory()});const s=document.getElementById("monitoring-events-date-start"),i=document.getElementById("monitoring-events-date-end");s&&s.addEventListener("change",p=>{_e=p.target.value,G=1,this.fetchAndRenderEventHistory()}),i&&i.addEventListener("change",p=>{Pe=p.target.value,G=1,this.fetchAndRenderEventHistory()});const r=document.getElementById("btn-clear-event-date-filter");r&&r.addEventListener("click",()=>{_e="",Pe="",G=1,s&&(s.value=""),i&&(i.value=""),this.fetchAndRenderEventHistory()});const c=document.getElementById("btn-clear-event-history");c&&c.addEventListener("click",()=>this.clearEventHistory());const u=document.getElementById("btn-refresh-monitoring");u&&u.addEventListener("click",()=>this.fetchDiagnostics());const d=document.getElementById("gnew-disk-accordion-header");d&&d.addEventListener("click",()=>{const p=document.getElementById("gnew-disk-accordion-content"),f=document.getElementById("gnew-disk-chevron");p&&f&&(p.style.maxHeight==="0px"?(p.style.maxHeight="1000px",f.style.transform="rotate(0deg)"):(p.style.maxHeight="0px",f.style.transform="rotate(-90deg)"))});const g=document.getElementById("btn-refresh-gnew-disk");g&&g.addEventListener("click",p=>{p.stopPropagation(),this.fetchDiagnostics()});const m=document.getElementById("btn-refresh-gnew-services");m&&m.addEventListener("click",async()=>{const p=m,f=p.querySelector("svg");if(!p.disabled){p.disabled=!0,p.style.opacity="0.6",p.style.cursor="not-allowed",f&&(f.style.animation="spin 0.8s linear infinite");try{await this.fetchDiagnostics()}finally{p.disabled=!1,p.style.opacity="",p.style.cursor="pointer",f&&(f.style.animation="")}}});const h=document.getElementById("monitoring-auto-refresh");h&&(h.addEventListener("change",p=>{p.target.checked?this._startAutoRefresh():this._stopAutoRefresh()}),h.checked&&this._startAutoRefresh()),window.monitoringHandler=this},_startAutoRefresh(){this._stopAutoRefresh(),Fe=setInterval(()=>{(he==="alerts"||he==="gnew")&&this.fetchDiagnostics()},3e4)},_stopAutoRefresh(){Fe&&(clearInterval(Fe),Fe=null)},fetch(){this.setActiveTab("alerts"),this.fetchDiagnostics()},setActiveTab(t){he=t;const e=document.getElementById("tab-monitoring-alerts"),n=document.getElementById("tab-monitoring-events"),o=document.getElementById("tab-monitoring-gnew");e&&e.classList.toggle("active",t==="alerts"),n&&n.classList.toggle("active",t==="events"),o&&o.classList.toggle("active",t==="gnew");const a=document.getElementById("monitoring-tab-content-alerts"),s=document.getElementById("monitoring-tab-content-events"),i=document.getElementById("monitoring-tab-content-gnew");a&&(a.classList.toggle("hidden",t!=="alerts"),a.classList.toggle("active",t==="alerts")),s&&(s.classList.toggle("hidden",t!=="events"),s.classList.toggle("active",t==="events")),i&&(i.classList.toggle("hidden",t!=="gnew"),i.classList.toggle("active",t==="gnew")),t==="gnew"?this.fetchDiagnostics():t==="events"?(G=1,this.fetchAndRenderEventHistory()):this.renderGnewServicesStatus()},render(){he==="alerts"?this.renderGnewServicesStatus():he==="events"&&this.fetchAndRenderEventHistory()},renderGnewServicesStatus(){const t=document.getElementById("monitoring-alerts-grid");if(!t)return;if(t.style.display="flex",t.style.flexDirection="column",t.style.gap="0",!v||!v.servicos||!Array.isArray(v.servicos.servicos)||v.servicos.servicos.length===0){t.innerHTML=`
            ...
            <div class="monitor-list">
                ...
                ${e.map((c,u)=>{const d=c.status==="active"||c.status_label==="ativo",g=d?"#10b981":"#ef4444",m=d?"Online":c.status_label||c.status||"Offline",h=d?"rgba(16,185,129,0.12)":"rgba(239,68,68,0.12)",p=d?"#6ee7b7":"#fca5a5",f=d?"rgba(16,185,129,0.3)":"rgba(239,68,68,0.3)";return`
                        <div class="monitor-list-row" style="background: ${u%2===0?"transparent":"rgba(255,255,255,0.015)"};">
                            <div class="monitor-list-col-name">
                                <span class="monitor-dot" style="background: ${g};"></span>
                                <span class="monitor-svc-name">${c.nome}</span>
                            </div>
                            <div class="monitor-list-col-status">
                                <span class="monitor-badge" style="background:${h}; color:${p}; border-color:${f};">${m}</span>
                            </div>
                        </div>`}).join("")}
            ...
            </div>`;try{let e=await T.get("/monitoring/events?limit=1000");if(Be&&(e=e.filter(i=>(i.title||"").toLowerCase().includes(Be)||(i.description||"").toLowerCase().includes(Be)||(i.source||"").toLowerCase().includes(Be))),Qe!=="all"&&(e=e.filter(i=>i.severity===Qe)),_e){const i=new Date(_e+"T00:00:00").getTime();e=e.filter(r=>r.created_at?new Date(r.created_at).getTime()>=i:!1)}if(Pe){const i=new Date(Pe+"T23:59:59").getTime();e=e.filter(r=>r.created_at?new Date(r.created_at).getTime()<=i:!1)}fe=e.length;const n=Math.max(1,Math.ceil(fe/Ie));G>n&&(G=n);const o=document.getElementById("event-history-count");o&&(o.textContent=fe>0?fe:"",o.style.display=fe>0?"inline-flex":"none");const a=(G-1)*Ie,s=e.slice(a,a+Ie);this.renderEvents(s),this.renderPagination(fe,n)}catch(e){console.error("Erro ao buscar histórico de eventos:",e),t.innerHTML=`
                <div style="text-align: center; padding: 3rem; color: var(--text-muted);">
            ...
            <div class="eh-pagination">
            ...
            </div>`,n.querySelectorAll(".eh-page-btn[data-page]").forEach(d=>{d.addEventListener("click",()=>{const g=parseInt(d.dataset.page,10);if(!isNaN(g)&&g>=1&&g<=e&&g!==G){G=g,this.fetchAndRenderEventHistory();const m=document.getElementById("monitoring-events-grid");m&&m.scrollIntoView({behavior:"smooth",block:"start"})}})})},renderEvents(t){const e=document.getElementById("monitoring-events-grid");if(!e)return;e.style.display="flex",e.style.flexDirection="column",e.style.gap="0";const n=t||[];if(n.length===0){e.innerHTML=`
                ...
                </div>`;return}const o={};n.forEach(s=>{const i=s.created_at?new Date(s.created_at).toLocaleDateString("pt-BR",{weekday:"long",year:"numeric",month:"long",day:"numeric"}):"Data desconhecida";o[i]||(o[i]=[]),o[i].push(s)});const a=Object.entries(o).map(([s,i])=>{const r=i.map(c=>{const u=c.severity||"info";let d="Info",g="#3b82f6",m="rgba(59,130,246,0.12)",h="#93c5fd",p="rgba(59,130,246,0.3)",f="#3b82f6";u==="critical"?(d="Crítico",g="#ef4444",f="#ef4444",m="rgba(239,68,68,0.12)",h="#fca5a5",p="rgba(239,68,68,0.3)"):u==="warning"?(d="Alerta",g="#f59e0b",f="#f59e0b",m="rgba(245,158,11,0.12)",h="#fde047",p="rgba(245,158,11,0.3)"):u==="success"&&(d="Ok",g="#10b981",f="#10b981",m="rgba(16,185,129,0.12)",h="#6ee7b7",p="rgba(16,185,129,0.3)");const y=c.created_at?new Date(c.created_at):null,L=y?y.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit",second:"2-digit"}):"-",$=y?this._relativeTime(y):"",k=c.value_pct!=null?`${c.value_pct}%`:null;return`
                    <div class="event-history-row" style="border-left: 3px solid ${f};">
                        ...
                        <div class="event-history-row-meta">
                            ...
                            <span class="monitor-badge" style="background:${m}; color:${h}; border-color:${p}; flex-shrink: 0;">${d}</span>
