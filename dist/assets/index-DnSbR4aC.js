(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))o(a);new MutationObserver(a=>{for(const s of a)if(s.type==="childList")for(const i of s.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&o(i)}).observe(document,{childList:!0,subtree:!0});function n(a){const s={};return a.integrity&&(s.integrity=a.integrity),a.referrerPolicy&&(s.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?s.credentials="include":a.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function o(a){if(a.ep)return;a.ep=!0;const s=n(a);fetch(a.href,s)}})();const ve="/api",L={async get(t){const e=await fetch(`${ve}${t}`);if(!e.ok){const n=await e.json().catch(()=>({}));throw new Error(n.error||`HTTP error! status: ${e.status}`)}return await e.json()},async post(t,e){const n=await fetch(`${ve}${t}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)});if(!n.ok){const o=await n.json().catch(()=>({}));throw new Error(o.error||`HTTP error! status: ${n.status}`)}return await n.json()},async put(t,e){const n=await fetch(`${ve}${t}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)});if(!n.ok){const o=await n.json().catch(()=>({}));throw new Error(o.error||`HTTP error! status: ${n.status}`)}return await n.json()},async delete(t){const e=await fetch(`${ve}${t}`,{method:"DELETE"});if(!e.ok){const n=await e.json().catch(()=>({}));throw new Error(n.error||`HTTP error! status: ${e.status}`)}return await e.json()},async upload(t,e){const n=await fetch(`${ve}${t}`,{method:"POST",body:e});if(!n.ok){const o=await n.json().catch(()=>({}));throw new Error(o.error||`HTTP error! status: ${n.status}`)}return await n.json()}};let me=null;const V={init(){const t=localStorage.getItem("user");if(t)try{return me=JSON.parse(t),!0}catch{return this.logout(),!1}return!1},getUser(){return me},isAdmin(){return me&&me.role==="Administrador"},async login(t,e){try{const n=await L.post("/login",{email:t,password:e});return me=n,localStorage.setItem("user",JSON.stringify(n)),{success:!0,user:n}}catch(n){return{success:!1,error:n.message}}},logout(){me=null,localStorage.removeItem("user")}},l={show(t){const e=document.getElementById(t);e&&e.classList.remove("hidden")},hide(t){const e=document.getElementById(t);e&&e.classList.add("hidden")},toggle(t,e){const n=document.getElementById(t);n&&n.classList.toggle("hidden",e)},setText(t,e){const n=document.getElementById(t);n&&(n.innerText=e)},setValue(t,e){const n=document.getElementById(t);n&&(n.value=e)},getValue(t){const e=document.getElementById(t);return e?e.value:null},on(t,e,n){const o=document.getElementById(t);o&&o.addEventListener(e,n)}},rt={canvas:null,ctx:null,particles:[],animationFrameId:null,isActive:!1,init(){if(this.canvas=document.getElementById("account-network-bg"),!this.canvas)return;this.ctx=this.canvas.getContext("2d"),this.resize(),window.addEventListener("resize",()=>{this.isActive&&this.resize()});const t=window.innerWidth<=768;this.particleCount=t?30:60,this.connectDistance=150,this.particleColor="rgba(34, 211, 238, 0.5)",this.particles=[];for(let e=0;e<this.particleCount;e++)this.particles.push({x:Math.random()*this.canvas.width,y:Math.random()*this.canvas.height,vx:(Math.random()-.5)*1.5,vy:(Math.random()-.5)*1.5,radius:Math.random()*2+1})},resize(){if(!this.canvas)return;const t=document.getElementById("account-section");t&&(this.canvas.width=t.clientWidth,this.canvas.height=t.clientHeight)},updateAndDraw(){if(!(!this.isActive||!this.canvas)){this.ctx.clearRect(0,0,this.canvas.width,this.canvas.height);for(let t=0;t<this.particles.length;t++){const e=this.particles[t];e.x+=e.vx,e.y+=e.vy,(e.x<0||e.x>this.canvas.width)&&(e.vx*=-1),(e.y<0||e.y>this.canvas.height)&&(e.vy*=-1),this.ctx.beginPath(),this.ctx.arc(e.x,e.y,e.radius,0,Math.PI*2),this.ctx.fillStyle=this.particleColor,this.ctx.fill();for(let n=t+1;n<this.particles.length;n++){const o=this.particles[n],a=e.x-o.x,s=e.y-o.y,i=Math.sqrt(a*a+s*s);if(i<this.connectDistance){this.ctx.beginPath(),this.ctx.lineWidth=1;const r=1-i/this.connectDistance;this.ctx.strokeStyle=`rgba(34, 211, 238, ${r*.4})`,this.ctx.moveTo(e.x,e.y),this.ctx.lineTo(o.x,o.y),this.ctx.stroke()}}}this.animationFrameId=requestAnimationFrame(()=>this.updateAndDraw())}},start(){this.canvas||this.init(),this.isActive||(this.isActive=!0,this.resize(),this.updateAndDraw())},stop(){this.isActive=!1,this.animationFrameId&&(cancelAnimationFrame(this.animationFrameId),this.animationFrameId=null)}};let be=[];const Xe={async fetch(){try{be=await L.get("/users"),this.render(be)}catch(t){console.error("Error fetching Users:",t)}},getUsers(){return be},render(t){const e=document.getElementById("user-table-body");e&&(e.innerHTML=t.map(n=>{const o=n.role==="Administrador",a=V.isAdmin()?`
            ...
            `}).join("")},render(t){const e=document.getElementById("doc-list-body");if(!e)return;const n=document.getElementById("doc-list-thead"),o=ie.toLowerCase()==="contratos"||ie.toLowerCase()==="termo de uso",a=window.auth&&window.auth.isAdmin(),s=a?"":'class="role-hidden"';lt=t;const i=t.length,r=Math.ceil(i/we);P>r&&(P=Math.max(1,r)),P<1&&(P=1);const c=(P-1)*we,u=t.slice(c,c+we);if(n&&(o?n.innerHTML=`
                    <tr>
                        <th>Nome</th>
                        <th>Tamanho</th>
                        <th>Tipo</th>
            ...
            `,this.renderPaginationControls("doc-pagination",0,0);return}e.innerHTML=u.map(d=>{const p=d.mimetype==="application/pdf"?"📕":"🖼️",m=(d.size/1024).toFixed(1)+" KB",f=d.created_at?new Date(d.created_at).toLocaleDateString("pt-BR"):"-",g=d.mimetype==="application/pdf"?"PDF":"Imagem",h=a?`<button class="btn-delete" onclick="window.DocsHandler.delete(${d.id})" style="padding: 4px 10px; font-size: 0.85rem;">Deletar</button>`:"",v=d.start_date?new Date(d.start_date+"T00:00:00").toLocaleDateString("pt-BR"):"-",y=d.end_date?d.end_date==="Indefinido"?"Indefinido":new Date(d.end_date+"T00:00:00").toLocaleDateString("pt-BR"):"-";return o?`
                    <tr>
                        <td>
                            <span style="font-weight: 500; display: flex; align-items: center; gap: 8px;">
                                <span>${p}</span>
        ...
        `,o.innerHTML=a}};let ee=[],E={summaries:[]},ct=null,k=null,qe="list",xe=null,X=null,dt=null,_=1;const Ee=10;let He=[];const j={getPendingProcId(){return ct},async fetch(){try{_=1,ee=await L.get("/procedures"),this.renderTable(ee)}catch(t){console.error("Error fetching FAQs:",t)}},getFaqs(){return ee},setListingMode(t){qe=t,_=1,this.renderTable(He.length?He:ee)},renderTable(t){const e=document.getElementById("list-table-container"),n=document.getElementById("list-cards-container"),o=document.getElementById("proc-table-body");if(!e||!n||!o)return;He=t;const a=t.length,s=Math.ceil(a/Ee);_>s&&(_=Math.max(1,s)),_<1&&(_=1);const i=(_-1)*Ee,r=t.slice(i,i+Ee);qe==="list"?(l.show("list-table-container"),l.hide("list-cards-container"),r.length===0?o.innerHTML=`
                ...
                `:n.innerHTML=r.map(u=>{const d=V.isAdmin()?`
             ...
             <li class="sidebar-index-item ${a.id===k?"active":""} editable-section style-none"
                 draggable="true" 
            ...
            `).join("");const o=E.summaries.find(a=>a.id===k);o?(n.textContent=o.title,n.style.color="var(--text-main)",e.classList.remove("hidden")):(n.textContent="Nenhum sumário selecionado",n.style.color="var(--accent)",e.classList.add("hidden"))},selectSummary(t){k=t,this.renderProcedureBuilderSidebar(),this.renderProcedureBuilder()},updateSummaryTitle(t,e){const n=E.summaries.find(a=>a.id===t);n&&(n.title=e||"Sem título"),this.renderProcedureBuilderSidebar();const o=E.summaries.find(a=>a.id===k);o&&(document.getElementById("current-summary-name").textContent=o.title)},addSummary(){const t="sum_"+Date.now();E.summaries.push({id:t,title:`Sumário ${E.summaries.length+1}`,sections:[]}),k=t,this.renderProcedureBuilderSidebar(),this.renderProcedureBuilder()},removeSummary(t){confirm("Excluir este sumário apagará todos os campos dentro dele. Deseja continuar?")&&(E.summaries=E.summaries.filter(e=>e.id!==t),k===t&&(k=E.summaries.length>0?E.summaries[0].id:null),this.renderProcedureBuilderSidebar(),this.renderProcedureBuilder())},renderProcedureBuilder(){const t=document.getElementById("procedure-edit-container");if(!t)return;if(!k){t.innerHTML='<p class="empty-state">Crie um novo sumário na barra lateral para adicionar conteúdo.</p>';return}const e=E.summaries.find(o=>o.id===k);if(!e)return;const n=e.sections;if(n.length===0){t.innerHTML=`<p class="empty-state">Nenhum campo em "${e.title}". Clique em "+ Novo Container" para começar.</p>`;return}t.innerHTML=n.map((o,a)=>`
             ...
             </div>`).join("")},handleSumDragStart(t,e){xe="summary",X=e,t.dataTransfer.effectAllowed="move",setTimeout(()=>{t.target&&t.target.classList.add("dragging")},0)},handleSumDrop(t,e){if(t.preventDefault(),xe!=="summary"||X===null||X===e)return;const n=E.summaries.splice(X,1)[0];E.summaries.splice(e,0,n),this.renderProcedureBuilderSidebar()},handleSecDragStart(t,e,n){xe="container",X=e,dt=n,t.dataTransfer.effectAllowed="move",setTimeout(()=>{const o=t.target.nodeType===1?t.target.closest(".editable-section"):null;o&&o.classList.add("dragging")},0)},handleDragOver(t){t.preventDefault(),t.dataTransfer.dropEffect="move"},handleSecDrop(t,e,n){if(t.preventDefault(),xe!=="container"||X===null||dt!==n)return;const o=E.summaries.find(s=>s.id===n);if(!o||X===e)return;const a=o.sections.splice(X,1)[0];o.sections.splice(e,0,a),this.renderProcedureBuilder()},handleDragEnd(t){document.querySelectorAll(".editable-section.dragging").forEach(e=>e.classList.remove("dragging")),t&&t.target&&t.target.setAttribute&&t.target.setAttribute("draggable","false"),xe=null,X=null},updateSectionTitle(t,e){const n=E.summaries.find(o=>o.id===k);n&&(n.sections[t].title=e)},updateSectionData(t,e){const n=E.summaries.find(o=>o.id===k);n&&(n.sections[t].data=e)},removeSection(t){const e=E.summaries.find(n=>n.id===k);e&&e.sections.splice(t,1),this.renderProcedureBuilder()},getRteToolbarHTML(){return`
        ...
        `},addFaqItem(t){const e=E.summaries.find(n=>n.id===k);e&&(e.sections[t].data=e.sections[t].data||[],e.sections[t].data.push({q:"",a:""}),this.renderProcedureBuilder())},updateFaqItem(t,e,n,o){const a=E.summaries.find(s=>s.id===k);a&&(a.sections[t].data[e][n]=o)},removeFaqItem(t,e){const n=E.summaries.find(o=>o.id===k);n&&n.sections[t].data.splice(e,1),this.renderProcedureBuilder()},addSection(t,e){if(!k){alert("Selecione primeiro um sumário na barra lateral.");return}const n=E.summaries.find(o=>o.id===k);n&&(n.sections.push({id:Date.now(),title:t,type:e,data:e==="FAQ"?[]:e==="TEXTO"?"":null}),this.renderProcedureBuilder())},async handleSectionFileDrop(t,e){e.dataTransfer.files&&e.dataTransfer.files.length>0&&await this.uploadSectionFile(t,e.dataTransfer.files[0])},async handleSectionFileUpload(t,e){const n=e.files[0];n&&await this.uploadSectionFile(t,n)},async uploadSectionFile(t,e){const n=new FormData;n.append("file",e);try{const o=await L.upload("/upload",n),a=E.summaries.find(s=>s.id===k);a&&(a.sections[t].data={name:e.name,path:o.path,mimetype:e.type},this.renderProcedureBuilder())}catch{alert("Erro no upload")}},async handleSaveProcedure(){const t=parseInt(l.getValue("proc-id"));if(!t)return;const n={...ee.find(o=>o.id===t),content:JSON.stringify(E)};try{await L.put(`/procedures/${t}`,n),alert("Salvo com sucesso!"),this.toggleEditMode(!1),this.openDetail(t),this.fetch()}catch{alert("Erro ao salvar")}},search(t){_=1;const e=ee.filter(n=>(n.name||n.title||"").toLowerCase().includes(t)||(n.responsible||"").toLowerCase().includes(t)||(n.group_name||"").toLowerCase().includes(t));this.renderTable(e)},changePage(t){_=t,this.renderTable(He)},renderPaginationControls(t,e,n){const o=document.getElementById(t);if(!o)return;if(e===0){o.innerHTML="";return}let a="";a+=`
            <button class="pagination-btn" 
                    ${_===1?"disabled":""} 
        ...
        `,o.innerHTML=a}};window.toggleGhAccordion=function(t){const e=document.getElementById(t);e&&e.classList.toggle("open")};let O=[],pe="list",se="month",$=new Date,z=1;const $e=10;let ut=[];const F={async fetch(){try{z=1,O=await L.get("/accounts"),this.initDashboardMultiselects(),this.populateCompanyFilter(),this.handleSearch(),this.checkAccountAlerts()}catch(t){console.error("Falha ao obter contas",t)}},populateCompanyFilter(){const t=document.getElementById("dash-filter-company-dynamic-options");if(t){const e=new Set;t.querySelectorAll('input[type="checkbox"]:checked').forEach(a=>{e.add(a.value)});const n=[...new Set(O.map(a=>a.company_name).filter(Boolean))].sort((a,s)=>a.localeCompare(s));let o="";n.forEach(a=>{const s=e.has(a)?"checked":"";o+=`<label class="multiselect-option"><input type="checkbox" value="${a}" ${s}> <span>${a}</span></label>`}),t.innerHTML=o,this.setupMultiselectListeners("dash-filter-company")}},setupMultiselectListeners(t){if(!document.getElementById(`${t}-container`))return;const n=document.getElementById(`${t}-trigger`),o=document.getElementById(`${t}-dropdown`);if(!n||!o)return;n.dataset.listenerBound||(n.addEventListener("click",r=>{r.stopPropagation(),document.querySelectorAll(".multiselect-dropdown").forEach(c=>{c!==o&&c.classList.add("hidden")}),o.classList.toggle("hidden")}),n.dataset.listenerBound="true");const a=o.querySelector('input[value="Todos"]'),s=Array.from(o.querySelectorAll('input[type="checkbox"]')).filter(r=>r.value!=="Todos"),i=()=>{const r=s.filter(u=>u.checked).map(u=>u.value),c=n.querySelector(".trigger-label");a.checked||s.length>0&&r.length===s.length?(a.checked=!0,c&&(c.innerText="Todos")):r.length===0?c&&(c.innerText="Nenhum"):r.length===1?c&&(c.innerText=r[0]):c&&(c.innerText=`${r.length} selecionados`)};a&&!a.dataset.listenerBound&&(a.addEventListener("change",()=>{s.forEach(r=>{r.checked=a.checked}),i(),this.renderDashboard()}),a.dataset.listenerBound="true"),s.forEach(r=>{r.dataset.listenerBound||(r.addEventListener("change",()=>{s.every(u=>u.checked)?a.checked=!0:a.checked=!1,i(),this.renderDashboard()}),r.dataset.listenerBound="true")}),i()},initDashboardMultiselects(){this.setupMultiselectListeners("dash-filter-category"),window.multiselectOutsideClickListenerBound||(document.addEventListener("click",t=>{t.target.closest(".custom-multiselect-container")||document.querySelectorAll(".multiselect-dropdown").forEach(e=>{e.classList.add("hidden")})}),window.multiselectOutsideClickListenerBound=!0)},getMultiselectValues(t){const e=document.getElementById(`${t}-dropdown`);if(!e)return["Todos"];const n=e.querySelector('input[value="Todos"]');return n&&n.checked?["Todos"]:Array.from(e.querySelectorAll('input[type="checkbox"]:checked')).map(o=>o.value).filter(o=>o!=="Todos")},resetMultiselects(){["dash-filter-category","dash-filter-company"].forEach(t=>{const e=document.getElementById(`${t}-dropdown`);if(e){e.querySelectorAll('input[type="checkbox"]').forEach(a=>{a.checked=a.value==="Todos"});const o=document.getElementById(`${t}-trigger`);if(o){const a=o.querySelector(".trigger-label");a&&(a.innerText="Todos")}}})},getAccounts(){return O},setAccountsViewMode(t){pe=t,this.handleSearch()},setCalendarSubView(t){se=t,this.handleSearch()},shiftCalendarDate(t){se==="day"?$.setDate($.getDate()+t):se==="month"?$.setMonth($.getMonth()+t):se==="year"&&$.setFullYear($.getFullYear()+t),l.setValue("filter-day",$.getDate()),l.setValue("filter-month",$.getMonth()),l.setValue("filter-year",$.getFullYear()),this.handleSearch()},handleFilterChange(t=!1){if(t){const e=l.getValue("filter-cal-year")?parseInt(l.getValue("filter-cal-year")):$.getFullYear(),n=l.getValue("filter-cal-month")?parseInt(l.getValue("filter-cal-month")):$.getMonth();$=new Date(e,n,1)}else{const e=l.getValue("filter-year")?parseInt(l.getValue("filter-year")):$.getFullYear(),n=l.getValue("filter-month")?parseInt(l.getValue("filter-month")):$.getMonth(),o=l.getValue("filter-day")?parseInt(l.getValue("filter-day")):$.getDate();$=new Date(e,n,o)}l.setValue("filter-month",$.getMonth()),l.setValue("filter-year",$.getFullYear()),this.handleSearch()},handleSearch(){const t=(l.getValue("accounts-search")||"").toLowerCase();let e=O.filter(n=>n.company_name.toLowerCase().includes(t)||n.description&&n.description.toLowerCase().includes(t));if(pe==="list"){z=1;const n=l.getValue("filter-status")||"",o=document.getElementById("filter-date-toggle"),a=o?o.checked:!1,s=$.getFullYear(),i=$.getMonth(),r=$.getDate();e=e.filter(c=>{if(n&&c.status!==n)return!1;if(!a||!c.due_date)return!0;const[u,d,p]=c.due_date.split("-"),m=parseInt(u,10),f=parseInt(d,10)-1,g=parseInt(p,10);return c.type==="Único"?m===s&&f===i&&g===r:c.type==="Recorrente"?g===r:!0}),this.renderAccountsList(e)}else pe==="notificacoes"?this.renderNotifications():pe==="dashboard"?this.renderDashboard():this.renderCalendarWrapper(e)},checkAccountAlerts(){let t=!1;const e=new Date;e.setHours(0,0,0,0),O.forEach(o=>{const a=(o.status||"").trim().toLowerCase(),s=(o.payment_status||"").trim().toLowerCase();if(a==="on"&&s==="pendente"&&o.due_date){const[i,r,c]=o.due_date.split("-");let u=new Date(parseInt(i,10),parseInt(r,10)-1,parseInt(c,10));u.setHours(0,0,0,0),u.getTime()<=e.getTime()&&(t=!0)}});const n=document.getElementById("icon-alert-bell");n&&(t?n.classList.add("alert-pulse"):n.classList.remove("alert-pulse"))},renderNotifications(){const t=document.getElementById("accounts-notifications-body");if(!t)return;t.innerHTML="";const e=new Date;e.setHours(0,0,0,0);let n=O.filter(o=>{const a=(o.status||"").trim().toLowerCase(),s=(o.payment_status||"").trim().toLowerCase();if(a!=="on"||s!=="pendente"||!o.due_date)return!1;const[i,r,c]=o.due_date.split("-");let u=new Date(parseInt(i,10),parseInt(r,10)-1,parseInt(c,10));return u.setHours(0,0,0,0),u.getTime()<=e.getTime()});if(n.length===0){t.innerHTML='<tr><td colspan="6" style="text-align:center; color: var(--text-muted); padding: 20px;">Nenhuma conta urgente ou atrasada.</td></tr>';return}n.forEach(o=>{const a=document.createElement("tr");let s="Sem Data";if(o.due_date){const r=o.due_date.split("-");r.length===3&&(s=`${r[2]}/${r[1]}/${r[0]}`)}const i=V.isAdmin()?`
            ...
            `,t.appendChild(a)})},renderAccountsList(t){const e=document.getElementById("accounts-table-body");if(!e)return;e.innerHTML="",this.renderSidebarMiniCalendar(),ut=t;const n=t.length,o=Math.ceil(n/$e);z>o&&(z=Math.max(1,o)),z<1&&(z=1);const a=(z-1)*$e,s=t.slice(a,a+$e);if(s.length===0){e.innerHTML='<tr><td colspan="7" style="text-align:center; color: var(--text-muted); padding: 20px;">Nenhuma conta encontrada.</td></tr>',this.renderPaginationControls("accounts-list-pagination",0,0),this.renderDashboard();return}s.forEach(i=>{const r=document.createElement("tr");let c="Sem Data";if(i.due_date){const p=i.due_date.split("-");p.length===3&&(c=`${p[2]}/${p[1]}/${p[0]}`)}const u=i.status==="Off",d=V.isAdmin()?`
        ...
        `;let s=0;for(let c=1;c<=e;c++)(c===1||c===e||c>=z-1&&c<=z+1)&&(s&&c-s>1&&(a+='<span style="color: var(--text-muted); padding: 0 4px;">...</span>'),a+=`
        ...
        `;const i=(z-1)*$e+1,r=Math.min(z*$e,n);a+=`
            <span class="pagination-info">
                Exibindo ${i}-${r} de ${n}
            </span>
        `,o.innerHTML=a}};let ce=[],W={},oe=null,Ne=null,Ue=null,N=[],Re=[],Ze={},K={},ye,tt,ze,je,gt,Oe;const ft={init(){ye=document.getElementById("timeline-event-form"),tt=document.getElementById("view-visualizacao"),ze=document.getElementById("view-attention"),je=document.getElementById("view-anexo"),gt=document.getElementById("view-relatorio"),Oe=document.getElementById("view-config"),window.timelineHandler=ft,window.applyFilters=Lt,window.clearFilters=It,window.toggleFilters=Bt,window.handleDelete=Tt,window.resetForm=at,window.toggleAccordion=yt,window.handleFormSubmit=pt,window.editEvent=ot,window.deleteTopic=At,window.deleteSubtopic=Ht,window.handleTrackDragStart=Pt,window.handleTrackDragOver=_t,window.handleTrackDragEnd=Ft;const t=document.getElementById("timeline-topic-form");t&&t.addEventListener("submit",St);const e=document.getElementById("timeline-subtopic-form");e&&e.addEventListener("submit",Mt);const n=document.getElementById("topico");n&&n.addEventListener("change",u=>{nt(u.target.value)});const o=document.getElementById("em-ocorrencia");o&&o.addEventListener("change",u=>{const d=document.getElementById("fim"),p=document.getElementById("inicio");if(u.target.checked){if(!p.value){const m=new Date;m.setMinutes(m.getMinutes()-m.getTimezoneOffset()),p.value=m.toISOString().slice(0,16)}d.required=!1}else{const m=new Date;m.setMinutes(m.getMinutes()-m.getTimezoneOffset()),d.value=m.toISOString().slice(0,16),d.required=!0}});const a=document.getElementById("auto-refresh-toggle");a&&a.addEventListener("change",u=>{ht(u.target.checked)}),document.querySelectorAll("[data-timeline-tab]").forEach(u=>{u.addEventListener("click",d=>{const p=d.currentTarget.getAttribute("data-timeline-tab");Me(p)})}),ye&&ye.addEventListener("submit",pt);const s=document.getElementById("rep-filter-start"),i=document.getElementById("rep-filter-end"),r=document.getElementById("rep-filter-topic"),c=document.getElementById("rep-filter-subtopic");s&&s.addEventListener("change",()=>Ce()),i&&i.addEventListener("change",()=>Ce()),r&&r.addEventListener("change",u=>{Dt(u.target.value),Ce()}),c&&c.addEventListener("change",()=>Ce()),window.addEventListener("SectionChange",u=>{u.detail&&u.detail.section==="timeline"&&de().then(()=>{Z(),mt()})}),de().then(()=>{Z(),mt()})}};window.addEventListener("focus",()=>{tt&&Z()});function nt(t,e=null){const n=document.getElementById("sub-topico");if(!n)return;const o=t?t.toLowerCase().trim():"";if(!o||!K[o]){n.innerHTML='<option value="">Selecione o tópico primeiro...</option>',n.classList.remove("has-options");return}n.innerHTML='<option value="" disabled selected>Escolha o evento...</option>',K[o].forEach(a=>{const s=document.createElement("option");s.value=a.toLowerCase(),s.textContent=a,e&&s.value===e.toLowerCase()&&(s.selected=!0),n.appendChild(s)}),e||(n.selectedIndex=1),n.classList.add("has-options")}async function de(){try{const t=await fetch("/api/timeline/config");if(!t.ok)throw new Error("Falha ao buscar configurações");const e=await t.json();N=e.topics||[],Re=e.subtopics||[],Ze={},K={},N.forEach(o=>{Ze[o.id]=o.color,K[o.id]=[]}),Re.forEach(o=>{const a=o.topic_id;K[a]&&K[a].push(o.name)}),$t();const n=document.getElementById("view-config");n&&n.classList.contains("active")&&bt()}catch(t){console.error("Error loading config:",t)}}function $t(){const t=document.getElementById("topico");if(t){const o=t.value;t.innerHTML='<option value="" disabled selected>Selecione um tópico...</option>',N.forEach(a=>{const s=document.createElement("option");s.value=a.id,s.textContent=a.name,t.appendChild(s)}),t.value=o}const e=document.getElementById("rep-filter-topic");if(e){const o=e.value;e.innerHTML='<option value="Todos">Todos</option>',N.forEach(a=>{const s=document.createElement("option");s.value=a.id,s.textContent=a.name,e.appendChild(s)}),o&&[...e.options].some(a=>a.value===o)?e.value=o:e.value="Todos"}const n=document.getElementById("subtopic-topic-id");n&&(n.innerHTML='<option value="" disabled selected>Selecione um tópico...</option>',N.forEach(o=>{const a=document.createElement("option");a.value=o.id,a.textContent=o.name,n.appendChild(a)}))}function Z(){fetch("/api/timeline/events").then(t=>{if(!t.ok)throw new Error("Failed to fetch");return t.json()}).then(t=>{ce=t,it(),ze&&ze.classList.contains("active")&&vt()}).catch(t=>{console.error("Error loading events:",t)})}function mt(){const t=document.getElementById("timeline-tab-anexo"),e=document.getElementById("timeline-tab-config");if(window.auth&&window.auth.isAdmin())t&&t.classList.remove("role-hidden"),e&&e.classList.remove("role-hidden");else{t&&t.classList.add("role-hidden"),e&&e.classList.add("role-hidden");const o=je&&je.classList.contains("active"),a=Oe&&Oe.classList.contains("active");(o||a)&&Me("visualizacao")}}function Me(t){const e={visualizacao:{section:tt,button:document.querySelector('[data-timeline-tab="visualizacao"]')},attention:{section:ze,button:document.querySelector('[data-timeline-tab="attention"]')},anexo:{section:je,button:document.querySelector('[data-timeline-tab="anexo"]')},relatorio:{section:gt,button:document.querySelector('[data-timeline-tab="relatorio"]')},config:{section:Oe,button:document.querySelector('[data-timeline-tab="config"]')}};Object.values(e).forEach(n=>{n.section&&n.section.classList.remove("active"),n.button&&n.button.classList.remove("active")}),e[t]&&(e[t].section&&e[t].section.classList.add("active"),e[t].button&&e[t].button.classList.add("active")),t==="visualizacao"?(Z(),Te(!0)):t==="attention"?(vt(),Te(!0)):t==="relatorio"?(Ce(),Te(!1)):(t==="config"&&bt(),Te(!1))}function Te(t){const e=document.getElementById("floating-refresh-control");if(e)if(t){e.classList.remove("hidden");const n=document.getElementById("auto-refresh-toggle");n&&n.checked&&!oe&&ht(!0)}else e.classList.add("hidden"),oe&&(clearInterval(oe),oe=null)}function ht(t){oe&&(clearInterval(oe),oe=null),t&&(Z(),oe=setInterval(Z,6e4))}function pt(t){t.preventDefault();const e=ye.querySelector('button[type="submit"]');e&&(e.textContent="Salvando...",e.disabled=!0);const o={id:document.getElementById("event-id").value||Date.now().toString(),nome:document.getElementById("nome").value,topico:document.getElementById("topico").value,sub_topico:document.getElementById("sub-topico").value,em_ocorrencia:document.getElementById("em-ocorrencia").checked?1:0,inicio:document.getElementById("inicio").value,fim:document.getElementById("fim").value,descricao:document.getElementById("descricao").value,anotacao:document.getElementById("anotacao").value,cor:document.getElementById("cor").value};fetch("/api/timeline/events",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)}).then(async a=>{const s=await a.text();if(!a.ok)throw new Error(`Server error (${a.status}): ${s}`);return JSON.parse(s)}).then(()=>{alert("Evento salvo com sucesso!"),at(),Me("visualizacao")}).catch(a=>{console.error("Error saving event:",a),alert("Erro ao salvar evento: "+a.message)}).finally(()=>{e&&(e.textContent="Salvar Evento",e.disabled=!1)})}function ot(t){const e=ce.find(s=>s.id===t);if(!e)return;document.getElementById("event-id").value=e.id,document.getElementById("nome").value=e.nome;const n=ke(e.topico);document.getElementById("topico").value=n,nt(n,e.sub_topico);const o=document.getElementById("em-ocorrencia");o.checked=e.em_ocorrencia==1||e.em_ocorrencia==="true"||!e.fim,o.dispatchEvent(new Event("change")),document.getElementById("inicio").value=e.inicio,document.getElementById("fim").value=e.fim||"",document.getElementById("descricao").value=e.descricao||"",document.getElementById("anotacao").value=e.anotacao||"",document.getElementById("cor").value=e.cor||"#000000",Me("anexo");const a=document.getElementById("btn-delete");a&&(a.style.display="block")}function at(){ye&&ye.reset();const t=document.getElementById("event-id");t&&(t.value=""),nt("");const e=document.getElementById("fim");e&&(e.required=!0);const n=document.getElementById("cor");n&&(n.value="#000000");const o=document.getElementById("btn-delete");o&&(o.style.display="none")}function Tt(){const t=document.getElementById("event-id").value;t&&confirm("Tem certeza que deseja excluir este evento?")&&fetch(`/api/timeline/events/${t}`,{method:"DELETE"}).then(e=>{if(!e.ok)throw new Error("Failed to delete");return e.json()}).then(()=>{alert("Evento excluído!"),at(),Me("visualizacao")}).catch(e=>{console.error("Error deleting:",e),alert("Erro ao excluir: "+e.message)})}function Lt(t){const e=document.getElementById(`filter-start-${t}`),n=document.getElementById(`filter-end-${t}`),o=document.getElementById(`filter-sub-topic-${t}`),a=e&&e.value?new Date(e.value).getTime():null,s=n&&n.value?new Date(n.value).getTime():null,i=o?o.value:"";W[t]={start:a,end:s,subTopic:i},it()}function It(t){const e=document.getElementById(`filter-start-${t}`),n=document.getElementById(`filter-end-${t}`),o=document.getElementById(`filter-sub-topic-${t}`);e&&(e.value=""),n&&(n.value=""),o&&(o.value=""),W[t]=null,it()}function Bt(t){const e=document.getElementById(`filters-panel-${t}`),n=document.getElementById(`btn-toggle-${t}`);e&&n&&(e.classList.toggle("hidden"),n.classList.toggle("active"))}function yt(t){const e=document.getElementById(t);e&&e.classList.toggle("active")}function it(){const t=document.getElementById("timeline-tracks-container");if(!t)return;const e=Array.from(t.querySelectorAll(".timeline-container")).map(a=>a.dataset.topicId),n=N.map(a=>a.id);if(e.length!==n.length||!n.every(a=>e.includes(a))){t.innerHTML="";const a=window.auth&&window.auth.isAdmin(),s=a?'style="cursor: grab;"':"";N.forEach(i=>{const r=`
            ...
            `;t.insertAdjacentHTML("beforeend",r);const c=document.getElementById(`filter-sub-topic-${i.id}`);c&&K[i.id]&&K[i.id].forEach(u=>{const d=document.createElement("option");d.value=u.toLowerCase(),d.textContent=u,c.appendChild(d)})})}N.forEach(a=>{const s=document.getElementById(`track-${a.id}`),i=document.getElementById(`min-date-${a.id}`),r=document.getElementById(`max-date-${a.id}`);s&&(s.innerHTML=""),i&&(i.textContent=""),r&&(r.textContent="")}),ce.length!==0&&N.forEach(a=>{const s=a.id,i=ce.filter(v=>ke(v.topico)===s);let r=i;W[s]&&W[s].subTopic&&(r=i.filter(v=>(v.sub_topico?v.sub_topico.toLowerCase():"")===W[s].subTopic.toLowerCase()));const c=W[s]&&W[s].start?W[s].start:new Date("2026-01-01T00:00:00").getTime(),u=W[s]&&W[s].end?W[s].end:Date.now();Ct(s,r,c,u);const d=c,p=u,m=p-d,f=document.getElementById(`min-date-${s}`),g=document.getElementById(`max-date-${s}`);f&&(f.textContent=new Date(d).toLocaleDateString()+" "+new Date(d).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})),g&&(g.textContent=new Date(p).toLocaleDateString()+" "+new Date(p).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}));const h=document.getElementById(`track-${s}`);h&&r.forEach(v=>{const y=new Date(v.inicio).getTime(),b=v.fim?new Date(v.fim).getTime():Date.now();if(b<d||y>p)return;const I=Math.max(y,d),R=Math.min(b,p),S=(I-d)/m*100,x=(R-I)/m*100;if(x<=0)return;const C=document.createElement("div");C.className="timeline-bar",C.style.left=`${S}%`,C.style.width=`${x}%`,C.style.color=v.cor&&v.cor!=="#000000"?v.cor:Ze[s]||"#6b7280";const w=document.createElement("div");w.className="timeline-bar-visual",C.appendChild(w);const H=document.createElement("div");H.className="timeline-identifier-point";const q=new Date(v.inicio).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),U=v.fim?new Date(v.fim).toLocaleString([],{dateStyle:"short",timeStyle:"short"}):"Em andamento",Y=a.name,D=v.sub_topico?v.sub_topico.charAt(0).toUpperCase()+v.sub_topico.slice(1):"-";H.setAttribute("data-tooltip",`Tópico: ${Y}
Eventos: ${D}
Início: ${q} - Fim: ${U}
Descrição: ${v.descricao||"-"}`),!v.fim&&H.classList.add("pulsing"),window.auth&&window.auth.isAdmin()?(H.style.cursor="pointer",H.onclick=ue=>{ue.stopPropagation(),ot(v.id)}):H.style.cursor="default",C.appendChild(H),h.appendChild(C)})})}function ke(t){return t?t.toLowerCase().trim():""}function Ct(t,e,n,o){const a=document.getElementById(`sla-${t}`);if(!a)return;const s=o-n;if(s<=0){a.textContent="N/A";return}const r=e.filter(m=>{const f=new Date(m.inicio).getTime();return(m.fim?new Date(m.fim).getTime():Date.now())>n&&f<o}).map(m=>({start:Math.max(new Date(m.inicio).getTime(),n),end:Math.min(m.fim?new Date(m.fim).getTime():Date.now(),o)}));r.sort((m,f)=>m.start-f.start);const c=[];if(r.length>0){let m=r[0];for(let f=1;f<r.length;f++){const g=r[f];g.start<m.end?m.end=Math.max(m.end,g.end):(c.push(m),m=g)}c.push(m)}let u=0;c.forEach(m=>{u+=m.end-m.start});const d=(s-u)/s*100;let p="#10b981";d<50?p="#ef4444":d<90&&(p="#f97316"),a.style.color=p,a.textContent=d.toFixed(4)+"%"}function vt(){const t=document.getElementById("attention-topics-container");if(!t)return;t.innerHTML="";const e=ce.filter(n=>!n.fim);N.forEach(n=>{const o=n.id,a=e.filter(h=>ke(h.topico)===o),s=document.createElement("div");s.className=a.length>0?"accordion-item active":"accordion-item",s.id=`attn-acc-${o}`;const i=document.createElement("div");i.className="accordion-header",i.onclick=()=>yt(`attn-acc-${o}`);const r=document.createElement("div");r.className="accordion-title-group";const c=document.createElement("div");c.className="topic-indicator",c.style.backgroundColor=n.color;const u=document.createElement("h3");u.textContent=n.name;const d=document.createElement("span");d.style.cssText="background: #f1f5f9; padding: 2px 8px; border-radius: 12px; font-size: 0.95rem; font-weight: 900; color: #0f172a; margin-left: 0.5rem; border: 1px solid #cbd5e1;",d.textContent=`${a.length}`,r.appendChild(c),r.appendChild(u),r.appendChild(d);const p=document.createElement("span");p.className="accordion-chevron",p.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>',i.appendChild(r),i.appendChild(p);const m=document.createElement("div");m.className="accordion-content";const f=document.createElement("div");f.className="accordion-body";const g=document.createElement("div");if(g.className="attention-carousel",a.length===0){const h=document.createElement("div");h.className="empty-state",h.textContent="Nenhum evento em andamento.",g.appendChild(h)}else a.forEach(h=>{const v=document.createElement("div");v.className="attention-card",v.style.borderLeftColor=h.cor&&h.cor!=="#000000"?h.cor:n.color;const y=document.createElement("h3");y.textContent=h.nome;const b=document.createElement("div");b.className="sub-topic",b.textContent=h.sub_topico||"-";const I=document.createElement("div");I.className="card-detail",I.innerHTML=`<strong>Início:</strong> ${new Date(h.inicio).toLocaleString()}`;const R=Date.now()-new Date(h.inicio).getTime(),S=document.createElement("div");S.className="card-duration",S.innerHTML=`<strong>Tempo:</strong> <span>${kt(R)}</span>`;const x=document.createElement("div");x.className="card-description",x.textContent=h.descricao||"-",v.appendChild(y),v.appendChild(b),v.appendChild(I),v.appendChild(S),v.appendChild(x),window.auth&&window.auth.isAdmin()?(v.style.cursor="pointer",v.onclick=()=>ot(h.id)):v.style.cursor="default",g.appendChild(v)});f.appendChild(g),m.appendChild(f),s.appendChild(i),s.appendChild(m),t.appendChild(s)})}function kt(t){if(t<0)return"0s";const e=Math.floor(t/1e3),n=Math.floor(e/60),o=Math.floor(n/60),a=Math.floor(o/24),s=[];return a>0&&s.push(`${a}d`),(o%24>0||a>0)&&s.push(`${o%24}h`),(n%60>0||o>0)&&s.push(`${n%60}m`),s.push(`${e%60}s`),s.join(" ")}function Dt(t){const e=document.getElementById("rep-filter-subtopic");if(!e)return;e.innerHTML='<option value="Todos">Todos</option>';const n=t?t.toLowerCase().trim():"";n&&K[n]&&K[n].forEach(o=>{const a=document.createElement("option");a.value=o.toLowerCase(),a.textContent=o,e.appendChild(a)})}function Ce(){let t=ce;const e=document.getElementById("rep-filter-start")?.value,n=document.getElementById("rep-filter-end")?.value,o=document.getElementById("rep-filter-topic")?.value,a=document.getElementById("rep-filter-subtopic")?.value;if(e){const x=new Date(e+"T00:00:00").getTime();t=t.filter(C=>new Date(C.inicio).getTime()>=x)}if(n){const x=new Date(n+"T23:59:59").getTime();t=t.filter(C=>new Date(C.inicio).getTime()<=x)}o&&o!=="Todos"&&(t=t.filter(x=>ke(x.topico)===o.toLowerCase())),a&&a!=="Todos"&&(t=t.filter(x=>x.sub_topico&&x.sub_topico.toLowerCase()===a.toLowerCase()));const s=document.getElementById("rep-kpi-total"),i=document.getElementById("rep-kpi-active"),r=document.getElementById("rep-kpi-avg-time");s&&(s.textContent=t.length);const c=t.filter(x=>x.em_ocorrencia==1||x.em_ocorrencia==="true"||!x.fim);i&&(i.textContent=c.length);const u=t.filter(x=>x.fim);let d="0h 0m";if(u.length>0){const C=u.reduce((U,Y)=>U+(new Date(Y.fim).getTime()-new Date(Y.inicio).getTime()),0)/u.length,w=Math.floor(C/6e4),H=Math.floor(w/60),q=w%60;d=`${H}h ${q}m`}if(r&&(r.textContent=d),!window.Chart){console.warn("Chart.js is not loaded.");return}const p=N,m=e?new Date(e+"T00:00:00").getTime():new Date(new Date().getFullYear()+"-01-01T00:00:00").getTime(),f=n?new Date(n+"T23:59:59").getTime():Date.now(),g=p.map(x=>x.name),h=p.map(x=>{const C=x.id,w=ce.filter(M=>ke(M.topico)===C),H=f-m;if(H<=0)return 100;const U=w.filter(M=>{const Q=new Date(M.inicio).getTime();return(M.fim?new Date(M.fim).getTime():Date.now())>m&&Q<f}).map(M=>({start:Math.max(new Date(M.inicio).getTime(),m),end:Math.min(M.fim?new Date(M.fim).getTime():Date.now(),f)}));U.sort((M,Q)=>M.start-Q.start);const Y=[];if(U.length>0){let M=U[0];for(let Q=1;Q<U.length;Q++){const ae=U[Q];ae.start<M.end?M.end=Math.max(M.end,ae.end):(Y.push(M),M=ae)}Y.push(M)}const J=(M=>{let Q=0;return M.forEach(ae=>{Q+=ae.end-ae.start}),Q})(Y),ue=(H-J)/H*100;return parseFloat(ue.toFixed(4))}),v=p.map(x=>x.color||"#6b7280"),y=document.getElementById("chart-rep-sla");y&&(Ne&&Ne.destroy(),Ne=new window.Chart(y,{type:"bar",data:{labels:g,datasets:[{label:"Disponibilidade %",data:h,backgroundColor:v,borderRadius:6}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{y:{min:Math.max(0,Math.min(...h)-5),max:100,ticks:{callback:x=>x+"%"}}}}}));const b={};t.forEach(x=>{const C=x.sub_topico?x.sub_topico.charAt(0).toUpperCase()+x.sub_topico.slice(1).toLowerCase():"Não especificado";b[C]=(b[C]||0)+1});const I=Object.keys(b),R=Object.values(b),S=document.getElementById("chart-rep-qty");S&&(Ue&&Ue.destroy(),Ue=new window.Chart(S,{type:"doughnut",data:{labels:I.length>0?I:["Nenhum evento"],datasets:[{data:R.length>0?R:[0],backgroundColor:["#3b82f6","#10b981","#f59e0b","#8b5cf6","#ec4899","#6366f1","#14b8a6","#f43f5e","#a855f7","#06b6d4"]}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{position:"right",labels:{boxWidth:12}}}}}))}function St(t){t.preventDefault();const e=document.getElementById("topic-id"),n=document.getElementById("topic-name"),o=document.getElementById("topic-color");if(!e||!n||!o)return;const a={id:e.value.trim().toLowerCase(),name:n.value.trim(),color:o.value};if(!a.id){alert("Por favor, defina um ID para o tópico.");return}fetch("/api/timeline/config/topics",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)}).then(s=>{if(!s.ok)throw new Error("Erro ao salvar tópico");return s.json()}).then(()=>{alert("Tópico salvo com sucesso!"),e.value="",n.value="",o.value="#3b82f6",de().then(()=>{Z()})}).catch(s=>{console.error(s),alert("Erro: "+s.message)})}function Mt(t){t.preventDefault();const e=document.getElementById("subtopic-topic-id"),n=document.getElementById("subtopic-name");if(!e||!n)return;const o={topic_id:e.value,name:n.value.trim()};if(!o.topic_id||!o.name){alert("Preencha todos os campos do evento.");return}fetch("/api/timeline/config/subtopics",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)}).then(a=>{if(!a.ok)throw new Error("Erro ao adicionar evento");return a.json()}).then(()=>{alert("Evento adicionado!"),n.value="",de()}).catch(a=>{console.error(a),alert("Erro: "+a.message)})}function At(t){confirm("Excluir este tópico também removerá todos os seus eventos associados. Deseja continuar?")&&fetch(`/api/timeline/config/topics/${t}`,{method:"DELETE"}).then(e=>{if(!e.ok)throw new Error("Erro ao excluir tópico");return e.json()}).then(()=>{alert("Tópico excluído!"),de().then(()=>{Z()})}).catch(e=>{console.error(e),alert("Erro: "+e.message)})}function Ht(t){confirm("Deseja realmente excluir este evento?")&&fetch(`/api/timeline/config/subtopics/${t}`,{method:"DELETE"}).then(e=>{if(!e.ok)throw new Error("Erro ao excluir evento");return e.json()}).then(()=>{alert("Evento excluído!"),de()}).catch(e=>{console.error(e),alert("Erro: "+e.message)})}function bt(){const t=document.getElementById("config-topics-list");t&&(t.innerHTML="",N.length===0?t.innerHTML='<div style="color: var(--text-muted); font-size: 0.9rem;">Nenhum tópico cadastrado.</div>':N.forEach(n=>{const o=document.createElement("div");o.style.cssText="display: flex; align-items: center; justify-content: space-between; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 8px; border: 1px solid var(--glass-border); margin-bottom: 6px;",o.innerHTML=`
                ...
                `,t.appendChild(o)}));const e=document.getElementById("config-subtopics-list");e&&(e.innerHTML="",Re.length===0?e.innerHTML='<div style="color: var(--text-muted); font-size: 0.9rem;">Nenhum evento cadastrado.</div>':Re.forEach(n=>{const o=N.find(r=>r.id===n.topic_id),a=o?o.name:n.topic_id,s=o?o.color:"#6b7280",i=document.createElement("div");i.style.cssText="display: flex; align-items: center; justify-content: space-between; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 8px; border: 1px solid var(--glass-border); margin-bottom: 6px;",i.innerHTML=`
                    <div>
                ...
                `,e.appendChild(i)}))}function Pt(t,e){t.currentTarget.classList.add("dragging"),t.dataTransfer.effectAllowed="move"}function _t(t){t.preventDefault();const e=document.querySelector(".timeline-container.dragging");if(!e)return;const n=document.getElementById("timeline-tracks-container");if(!n)return;const a=[...n.querySelectorAll(".timeline-container:not(.dragging)")].find(s=>{const i=s.getBoundingClientRect();return t.clientY<=i.top+i.height/2});a?n.insertBefore(e,a):n.appendChild(e)}function Ft(t){const e=document.querySelector(".timeline-container.dragging");e&&e.classList.remove("dragging"),document.querySelectorAll(".timeline-container").forEach(a=>{a.setAttribute("draggable","false")});const n=document.getElementById("timeline-tracks-container");if(!n)return;const o=Array.from(n.querySelectorAll(".timeline-container")).map(a=>a.dataset.topicId);Vt(o)}function Vt(t){fetch("/api/timeline/config/topics/reorder",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({order:t})}).then(e=>{if(!e.ok)throw new Error("Erro ao salvar nova ordenação");return e.json()}).then(()=>{console.log("Ordem dos tópicos atualizada com sucesso."),de().then(()=>{Z()})}).catch(e=>{console.error(e),alert("Erro ao salvar ordenação: "+e.message)})}let ge=[],Ge=[],Ye=[],Je=[],B="extensions",A=1,fe=100,We=[];const Le={setActiveTab(t){B=t,A=1;const e=document.getElementById("telephony-search");e&&(e.value="",t==="extensions"?e.placeholder="Pesquisar ramais por número, nome ou usuário...":t==="queues"?e.placeholder="Pesquisar filas por número ou nome...":t==="blf"?e.placeholder="Pesquisar BLF por nome...":t==="users"&&(e.placeholder="Pesquisar usuários por nome ou perfil...")),document.querySelectorAll(".telephony-tabs-nav .acc-tab-btn").forEach(s=>{s.id===`tab-telephony-${t}`?s.classList.add("active"):s.classList.remove("active")}),document.querySelectorAll(".telephony-tab-content").forEach(s=>{s.id===`telephony-view-${t==="users"?"users":t==="queues"?"queues":t}`?s.classList.remove("hidden"):s.classList.add("hidden")});const a=this.getActiveDataList();this.render(a)},getActiveDataList(){return B==="extensions"?ge:B==="queues"?Ge:B==="blf"?Ye:B==="users"?Je:[]},async fetch(){const t=this.getActiveTableBody();t&&(t.innerHTML='<tr><td colspan="10" style="text-align: center; padding: 2rem; color: var(--text-muted);">Carregando dados...</td></tr>');try{if(A=1,B==="extensions")ge=await L.get("/telephony/extensions"),this.render(ge);else if(B==="queues")Ge=await L.get("/telephony/queues"),this.render(Ge);else if(B==="blf"){if(ge.length===0)try{ge=await L.get("/telephony/extensions")}catch(e){console.warn("Could not pre-fetch extensions for BLF mapping:",e)}Ye=await L.get("/telephony/blfs"),this.render(Ye)}else B==="users"&&(Je=await L.get("/telephony/users"),this.render(Je))}catch(e){console.error(`Error fetching telephony ${B}:`,e),t&&(t.innerHTML=`<tr><td colspan="10" style="text-align: center; padding: 2rem; color: #ef4444;">Erro ao carregar dados: ${e.message||"Erro de rede"}</td></tr>`)}},getActiveTableBody(){return B==="extensions"?document.getElementById("telephony-table-body"):B==="queues"?document.getElementById("telephony-queues-table-body"):B==="blf"?document.getElementById("telephony-blf-table-body"):B==="users"?document.getElementById("telephony-users-table-body"):null},render(t){const e=this.getActiveTableBody();if(!e)return;We=t;const n=t.length,o=Math.ceil(n/fe);A>o&&(A=Math.max(1,o)),A<1&&(A=1);const a=(A-1)*fe,s=t.slice(a,a+fe);if(s.length===0){const i=B==="extensions"?7:B==="queues"?6:B==="blf"?4:5;e.innerHTML=`
                <tr>
                    <td colspan="${i}" style="text-align: center; padding: 2rem; color: var(--text-muted);">
                        Nenhum registro encontrado.
                    </td>
                </tr>
            `,this.renderPaginationControls("telephony-pagination",0,0);return}B==="extensions"?this.renderExtensionsList(e,s):B==="queues"?this.renderQueuesList(e,s):B==="blf"?this.renderBlfsList(e,s):B==="users"&&this.renderUsersList(e,s),this.renderPaginationControls("telephony-pagination",o,n)},renderExtensionsList(t,e){t.innerHTML=e.map(n=>{const o=n.exten||"-",a=n.nome||"-",s=n.ddr||"-",i=n.Username||"-",r=n.Secret||"",c=n.regra_saida_nome?`<span class="badge" style="background: rgba(255,255,255,0.05); color: var(--text-main); font-size: 0.8rem; padding: 4px 8px; border-radius: 6px;">${n.regra_saida_nome}</span>`:"-",u=n.observacao||"-",d=r.replace(/'/g,"\\'");return`
                <tr>
                    <td>
            ...
            <button class="pagination-btn" 
                    ${A===1?"disabled":""} 
                    onclick="window.TelephonyHandler.changePage(${A-1})"
                    title="Página Anterior">
                &laquo;
            </button>
        ...
        `;const i=(A-1)*fe+1,r=Math.min(A*fe,n);a+=`
            <span class="pagination-info">
                Exibindo ${i}-${r} de ${n}
            </span>
        `,o.innerHTML=a}},Ie=30;let Be="",Qe="all",Pe="",_e="",G=1,he=0,te="alerts",T=null,Ke=[],Fe=null;const st={init(){const t=document.getElementById("tab-monitoring-alerts");t&&t.addEventListener("click",()=>this.setActiveTab("alerts"));const e=document.getElementById("tab-monitoring-events");e&&e.addEventListener("click",()=>this.setActiveTab("events"));const n=document.getElementById("tab-monitoring-apis");n&&n.addEventListener("click",()=>this.setActiveTab("apis"));const o=document.getElementById("tab-monitoring-gnew");o&&o.addEventListener("click",()=>this.setActiveTab("gnew"));const a=document.getElementById("monitoring-events-search-input");a&&a.addEventListener("input",y=>{Be=y.target.value.toLowerCase(),G=1,this.fetchAndRenderEventHistory()});const s=document.getElementById("monitoring-search-input");s&&s.addEventListener("input",()=>{this.renderGnewServicesStatus()});const i=document.getElementById("monitoring-events-severity-filter");i&&i.addEventListener("change",y=>{Qe=y.target.value,G=1,this.fetchAndRenderEventHistory()});const r=document.getElementById("monitoring-events-date-start"),c=document.getElementById("monitoring-events-date-end");r&&r.addEventListener("change",y=>{Pe=y.target.value,G=1,this.fetchAndRenderEventHistory()}),c&&c.addEventListener("change",y=>{_e=y.target.value,G=1,this.fetchAndRenderEventHistory()});const u=document.getElementById("btn-clear-event-date-filter");u&&u.addEventListener("click",()=>{Pe="",_e="",G=1,r&&(r.value=""),c&&(c.value=""),this.fetchAndRenderEventHistory()});const d=document.getElementById("btn-clear-event-history");d&&d.addEventListener("click",()=>this.clearEventHistory());const p=document.getElementById("btn-refresh-monitoring");p&&p.addEventListener("click",()=>this.fetchDiagnostics());const m=document.getElementById("gnew-disk-accordion-header");m&&m.addEventListener("click",()=>{const y=document.getElementById("gnew-disk-accordion-content"),b=document.getElementById("gnew-disk-chevron");y&&b&&(y.style.maxHeight==="0px"?(y.style.maxHeight="1000px",b.style.transform="rotate(0deg)"):(y.style.maxHeight="0px",b.style.transform="rotate(-90deg)"))});const f=document.getElementById("btn-refresh-gnew-disk");f&&f.addEventListener("click",y=>{y.stopPropagation(),this.fetchDiagnostics()});const g=document.getElementById("btn-refresh-gnew-services");g&&g.addEventListener("click",async()=>{const y=g,b=y.querySelector("svg");if(!y.disabled){y.disabled=!0,y.style.opacity="0.6",y.style.cursor="not-allowed",b&&(b.style.animation="spin 0.8s linear infinite");try{await this.fetchDiagnostics()}finally{y.disabled=!1,y.style.opacity="",y.style.cursor="pointer",b&&(b.style.animation="")}}});const h=document.getElementById("btn-refresh-apis-status");h&&h.addEventListener("click",()=>this.fetchAndRenderApisStatus());const v=document.getElementById("monitoring-auto-refresh");v&&(v.addEventListener("change",y=>{y.target.checked?this._startAutoRefresh():this._stopAutoRefresh()}),v.checked&&this._startAutoRefresh()),window.monitoringHandler=this},_startAutoRefresh(){this._stopAutoRefresh(),Fe=setInterval(()=>{(te==="alerts"||te==="gnew")&&this.fetchDiagnostics()},3e4)},_stopAutoRefresh(){Fe&&(clearInterval(Fe),Fe=null)},fetch(){this.setActiveTab("alerts"),this.fetchDiagnostics()},setActiveTab(t){te=t;const e=document.getElementById("tab-monitoring-alerts"),n=document.getElementById("tab-monitoring-events"),o=document.getElementById("tab-monitoring-apis"),a=document.getElementById("tab-monitoring-gnew");e&&e.classList.toggle("active",t==="alerts"),n&&n.classList.toggle("active",t==="events"),o&&o.classList.toggle("active",t==="apis"),a&&a.classList.toggle("active",t==="gnew");const s=document.getElementById("monitoring-tab-content-alerts"),i=document.getElementById("monitoring-tab-content-events"),r=document.getElementById("monitoring-tab-content-apis"),c=document.getElementById("monitoring-tab-content-gnew");s&&(s.classList.toggle("hidden",t!=="alerts"),s.classList.toggle("active",t==="alerts")),i&&(i.classList.toggle("hidden",t!=="events"),i.classList.toggle("active",t==="events")),r&&(r.classList.toggle("hidden",t!=="apis"),r.classList.toggle("active",t==="apis")),c&&(c.classList.toggle("hidden",t!=="gnew"),c.classList.toggle("active",t==="gnew")),t==="gnew"?this.fetchDiagnostics():t==="events"?(G=1,this.fetchAndRenderEventHistory()):t==="apis"?this.fetchAndRenderApisStatus():this.renderGnewServicesStatus()},render(){te==="alerts"?this.renderGnewServicesStatus():te==="events"?this.fetchAndRenderEventHistory():te==="apis"&&this.fetchAndRenderApisStatus()},renderGnewServicesStatus(){const t=document.getElementById("monitoring-alerts-grid");if(!t)return;t.style.display="flex",t.style.flexDirection="column",t.style.gap="0";const e=T&&T.servicos&&Array.isArray(T.servicos.servicos)?T.servicos.servicos:[],n=Ke||[];if(e.length===0&&n.length===0){t.innerHTML=`
            ...
            `;return}const o=e.length+n.length,a=e.filter(y=>y.status!=="active"&&y.status_label!=="ativo").length,s=n.filter(y=>!y.online).length,i=a+s,r=o-i,c=document.getElementById("monitor-kpi-total"),u=document.getElementById("monitor-kpi-warning"),d=document.getElementById("monitor-kpi-info");c&&(c.textContent=o),u&&(u.textContent=i),d&&(d.textContent=r);const p=document.getElementById("monitoring-search-input"),m=p?p.value.toLowerCase().trim():"";let f=e,g=n;m&&(f=e.filter(y=>y.nome.toLowerCase().includes(m)),g=n.filter(y=>y.name.toLowerCase().includes(m)||y.description.toLowerCase().includes(m)));let h=`
        ...
        `,v=0;f.forEach(y=>{const b=y.status==="active"||y.status_label==="ativo",I=b?"#10b981":"#ef4444",R=b?"Online":y.status_label||y.status||"Offline",S=b?"rgba(16,185,129,0.12)":"rgba(239,68,68,0.12)",x=b?"#6ee7b7":"#fca5a5",C=b?"rgba(16,185,129,0.3)":"rgba(239,68,68,0.3)",w=v%2===0?"transparent":"rgba(255,255,255,0.015)";v++,h+=`
                <div class="monitor-list-row" style="background: ${w};">
                    <div class="monitor-list-col-name">
                        <span class="monitor-dot" style="background: ${I};"></span>
                        <span class="monitor-svc-name" style="font-size:0.88rem;">[Serviço PABX] ${y.nome}</span>
                    </div>
                    <div class="monitor-list-col-status">
                        <span class="monitor-badge" style="background:${S}; color:${x}; border-color:${C};">${R}</span>
                    </div>
                </div>`}),g.forEach(y=>{const b=y.online,I=b?"#10b981":"#ef4444",R=b?"Online":"Offline",S=b?"rgba(16,185,129,0.12)":"rgba(239,68,68,0.12)",x=b?"#6ee7b7":"#fca5a5",C=b?"rgba(16,185,129,0.3)":"rgba(239,68,68,0.3)",w=v%2===0?"transparent":"rgba(255,255,255,0.015)";v++,h+=`
                <div class="monitor-list-row" style="background: ${w};">
                    <div class="monitor-list-col-name">
                        <span class="monitor-dot" style="background: ${I};"></span>
                ...
                </div>`}),h+="</div>",t.innerHTML=h},async fetchAndRenderEventHistory(){const t=document.getElementById("monitoring-events-grid");if(t){t.innerHTML=`
            ...
            </div>`;try{let e=await L.get("/monitoring/events?limit=1000");if(Be&&(e=e.filter(i=>(i.title||"").toLowerCase().includes(Be)||(i.description||"").toLowerCase().includes(Be)||(i.source||"").toLowerCase().includes(Be))),Qe!=="all"&&(e=e.filter(i=>i.severity===Qe)),Pe){const i=new Date(Pe+"T00:00:00").getTime();e=e.filter(r=>r.created_at?new Date(r.created_at).getTime()>=i:!1)}if(_e){const i=new Date(_e+"T23:59:59").getTime();e=e.filter(r=>r.created_at?new Date(r.created_at).getTime()<=i:!1)}he=e.length;const n=Math.max(1,Math.ceil(he/Ie));G>n&&(G=n);const o=document.getElementById("event-history-count");o&&(o.textContent=he>0?he:"",o.style.display=he>0?"inline-flex":"none");const a=(G-1)*Ie,s=e.slice(a,a+Ie);this.renderEvents(s),this.renderPagination(he,n)}catch(e){console.error("Erro ao buscar histórico de eventos:",e),t.innerHTML=`
                <div style="text-align: center; padding: 3rem; color: var(--text-muted);">
...
`).find(o=>o.trim().endsWith(" /"));if(n){const o=n.trim().split(/\s+/);if(o.length>=5){const a=o[1],s=o[2],i=o[4].replace("%",""),r=parseInt(i,10);if(!isNaN(r))return{percentage:r,detail:`${s} em uso de ${a} (Montagem em /)`}}}}catch(e){console.warn("Erro ao fazer parse do disco:",e)}return{percentage:0,detail:"Erro no parse"}},renderGnewDiagnostics(){if(!T)return;if(T.memoria){let n={percentage:0,detail:"Dados de memória indisponíveis"};if(T.memoria.output)n=this.parseMemoryOutput(T.memoria.output);else if(typeof T.memoria.percent<"u"){const i=(T.memoria.total_mb/1024).toFixed(1),r=(T.memoria.used_mb/1024).toFixed(1);n={percentage:Math.round(T.memoria.percent),detail:`${r}GB em uso de ${i}GB total`}}const o=document.getElementById("gnew-kpi-mem-text"),a=document.getElementById("gnew-kpi-mem-bar"),s=document.getElementById("gnew-kpi-mem-detail");o&&(o.textContent=`${n.percentage}%`),a&&(a.style.width=`${n.percentage}%`),s&&(s.textContent=n.detail)}if(T.disco){let n={percentage:0,detail:"Dados de disco indisponíveis"};if(T.disco.output)n=this.parseDiskOutput(T.disco.output);else if(Array.isArray(T.disco)){const i=T.disco.find(r=>r.mountpoint==="/");i&&(n={percentage:Math.round(i.percent),detail:`${i.used_gb.toFixed(1)}GB em uso de ${i.total_gb.toFixed(1)}GB (Montagem em /)`})}const o=document.getElementById("gnew-kpi-disk-text"),a=document.getElementById("gnew-kpi-disk-bar"),s=document.getElementById("gnew-kpi-disk-detail");o&&(o.textContent=`${n.percentage}%`),a&&(a.style.width=`${n.percentage}%`),s&&(s.textContent=n.detail)}const t=document.getElementById("gnew-disk-table-body");if(t){let n=[];if(T.disco)if(T.disco.output)try{const o=T.disco.output.trim().split(`
`);for(let a=1;a<o.length;a++){const s=o[a].trim().split(/\s+/);s.length>=6&&n.push({mountpoint:s[5],total:s[1],used:s[2],free:s[3],percent:parseInt(s[4].replace("%",""),10)||0})}}catch(o){console.warn("Erro ao fazer parse da tabela de disco offline:",o)}else Array.isArray(T.disco)&&(n=T.disco.map(o=>({mountpoint:o.mountpoint,total:typeof o.total_gb=="number"?`${o.total_gb.toFixed(2)} GB`:o.total_gb||"0 GB",used:typeof o.used_gb=="number"?`${o.used_gb.toFixed(2)} GB`:o.used_gb||"0 GB",free:typeof o.free_gb=="number"?`${o.free_gb.toFixed(2)} GB`:o.free_gb||"0 GB",percent:typeof o.percent=="number"?Math.round(o.percent):parseInt(o.percent,10)||0})));n.length>0?t.innerHTML=n.map(o=>{const a=o.percent;return`
                ...
                `}if(T.servicos&&T.servicos.timestamp)try{const o=new Date(T.servicos.timestamp).toLocaleString("pt-BR"),a=document.getElementById("gnew-services-timestamp");a&&(a.textContent=`Última verificação: ${o}`)}catch(n){console.warn("Erro ao formatar timestamp dos serviços:",n)}const e=document.getElementById("gnew-services-list");if(e){let n=[];T.servicos&&Array.isArray(T.servicos.servicos)&&(n=T.servicos.servicos),n.length>0?(e.innerHTML=n.map(a=>{const s=a.status==="active"||a.status_label==="ativo",i=s?"rgba(16, 185, 129, 0.1)":"rgba(239, 68, 68, 0.1)",r=s?"#6ee7b7":"#fca5a5",c=s?"#10b981":"#ef4444",u=s?"#10b981":"#ef4444";return`
