(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))o(a);new MutationObserver(a=>{for(const s of a)if(s.type==="childList")for(const i of s.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&o(i)}).observe(document,{childList:!0,subtree:!0});function n(a){const s={};return a.integrity&&(s.integrity=a.integrity),a.referrerPolicy&&(s.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?s.credentials="include":a.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function o(a){if(a.ep)return;a.ep=!0;const s=n(a);fetch(a.href,s)}})();const we="/api",$={async get(e){const t=await fetch(`${we}${e}`);if(!t.ok){const n=await t.json().catch(()=>({}));throw new Error(n.error||`HTTP error! status: ${t.status}`)}return await t.json()},async post(e,t){const n=await fetch(`${we}${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});if(!n.ok){const o=await n.json().catch(()=>({}));throw new Error(o.error||`HTTP error! status: ${n.status}`)}return await n.json()},async put(e,t){const n=await fetch(`${we}${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});if(!n.ok){const o=await n.json().catch(()=>({}));throw new Error(o.error||`HTTP error! status: ${n.status}`)}return await n.json()},async delete(e){const t=await fetch(`${we}${e}`,{method:"DELETE"});if(!t.ok){const n=await t.json().catch(()=>({}));throw new Error(n.error||`HTTP error! status: ${t.status}`)}return await t.json()},async upload(e,t){const n=await fetch(`${we}${e}`,{method:"POST",body:t});if(!n.ok){const o=await n.json().catch(()=>({}));throw new Error(o.error||`HTTP error! status: ${n.status}`)}return await n.json()}};let pe=null;const j={init(){const e=localStorage.getItem("user");if(e)try{return pe=JSON.parse(e),!0}catch{return this.logout(),!1}return!1},getUser(){return pe},isAdmin(){return pe&&pe.role==="Administrador"},async login(e,t){try{const n=await $.post("/login",{email:e,password:t});return pe=n,localStorage.setItem("user",JSON.stringify(n)),{success:!0,user:n}}catch(n){return{success:!1,error:n.message}}},logout(){pe=null,localStorage.removeItem("user")}},c={show(e){const t=document.getElementById(e);t&&t.classList.remove("hidden")},hide(e){const t=document.getElementById(e);t&&t.classList.add("hidden")},toggle(e,t){const n=document.getElementById(e);n&&n.classList.toggle("hidden",t)},setText(e,t){const n=document.getElementById(e);n&&(n.innerText=t)},setValue(e,t){const n=document.getElementById(e);n&&(n.value=t)},getValue(e){const t=document.getElementById(e);return t?t.value:null},on(e,t,n){const o=document.getElementById(e);o&&o.addEventListener(t,n)}},ft={canvas:null,ctx:null,particles:[],animationFrameId:null,isActive:!1,init(){if(this.canvas=document.getElementById("account-network-bg"),!this.canvas)return;this.ctx=this.canvas.getContext("2d"),this.resize(),window.addEventListener("resize",()=>{this.isActive&&this.resize()});const e=window.innerWidth<=768;this.particleCount=e?30:60,this.connectDistance=150,this.particleColor="rgba(34, 211, 238, 0.5)",this.particles=[];for(let t=0;t<this.particleCount;t++)this.particles.push({x:Math.random()*this.canvas.width,y:Math.random()*this.canvas.height,vx:(Math.random()-.5)*1.5,vy:(Math.random()-.5)*1.5,radius:Math.random()*2+1})},resize(){if(!this.canvas)return;const e=document.getElementById("account-section");e&&(this.canvas.width=e.clientWidth,this.canvas.height=e.clientHeight)},updateAndDraw(){if(!(!this.isActive||!this.canvas)){this.ctx.clearRect(0,0,this.canvas.width,this.canvas.height);for(let e=0;e<this.particles.length;e++){const t=this.particles[e];t.x+=t.vx,t.y+=t.vy,(t.x<0||t.x>this.canvas.width)&&(t.vx*=-1),(t.y<0||t.y>this.canvas.height)&&(t.vy*=-1),this.ctx.beginPath(),this.ctx.arc(t.x,t.y,t.radius,0,Math.PI*2),this.ctx.fillStyle=this.particleColor,this.ctx.fill();for(let n=e+1;n<this.particles.length;n++){const o=this.particles[n],a=t.x-o.x,s=t.y-o.y,i=Math.sqrt(a*a+s*s);if(i<this.connectDistance){this.ctx.beginPath(),this.ctx.lineWidth=1;const r=1-i/this.connectDistance;this.ctx.strokeStyle=`rgba(34, 211, 238, ${r*.4})`,this.ctx.moveTo(t.x,t.y),this.ctx.lineTo(o.x,o.y),this.ctx.stroke()}}}this.animationFrameId=requestAnimationFrame(()=>this.updateAndDraw())}},start(){this.canvas||this.init(),this.isActive||(this.isActive=!0,this.resize(),this.updateAndDraw())},stop(){this.isActive=!1,this.animationFrameId&&(cancelAnimationFrame(this.animationFrameId),this.animationFrameId=null)}};let xe=[];const st={async fetch(){try{xe=await $.get("/users"),this.render(xe)}catch(e){console.error("Error fetching Users:",e)}},getUsers(){return xe},render(e){const t=document.getElementById("user-table-body");t&&(t.innerHTML=e.map(n=>{const o=n.role==="Administrador",a=j.isAdmin()?`
            ...
            `}).join("")},render(e){const t=document.getElementById("doc-list-body");if(!t)return;const n=document.getElementById("doc-list-thead"),o=re.toLowerCase()==="contratos"||re.toLowerCase()==="termo de uso",a=window.auth&&window.auth.isAdmin(),s=a?"":'class="role-hidden"';ht=e;const i=e.length,r=Math.ceil(i/Ee);O>r&&(O=Math.max(1,r)),O<1&&(O=1);const l=(O-1)*Ee,u=e.slice(l,l+Ee);if(n&&(o?n.innerHTML=`
                    <tr>
                        <th>Nome</th>
                        <th>Tamanho</th>
                        <th>Tipo</th>
            ...
            `,this.renderPaginationControls("doc-pagination",0,0);return}t.innerHTML=u.map(d=>{const p=d.mimetype==="application/pdf"?"📕":"🖼️",m=(d.size/1024).toFixed(1)+" KB",f=d.created_at?new Date(d.created_at).toLocaleDateString("pt-BR"):"-",g=d.mimetype==="application/pdf"?"PDF":"Imagem",h=a?`<button class="btn-delete" onclick="window.DocsHandler.delete(${d.id})" style="padding: 4px 10px; font-size: 0.85rem;">Deletar</button>`:"",y=d.start_date?new Date(d.start_date+"T00:00:00").toLocaleDateString("pt-BR"):"-",k=d.end_date?d.end_date==="Indefinido"?"Indefinido":new Date(d.end_date+"T00:00:00").toLocaleDateString("pt-BR"):"-";return o?`
                    <tr>
                        <td>
                            <span style="font-weight: 500; display: flex; align-items: center; gap: 8px;">
                                <span>${p}</span>
                ...
                `}).join(""),this.renderPaginationControls("doc-pagination",r,i)},async handleUpload(e){e.preventDefault();const t=document.getElementById("doc-file"),n=document.getElementById("doc-category"),o=document.getElementById("doc-display-name");if(!t.files.length){alert("Selecione um arquivo.");return}const a=new FormData,s=n?n.value:"Geral";a.append("category",s),a.append("customName",o?o.value:""),a.append("document",t.files[0]);const i=s.toLowerCase();if(i==="contratos"||i==="termo de uso"){const r=document.getElementById("doc-start-date"),l=document.getElementById("doc-end-date"),u=document.getElementById("doc-indefinite");r&&r.value&&a.append("startDate",r.value),u&&u.checked?a.append("endDate","Indefinido"):l&&l.value&&a.append("endDate",l.value)}try{await $.upload("/documents",a),c.hide("modal-upload"),document.getElementById("doc-form").reset();const r=document.getElementById("doc-dates-container");r&&(r.style.display="none");const l=document.getElementById("doc-end-date");l&&(l.disabled=!1),c.setText("file-name-display","Respeite o formato .png ou .pdf"),this.fetch(),alert("Documento adicionado com sucesso!")}catch(r){console.error(r),alert("Erro ao subir arquivo.")}},async delete(e){if(confirm("Deletar este documento?"))try{await $.delete(`/documents/${e}`),this.fetch()}catch{alert("Erro ao excluir documento.")}},search(e){if(re.toLowerCase()==="dashboard")this.renderDashboard();else{O=1;const t=Pe.filter(n=>(n.category||"Geral").toLowerCase()===re.toLowerCase()&&n.original_name.toLowerCase().includes(e));this.render(t)}},changePage(e){O=e,this.render(ht)},renderPaginationControls(e,t,n){const o=document.getElementById(e);if(!o)return;if(t===0){o.innerHTML="";return}let a="";a+=`
            <button class="pagination-btn" 
                    ${O===1?"disabled":""} 
                    onclick="window.DocsHandler.changePage(${O-1})"
        ...
        `;const i=(O-1)*Ee+1,r=Math.min(O*Ee,n);a+=`
            <span class="pagination-info">
                Exibindo ${i}-${r} de ${n}
            </span>
        `,o.innerHTML=a}};let oe=[],C={summaries:[]},yt=null,V=null,Qe="list",$e=null,te=null,vt=null,q=1;const Ie=10;let Fe=[];const U={getPendingProcId(){return yt},async fetch(){try{q=1,oe=await $.get("/procedures"),this.renderTable(oe)}catch(e){console.error("Error fetching FAQs:",e)}},getFaqs(){return oe},setListingMode(e){Qe=e,q=1,this.renderTable(Fe.length?Fe:oe)},renderTable(e){const t=document.getElementById("list-table-container"),n=document.getElementById("list-cards-container"),o=document.getElementById("proc-table-body");if(!t||!n||!o)return;Fe=e;const a=e.length,s=Math.ceil(a/Ie);q>s&&(q=Math.max(1,s)),q<1&&(q=1);const i=(q-1)*Ie,r=e.slice(i,i+Ie);Qe==="list"?(c.show("list-table-container"),c.hide("list-cards-container"),r.length===0?o.innerHTML=`
                    <tr>
                        <td colspan="5" style="text-align: center; padding: 2rem; color: var(--text-muted);">
                            Nenhum procedimento encontrado.
                        </td>
                    </tr>
                `:o.innerHTML=r.map(u=>{const d=j.isAdmin()?`
                        <td>
                            <div class="btn-actions-container">
                ...
                `:n.innerHTML=r.map(u=>{const d=j.isAdmin()?`
             ...
             <li class="sidebar-index-item ${a.id===V?"active":""} editable-section style-none"
                 draggable="true" 
            ...
            `).join("");const o=C.summaries.find(a=>a.id===V);o?(n.textContent=o.title,n.style.color="var(--text-main)",t.classList.remove("hidden")):(n.textContent="Nenhum sumário selecionado",n.style.color="var(--accent)",t.classList.add("hidden"))},selectSummary(e){V=e,this.renderProcedureBuilderSidebar(),this.renderProcedureBuilder()},updateSummaryTitle(e,t){const n=C.summaries.find(a=>a.id===e);n&&(n.title=t||"Sem título"),this.renderProcedureBuilderSidebar();const o=C.summaries.find(a=>a.id===V);o&&(document.getElementById("current-summary-name").textContent=o.title)},addSummary(){const e="sum_"+Date.now();C.summaries.push({id:e,title:`Sumário ${C.summaries.length+1}`,sections:[]}),V=e,this.renderProcedureBuilderSidebar(),this.renderProcedureBuilder()},removeSummary(e){confirm("Excluir este sumário apagará todos os campos dentro dele. Deseja continuar?")&&(C.summaries=C.summaries.filter(t=>t.id!==e),V===e&&(V=C.summaries.length>0?C.summaries[0].id:null),this.renderProcedureBuilderSidebar(),this.renderProcedureBuilder())},renderProcedureBuilder(){const e=document.getElementById("procedure-edit-container");if(!e)return;if(!V){e.innerHTML='<p class="empty-state">Crie um novo sumário na barra lateral para adicionar conteúdo.</p>';return}const t=C.summaries.find(o=>o.id===V);if(!t)return;const n=t.sections;if(n.length===0){e.innerHTML=`<p class="empty-state">Nenhum campo em "${t.title}". Clique em "+ Novo Container" para começar.</p>`;return}e.innerHTML=n.map((o,a)=>`
        ...
        `},addFaqItem(e){const t=C.summaries.find(n=>n.id===V);t&&(t.sections[e].data=t.sections[e].data||[],t.sections[e].data.push({q:"",a:""}),this.renderProcedureBuilder())},updateFaqItem(e,t,n,o){const a=C.summaries.find(s=>s.id===V);a&&(a.sections[e].data[t][n]=o)},removeFaqItem(e,t){const n=C.summaries.find(o=>o.id===V);n&&n.sections[e].data.splice(t,1),this.renderProcedureBuilder()},addSection(e,t){if(!V){alert("Selecione primeiro um sumário na barra lateral.");return}const n=C.summaries.find(o=>o.id===V);n&&(n.sections.push({id:Date.now(),title:e,type:t,data:t==="FAQ"?[]:t==="TEXTO"?"":null}),this.renderProcedureBuilder())},async handleSectionFileDrop(e,t){t.dataTransfer.files&&t.dataTransfer.files.length>0&&await this.uploadSectionFile(e,t.dataTransfer.files[0])},async handleSectionFileUpload(e,t){const n=t.files[0];n&&await this.uploadSectionFile(e,n)},async uploadSectionFile(e,t){const n=new FormData;n.append("file",t);try{const o=await $.upload("/upload",n),a=C.summaries.find(s=>s.id===V);a&&(a.sections[e].data={name:t.name,path:o.path,mimetype:t.type},this.renderProcedureBuilder())}catch{alert("Erro no upload")}},async handleSaveProcedure(){const e=parseInt(c.getValue("proc-id"));if(!e)return;const n={...oe.find(o=>o.id===e),content:JSON.stringify(C)};try{await $.put(`/procedures/${e}`,n),alert("Salvo com sucesso!"),this.toggleEditMode(!1),this.openDetail(e),this.fetch()}catch{alert("Erro ao salvar")}},search(e){q=1;const t=oe.filter(n=>(n.name||n.title||"").toLowerCase().includes(e)||(n.responsible||"").toLowerCase().includes(e)||(n.group_name||"").toLowerCase().includes(e));this.renderTable(t)},changePage(e){q=e,this.renderTable(Fe)},renderPaginationControls(e,t,n){const o=document.getElementById(e);if(!o)return;if(t===0){o.innerHTML="";return}let a="";a+=`
            <button class="pagination-btn" 
                    ${q===1?"disabled":""} 
        ...
        `,o.innerHTML=a}};window.toggleGhAccordion=function(e){const t=document.getElementById(e);t&&t.classList.toggle("open")};let Y=[],ge="list",le="month",A=new Date,G=1;const Te=10;let bt=[];const z={async fetch(){try{G=1,Y=await $.get("/accounts"),this.initDashboardMultiselects(),this.populateCompanyFilter(),this.handleSearch(),this.checkAccountAlerts()}catch(e){console.error("Falha ao obter contas",e)}},populateCompanyFilter(){const e=document.getElementById("dash-filter-company-dynamic-options");if(e){const t=new Set;e.querySelectorAll('input[type="checkbox"]:checked').forEach(a=>{t.add(a.value)});const n=[...new Set(Y.map(a=>a.company_name).filter(Boolean))].sort((a,s)=>a.localeCompare(s));let o="";n.forEach(a=>{const s=t.has(a)?"checked":"";o+=`<label class="multiselect-option"><input type="checkbox" value="${a}" ${s}> <span>${a}</span></label>`}),e.innerHTML=o,this.setupMultiselectListeners("dash-filter-company")}},setupMultiselectListeners(e){if(!document.getElementById(`${e}-container`))return;const n=document.getElementById(`${e}-trigger`),o=document.getElementById(`${e}-dropdown`);if(!n||!o)return;n.dataset.listenerBound||(n.addEventListener("click",r=>{r.stopPropagation(),document.querySelectorAll(".multiselect-dropdown").forEach(l=>{l!==o&&l.classList.add("hidden")}),o.classList.toggle("hidden")}),n.dataset.listenerBound="true");const a=o.querySelector('input[value="Todos"]'),s=Array.from(o.querySelectorAll('input[type="checkbox"]')).filter(r=>r.value!=="Todos"),i=()=>{const r=s.filter(u=>u.checked).map(u=>u.value),l=n.querySelector(".trigger-label");a.checked||s.length>0&&r.length===s.length?(a.checked=!0,l&&(l.innerText="Todos")):r.length===0?l&&(l.innerText="Nenhum"):r.length===1?l&&(l.innerText=r[0]):l&&(l.innerText=`${r.length} selecionados`)};a&&!a.dataset.listenerBound&&(a.addEventListener("change",()=>{s.forEach(r=>{r.checked=a.checked}),i(),this.renderDashboard()}),a.dataset.listenerBound="true"),s.forEach(r=>{r.dataset.listenerBound||(r.addEventListener("change",()=>{s.every(u=>u.checked)?a.checked=!0:a.checked=!1,i(),this.renderDashboard()}),r.dataset.listenerBound="true")}),i()},initDashboardMultiselects(){this.setupMultiselectListeners("dash-filter-category"),window.multiselectOutsideClickListenerBound||(document.addEventListener("click",e=>{e.target.closest(".custom-multiselect-container")||document.querySelectorAll(".multiselect-dropdown").forEach(t=>{t.classList.add("hidden")})}),window.multiselectOutsideClickListenerBound=!0)},getMultiselectValues(e){const t=document.getElementById(`${e}-dropdown`);if(!t)return["Todos"];const n=t.querySelector('input[value="Todos"]');return n&&n.checked?["Todos"]:Array.from(t.querySelectorAll('input[type="checkbox"]:checked')).map(o=>o.value).filter(o=>o!=="Todos")},resetMultiselects(){["dash-filter-category","dash-filter-company"].forEach(e=>{const t=document.getElementById(`${e}-dropdown`);if(t){t.querySelectorAll('input[type="checkbox"]').forEach(a=>{a.checked=a.value==="Todos"});const o=document.getElementById(`${e}-trigger`);if(o){const a=o.querySelector(".trigger-label");a&&(a.innerText="Todos")}}})},getAccounts(){return Y},setAccountsViewMode(e){ge=e,this.handleSearch()},setCalendarSubView(e){le=e,this.handleSearch()},shiftCalendarDate(e){le==="day"?A.setDate(A.getDate()+e):le==="month"?A.setMonth(A.getMonth()+e):le==="year"&&A.setFullYear(A.getFullYear()+e),c.setValue("filter-day",A.getDate()),c.setValue("filter-month",A.getMonth()),c.setValue("filter-year",A.getFullYear()),this.handleSearch()},handleFilterChange(e=!1){if(e){const t=c.getValue("filter-cal-year")?parseInt(c.getValue("filter-cal-year")):A.getFullYear(),n=c.getValue("filter-cal-month")?parseInt(c.getValue("filter-cal-month")):A.getMonth();A=new Date(t,n,1)}else{const t=c.getValue("filter-year")?parseInt(c.getValue("filter-year")):A.getFullYear(),n=c.getValue("filter-month")?parseInt(c.getValue("filter-month")):A.getMonth(),o=c.getValue("filter-day")?parseInt(c.getValue("filter-day")):A.getDate();A=new Date(t,n,o)}c.setValue("filter-month",A.getMonth()),c.setValue("filter-year",A.getFullYear()),this.handleSearch()},handleSearch(){const e=(c.getValue("accounts-search")||"").toLowerCase();let t=Y.filter(n=>n.company_name.toLowerCase().includes(e)||n.description&&n.description.toLowerCase().includes(e));if(ge==="list"){G=1;const n=c.getValue("filter-status")||"",o=document.getElementById("filter-date-toggle"),a=o?o.checked:!1,s=A.getFullYear(),i=A.getMonth(),r=A.getDate();t=t.filter(l=>{if(n&&l.status!==n)return!1;if(!a||!l.due_date)return!0;const[u,d,p]=l.due_date.split("-"),m=parseInt(u,10),f=parseInt(d,10)-1,g=parseInt(p,10);return l.type==="Único"?m===s&&f===i&&g===r:l.type==="Recorrente"?g===r:!0}),this.renderAccountsList(t)}else ge==="notificacoes"?this.renderNotifications():ge==="dashboard"?this.renderDashboard():this.renderCalendarWrapper(t)},checkAccountAlerts(){let e=!1;const t=new Date;t.setHours(0,0,0,0),Y.forEach(o=>{const a=(o.status||"").trim().toLowerCase(),s=(o.payment_status||"").trim().toLowerCase();if(a==="on"&&s==="pendente"&&o.due_date){const[i,r,l]=o.due_date.split("-");let u=new Date(parseInt(i,10),parseInt(r,10)-1,parseInt(l,10));u.setHours(0,0,0,0),u.getTime()<=t.getTime()&&(e=!0)}});const n=document.getElementById("icon-alert-bell");n&&(e?n.classList.add("alert-pulse"):n.classList.remove("alert-pulse"))},renderNotifications(){const e=document.getElementById("accounts-notifications-body");if(!e)return;e.innerHTML="";const t=new Date;t.setHours(0,0,0,0);let n=Y.filter(o=>{const a=(o.status||"").trim().toLowerCase(),s=(o.payment_status||"").trim().toLowerCase();if(a!=="on"||s!=="pendente"||!o.due_date)return!1;const[i,r,l]=o.due_date.split("-");let u=new Date(parseInt(i,10),parseInt(r,10)-1,parseInt(l,10));return u.setHours(0,0,0,0),u.getTime()<=t.getTime()});if(n.length===0){e.innerHTML='<tr><td colspan="6" style="text-align:center; color: var(--text-muted); padding: 20px;">Nenhuma conta urgente ou atrasada.</td></tr>';return}n.forEach(o=>{const a=document.createElement("tr");let s="Sem Data";if(o.due_date){const r=o.due_date.split("-");r.length===3&&(s=`${r[2]}/${r[1]}/${r[0]}`)}const i=j.isAdmin()?`
            ...
            `,e.appendChild(a)})},renderAccountsList(e){const t=document.getElementById("accounts-table-body");if(!t)return;t.innerHTML="",this.renderSidebarMiniCalendar(),bt=e;const n=e.length,o=Math.ceil(n/Te);G>o&&(G=Math.max(1,o)),G<1&&(G=1);const a=(G-1)*Te,s=e.slice(a,a+Te);if(s.length===0){t.innerHTML='<tr><td colspan="7" style="text-align:center; color: var(--text-muted); padding: 20px;">Nenhuma conta encontrada.</td></tr>',this.renderPaginationControls("accounts-list-pagination",0,0),this.renderDashboard();return}s.forEach(i=>{const r=document.createElement("tr");let l="Sem Data";if(i.due_date){const p=i.due_date.split("-");p.length===3&&(l=`${p[2]}/${p[1]}/${p[0]}`)}const u=i.status==="Off",d=j.isAdmin()?`
        ...
        `;const i=(G-1)*Te+1,r=Math.min(G*Te,n);a+=`
            <span class="pagination-info">
                Exibindo ${i}-${r} de ${n}
            </span>
        `,o.innerHTML=a}};let ue=[],X={},ie=null,Ke=null,Xe=null,Ze=!1,ke=!1,Le=!1,W=[],Ue=[],rt={},ee={},be,ct,Ye,Je,Tt,We;const kt={init(){be=document.getElementById("timeline-event-form"),ct=document.getElementById("view-visualizacao"),Ye=document.getElementById("view-attention"),Je=document.getElementById("view-anexo"),Tt=document.getElementById("view-relatorio"),We=document.getElementById("view-config"),window.timelineHandler=kt,window.applyFilters=Rt,window.clearFilters=Pt,window.toggleFilters=Ft,window.handleDelete=_t,window.resetForm=mt,window.toggleAccordion=Bt,window.handleFormSubmit=xt,window.editEvent=ut,window.deleteTopic=jt,window.deleteSubtopic=Gt,window.handleTrackDragStart=Ut,window.handleTrackDragOver=Yt,window.handleTrackDragEnd=Jt;const e=document.getElementById("timeline-topic-form");e&&(e.onsubmit=qt);const t=document.getElementById("timeline-subtopic-form");t&&(t.onsubmit=zt);const n=document.getElementById("topico");n&&(n.onchange=u=>{dt(u.target.value)});const o=document.getElementById("em-ocorrencia");o&&(o.onchange=u=>{const d=document.getElementById("fim"),p=document.getElementById("inicio");if(u.target.checked){if(!p.value){const m=new Date;m.setMinutes(m.getMinutes()-m.getTimezoneOffset()),p.value=m.toISOString().slice(0,16)}d.required=!1}else{const m=new Date;m.setMinutes(m.getMinutes()-m.getTimezoneOffset()),d.value=m.toISOString().slice(0,16),d.required=!0}});const a=document.getElementById("auto-refresh-toggle");a&&(a.onchange=u=>{Lt(u.target.checked)}),document.querySelectorAll("[data-timeline-tab]").forEach(u=>{u.onclick=d=>{const p=d.currentTarget.getAttribute("data-timeline-tab");Re(p)}}),be&&(be.onsubmit=xt);const s=document.getElementById("rep-filter-start"),i=document.getElementById("rep-filter-end"),r=document.getElementById("rep-filter-topic"),l=document.getElementById("rep-filter-subtopic");s&&(s.onchange=()=>Ae()),i&&(i.onchange=()=>Ae()),r&&(r.onchange=u=>{Ot(u.target.value),Ae()}),l&&(l.onchange=()=>Ae()),window._timelineSectionChangeHandler&&window.removeEventListener("SectionChange",window._timelineSectionChangeHandler),window._timelineSectionChangeHandler=u=>{u.detail&&u.detail.section==="timeline"&&me().then(()=>{ne(),wt()})},window.addEventListener("SectionChange",window._timelineSectionChangeHandler),me().then(()=>{ne(),wt()})}};window._timelineFocusHandler&&window.removeEventListener("focus",window._timelineFocusHandler);window._timelineFocusHandler=()=>{ct&&ne()};window.addEventListener("focus",window._timelineFocusHandler);function dt(e,t=null){const n=document.getElementById("sub-topico");if(!n)return;const o=e?e.toLowerCase().trim():"";if(!o||!ee[o]){n.innerHTML='<option value="">Selecione o tópico primeiro...</option>',n.classList.remove("has-options");return}n.innerHTML='<option value="" disabled selected>Escolha o evento...</option>',ee[o].forEach(a=>{const s=document.createElement("option");s.value=a.toLowerCase(),s.textContent=a,t&&s.value===t.toLowerCase()&&(s.selected=!0),n.appendChild(s)}),t||(n.selectedIndex=1),n.classList.add("has-options")}async function me(){try{const e=await fetch("/api/timeline/config");if(!e.ok)throw new Error("Falha ao buscar configurações");const t=await e.json();W=t.topics||[],Ue=t.subtopics||[],rt={},ee={},W.forEach(o=>{rt[o.id]=o.color,ee[o.id]=[]}),Ue.forEach(o=>{const a=o.topic_id;ee[a]&&ee[a].push(o.name)}),Ht();const n=document.getElementById("view-config");n&&n.classList.contains("active")&&St()}catch(e){console.error("Error loading config:",e)}}function Ht(){const e=document.getElementById("topico");if(e){const o=e.value;e.innerHTML='<option value="" disabled selected>Selecione um tópico...</option>',W.forEach(a=>{const s=document.createElement("option");s.value=a.id,s.textContent=a.name,e.appendChild(s)}),e.value=o}const t=document.getElementById("rep-filter-topic");if(t){const o=t.value;t.innerHTML='<option value="Todos">Todos</option>',W.forEach(a=>{const s=document.createElement("option");s.value=a.id,s.textContent=a.name,t.appendChild(s)}),o&&[...t.options].some(a=>a.value===o)?t.value=o:t.value="Todos"}const n=document.getElementById("subtopic-topic-id");n&&(n.innerHTML='<option value="" disabled selected>Selecione um tópico...</option>',W.forEach(o=>{const a=document.createElement("option");a.value=o.id,a.textContent=o.name,n.appendChild(a)}))}function ne(){fetch("/api/timeline/events").then(e=>{if(!e.ok)throw new Error("Failed to fetch");return e.json()}).then(e=>{ue=e,pt(),Ye&&Ye.classList.contains("active")&&Ct()}).catch(e=>{console.error("Error loading events:",e)})}function wt(){const e=document.getElementById("timeline-tab-anexo"),t=document.getElementById("timeline-tab-config");if(window.auth&&window.auth.isAdmin())e&&e.classList.remove("role-hidden"),t&&t.classList.remove("role-hidden");else{e&&e.classList.add("role-hidden"),t&&t.classList.add("role-hidden");const o=Je&&Je.classList.contains("active"),a=We&&We.classList.contains("active");(o||a)&&Re("visualizacao")}}function Re(e){const t={visualizacao:{section:ct,button:document.querySelector('[data-timeline-tab="visualizacao"]')},attention:{section:Ye,button:document.querySelector('[data-timeline-tab="attention"]')},anexo:{section:Je,button:document.querySelector('[data-timeline-tab="anexo"]')},relatorio:{section:Tt,button:document.querySelector('[data-timeline-tab="relatorio"]')},config:{section:We,button:document.querySelector('[data-timeline-tab="config"]')}};Object.values(t).forEach(n=>{n.section&&n.section.classList.remove("active"),n.button&&n.button.classList.remove("active")}),t[e]&&(t[e].section&&t[e].section.classList.add("active"),t[e].button&&t[e].button.classList.add("active")),e==="visualizacao"?(ne(),Be(!0)):e==="attention"?(Ct(),Be(!0)):e==="relatorio"?(Ae(),Be(!1)):(e==="config"&&St(),Be(!1))}function Be(e){const t=document.getElementById("floating-refresh-control");if(t)if(e){t.classList.remove("hidden");const n=document.getElementById("auto-refresh-toggle");n&&n.checked&&!ie&&Lt(!0)}else t.classList.add("hidden"),ie&&(clearInterval(ie),ie=null)}function Lt(e){ie&&(clearInterval(ie),ie=null),e&&(ne(),ie=setInterval(ne,6e4))}function xt(e){if(e.preventDefault(),Ze){console.warn("[Timeline] O salvamento já está em andamento. Ignorando envio duplicado.");return}Ze=!0;const t=be.querySelector('button[type="submit"]');t&&(t.textContent="Salvando...",t.disabled=!0);const o={id:document.getElementById("event-id").value||Date.now().toString(),nome:document.getElementById("nome").value,topico:document.getElementById("topico").value,sub_topico:document.getElementById("sub-topico").value,em_ocorrencia:document.getElementById("em-ocorrencia").checked?1:0,inicio:document.getElementById("inicio").value,fim:document.getElementById("fim").value,descricao:document.getElementById("descricao").value,anotacao:document.getElementById("anotacao").value,cor:document.getElementById("cor").value};fetch("/api/timeline/events",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)}).then(async a=>{const s=await a.text();if(!a.ok)throw new Error(`Server error (${a.status}): ${s}`);return JSON.parse(s)}).then(()=>{alert("Evento salvo com sucesso!"),mt(),Re("visualizacao")}).catch(a=>{console.error("Error saving event:",a),alert("Erro ao salvar evento: "+a.message)}).finally(()=>{t&&(t.textContent="Salvar Evento",t.disabled=!1),Ze=!1})}function ut(e){const t=ue.find(s=>s.id===e);if(!t)return;document.getElementById("event-id").value=t.id,document.getElementById("nome").value=t.nome;const n=Me(t.topico);document.getElementById("topico").value=n,dt(n,t.sub_topico);const o=document.getElementById("em-ocorrencia");o.checked=t.em_ocorrencia==1||t.em_ocorrencia==="true"||!t.fim,o.dispatchEvent(new Event("change")),document.getElementById("inicio").value=t.inicio,document.getElementById("fim").value=t.fim||"",document.getElementById("descricao").value=t.descricao||"",document.getElementById("anotacao").value=t.anotacao||"",document.getElementById("cor").value=t.cor||"#000000",Re("anexo");const a=document.getElementById("btn-delete");a&&(a.style.display="block")}function mt(){be&&be.reset();const e=document.getElementById("event-id");e&&(e.value=""),dt("");const t=document.getElementById("fim");t&&(t.required=!0);const n=document.getElementById("cor");n&&(n.value="#000000");const o=document.getElementById("btn-delete");o&&(o.style.display="none")}function _t(){const e=document.getElementById("event-id").value;e&&confirm("Tem certeza que deseja excluir este evento?")&&fetch(`/api/timeline/events/${e}`,{method:"DELETE"}).then(t=>{if(!t.ok)throw new Error("Failed to delete");return t.json()}).then(()=>{alert("Evento excluído!"),mt(),Re("visualizacao")}).catch(t=>{console.error("Error deleting:",t),alert("Erro ao excluir: "+t.message)})}function Rt(e){const t=document.getElementById(`filter-start-${e}`),n=document.getElementById(`filter-end-${e}`),o=document.getElementById(`filter-sub-topic-${e}`),a=t&&t.value?new Date(t.value).getTime():null,s=n&&n.value?new Date(n.value).getTime():null,i=o?o.value:"";X[e]={start:a,end:s,subTopic:i},pt()}function Pt(e){const t=document.getElementById(`filter-start-${e}`),n=document.getElementById(`filter-end-${e}`),o=document.getElementById(`filter-sub-topic-${e}`);t&&(t.value=""),n&&(n.value=""),o&&(o.value=""),X[e]=null,pt()}function Ft(e){const t=document.getElementById(`filters-panel-${e}`),n=document.getElementById(`btn-toggle-${e}`);t&&n&&(t.classList.toggle("hidden"),n.classList.toggle("active"))}function Bt(e){const t=document.getElementById(e);t&&t.classList.toggle("active")}function pt(){const e=document.getElementById("timeline-tracks-container");if(!e)return;const t=Array.from(e.querySelectorAll(".timeline-container")).map(a=>a.dataset.topicId),n=W.map(a=>a.id);if(t.length!==n.length||!n.every(a=>t.includes(a))){e.innerHTML="";const a=window.auth&&window.auth.isAdmin(),s=a?'style="cursor: grab;"':"";W.forEach(i=>{const r=`
            ...
            `;e.insertAdjacentHTML("beforeend",r);const l=document.getElementById(`filter-sub-topic-${i.id}`);l&&ee[i.id]&&ee[i.id].forEach(u=>{const d=document.createElement("option");d.value=u.toLowerCase(),d.textContent=u,l.appendChild(d)})})}W.forEach(a=>{const s=document.getElementById(`track-${a.id}`),i=document.getElementById(`min-date-${a.id}`),r=document.getElementById(`max-date-${a.id}`);s&&(s.innerHTML=""),i&&(i.textContent=""),r&&(r.textContent="")}),ue.length!==0&&W.forEach(a=>{const s=a.id,i=ue.filter(y=>Me(y.topico)===s);let r=i;X[s]&&X[s].subTopic&&(r=i.filter(y=>(y.sub_topico?y.sub_topico.toLowerCase():"")===X[s].subTopic.toLowerCase()));const l=X[s]&&X[s].start?X[s].start:new Date("2026-01-01T00:00:00").getTime(),u=X[s]&&X[s].end?X[s].end:Date.now();Vt(s,r,l,u);const d=l,p=u,m=p-d,f=document.getElementById(`min-date-${s}`),g=document.getElementById(`max-date-${s}`);f&&(f.textContent=new Date(d).toLocaleDateString()+" "+new Date(d).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})),g&&(g.textContent=new Date(p).toLocaleDateString()+" "+new Date(p).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}));const h=document.getElementById(`track-${s}`);h&&r.forEach(y=>{const k=new Date(y.inicio).getTime(),w=y.fim?new Date(y.fim).getTime():Date.now();if(w<d||k>p)return;const E=Math.max(k,d),D=Math.min(w,p),L=(E-d)/m*100,I=(D-E)/m*100;if(I<=0)return;const R=document.createElement("div");R.className="timeline-bar",R.style.left=`${L}%`,R.style.width=`${I}%`,R.style.color=y.cor&&y.cor!=="#000000"?y.cor:rt[s]||"#6b7280";const x=document.createElement("div");x.className="timeline-bar-visual",R.appendChild(x);const S=document.createElement("div");S.className="timeline-identifier-point";const v=new Date(y.inicio).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),b=y.fim?new Date(y.fim).toLocaleString([],{dateStyle:"short",timeStyle:"short"}):"Em andamento",B=a.name,T=y.sub_topico?y.sub_topico.charAt(0).toUpperCase()+y.sub_topico.slice(1):"-";S.setAttribute("data-tooltip",`Tópico: ${B}
Eventos: ${T}
Início: ${v} - Fim: ${b}
Descrição: ${y.descricao||"-"}`),!y.fim&&S.classList.add("pulsing"),window.auth&&window.auth.isAdmin()?(S.style.cursor="pointer",S.onclick=F=>{F.stopPropagation(),ut(y.id)}):S.style.cursor="default",R.appendChild(S),h.appendChild(R)})})}function Me(e){return e?e.toLowerCase().trim():""}function Vt(e,t,n,o){const a=document.getElementById(`sla-${e}`);if(!a)return;const s=o-n;if(s<=0){a.textContent="N/A";return}const r=t.filter(m=>{const f=new Date(m.inicio).getTime();return(m.fim?new Date(m.fim).getTime():Date.now())>n&&f<o}).map(m=>({start:Math.max(new Date(m.inicio).getTime(),n),end:Math.min(m.fim?new Date(m.fim).getTime():Date.now(),o)}));r.sort((m,f)=>m.start-f.start);const l=[];if(r.length>0){let m=r[0];for(let f=1;f<r.length;f++){const g=r[f];g.start<m.end?m.end=Math.max(m.end,g.end):(l.push(m),m=g)}l.push(m)}let u=0;l.forEach(m=>{u+=m.end-m.start});const d=(s-u)/s*100;let p="#10b981";d<50?p="#ef4444":d<90&&(p="#f97316"),a.style.color=p,a.textContent=d.toFixed(4)+"%"}function Ct(){const e=document.getElementById("attention-topics-container");if(!e)return;e.innerHTML="";const t=ue.filter(n=>!n.fim);W.forEach(n=>{const o=n.id,a=t.filter(h=>Me(h.topico)===o),s=document.createElement("div");s.className=a.length>0?"accordion-item active":"accordion-item",s.id=`attn-acc-${o}`;const i=document.createElement("div");i.className="accordion-header",i.onclick=()=>Bt(`attn-acc-${o}`);const r=document.createElement("div");r.className="accordion-title-group";const l=document.createElement("div");l.className="topic-indicator",l.style.backgroundColor=n.color;const u=document.createElement("h3");u.textContent=n.name;const d=document.createElement("span");d.style.cssText="background: #f1f5f9; padding: 2px 8px; border-radius: 12px; font-size: 0.95rem; font-weight: 900; color: #0f172a; margin-left: 0.5rem; border: 1px solid #cbd5e1;",d.textContent=`${a.length}`,r.appendChild(l),r.appendChild(u),r.appendChild(d);const p=document.createElement("span");p.className="accordion-chevron",p.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>',i.appendChild(r),i.appendChild(p);const m=document.createElement("div");m.className="accordion-content";const f=document.createElement("div");f.className="accordion-body";const g=document.createElement("div");if(g.className="attention-carousel",a.length===0){const h=document.createElement("div");h.className="empty-state",h.textContent="Nenhum evento em andamento.",g.appendChild(h)}else a.forEach(h=>{const y=document.createElement("div");y.className="attention-card",y.style.borderLeftColor=h.cor&&h.cor!=="#000000"?h.cor:n.color;const k=document.createElement("h3");k.textContent=h.nome;const w=document.createElement("div");w.className="sub-topic",w.textContent=h.sub_topico||"-";const E=document.createElement("div");E.className="card-detail",E.innerHTML=`<strong>Início:</strong> ${new Date(h.inicio).toLocaleString()}`;const D=Date.now()-new Date(h.inicio).getTime(),L=document.createElement("div");L.className="card-duration",L.innerHTML=`<strong>Tempo:</strong> <span>${Nt(D)}</span>`;const I=document.createElement("div");I.className="card-description",I.textContent=h.descricao||"-",y.appendChild(k),y.appendChild(w),y.appendChild(E),y.appendChild(L),y.appendChild(I),window.auth&&window.auth.isAdmin()?(y.style.cursor="pointer",y.onclick=()=>ut(h.id)):y.style.cursor="default",g.appendChild(y)});f.appendChild(g),m.appendChild(f),s.appendChild(i),s.appendChild(m),e.appendChild(s)})}function Nt(e){if(e<0)return"0s";const t=Math.floor(e/1e3),n=Math.floor(t/60),o=Math.floor(n/60),a=Math.floor(o/24),s=[];return a>0&&s.push(`${a}d`),(o%24>0||a>0)&&s.push(`${o%24}h`),(n%60>0||o>0)&&s.push(`${n%60}m`),s.push(`${t%60}s`),s.join(" ")}function Ot(e){const t=document.getElementById("rep-filter-subtopic");if(!t)return;t.innerHTML='<option value="Todos">Todos</option>';const n=e?e.toLowerCase().trim():"";n&&ee[n]&&ee[n].forEach(o=>{const a=document.createElement("option");a.value=o.toLowerCase(),a.textContent=o,t.appendChild(a)})}function Ae(){let e=ue;const t=document.getElementById("rep-filter-start")?.value,n=document.getElementById("rep-filter-end")?.value,o=document.getElementById("rep-filter-topic")?.value,a=document.getElementById("rep-filter-subtopic")?.value;if(t){const I=new Date(t+"T00:00:00").getTime();e=e.filter(R=>new Date(R.inicio).getTime()>=I)}if(n){const I=new Date(n+"T23:59:59").getTime();e=e.filter(R=>new Date(R.inicio).getTime()<=I)}o&&o!=="Todos"&&(e=e.filter(I=>Me(I.topico)===o.toLowerCase())),a&&a!=="Todos"&&(e=e.filter(I=>I.sub_topico&&I.sub_topico.toLowerCase()===a.toLowerCase()));const s=document.getElementById("rep-kpi-total"),i=document.getElementById("rep-kpi-active"),r=document.getElementById("rep-kpi-avg-time");s&&(s.textContent=e.length);const l=e.filter(I=>I.em_ocorrencia==1||I.em_ocorrencia==="true"||!I.fim);i&&(i.textContent=l.length);const u=e.filter(I=>I.fim);let d="0h 0m";if(u.length>0){const R=u.reduce((b,B)=>b+(new Date(B.fim).getTime()-new Date(B.inicio).getTime()),0)/u.length,x=Math.floor(R/6e4),S=Math.floor(x/60),v=x%60;d=`${S}h ${v}m`}if(r&&(r.textContent=d),!window.Chart){console.warn("Chart.js is not loaded.");return}const p=W,m=t?new Date(t+"T00:00:00").getTime():new Date(new Date().getFullYear()+"-01-01T00:00:00").getTime(),f=n?new Date(n+"T23:59:59").getTime():Date.now(),g=p.map(I=>I.name),h=p.map(I=>{const R=I.id,x=ue.filter(H=>Me(H.topico)===R),S=f-m;if(S<=0)return 100;const b=x.filter(H=>{const Q=new Date(H.inicio).getTime();return(H.fim?new Date(H.fim).getTime():Date.now())>m&&Q<f}).map(H=>({start:Math.max(new Date(H.inicio).getTime(),m),end:Math.min(H.fim?new Date(H.fim).getTime():Date.now(),f)}));b.sort((H,Q)=>H.start-Q.start);const B=[];if(b.length>0){let H=b[0];for(let Q=1;Q<b.length;Q++){const se=b[Q];se.start<H.end?H.end=Math.max(H.end,se.end):(B.push(H),H=se)}B.push(H)}const _=(H=>{let Q=0;return H.forEach(se=>{Q+=se.end-se.start}),Q})(B),F=(S-_)/S*100;return parseFloat(F.toFixed(4))}),y=p.map(I=>I.color||"#6b7280"),k=document.getElementById("chart-rep-sla");k&&(Ke&&Ke.destroy(),Ke=new window.Chart(k,{type:"bar",data:{labels:g,datasets:[{label:"Disponibilidade %",data:h,backgroundColor:y,borderRadius:6}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{y:{min:Math.max(0,Math.min(...h)-5),max:100,ticks:{callback:I=>I+"%"}}}}}));const w={};e.forEach(I=>{const R=I.sub_topico?I.sub_topico.charAt(0).toUpperCase()+I.sub_topico.slice(1).toLowerCase():"Não especificado";w[R]=(w[R]||0)+1});const E=Object.keys(w),D=Object.values(w),L=document.getElementById("chart-rep-qty");L&&(Xe&&Xe.destroy(),Xe=new window.Chart(L,{type:"doughnut",data:{labels:E.length>0?E:["Nenhum evento"],datasets:[{data:D.length>0?D:[0],backgroundColor:["#3b82f6","#10b981","#f59e0b","#8b5cf6","#ec4899","#6366f1","#14b8a6","#f43f5e","#a855f7","#06b6d4"]}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{position:"right",labels:{boxWidth:12}}}}}))}function qt(e){if(e.preventDefault(),ke)return;ke=!0;const t=document.getElementById("topic-id"),n=document.getElementById("topic-name"),o=document.getElementById("topic-color");if(!t||!n||!o){ke=!1;return}const a={id:t.value.trim().toLowerCase(),name:n.value.trim(),color:o.value};if(!a.id){alert("Por favor, defina um ID para o tópico."),ke=!1;return}fetch("/api/timeline/config/topics",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)}).then(s=>{if(!s.ok)throw new Error("Erro ao salvar tópico");return s.json()}).then(()=>(alert("Tópico salvo com sucesso!"),t.value="",n.value="",o.value="#3b82f6",me().then(()=>{ne()}))).catch(s=>{console.error(s),alert("Erro: "+s.message)}).finally(()=>{ke=!1})}function zt(e){if(e.preventDefault(),Le)return;Le=!0;const t=document.getElementById("subtopic-topic-id"),n=document.getElementById("subtopic-name");if(!t||!n){Le=!1;return}const o={topic_id:t.value,name:n.value.trim()};if(!o.topic_id||!o.name){alert("Preencha todos os campos do evento."),Le=!1;return}fetch("/api/timeline/config/subtopics",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)}).then(a=>{if(!a.ok)throw new Error("Erro ao adicionar evento");return a.json()}).then(()=>(alert("Evento adicionado!"),n.value="",me())).catch(a=>{console.error(a),alert("Erro: "+a.message)}).finally(()=>{Le=!1})}function jt(e){confirm("Excluir este tópico também removerá todos os seus eventos associados. Deseja continuar?")&&fetch(`/api/timeline/config/topics/${e}`,{method:"DELETE"}).then(t=>{if(!t.ok)throw new Error("Erro ao excluir tópico");return t.json()}).then(()=>{alert("Tópico excluído!"),me().then(()=>{ne()})}).catch(t=>{console.error(t),alert("Erro: "+t.message)})}function Gt(e){confirm("Deseja realmente excluir este evento?")&&fetch(`/api/timeline/config/subtopics/${e}`,{method:"DELETE"}).then(t=>{if(!t.ok)throw new Error("Erro ao excluir evento");return t.json()}).then(()=>{alert("Evento excluído!"),me()}).catch(t=>{console.error(t),alert("Erro: "+t.message)})}function St(){const e=document.getElementById("config-topics-list");e&&(e.innerHTML="",W.length===0?e.innerHTML='<div style="color: var(--text-muted); font-size: 0.9rem;">Nenhum tópico cadastrado.</div>':W.forEach(n=>{const o=document.createElement("div");o.style.cssText="display: flex; align-items: center; justify-content: space-between; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 8px; border: 1px solid var(--glass-border); margin-bottom: 6px;",o.innerHTML=`
            ...
            `}).join("")},toggleQueueRow(e){const t=document.getElementById(`queue-details-${e}`),n=document.getElementById(`queue-arrow-${e}`);t&&(t.classList.toggle("hidden"),n&&(t.classList.contains("hidden")?n.style.transform="rotate(0deg)":n.style.transform="rotate(180deg)"))},toggleBlfRow(e){const t=document.getElementById(`blf-details-${e}`),n=document.getElementById(`blf-arrow-${e}`);t&&(t.classList.toggle("hidden"),n&&(t.classList.contains("hidden")?n.style.transform="rotate(0deg)":n.style.transform="rotate(180deg)"))},toggleUserSecret(e){alert("Por segurança do PABX Gnew, as senhas dos usuários do portal são armazenadas com criptografia unidirecional na base e não podem ser lidas em texto claro.")},search(e){N=1;const n=this.getActiveDataList().filter(o=>P==="extensions"?(o.exten||"").toLowerCase().includes(e)||(o.nome||"").toLowerCase().includes(e)||(o.Username||"").toLowerCase().includes(e)||(o.ddr||"").toLowerCase().includes(e)||(o.observacao||"").toLowerCase().includes(e):P==="queues"?(o.exten||"").toLowerCase().includes(e)||(o.nome||"").toLowerCase().includes(e)||(o.Estrategia||"").toLowerCase().includes(e):P==="blf"?(o.Nome||"").toLowerCase().includes(e):P==="users"?(o.username||"").toLowerCase().includes(e)||(o.email||"").toLowerCase().includes(e)||(o.Tipo||"").toLowerCase().includes(e):!1);this.render(n)},changePage(e){N=e,this.render(ot)},setPageSize(e){he=parseInt(e,10),N=1,this.render(ot)},toggleSecret(e,t){const n=document.getElementById(`secret-txt-${e}`),o=document.getElementById(`secret-icon-${e}`);!n||!o||(n.textContent==="••••••••"?(n.textContent=t,o.innerHTML=`
            ...
            <button class="pagination-btn" 
                    ${N===1?"disabled":""} 
                    onclick="window.TelephonyHandler.changePage(${N-1})"
                    title="Página Anterior">
                &laquo;
            </button>
        `;let s=0;for(let l=1;l<=t;l++)(l===1||l===t||l>=N-1&&l<=N+1)&&(s&&l-s>1&&(a+='<span style="color: var(--text-muted); padding: 0 4px;">...</span>'),a+=`
        ...
        `,o.innerHTML=a}},Se=30;let De="",at="all",Ve="",Ne="",K=1,ye=0,J="alerts",Z="switches",M=null,it=[],Et=[],$t=[],It=[],Oe=null,qe=null,ze=null,je=null,ve=!1;const gt={init(){console.log("📊 [MONITORING] Initializing monitoringHandler...");const e=document.getElementById("tab-monitoring-alerts");e&&e.addEventListener("click",()=>this.setActiveTab("alerts"));const t=document.getElementById("tab-monitoring-events");t&&t.addEventListener("click",()=>this.setActiveTab("events"));const n=document.getElementById("tab-monitoring-apis");n&&n.addEventListener("click",()=>this.setActiveTab("apis"));const o=document.getElementById("tab-monitoring-gnew");o&&o.addEventListener("click",()=>this.setActiveTab("gnew"));const a=document.getElementById("tab-monitoring-infra");a&&a.addEventListener("click",()=>{console.log("📊 [MONITORING] Clicked on Infraestrutura tab"),this.setActiveTab("infra")});const s=document.getElementById("tab-infra-switches");s&&s.addEventListener("click",()=>{console.log("📊 [MONITORING] Clicked on Switches subtab"),this.setInfraTab("switches")});const i=document.getElementById("tab-infra-routers");i&&i.addEventListener("click",()=>{console.log("📊 [MONITORING] Clicked on Routers subtab"),this.setInfraTab("routers")});const r=document.getElementById("tab-infra-nas");r&&r.addEventListener("click",()=>{console.log("📊 [MONITORING] Clicked on NAS subtab"),this.setInfraTab("nas")});const l=document.getElementById("btn-refresh-switches-status");l&&l.addEventListener("click",()=>this.fetchAndRenderSwitchesStatus(!0));const u=document.getElementById("btn-refresh-routers-status");u&&u.addEventListener("click",()=>this.fetchAndRenderRoutersStatus(!0));const d=document.getElementById("btn-refresh-nas-status");d&&d.addEventListener("click",()=>this.fetchAndRenderNasStatus(!0));const p=document.getElementById("monitoring-events-search-input");p&&p.addEventListener("input",b=>{De=b.target.value.toLowerCase(),K=1,this.fetchAndRenderEventHistory()});const m=document.getElementById("monitoring-search-input");m&&m.addEventListener("input",()=>{this.renderGnewServicesStatus()});const f=document.getElementById("monitoring-events-severity-filter");f&&f.addEventListener("change",b=>{at=b.target.value,K=1,this.fetchAndRenderEventHistory()});const g=document.getElementById("monitoring-events-date-start"),h=document.getElementById("monitoring-events-date-end");g&&g.addEventListener("change",b=>{Ve=b.target.value,K=1,this.fetchAndRenderEventHistory()}),h&&h.addEventListener("change",b=>{Ne=b.target.value,K=1,this.fetchAndRenderEventHistory()});const y=document.getElementById("btn-clear-event-date-filter");y&&y.addEventListener("click",()=>{Ve="",Ne="",K=1,g&&(g.value=""),h&&(h.value=""),this.fetchAndRenderEventHistory()});const k=document.getElementById("btn-clear-event-history");k&&k.addEventListener("click",()=>this.clearEventHistory());const w=document.getElementById("btn-refresh-monitoring");w&&w.addEventListener("click",()=>this.fetchDiagnostics());const E=document.getElementById("gnew-disk-accordion-header");E&&E.addEventListener("click",()=>{const b=document.getElementById("gnew-disk-accordion-content"),B=document.getElementById("gnew-disk-chevron");b&&B&&(b.style.maxHeight==="0px"?(b.style.maxHeight="1000px",B.style.transform="rotate(0deg)"):(b.style.maxHeight="0px",B.style.transform="rotate(-90deg)"))});const D=document.getElementById("btn-refresh-gnew-disk");D&&D.addEventListener("click",b=>{b.stopPropagation(),this.fetchDiagnostics()});const L=document.getElementById("btn-refresh-gnew-services");L&&L.addEventListener("click",async()=>{const b=L,B=b.querySelector("svg");if(!b.disabled){b.disabled=!0,b.style.opacity="0.6",b.style.cursor="not-allowed",B&&(B.style.animation="spin 0.8s linear infinite");try{await this.fetchDiagnostics()}finally{b.disabled=!1,b.style.opacity="",b.style.cursor="pointer",B&&(B.style.animation="")}}});const I=document.getElementById("btn-refresh-apis-status");I&&I.addEventListener("click",()=>this.fetchAndRenderApisStatus());const R=document.getElementById("monitoring-auto-refresh");R&&(R.addEventListener("change",b=>{b.target.checked?this._startAutoRefresh():this._stopAutoRefresh()}),R.checked&&this._startAutoRefresh());const x=document.getElementById("switches-auto-refresh");x&&(x.addEventListener("change",b=>{b.target.checked?this._startSwitchesAutoRefresh():this._stopSwitchesAutoRefresh()}),x.checked&&this._startSwitchesAutoRefresh());const S=document.getElementById("routers-auto-refresh");S&&(S.addEventListener("change",b=>{b.target.checked?this._startRoutersAutoRefresh():this._stopRoutersAutoRefresh()}),S.checked&&this._startRoutersAutoRefresh());const v=document.getElementById("nas-auto-refresh");v&&(v.addEventListener("change",b=>{b.target.checked?this._startNasAutoRefresh():this._stopNasAutoRefresh()}),v.checked&&this._startNasAutoRefresh()),window.monitoringHandler=this},_startAutoRefresh(){this._stopAutoRefresh(),Oe=setInterval(()=>{(J==="alerts"||J==="gnew")&&this.fetchDiagnostics()},3e4)},_stopAutoRefresh(){Oe&&(clearInterval(Oe),Oe=null)},_startSwitchesAutoRefresh(){this._stopSwitchesAutoRefresh(),qe=setInterval(()=>{J==="infra"&&Z==="switches"&&this.fetchAndRenderSwitchesStatus(!1,!0)},6e4)},_stopSwitchesAutoRefresh(){qe&&(clearInterval(qe),qe=null)},_startRoutersAutoRefresh(){this._stopRoutersAutoRefresh(),ze=setInterval(()=>{J==="infra"&&Z==="routers"&&this.fetchAndRenderRoutersStatus(!1,!0)},6e4)},_stopRoutersAutoRefresh(){ze&&(clearInterval(ze),ze=null)},_startNasAutoRefresh(){this._stopNasAutoRefresh(),je=setInterval(()=>{J==="infra"&&Z==="nas"&&this.fetchAndRenderNasStatus(!1,!0)},6e4)},_stopNasAutoRefresh(){je&&(clearInterval(je),je=null)},fetch(){this.setActiveTab("alerts"),this.fetchDiagnostics()},setActiveTab(e){J=e;const t=document.getElementById("tab-monitoring-alerts"),n=document.getElementById("tab-monitoring-events"),o=document.getElementById("tab-monitoring-apis"),a=document.getElementById("tab-monitoring-gnew"),s=document.getElementById("tab-monitoring-infra");t&&t.classList.toggle("active",e==="alerts"),n&&n.classList.toggle("active",e==="events"),o&&o.classList.toggle("active",e==="apis"),a&&a.classList.toggle("active",e==="gnew"),s&&s.classList.toggle("active",e==="infra");const i=document.getElementById("monitoring-tab-content-alerts"),r=document.getElementById("monitoring-tab-content-events"),l=document.getElementById("monitoring-tab-content-apis"),u=document.getElementById("monitoring-tab-content-gnew"),d=document.getElementById("monitoring-tab-content-infra");i&&(i.classList.toggle("hidden",e!=="alerts"),i.classList.toggle("active",e==="alerts")),r&&(r.classList.toggle("hidden",e!=="events"),r.classList.toggle("active",e==="events")),l&&(l.classList.toggle("hidden",e!=="apis"),l.classList.toggle("active",e==="apis")),u&&(u.classList.toggle("hidden",e!=="gnew"),u.classList.toggle("active",e==="gnew")),d&&(d.classList.toggle("hidden",e!=="infra"),d.classList.toggle("active",e==="infra")),e==="gnew"?this.fetchDiagnostics():e==="events"?(K=1,this.fetchAndRenderEventHistory()):e==="apis"?this.fetchAndRenderApisStatus():e==="infra"?this.setInfraTab(Z):this.renderGnewServicesStatus()},setInfraTab(e){console.log("📊 [MONITORING] setInfraTab called with:",e),Z=e;const t=document.getElementById("tab-infra-switches"),n=document.getElementById("tab-infra-routers"),o=document.getElementById("tab-infra-nas");t&&t.classList.toggle("active",e==="switches"),n&&n.classList.toggle("active",e==="routers"),o&&o.classList.toggle("active",e==="nas");const a=document.getElementById("infra-tab-content-switches"),s=document.getElementById("infra-tab-content-routers"),i=document.getElementById("infra-tab-content-nas");a&&(a.classList.toggle("hidden",e!=="switches"),a.classList.toggle("active",e==="switches")),s&&(s.classList.toggle("hidden",e!=="routers"),s.classList.toggle("active",e==="routers")),i&&(i.classList.toggle("hidden",e!=="nas"),i.classList.toggle("active",e==="nas")),e==="switches"?this.fetchAndRenderSwitchesStatus():e==="routers"?this.fetchAndRenderRoutersStatus():e==="nas"&&this.fetchAndRenderNasStatus()},render(){J==="alerts"?this.renderGnewServicesStatus():J==="events"?this.fetchAndRenderEventHistory():J==="apis"?this.fetchAndRenderApisStatus():J==="infra"&&(Z==="switches"?this.fetchAndRenderSwitchesStatus():Z==="routers"?this.fetchAndRenderRoutersStatus():Z==="nas"&&this.fetchAndRenderNasStatus())},renderGnewServicesStatus(){const e=document.getElementById("monitoring-alerts-grid");if(!e)return;e.style.display="flex",e.style.flexDirection="column",e.style.gap="0";const t=M&&M.servicos&&Array.isArray(M.servicos.servicos)?M.servicos.servicos:[],n=it||[],o=Et||[],a=$t||[],s=It||[];if(t.length===0&&n.length===0&&o.length===0&&a.length===0&&s.length===0){e.innerHTML=`
            ...
            `;return}const i=t.length+n.length+o.length+a.length+s.length,r=t.filter(v=>v.status!=="active"&&v.status_label!=="ativo").length,l=n.filter(v=>!v.online||v.status==="warning").length,u=o.filter(v=>!v.online).length,d=a.filter(v=>!v.online).length,p=s.filter(v=>!v.online).length,m=r+l+u+d+p,f=i-m,g=document.getElementById("monitor-kpi-total"),h=document.getElementById("monitor-kpi-warning"),y=document.getElementById("monitor-kpi-info");g&&(g.textContent=i),h&&(h.textContent=m),y&&(y.textContent=f);const k=document.getElementById("monitoring-search-input"),w=k?k.value.toLowerCase().trim():"";let E=t,D=n,L=o,I=a,R=s;w&&(E=t.filter(v=>v.nome.toLowerCase().includes(w)),D=n.filter(v=>v.name.toLowerCase().includes(w)||v.description.toLowerCase().includes(w)),L=o.filter(v=>v.name.toLowerCase().includes(w)||v.ip.toLowerCase().includes(w)),I=a.filter(v=>v.name.toLowerCase().includes(w)||v.ip.toLowerCase().includes(w)),R=s.filter(v=>v.name.toLowerCase().includes(w)||v.ip.toLowerCase().includes(w)));let x=`
        ...
        `,S=0;E.forEach(v=>{const b=v.status==="active"||v.status_label==="ativo",B=b?"#10b981":"#ef4444",T=b?"Online":v.status_label||v.status||"Offline",_=b?"rgba(16,185,129,0.12)":"rgba(239,68,68,0.12)",F=b?"#6ee7b7":"#fca5a5",H=b?"rgba(16,185,129,0.3)":"rgba(239,68,68,0.3)",Q=S%2===0?"transparent":"rgba(255,255,255,0.015)";S++,x+=`
                ...
                </div>`}),D.forEach(v=>{let b="#10b981",B="Online",T="rgba(16,185,129,0.12)",_="#6ee7b7",F="rgba(16,185,129,0.3)";v.status==="warning"?(b="#f59e0b",B="Alerta",T="rgba(245,158,11,0.12)",_="#fde047",F="rgba(245,158,11,0.3)"):(v.status==="offline"||!v.online)&&(b="#ef4444",B="Offline",T="rgba(239,68,68,0.12)",_="#fca5a5",F="rgba(239,68,68,0.3)");const H=S%2===0?"transparent":"rgba(255,255,255,0.015)";S++,x+=`
                <div class="monitor-list-row" style="background: ${H};">
                    ...
                    <div class="monitor-list-col-status">
                        <span class="monitor-badge" style="background:${T}; color:${_}; border-color:${F};">${B}</span>
                    </div>
                </div>`}),L.forEach(v=>{let b="#10b981",B="Online",T="rgba(16,185,129,0.12)",_="#6ee7b7",F="rgba(16,185,129,0.3)";v.online===null?(b="#94a3b8",B="Aguardando...",T="rgba(255, 255, 255, 0.05)",_="var(--text-muted)",F="rgba(255, 255, 255, 0.1)"):v.online||(b="#ef4444",B="Offline",T="rgba(239,68,68,0.12)",_="#fca5a5",F="rgba(239,68,68,0.3)");const H=S%2===0?"transparent":"rgba(255,255,255,0.015)";S++,x+=`
                <div class="monitor-list-row" style="background: ${H};">
                    <div class="monitor-list-col-name">
                        <span class="monitor-dot" style="background: ${b};"></span>
                ...
                </div>`}),I.forEach(v=>{let b="#10b981",B="Online",T="rgba(16,185,129,0.12)",_="#6ee7b7",F="rgba(16,185,129,0.3)";v.online===null?(b="#94a3b8",B="Aguardando...",T="rgba(255, 255, 255, 0.05)",_="var(--text-muted)",F="rgba(255, 255, 255, 0.1)"):v.online||(b="#ef4444",B="Offline",T="rgba(239,68,68,0.12)",_="#fca5a5",F="rgba(239,68,68,0.3)");const H=S%2===0?"transparent":"rgba(255,255,255,0.015)";S++,x+=`
                <div class="monitor-list-row" style="background: ${H};">
                    <div class="monitor-list-col-name">
                        <span class="monitor-dot" style="background: ${b};"></span>
                ...
                </div>`}),R.forEach(v=>{let b="#10b981",B="Online",T="rgba(16,185,129,0.12)",_="#6ee7b7",F="rgba(16,185,129,0.3)";v.online===null?(b="#94a3b8",B="Aguardando...",T="rgba(255, 255, 255, 0.05)",_="var(--text-muted)",F="rgba(255, 255, 255, 0.1)"):v.online||(b="#ef4444",B="Offline",T="rgba(239,68,68,0.12)",_="#fca5a5",F="rgba(239,68,68,0.3)");const H=S%2===0?"transparent":"rgba(255,255,255,0.015)";S++,x+=`
                <div class="monitor-list-row" style="background: ${H};">
                    <div class="monitor-list-col-name">
                        <span class="monitor-dot" style="background: ${b};"></span>
            ...
            </div>`;try{let t=await $.get("/monitoring/events?limit=1000");if(De&&(t=t.filter(i=>(i.title||"").toLowerCase().includes(De)||(i.description||"").toLowerCase().includes(De)||(i.source||"").toLowerCase().includes(De))),at!=="all"&&(t=t.filter(i=>i.severity===at)),Ve){const i=new Date(Ve+"T00:00:00").getTime();t=t.filter(r=>r.created_at?new Date(r.created_at).getTime()>=i:!1)}if(Ne){const i=new Date(Ne+"T23:59:59").getTime();t=t.filter(r=>r.created_at?new Date(r.created_at).getTime()<=i:!1)}ye=t.length;const n=Math.max(1,Math.ceil(ye/Se));K>n&&(K=n);const o=document.getElementById("event-history-count");o&&(o.textContent=ye>0?ye:"",o.style.display=ye>0?"inline-flex":"none");const a=(K-1)*Se,s=t.slice(a,a+Se);this.renderEvents(s),this.renderPagination(ye,n)}catch(t){console.error("Erro ao buscar histórico de eventos:",t),e.innerHTML=`
                <div style="text-align: center; padding: 3rem; color: var(--text-muted);">
            ...
            </div>`,n.querySelectorAll(".eh-page-btn[data-page]").forEach(d=>{d.addEventListener("click",()=>{const p=parseInt(d.dataset.page,10);if(!isNaN(p)&&p>=1&&p<=t&&p!==K){K=p,this.fetchAndRenderEventHistory();const m=document.getElementById("monitoring-events-grid");m&&m.scrollIntoView({behavior:"smooth",block:"start"})}})})},renderEvents(e){const t=document.getElementById("monitoring-events-grid");if(!t)return;t.style.display="flex",t.style.flexDirection="column",t.style.gap="0";const n=e||[];if(n.length===0){t.innerHTML=`
...
`);for(let a=1;a<o.length;a++){const s=o[a].trim().split(/\s+/);s.length>=6&&n.push({mountpoint:s[5],total:s[1],used:s[2],free:s[3],percent:parseInt(s[4].replace("%",""),10)||0})}}catch(o){console.warn("Erro ao fazer parse da tabela de disco offline:",o)}else Array.isArray(M.disco)&&(n=M.disco.map(o=>({mountpoint:o.mountpoint,total:typeof o.total_gb=="number"?`${o.total_gb.toFixed(2)} GB`:o.total_gb||"0 GB",used:typeof o.used_gb=="number"?`${o.used_gb.toFixed(2)} GB`:o.used_gb||"0 GB",free:typeof o.free_gb=="number"?`${o.free_gb.toFixed(2)} GB`:o.free_gb||"0 GB",percent:typeof o.percent=="number"?Math.round(o.percent):parseInt(o.percent,10)||0})));n.length>0?e.innerHTML=n.map(o=>{const a=o.percent;return`
        ...
        `;const t=document.getElementById("btn-refresh-apis-status");let n=null;t&&(n=t.querySelector("svg"),t.disabled=!0,t.style.opacity="0.6",t.style.cursor="not-allowed",n&&(n.style.animation="spin 0.8s linear infinite"));try{const o=await $.get("/monitoring/apis-status?refresh=true&t="+Date.now());if(o&&o.success&&Array.isArray(o.apis))it=o.apis,J==="alerts"&&this.renderGnewServicesStatus(),this.renderApisGrid(o.apis);else throw new Error("Resposta inválida do servidor.")}catch(o){console.error("Erro ao buscar status das APIs:",o),e.innerHTML=`
            ...
            `}).join("")}},async fetchAndRenderSwitchesStatus(e=!1,t=!1){const n=document.getElementById("switches-auto-refresh"),o=t||n&&n.checked,a=document.getElementById("monitoring-switches-tbody");if(!a)return;const s=document.getElementById("btn-refresh-switches-status");let i=null;s&&(i=s.querySelector("svg"),s.disabled=!0,s.style.opacity="0.6",s.style.cursor="not-allowed",i&&(i.style.animation="spin 0.8s linear infinite"));try{if(o){const r=await $.get(`/monitoring/switches?ping=false&refresh=${e}&t=${Date.now()}`);if(r&&r.success&&Array.isArray(r.switches)){this.renderSwitchesTable(r.switches),r.switches.forEach(l=>{const u=document.getElementById(`switch-row-${l.id}`);if(u){const d=u.querySelector(".switch-sync-indicator");d&&(d.innerHTML=`
                                ...
                                `)}}),ve=!0;for(const l of r.switches){if(J!=="infra")break;const u=document.getElementById(`switch-row-${l.id}`);if(u){const d=u.querySelector(".switch-sync-indicator");d&&(d.innerHTML=`
                                ...
                                `)}try{const d=await $.get(`/monitoring/switches/${l.id}/ping?t=${Date.now()}`);if(d&&d.success&&d.switch){const p=d.switch,m=document.getElementById(`switch-row-${p.id}`);if(m){let f="rgba(16, 185, 129, 0.12)",g="#6ee7b7",h="rgba(16, 185, 129, 0.3)",y="Online";p.online||(f="rgba(239, 68, 68, 0.12)",g="#fca5a5",h="rgba(239, 68, 68, 0.3)",y="Offline");const k=p.latency<50?"#6ee7b7":p.latency<150?"#fde047":"#fca5a5",w=p.online?`${p.latency}ms`:"-",E=m.querySelector(".monitor-badge");E&&(E.style.background=f,E.style.color=g,E.style.borderColor=h,E.textContent=y);const D=m.querySelector(".switch-latency");D&&(D.style.color=k,D.textContent=w);const L=m.querySelector(".switch-sync-indicator");L&&(L.innerHTML=`
            ...
            `}).join("")}},async fetchAndRenderRoutersStatus(e=!1,t=!1){console.log("📊 [MONITORING] fetchAndRenderRoutersStatus called. forceRefresh:",e,"sequential:",t);const n=document.getElementById("routers-auto-refresh"),o=t||n&&n.checked,a=document.getElementById("monitoring-routers-tbody");if(!a){console.error("📊 [MONITORING] Element #monitoring-routers-tbody not found in DOM!");return}const s=document.getElementById("btn-refresh-routers-status");let i=null;s&&(i=s.querySelector("svg"),s.disabled=!0,s.style.opacity="0.6",s.style.cursor="not-allowed",i&&(i.style.animation="spin 0.8s linear infinite"));try{if(console.log("📊 [MONITORING] Fetching routers, sequential mode:",o),o){const r=await $.get(`/monitoring/routers?ping=false&refresh=${e}&t=${Date.now()}`);if(r&&r.success&&Array.isArray(r.routers)){this.renderRoutersTable(r.routers),r.routers.forEach(l=>{const u=document.getElementById(`router-row-${l.id}`);if(u){const d=u.querySelector(".router-sync-indicator");d&&(d.innerHTML=`
                                    <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" style="color: var(--text-muted); opacity: 0.5;">
                                        <polyline points="23 4 23 10 17 10"></polyline>
                                        <polyline points="1 20 1 14 7 14"></polyline>
                                        <path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"></path>
                                    </svg>
                                `)}}),ve=!0;for(const l of r.routers){if(J!=="infra"||Z!=="routers")break;const u=document.getElementById(`router-row-${l.id}`);if(u){const d=u.querySelector(".router-sync-indicator");d&&(d.innerHTML=`
                                    <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" style="color: var(--accent); animation: spin 1s linear infinite;">
                                        <polyline points="23 4 23 10 17 10"></polyline>
                                        <polyline points="1 20 1 14 7 14"></polyline>
                                        <path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"></path>
                                    </svg>
                                `)}try{const d=await $.get(`/monitoring/routers/${l.id}/ping?t=${Date.now()}`);if(d&&d.success&&d.router){const p=d.router,m=document.getElementById(`router-row-${p.id}`);if(m){let f="rgba(16, 185, 129, 0.12)",g="#6ee7b7",h="rgba(16, 185, 129, 0.3)",y="Online";p.online||(f="rgba(239, 68, 68, 0.12)",g="#fca5a5",h="rgba(239, 68, 68, 0.3)",y="Offline");const k=p.latency<50?"#6ee7b7":p.latency<150?"#fde047":"#fca5a5",w=p.online?`${p.latency}ms`:"-",E=m.querySelector(".monitor-badge");E&&(E.style.background=f,E.style.color=g,E.style.borderColor=h,E.textContent=y);const D=m.querySelector(".router-latency");D&&(D.style.color=k,D.textContent=w);const L=m.querySelector(".router-sync-indicator");L&&(L.innerHTML=`
                                            <svg viewBox="0 0 24 24" width="14" height="14" stroke="#10b981" stroke-width="2.5" fill="none" stroke-linecap="round" stroke-linejoin="round" style="opacity: 0.8;">
                                                <polyline points="20 6 9 17 4 12"></polyline>
                                            </svg>
            ...
            `}).join("")}},async fetchAndRenderNasStatus(e=!1,t=!1){console.log("📊 [MONITORING] fetchAndRenderNasStatus called. forceRefresh:",e,"sequential:",t);const n=document.getElementById("nas-auto-refresh"),o=t||n&&n.checked,a=document.getElementById("monitoring-nas-tbody");if(!a){console.error("📊 [MONITORING] Element #monitoring-nas-tbody not found in DOM!");return}const s=document.getElementById("btn-refresh-nas-status");let i=null;s&&(i=s.querySelector("svg"),s.disabled=!0,s.style.opacity="0.6",s.style.cursor="not-allowed",i&&(i.style.animation="spin 0.8s linear infinite"));try{if(console.log("📊 [MONITORING] Fetching NAS devices, sequential mode:",o),o){const r=await $.get(`/monitoring/nas?ping=false&refresh=${e}&t=${Date.now()}`);if(r&&r.success&&Array.isArray(r.nas)){this.renderNasTable(r.nas),r.nas.forEach(l=>{const u=document.getElementById(`nas-row-${l.id}`);if(u){const d=u.querySelector(".nas-sync-indicator");d&&(d.innerHTML=`
                                ...
                                `)}}),ve=!0;for(const l of r.nas){if(J!=="infra"||Z!=="nas")break;const u=document.getElementById(`nas-row-${l.id}`);if(u){const d=u.querySelector(".nas-sync-indicator");d&&(d.innerHTML=`
                                    <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" style="color: var(--accent); animation: spin 1s linear infinite;">
                                        <polyline points="23 4 23 10 17 10"></polyline>
                                        <polyline points="1 20 1 14 7 14"></polyline>
                                        <path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"></path>
                                    </svg>
                                `)}try{const d=await $.get(`/monitoring/nas/${l.id}/ping?t=${Date.now()}`);if(d&&d.success&&d.nas){const p=d.nas,m=document.getElementById(`nas-row-${p.id}`);if(m){let f="rgba(16, 185, 129, 0.12)",g="#6ee7b7",h="rgba(16, 185, 129, 0.3)",y="Online";p.online||(f="rgba(239, 68, 68, 0.12)",g="#fca5a5",h="rgba(239, 68, 68, 0.3)",y="Offline");const k=p.latency<50?"#6ee7b7":p.latency<150?"#fde047":"#fca5a5",w=p.online?`${p.latency}ms`:"-",E=m.querySelector(".monitor-badge");E&&(E.style.background=f,E.style.color=g,E.style.borderColor=h,E.textContent=y);const D=m.querySelector(".nas-latency");D&&(D.style.color=k,D.textContent=w);const L=m.querySelector(".nas-sync-indicator");L&&(L.innerHTML=`
                                            <svg viewBox="0 0 24 24" width="14" height="14" stroke="#10b981" stroke-width="2.5" fill="none" stroke-linecap="round" stroke-linejoin="round" style="opacity: 0.8;">
                                                <polyline points="20 6 9 17 4 12"></polyline>
                                            </svg>
            ...
            `;return}t.innerHTML=e.map(n=>{let o="rgba(16, 185, 129, 0.12)",a="#6ee7b7",s="rgba(16, 185, 129, 0.3)",i="Online";n.online===null?(o="rgba(255, 255, 255, 0.05)",a="var(--text-muted)",s="rgba(255, 255, 255, 0.1)",i="Aguardando..."):n.online||(o="rgba(239, 68, 68, 0.12)",a="#fca5a5",s="rgba(239, 68, 68, 0.3)",i="Offline");const r=n.online?n.latency<50?"#6ee7b7":n.latency<150?"#fde047":"#fca5a5":"var(--text-muted)",l=n.online?`${n.latency}ms`:"-";return`
                <tr id="nas-row-${n.id}" style="border-bottom: 1px solid rgba(255, 255, 255, 0.05); transition: background 0.2s;">
                    <td style="padding: 12px; font-weight: 600; color: var(--text-main);">${n.name}</td>
                    <td style="padding: 12px; font-family: monospace; color: var(--text-muted);">${n.ip}</td>
