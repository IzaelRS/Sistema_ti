(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))o(a);new MutationObserver(a=>{for(const s of a)if(s.type==="childList")for(const i of s.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&o(i)}).observe(document,{childList:!0,subtree:!0});function n(a){const s={};return a.integrity&&(s.integrity=a.integrity),a.referrerPolicy&&(s.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?s.credentials="include":a.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function o(a){if(a.ep)return;a.ep=!0;const s=n(a);fetch(a.href,s)}})();const he="/api",E={async get(e){const t=await fetch(`${he}${e}`);if(!t.ok){const n=await t.json().catch(()=>({}));throw new Error(n.error||`HTTP error! status: ${t.status}`)}return await t.json()},async post(e,t){const n=await fetch(`${he}${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});if(!n.ok){const o=await n.json().catch(()=>({}));throw new Error(o.error||`HTTP error! status: ${n.status}`)}return await n.json()},async put(e,t){const n=await fetch(`${he}${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});if(!n.ok){const o=await n.json().catch(()=>({}));throw new Error(o.error||`HTTP error! status: ${n.status}`)}return await n.json()},async delete(e){const t=await fetch(`${he}${e}`,{method:"DELETE"});if(!t.ok){const n=await t.json().catch(()=>({}));throw new Error(n.error||`HTTP error! status: ${t.status}`)}return await t.json()},async upload(e,t){const n=await fetch(`${he}${e}`,{method:"POST",body:t});if(!n.ok){const o=await n.json().catch(()=>({}));throw new Error(o.error||`HTTP error! status: ${n.status}`)}return await n.json()}};let ue=null;const F={init(){const e=localStorage.getItem("user");if(e)try{return ue=JSON.parse(e),!0}catch{return this.logout(),!1}return!1},getUser(){return ue},isAdmin(){return ue&&ue.role==="Administrador"},async login(e,t){try{const n=await E.post("/login",{email:e,password:t});return ue=n,localStorage.setItem("user",JSON.stringify(n)),{success:!0,user:n}}catch(n){return{success:!1,error:n.message}}},logout(){ue=null,localStorage.removeItem("user")}},l={show(e){const t=document.getElementById(e);t&&t.classList.remove("hidden")},hide(e){const t=document.getElementById(e);t&&t.classList.add("hidden")},toggle(e,t){const n=document.getElementById(e);n&&n.classList.toggle("hidden",t)},setText(e,t){const n=document.getElementById(e);n&&(n.innerText=t)},setValue(e,t){const n=document.getElementById(e);n&&(n.value=t)},getValue(e){const t=document.getElementById(e);return t?t.value:null},on(e,t,n){const o=document.getElementById(e);o&&o.addEventListener(t,n)}},ot={canvas:null,ctx:null,particles:[],animationFrameId:null,isActive:!1,init(){if(this.canvas=document.getElementById("account-network-bg"),!this.canvas)return;this.ctx=this.canvas.getContext("2d"),this.resize(),window.addEventListener("resize",()=>{this.isActive&&this.resize()});const e=window.innerWidth<=768;this.particleCount=e?30:60,this.connectDistance=150,this.particleColor="rgba(34, 211, 238, 0.5)",this.particles=[];for(let t=0;t<this.particleCount;t++)this.particles.push({x:Math.random()*this.canvas.width,y:Math.random()*this.canvas.height,vx:(Math.random()-.5)*1.5,vy:(Math.random()-.5)*1.5,radius:Math.random()*2+1})},resize(){if(!this.canvas)return;const e=document.getElementById("account-section");e&&(this.canvas.width=e.clientWidth,this.canvas.height=e.clientHeight)},updateAndDraw(){if(!(!this.isActive||!this.canvas)){this.ctx.clearRect(0,0,this.canvas.width,this.canvas.height);for(let e=0;e<this.particles.length;e++){const t=this.particles[e];t.x+=t.vx,t.y+=t.vy,(t.x<0||t.x>this.canvas.width)&&(t.vx*=-1),(t.y<0||t.y>this.canvas.height)&&(t.vy*=-1),this.ctx.beginPath(),this.ctx.arc(t.x,t.y,t.radius,0,Math.PI*2),this.ctx.fillStyle=this.particleColor,this.ctx.fill();for(let n=e+1;n<this.particles.length;n++){const o=this.particles[n],a=t.x-o.x,s=t.y-o.y,i=Math.sqrt(a*a+s*s);if(i<this.connectDistance){this.ctx.beginPath(),this.ctx.lineWidth=1;const r=1-i/this.connectDistance;this.ctx.strokeStyle=`rgba(34, 211, 238, ${r*.4})`,this.ctx.moveTo(t.x,t.y),this.ctx.lineTo(o.x,o.y),this.ctx.stroke()}}}this.animationFrameId=requestAnimationFrame(()=>this.updateAndDraw())}},start(){this.canvas||this.init(),this.isActive||(this.isActive=!0,this.resize(),this.updateAndDraw())},stop(){this.isActive=!1,this.animationFrameId&&(cancelAnimationFrame(this.animationFrameId),this.animationFrameId=null)}};let ye=[];const Je={async fetch(){try{ye=await E.get("/users"),this.render(ye)}catch(e){console.error("Error fetching Users:",e)}},getUsers(){return ye},render(e){const t=document.getElementById("user-table-body");t&&(t.innerHTML=e.map(n=>{const o=n.role==="Administrador",a=F.isAdmin()?`
            ...
            `}).join("")},render(e){const t=document.getElementById("doc-list-body");if(!t)return;const n=document.getElementById("doc-list-thead"),o=ae.toLowerCase()==="contratos"||ae.toLowerCase()==="termo de uso",a=window.auth&&window.auth.isAdmin(),s=a?"":'class="role-hidden"';at=e;const i=e.length,r=Math.ceil(i/ve);H>r&&(H=Math.max(1,r)),H<1&&(H=1);const c=(H-1)*ve,u=e.slice(c,c+ve);if(n&&(o?n.innerHTML=`
                    <tr>
                        <th>Nome</th>
                        <th>Tamanho</th>
                        <th>Tipo</th>
            ...
            `,this.renderPaginationControls("doc-pagination",0,0);return}t.innerHTML=u.map(d=>{const m=d.mimetype==="application/pdf"?"📕":"🖼️",p=(d.size/1024).toFixed(1)+" KB",f=d.created_at?new Date(d.created_at).toLocaleDateString("pt-BR"):"-",g=d.mimetype==="application/pdf"?"PDF":"Imagem",h=a?`<button class="btn-delete" onclick="window.DocsHandler.delete(${d.id})" style="padding: 4px 10px; font-size: 0.85rem;">Deletar</button>`:"",y=d.start_date?new Date(d.start_date+"T00:00:00").toLocaleDateString("pt-BR"):"-",I=d.end_date?d.end_date==="Indefinido"?"Indefinido":new Date(d.end_date+"T00:00:00").toLocaleDateString("pt-BR"):"-";return o?`
                    <tr>
                        <td>
                            <span style="font-weight: 500; display: flex; align-items: center; gap: 8px;">
                                <span>${m}</span>
        ...
        `;const i=(H-1)*ve+1,r=Math.min(H*ve,n);a+=`
            <span class="pagination-info">
                Exibindo ${i}-${r} de ${n}
            </span>
        `,o.innerHTML=a}};let ee=[],w={summaries:[]},it=null,C=null,Oe="list",be=null,K=null,st=null,P=1;const we=10;let Be=[];const R={getPendingProcId(){return it},async fetch(){try{P=1,ee=await E.get("/procedures"),this.renderTable(ee)}catch(e){console.error("Error fetching FAQs:",e)}},getFaqs(){return ee},setListingMode(e){Oe=e,P=1,this.renderTable(Be.length?Be:ee)},renderTable(e){const t=document.getElementById("list-table-container"),n=document.getElementById("list-cards-container"),o=document.getElementById("proc-table-body");if(!t||!n||!o)return;Be=e;const a=e.length,s=Math.ceil(a/we);P>s&&(P=Math.max(1,s)),P<1&&(P=1);const i=(P-1)*we,r=e.slice(i,i+we);Oe==="list"?(l.show("list-table-container"),l.hide("list-cards-container"),r.length===0?o.innerHTML=`
                    <tr>
            ...
            `).join("");const o=w.summaries.find(a=>a.id===C);o?(n.textContent=o.title,n.style.color="var(--text-main)",t.classList.remove("hidden")):(n.textContent="Nenhum sumário selecionado",n.style.color="var(--accent)",t.classList.add("hidden"))},selectSummary(e){C=e,this.renderProcedureBuilderSidebar(),this.renderProcedureBuilder()},updateSummaryTitle(e,t){const n=w.summaries.find(a=>a.id===e);n&&(n.title=t||"Sem título"),this.renderProcedureBuilderSidebar();const o=w.summaries.find(a=>a.id===C);o&&(document.getElementById("current-summary-name").textContent=o.title)},addSummary(){const e="sum_"+Date.now();w.summaries.push({id:e,title:`Sumário ${w.summaries.length+1}`,sections:[]}),C=e,this.renderProcedureBuilderSidebar(),this.renderProcedureBuilder()},removeSummary(e){confirm("Excluir este sumário apagará todos os campos dentro dele. Deseja continuar?")&&(w.summaries=w.summaries.filter(t=>t.id!==e),C===e&&(C=w.summaries.length>0?w.summaries[0].id:null),this.renderProcedureBuilderSidebar(),this.renderProcedureBuilder())},renderProcedureBuilder(){const e=document.getElementById("procedure-edit-container");if(!e)return;if(!C){e.innerHTML='<p class="empty-state">Crie um novo sumário na barra lateral para adicionar conteúdo.</p>';return}const t=w.summaries.find(o=>o.id===C);if(!t)return;const n=t.sections;if(n.length===0){e.innerHTML=`<p class="empty-state">Nenhum campo em "${t.title}". Clique em "+ Novo Container" para começar.</p>`;return}e.innerHTML=n.map((o,a)=>`
             ...
             </div>`).join("")},handleSumDragStart(e,t){be="summary",K=t,e.dataTransfer.effectAllowed="move",setTimeout(()=>{e.target&&e.target.classList.add("dragging")},0)},handleSumDrop(e,t){if(e.preventDefault(),be!=="summary"||K===null||K===t)return;const n=w.summaries.splice(K,1)[0];w.summaries.splice(t,0,n),this.renderProcedureBuilderSidebar()},handleSecDragStart(e,t,n){be="container",K=t,st=n,e.dataTransfer.effectAllowed="move",setTimeout(()=>{const o=e.target.nodeType===1?e.target.closest(".editable-section"):null;o&&o.classList.add("dragging")},0)},handleDragOver(e){e.preventDefault(),e.dataTransfer.dropEffect="move"},handleSecDrop(e,t,n){if(e.preventDefault(),be!=="container"||K===null||st!==n)return;const o=w.summaries.find(s=>s.id===n);if(!o||K===t)return;const a=o.sections.splice(K,1)[0];o.sections.splice(t,0,a),this.renderProcedureBuilder()},handleDragEnd(e){document.querySelectorAll(".editable-section.dragging").forEach(t=>t.classList.remove("dragging")),e&&e.target&&e.target.setAttribute&&e.target.setAttribute("draggable","false"),be=null,K=null},updateSectionTitle(e,t){const n=w.summaries.find(o=>o.id===C);n&&(n.sections[e].title=t)},updateSectionData(e,t){const n=w.summaries.find(o=>o.id===C);n&&(n.sections[e].data=t)},removeSection(e){const t=w.summaries.find(n=>n.id===C);t&&t.sections.splice(e,1),this.renderProcedureBuilder()},getRteToolbarHTML(){return`
        ...
        `},addFaqItem(e){const t=w.summaries.find(n=>n.id===C);t&&(t.sections[e].data=t.sections[e].data||[],t.sections[e].data.push({q:"",a:""}),this.renderProcedureBuilder())},updateFaqItem(e,t,n,o){const a=w.summaries.find(s=>s.id===C);a&&(a.sections[e].data[t][n]=o)},removeFaqItem(e,t){const n=w.summaries.find(o=>o.id===C);n&&n.sections[e].data.splice(t,1),this.renderProcedureBuilder()},addSection(e,t){if(!C){alert("Selecione primeiro um sumário na barra lateral.");return}const n=w.summaries.find(o=>o.id===C);n&&(n.sections.push({id:Date.now(),title:e,type:t,data:t==="FAQ"?[]:t==="TEXTO"?"":null}),this.renderProcedureBuilder())},async handleSectionFileDrop(e,t){t.dataTransfer.files&&t.dataTransfer.files.length>0&&await this.uploadSectionFile(e,t.dataTransfer.files[0])},async handleSectionFileUpload(e,t){const n=t.files[0];n&&await this.uploadSectionFile(e,n)},async uploadSectionFile(e,t){const n=new FormData;n.append("file",t);try{const o=await E.upload("/upload",n),a=w.summaries.find(s=>s.id===C);a&&(a.sections[e].data={name:t.name,path:o.path,mimetype:t.type},this.renderProcedureBuilder())}catch{alert("Erro no upload")}},async handleSaveProcedure(){const e=parseInt(l.getValue("proc-id"));if(!e)return;const n={...ee.find(o=>o.id===e),content:JSON.stringify(w)};try{await E.put(`/procedures/${e}`,n),alert("Salvo com sucesso!"),this.toggleEditMode(!1),this.openDetail(e),this.fetch()}catch{alert("Erro ao salvar")}},search(e){P=1;const t=ee.filter(n=>(n.name||n.title||"").toLowerCase().includes(e)||(n.responsible||"").toLowerCase().includes(e)||(n.group_name||"").toLowerCase().includes(e));this.renderTable(t)},changePage(e){P=e,this.renderTable(Be)},renderPaginationControls(e,t,n){const o=document.getElementById(e);if(!o)return;if(t===0){o.innerHTML="";return}let a="";a+=`
            <button class="pagination-btn" 
                    ${P===1?"disabled":""} 
                    onclick="window.ProceduresHandler.changePage(${P-1})"
                    title="Página Anterior">
                &laquo;
            </button>
        `;let s=0;for(let c=1;c<=t;c++)(c===1||c===t||c>=P-1&&c<=P+1)&&(s&&c-s>1&&(a+='<span style="color: var(--text-muted); padding: 0 4px;">...</span>'),a+=`
        ...
        `,o.innerHTML=a}};window.toggleGhAccordion=function(e){const t=document.getElementById(e);t&&t.classList.toggle("open")};let N=[],me="list",ie="month",x=new Date,O=1;const xe=10;let rt=[];const _={async fetch(){try{O=1,N=await E.get("/accounts"),this.initDashboardMultiselects(),this.populateCompanyFilter(),this.handleSearch(),this.checkAccountAlerts()}catch(e){console.error("Falha ao obter contas",e)}},populateCompanyFilter(){const e=document.getElementById("dash-filter-company-dynamic-options");if(e){const t=new Set;e.querySelectorAll('input[type="checkbox"]:checked').forEach(a=>{t.add(a.value)});const n=[...new Set(N.map(a=>a.company_name).filter(Boolean))].sort((a,s)=>a.localeCompare(s));let o="";n.forEach(a=>{const s=t.has(a)?"checked":"";o+=`<label class="multiselect-option"><input type="checkbox" value="${a}" ${s}> <span>${a}</span></label>`}),e.innerHTML=o,this.setupMultiselectListeners("dash-filter-company")}},setupMultiselectListeners(e){if(!document.getElementById(`${e}-container`))return;const n=document.getElementById(`${e}-trigger`),o=document.getElementById(`${e}-dropdown`);if(!n||!o)return;n.dataset.listenerBound||(n.addEventListener("click",r=>{r.stopPropagation(),document.querySelectorAll(".multiselect-dropdown").forEach(c=>{c!==o&&c.classList.add("hidden")}),o.classList.toggle("hidden")}),n.dataset.listenerBound="true");const a=o.querySelector('input[value="Todos"]'),s=Array.from(o.querySelectorAll('input[type="checkbox"]')).filter(r=>r.value!=="Todos"),i=()=>{const r=s.filter(u=>u.checked).map(u=>u.value),c=n.querySelector(".trigger-label");a.checked||s.length>0&&r.length===s.length?(a.checked=!0,c&&(c.innerText="Todos")):r.length===0?c&&(c.innerText="Nenhum"):r.length===1?c&&(c.innerText=r[0]):c&&(c.innerText=`${r.length} selecionados`)};a&&!a.dataset.listenerBound&&(a.addEventListener("change",()=>{s.forEach(r=>{r.checked=a.checked}),i(),this.renderDashboard()}),a.dataset.listenerBound="true"),s.forEach(r=>{r.dataset.listenerBound||(r.addEventListener("change",()=>{s.every(u=>u.checked)?a.checked=!0:a.checked=!1,i(),this.renderDashboard()}),r.dataset.listenerBound="true")}),i()},initDashboardMultiselects(){this.setupMultiselectListeners("dash-filter-category"),window.multiselectOutsideClickListenerBound||(document.addEventListener("click",e=>{e.target.closest(".custom-multiselect-container")||document.querySelectorAll(".multiselect-dropdown").forEach(t=>{t.classList.add("hidden")})}),window.multiselectOutsideClickListenerBound=!0)},getMultiselectValues(e){const t=document.getElementById(`${e}-dropdown`);if(!t)return["Todos"];const n=t.querySelector('input[value="Todos"]');return n&&n.checked?["Todos"]:Array.from(t.querySelectorAll('input[type="checkbox"]:checked')).map(o=>o.value).filter(o=>o!=="Todos")},resetMultiselects(){["dash-filter-category","dash-filter-company"].forEach(e=>{const t=document.getElementById(`${e}-dropdown`);if(t){t.querySelectorAll('input[type="checkbox"]').forEach(a=>{a.checked=a.value==="Todos"});const o=document.getElementById(`${e}-trigger`);if(o){const a=o.querySelector(".trigger-label");a&&(a.innerText="Todos")}}})},getAccounts(){return N},setAccountsViewMode(e){me=e,this.handleSearch()},setCalendarSubView(e){ie=e,this.handleSearch()},shiftCalendarDate(e){ie==="day"?x.setDate(x.getDate()+e):ie==="month"?x.setMonth(x.getMonth()+e):ie==="year"&&x.setFullYear(x.getFullYear()+e),l.setValue("filter-day",x.getDate()),l.setValue("filter-month",x.getMonth()),l.setValue("filter-year",x.getFullYear()),this.handleSearch()},handleFilterChange(e=!1){if(e){const t=l.getValue("filter-cal-year")?parseInt(l.getValue("filter-cal-year")):x.getFullYear(),n=l.getValue("filter-cal-month")?parseInt(l.getValue("filter-cal-month")):x.getMonth();x=new Date(t,n,1)}else{const t=l.getValue("filter-year")?parseInt(l.getValue("filter-year")):x.getFullYear(),n=l.getValue("filter-month")?parseInt(l.getValue("filter-month")):x.getMonth(),o=l.getValue("filter-day")?parseInt(l.getValue("filter-day")):x.getDate();x=new Date(t,n,o)}l.setValue("filter-month",x.getMonth()),l.setValue("filter-year",x.getFullYear()),this.handleSearch()},handleSearch(){const e=(l.getValue("accounts-search")||"").toLowerCase();let t=N.filter(n=>n.company_name.toLowerCase().includes(e)||n.description&&n.description.toLowerCase().includes(e));if(me==="list"){O=1;const n=l.getValue("filter-status")||"",o=document.getElementById("filter-date-toggle"),a=o?o.checked:!1,s=x.getFullYear(),i=x.getMonth(),r=x.getDate();t=t.filter(c=>{if(n&&c.status!==n)return!1;if(!a||!c.due_date)return!0;const[u,d,m]=c.due_date.split("-"),p=parseInt(u,10),f=parseInt(d,10)-1,g=parseInt(m,10);return c.type==="Único"?p===s&&f===i&&g===r:c.type==="Recorrente"?g===r:!0}),this.renderAccountsList(t)}else me==="notificacoes"?this.renderNotifications():me==="dashboard"?this.renderDashboard():this.renderCalendarWrapper(t)},checkAccountAlerts(){let e=!1;const t=new Date;t.setHours(0,0,0,0),N.forEach(o=>{const a=(o.status||"").trim().toLowerCase(),s=(o.payment_status||"").trim().toLowerCase();if(a==="on"&&s==="pendente"&&o.due_date){const[i,r,c]=o.due_date.split("-");let u=new Date(parseInt(i,10),parseInt(r,10)-1,parseInt(c,10));u.setHours(0,0,0,0),u.getTime()<=t.getTime()&&(e=!0)}});const n=document.getElementById("icon-alert-bell");n&&(e?n.classList.add("alert-pulse"):n.classList.remove("alert-pulse"))},renderNotifications(){const e=document.getElementById("accounts-notifications-body");if(!e)return;e.innerHTML="";const t=new Date;t.setHours(0,0,0,0);let n=N.filter(o=>{const a=(o.status||"").trim().toLowerCase(),s=(o.payment_status||"").trim().toLowerCase();if(a!=="on"||s!=="pendente"||!o.due_date)return!1;const[i,r,c]=o.due_date.split("-");let u=new Date(parseInt(i,10),parseInt(r,10)-1,parseInt(c,10));return u.setHours(0,0,0,0),u.getTime()<=t.getTime()});if(n.length===0){e.innerHTML='<tr><td colspan="6" style="text-align:center; color: var(--text-muted); padding: 20px;">Nenhuma conta urgente ou atrasada.</td></tr>';return}n.forEach(o=>{const a=document.createElement("tr");let s="Sem Data";if(o.due_date){const r=o.due_date.split("-");r.length===3&&(s=`${r[2]}/${r[1]}/${r[0]}`)}const i=F.isAdmin()?`
            ...
            `,e.appendChild(a)})},renderAccountsList(e){const t=document.getElementById("accounts-table-body");if(!t)return;t.innerHTML="",this.renderSidebarMiniCalendar(),rt=e;const n=e.length,o=Math.ceil(n/xe);O>o&&(O=Math.max(1,o)),O<1&&(O=1);const a=(O-1)*xe,s=e.slice(a,a+xe);if(s.length===0){t.innerHTML='<tr><td colspan="7" style="text-align:center; color: var(--text-muted); padding: 20px;">Nenhuma conta encontrada.</td></tr>',this.renderPaginationControls("accounts-list-pagination",0,0),this.renderDashboard();return}s.forEach(i=>{const r=document.createElement("tr");let c="Sem Data";if(i.due_date){const m=i.due_date.split("-");m.length===3&&(c=`${m[2]}/${m[1]}/${m[0]}`)}const u=i.status==="Off",d=F.isAdmin()?`
            ...
            `,t.appendChild(r)}),this.renderPaginationControls("accounts-list-pagination",o,n),this.renderDashboard()},renderDashboard(){if(me!=="dashboard")return;this.initDashboardMultiselects();const e=l.getValue("dash-filter-start"),t=l.getValue("dash-filter-end"),n=l.getValue("dash-filter-type")||"Todos",o=l.getValue("dash-filter-status")||"Todos",a=l.getValue("dash-filter-payment")||"Todos",s=this.getMultiselectValues("dash-filter-category"),i=this.getMultiselectValues("dash-filter-company");let r=e?new Date(e+"T00:00:00"):null,c=t?new Date(t+"T23:59:59"):null;if(!r&&!c){const v=new Date;r=new Date(v.getFullYear(),v.getMonth(),1,0,0,0),c=new Date(v.getFullYear(),v.getMonth()+1,0,23,59,59)}else r?c||(c=new Date(2100,11,31)):r=new Date(2e3,0,1);let u=0,d=0,m=new Set,p=new Set,f=0,g=0,h=0,y="-",I=0,$=0,S={},q={},V={};N.forEach(v=>{if(!v.due_date||n!=="Todos"&&v.type!==n||o!=="Todos"&&v.status!==o||a!=="Todos"&&v.payment_status!==a)return;if(!s.includes("Todos")){if(s.length===0)return;const D=v.category||"Outros";if(!s.includes(D))return}if(!i.includes("Todos")&&(i.length===0||!i.includes(v.company_name)))return;let A=0,z=new Date(r);z.setHours(0,0,0,0);let U=new Date(c);U.setHours(0,0,0,0);let G=3650;for(;z<=U&&G>0;){if(this.isEventOnDate(v,z.getFullYear(),z.getMonth(),z.getDate())){A++;const D=`${z.getFullYear()}-${String(z.getMonth()+1).padStart(2,"0")}`;V[D]||(V[D]={total:0,pago:0,pendente:0,fixo:0,variavel:0});const Y=parseFloat(v.value||0);V[D].total+=Y,v.payment_status==="Pago"&&(V[D].pago+=Y),v.payment_status==="Pendente"&&(V[D].pendente+=Y),v.type==="Recorrente"&&(V[D].fixo+=Y),v.type==="Único"&&(V[D].variavel+=Y)}z.setDate(z.getDate()+1),G--}if(A>0){const D=parseFloat(v.value||0)*A;u+=D,d+=A,m.add(v.category||"Outros"),p.add(v.company_name),v.payment_status==="Pago"&&(f+=D),v.payment_status==="Pendente"&&(g+=D),v.type==="Recorrente"&&(I+=D),v.type==="Único"&&($+=D),D>h&&(h=D,y=v.company_name);const Y=v.category||"Outros";q[Y]=(q[Y]||0)+D;const de=v.company_name||"Sem Empresa";S[de]=(S[de]||0)+D}}),l.setText("dash-metric-valor","R$ "+u.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})),l.setText("dash-metric-contas",d.toString()),l.setText("dash-metric-tipos",m.size.toString()),l.setText("dash-metric-empresas",p.size.toString()),l.setText("dash-metric-pago","R$ "+f.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})),l.setText("dash-metric-pendente","R$ "+g.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})),l.setText("dash-metric-maior-valor","R$ "+h.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})),l.setText("dash-metric-maior-nome",y),l.setText("dash-metric-fixo","R$ "+I.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})),l.setText("dash-metric-variavel","R$ "+$.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2}));const b=l.getValue("dash-sort-empresas")||"desc",k=l.getValue("dash-sort-categorias")||"desc";this.renderTierList("dash-list-empresas",S,b),this.renderTierList("dash-list-categorias",q,k),this.renderTimeChart(V)},renderTimeChart(e){window.timeChartInstance&&window.timeChartInstance.destroy();const t=document.getElementById("chart-dashboard-time");if(!t)return;const n=Object.keys(e).sort(),o=n.map(d=>{const[m,p]=d.split("-");return`${p}/${m}`}),a=n.map(d=>e[d].total),s=n.map(d=>e[d].pago),i=n.map(d=>e[d].pendente),r=n.map(d=>e[d].fixo),c=n.map(d=>e[d].variavel),u={type:"line",data:{labels:o,datasets:[{label:"Valor Total (R$)",data:a,borderColor:"#3b82f6",backgroundColor:"rgba(59, 130, 246, 0.1)",borderWidth:2,pointBackgroundColor:"#3b82f6",pointRadius:4,fill:!0,tension:.3},{label:"Total Pago (R$)",data:s,borderColor:"#4ade80",backgroundColor:"transparent",borderWidth:2,pointBackgroundColor:"#4ade80",pointRadius:4,fill:!1,tension:.3},{label:"Total Pendente (R$)",data:i,borderColor:"#facc15",backgroundColor:"transparent",borderWidth:2,pointBackgroundColor:"#facc15",pointRadius:4,fill:!1,tension:.3},{label:"Custo Fixo (R$)",data:r,borderColor:"#60a5fa",backgroundColor:"transparent",borderWidth:2,pointBackgroundColor:"#60a5fa",pointRadius:4,fill:!1,tension:.3},{label:"Custo Variável (R$)",data:c,borderColor:"#c084fc",backgroundColor:"transparent",borderWidth:2,pointBackgroundColor:"#c084fc",pointRadius:4,fill:!1,tension:.3}]},options:{responsive:!0,maintainAspectRatio:!1,interaction:{mode:"index",intersect:!1},plugins:{legend:{position:"top",labels:{color:getComputedStyle(document.documentElement).getPropertyValue("--text-main").trim()||"#e2e8f0",usePointStyle:!0,boxWidth:8}},tooltip:{callbacks:{label:function(d){let m=d.dataset.label||"";return m&&(m+=": "),d.parsed.y!==null&&(m+=new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(d.parsed.y)),m}}}},scales:{y:{beginAtZero:!0,grid:{color:"rgba(255, 255, 255, 0.05)",drawBorder:!1},ticks:{color:getComputedStyle(document.documentElement).getPropertyValue("--text-muted").trim()||"#94a3b8",callback:function(d,m,p){return new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL",maximumFractionDigits:0}).format(d)}}},x:{grid:{color:"rgba(255, 255, 255, 0.05)",drawBorder:!1},ticks:{color:getComputedStyle(document.documentElement).getPropertyValue("--text-muted").trim()||"#94a3b8"}}}}};window.timeChartInstance=new Chart(t.getContext("2d"),u)},renderTierList(e,t,n){const o=document.getElementById(e);if(!o)return;const a=Object.entries(t);if(a.length===0){o.innerHTML='<div style="color: var(--text-muted); text-align: center; font-size: 0.9rem; padding: 10px;">Nenhum dado encontrado no período</div>';return}a.sort((r,c)=>n==="asc"?r[1]-c[1]:c[1]-r[1]);const s=a.slice(0,10);let i="";s.forEach(([r,c],u)=>{const d=u===0&&n==="desc",m=d?"🏆 ":u+1+". ";i+=`
            ...
            <div class="smc-grid">
        `;for(let g=0;g<a;g++)f+='<div class="smc-day empty"></div>';for(let g=1;g<=s;g++)f+=`<div class="smc-day ${g===o?"active":""}" onclick="window.AccountsHandler.selectDateFromMiniCalendar(${t}, ${n}, ${g})">${g}</div>`;f+="</div>",e.forEach(g=>{g&&(g.innerHTML=f)})},changeMiniCalendarMonthYear(e,t){let n=x.getDate();const o=new Date(e,parseInt(t)+1,0).getDate();n>o&&(n=o),x=new Date(e,t,n);try{l.setValue("filter-cal-year",e),l.setValue("filter-cal-month",t)}catch{}this.handleSearch(),this.renderSidebarMiniCalendar()},selectDateFromMiniCalendar(e,t,n){x=new Date(e,t,n);try{l.setValue("filter-cal-year",e),l.setValue("filter-cal-month",t)}catch{}if(me==="calendar"){const o=document.getElementById("toggle-accounts-cal-day");o&&o.click()}else this.handleSearch(),this.renderSidebarMiniCalendar()},renderCalendarMonth(e,t,n){const o=["Janeiro","Fevereiro","Março","Abril","Maio","Junho","Julho","Agosto","Setembro","Outubro","Novembro","Dezembro"];l.setText("calendar-date-display",`${o[n]} ${t}`);const a=document.getElementById("calendar-month-grid");a.innerHTML="";const s=new Date(t,n,1).getDay(),i=new Date(t,n+1,0).getDate(),r=new Date,c=r.getFullYear()===t&&r.getMonth()===n;new Date(r.getFullYear(),r.getMonth(),1);for(let d=0;d<s;d++)a.innerHTML+='<div class="calendar-day empty"></div>';for(let d=1;d<=i;d++){const m=c&&r.getDate()===d?"today":"";a.innerHTML+=`<div class="calendar-day ${m}" id="cal-day-cell-${d}">
                <div class="calendar-date">${d}</div>
                <div class="calendar-events" id="cal-events-${d}"></div>
            </div>`}this.getLatestRecorrenteAccounts(e).forEach(d=>{if(!d.due_date)return;const m=new Date(t,n,1),p=new Date(r.getFullYear(),r.getMonth(),1);let f=!0;if(d.status==="Off"&&m.getTime()>=p.getTime()&&(f=!1),!!f){for(let g=1;g<=i;g++)if(this.isEventOnDate(d,t,n,g)){const h=document.getElementById(`cal-events-${g}`);if(h){const y=`${t}-${String(n+1).padStart(2,"0")}-${String(g).padStart(2,"0")}`;let I=d.payment_status==="Pago"?"event-paid":d.payment_status==="Pendente"?"event-pending":"event-canceled";d.type==="Recorrente"&&y!==d.due_date&&(I="event-pending");const $=document.createElement("div");$.className=`event-pill event-${d.type.toLowerCase()} ${I}`,$.title=d.company_name,$.innerText=d.company_name,$.onclick=S=>{this.openDedicatedPage(d.id,y)},h.appendChild($)}}}})},renderCalendarDay(e,t,n,o){const a=["Janeiro","Fevereiro","Março","Abril","Maio","Junho","Julho","Agosto","Setembro","Outubro","Novembro","Dezembro"];l.setText("calendar-date-display",`${String(o).padStart(2,"0")} de ${a[n]} de ${t}`);const s=document.getElementById("calendar-day-list");s.innerHTML="";const i=new Date(t,n,o),r=new Date;r.setHours(0,0,0,0),i.setHours(0,0,0,0);let c=0;this.getLatestRecorrenteAccounts(e).forEach(d=>{let m=!0;if(d.status==="Off"&&i.getTime()>=r.getTime()&&(m=!1),!!m&&this.isEventOnDate(d,t,n,o)){c++;const p=`${t}-${String(n+1).padStart(2,"0")}-${String(o).padStart(2,"0")}`;let f=d.payment_status==="Pago"?"#4ade80":d.payment_status==="Pendente"?"#facc15":"#ef4444";d.type==="Recorrente"&&p!==d.due_date&&(f="#facc15"),s.innerHTML+=`
                    <div class="day-event-row ${d.type.toLowerCase()}">
                        ...
                        <button class="btn-icon" onclick="window.AccountsHandler.openDedicatedPage(${d.id}, '${p}')" title="Detalhes">
            ...
            `,e.appendChild(s)})}},selectHistoryItem(e,t=null){this.currentSelectedHistoryKey=t?e+"_"+t:e.toString(),this.renderDedicatedHistoryList();let n=null;if(t&&(n=this.currentCompanyHistory.find(r=>r.id===e&&r.due_date===t&&r.is_projection)),n||(n=this.currentCompanyHistory.find(r=>r.id===e&&!r.is_projection)),document.getElementById("ded-acc-details-empty"),document.getElementById("ded-acc-details-content"),!n){l.show("ded-acc-details-empty"),l.hide("ded-acc-details-content");return}l.hide("ded-acc-details-empty"),l.show("ded-acc-details-content");let o="DD/MM/YYYY";const a=t||n.due_date;if(a){const r=a.split("-");r.length===3&&(o=`${r[2]}/${r[1]}/${r[0]}`)}l.setText("ded-acc-det-date",o),l.setValue("ded-acc-det-val-input",parseFloat(n.value||0).toFixed(2)),l.setValue("ded-acc-det-date-input",a||""),l.setValue("ded-acc-det-status-input",n.payment_status||"Pendente"),l.setValue("ded-acc-det-account-status-input",n.status||"On"),l.setValue("ded-acc-det-obs-input",n.observation||""),n.type==="Recorrente"?(l.show("ded-acc-det-freq-group"),l.setValue("ded-acc-det-freq-input",n.frequency||"1 mes")):l.hide("ded-acc-det-freq-group");const s=document.getElementById("btn-ded-save-details");s&&(s.onclick=async()=>{const r={...n,value:l.getValue("ded-acc-det-val-input"),due_date:l.getValue("ded-acc-det-date-input"),payment_status:l.getValue("ded-acc-det-status-input"),status:l.getValue("ded-acc-det-account-status-input"),observation:l.getValue("ded-acc-det-obs-input"),frequency:n.type==="Recorrente"?l.getValue("ded-acc-det-freq-input"):"1 mes"};try{await E.put(`/accounts/${n.id}`,r),alert("Fatura atualizada com sucesso!"),await this.fetch(),this.currentCompanyHistory=N.filter(c=>c.company_name===n.company_name).sort((c,u)=>new Date(u.due_date||0)-new Date(c.due_date||0)),this.openDedicatedPage(n.id)}catch{alert("Erro ao atualizar fatura.")}},F.isAdmin()||(s.style.display="none"));const i=document.getElementById("btn-ded-delete-account");i&&(i.onclick=async()=>{if(confirm("Atenção: Tem certeza que deseja excluir DESTA fatura mensal especificamente?"))try{await E.delete(`/accounts/${n.id}`),await this.fetch();const r=N.filter(c=>c.company_name===n.company_name);r.length>0?this.openDedicatedPage(r[0].id):document.getElementById("btn-back-to-accounts").click()}catch{alert("Erro ao excluir fatura")}},F.isAdmin()||(i.style.display="none")),this.renderAttachmentArea(n)},renderAttachmentArea(e){document.getElementById("ded-acc-file-input");const t=document.getElementById("ded-acc-upload-area");if(document.getElementById("ded-acc-preview-area"),e.attachment_path){l.hide("ded-acc-upload-area"),l.show("ded-acc-preview-area");const n=e.attachment_path.match(/\.(jpeg|jpg|gif|png)$/)!=null,o=document.getElementById("ded-acc-preview-thumb"),a=e.attachment_path.split("/").pop()||"documento";l.setText("ded-acc-preview-name",a);const s=document.getElementById("ded-acc-preview-link");s.href="javascript:void(0)",s.onclick=async r=>{r.preventDefault();const c=s.innerText;s.innerText="Carregando...";try{const u=await fetch(e.attachment_path);if(!u.ok)throw new Error("Doc não encontrado");const d=await u.blob(),m=window.URL.createObjectURL(d);window.open(m,"_blank")}catch(u){alert("Erro ao visualizar documento. O arquivo pode ter sido movido ou o proxy falhou."),console.error("Blob fetch error:",u)}finally{s.innerText=c}},n?(o.innerHTML="",o.style.backgroundImage=`url('${e.attachment_path}')`):(o.style.backgroundImage="none",o.innerHTML=`
                ...
                `);const i=document.getElementById("btn-ded-remove-attachment");i.onclick=async()=>{if(confirm("Remover o anexo desta fatura? (O arquivo fisicamente não será deletado até limpeza de storage, mas a referência sumirá)"))try{await E.put(`/accounts/${e.id}`,{...e,attachment_path:null}),await this.fetch(),this.currentCompanyHistory=N.filter(r=>r.company_name===e.company_name).sort((r,c)=>new Date(c.due_date||0)-new Date(r.due_date||0)),this.selectHistoryItem(e.id)}catch{alert("Erro ao remover anexo")}},F.isAdmin()||(i.style.display="none")}else{if(l.show("ded-acc-upload-area"),l.hide("ded-acc-preview-area"),F.isAdmin())t.innerHTML=`
        ...
        `;let s=0;for(let c=1;c<=t;c++)(c===1||c===t||c>=O-1&&c<=O+1)&&(s&&c-s>1&&(a+='<span style="color: var(--text-muted); padding: 0 4px;">...</span>'),a+=`
                    <button class="pagination-btn ${c===O?"active":""}" 
        ...
        `;const i=(O-1)*xe+1,r=Math.min(O*xe,n);a+=`
            <span class="pagination-info">
                Exibindo ${i}-${r} de ${n}
            </span>
        `,o.innerHTML=a}};let le=[],J={},ne=null,Re=null,Ne=null,j=[],Pe=[],We={},Q={},fe,Ke,_e,Fe,dt,Ve;const ut={init(){fe=document.getElementById("timeline-event-form"),Ke=document.getElementById("view-visualizacao"),_e=document.getElementById("view-attention"),Fe=document.getElementById("view-anexo"),dt=document.getElementById("view-relatorio"),Ve=document.getElementById("view-config"),window.timelineHandler=ut,window.applyFilters=xt,window.clearFilters=Et,window.toggleFilters=$t,window.handleDelete=wt,window.resetForm=et,window.toggleAccordion=pt,window.handleFormSubmit=ct,window.editEvent=Ze,window.deleteTopic=kt,window.deleteSubtopic=Bt,window.handleTrackDragStart=St,window.handleTrackDragOver=Mt,window.handleTrackDragEnd=At;const e=document.getElementById("timeline-topic-form");e&&e.addEventListener("submit",Ct);const t=document.getElementById("timeline-subtopic-form");t&&t.addEventListener("submit",Dt);const n=document.getElementById("topico");n&&n.addEventListener("change",u=>{Xe(u.target.value)});const o=document.getElementById("em-ocorrencia");o&&o.addEventListener("change",u=>{const d=document.getElementById("fim"),m=document.getElementById("inicio");if(u.target.checked){if(!m.value){const p=new Date;p.setMinutes(p.getMinutes()-p.getTimezoneOffset()),m.value=p.toISOString().slice(0,16)}d.required=!1}else{const p=new Date;p.setMinutes(p.getMinutes()-p.getTimezoneOffset()),d.value=p.toISOString().slice(0,16),d.required=!0}});const a=document.getElementById("auto-refresh-toggle");a&&a.addEventListener("change",u=>{mt(u.target.checked)}),document.querySelectorAll("[data-timeline-tab]").forEach(u=>{u.addEventListener("click",d=>{const m=d.currentTarget.getAttribute("data-timeline-tab");De(m)})}),fe&&fe.addEventListener("submit",ct);const s=document.getElementById("rep-filter-start"),i=document.getElementById("rep-filter-end"),r=document.getElementById("rep-filter-topic"),c=document.getElementById("rep-filter-subtopic");s&&s.addEventListener("change",()=>Te()),i&&i.addEventListener("change",()=>Te()),r&&r.addEventListener("change",u=>{Lt(u.target.value),Te()}),c&&c.addEventListener("change",()=>Te()),window.addEventListener("SectionChange",u=>{u.detail&&u.detail.section==="timeline"&&ce().then(()=>{Z(),lt()})}),ce().then(()=>{Z(),lt()})}};window.addEventListener("focus",()=>{Ke&&Z()});function Xe(e,t=null){const n=document.getElementById("sub-topico");if(!n)return;const o=e?e.toLowerCase().trim():"";if(!o||!Q[o]){n.innerHTML='<option value="">Selecione o tópico primeiro...</option>',n.classList.remove("has-options");return}n.innerHTML='<option value="" disabled selected>Escolha o evento...</option>',Q[o].forEach(a=>{const s=document.createElement("option");s.value=a.toLowerCase(),s.textContent=a,t&&s.value===t.toLowerCase()&&(s.selected=!0),n.appendChild(s)}),t||(n.selectedIndex=1),n.classList.add("has-options")}async function ce(){try{const e=await fetch("/api/timeline/config");if(!e.ok)throw new Error("Falha ao buscar configurações");const t=await e.json();j=t.topics||[],Pe=t.subtopics||[],We={},Q={},j.forEach(o=>{We[o.id]=o.color,Q[o.id]=[]}),Pe.forEach(o=>{const a=o.topic_id;Q[a]&&Q[a].push(o.name)}),bt();const n=document.getElementById("view-config");n&&n.classList.contains("active")&&ft()}catch(e){console.error("Error loading config:",e)}}function bt(){const e=document.getElementById("topico");if(e){const o=e.value;e.innerHTML='<option value="" disabled selected>Selecione um tópico...</option>',j.forEach(a=>{const s=document.createElement("option");s.value=a.id,s.textContent=a.name,e.appendChild(s)}),e.value=o}const t=document.getElementById("rep-filter-topic");if(t){const o=t.value;t.innerHTML='<option value="Todos">Todos</option>',j.forEach(a=>{const s=document.createElement("option");s.value=a.id,s.textContent=a.name,t.appendChild(s)}),o&&[...t.options].some(a=>a.value===o)?t.value=o:t.value="Todos"}const n=document.getElementById("subtopic-topic-id");n&&(n.innerHTML='<option value="" disabled selected>Selecione um tópico...</option>',j.forEach(o=>{const a=document.createElement("option");a.value=o.id,a.textContent=o.name,n.appendChild(a)}))}function Z(){fetch("/api/timeline/events").then(e=>{if(!e.ok)throw new Error("Failed to fetch");return e.json()}).then(e=>{le=e,tt(),_e&&_e.classList.contains("active")&&gt()}).catch(e=>{console.error("Error loading events:",e)})}function lt(){const e=document.getElementById("timeline-tab-anexo"),t=document.getElementById("timeline-tab-config");if(window.auth&&window.auth.isAdmin())e&&e.classList.remove("role-hidden"),t&&t.classList.remove("role-hidden");else{e&&e.classList.add("role-hidden"),t&&t.classList.add("role-hidden");const o=Fe&&Fe.classList.contains("active"),a=Ve&&Ve.classList.contains("active");(o||a)&&De("visualizacao")}}function De(e){const t={visualizacao:{section:Ke,button:document.querySelector('[data-timeline-tab="visualizacao"]')},attention:{section:_e,button:document.querySelector('[data-timeline-tab="attention"]')},anexo:{section:Fe,button:document.querySelector('[data-timeline-tab="anexo"]')},relatorio:{section:dt,button:document.querySelector('[data-timeline-tab="relatorio"]')},config:{section:Ve,button:document.querySelector('[data-timeline-tab="config"]')}};Object.values(t).forEach(n=>{n.section&&n.section.classList.remove("active"),n.button&&n.button.classList.remove("active")}),t[e]&&(t[e].section&&t[e].section.classList.add("active"),t[e].button&&t[e].button.classList.add("active")),e==="visualizacao"?(Z(),Ee(!0)):e==="attention"?(gt(),Ee(!0)):e==="relatorio"?(Te(),Ee(!1)):(e==="config"&&ft(),Ee(!1))}function Ee(e){const t=document.getElementById("floating-refresh-control");if(t)if(e){t.classList.remove("hidden");const n=document.getElementById("auto-refresh-toggle");n&&n.checked&&!ne&&mt(!0)}else t.classList.add("hidden"),ne&&(clearInterval(ne),ne=null)}function mt(e){ne&&(clearInterval(ne),ne=null),e&&(Z(),ne=setInterval(Z,6e4))}function ct(e){e.preventDefault();const t=fe.querySelector('button[type="submit"]');t&&(t.textContent="Salvando...",t.disabled=!0);const o={id:document.getElementById("event-id").value||Date.now().toString(),nome:document.getElementById("nome").value,topico:document.getElementById("topico").value,sub_topico:document.getElementById("sub-topico").value,em_ocorrencia:document.getElementById("em-ocorrencia").checked?1:0,inicio:document.getElementById("inicio").value,fim:document.getElementById("fim").value,descricao:document.getElementById("descricao").value,anotacao:document.getElementById("anotacao").value,cor:document.getElementById("cor").value};fetch("/api/timeline/events",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)}).then(async a=>{const s=await a.text();if(!a.ok)throw new Error(`Server error (${a.status}): ${s}`);return JSON.parse(s)}).then(()=>{alert("Evento salvo com sucesso!"),et(),De("visualizacao")}).catch(a=>{console.error("Error saving event:",a),alert("Erro ao salvar evento: "+a.message)}).finally(()=>{t&&(t.textContent="Salvar Evento",t.disabled=!1)})}function Ze(e){const t=le.find(s=>s.id===e);if(!t)return;document.getElementById("event-id").value=t.id,document.getElementById("nome").value=t.nome;const n=Ie(t.topico);document.getElementById("topico").value=n,Xe(n,t.sub_topico);const o=document.getElementById("em-ocorrencia");o.checked=t.em_ocorrencia==1||t.em_ocorrencia==="true"||!t.fim,o.dispatchEvent(new Event("change")),document.getElementById("inicio").value=t.inicio,document.getElementById("fim").value=t.fim||"",document.getElementById("descricao").value=t.descricao||"",document.getElementById("anotacao").value=t.anotacao||"",document.getElementById("cor").value=t.cor||"#000000",De("anexo");const a=document.getElementById("btn-delete");a&&(a.style.display="block")}function et(){fe&&fe.reset();const e=document.getElementById("event-id");e&&(e.value=""),Xe("");const t=document.getElementById("fim");t&&(t.required=!0);const n=document.getElementById("cor");n&&(n.value="#000000");const o=document.getElementById("btn-delete");o&&(o.style.display="none")}function wt(){const e=document.getElementById("event-id").value;e&&confirm("Tem certeza que deseja excluir este evento?")&&fetch(`/api/timeline/events/${e}`,{method:"DELETE"}).then(t=>{if(!t.ok)throw new Error("Failed to delete");return t.json()}).then(()=>{alert("Evento excluído!"),et(),De("visualizacao")}).catch(t=>{console.error("Error deleting:",t),alert("Erro ao excluir: "+t.message)})}function xt(e){const t=document.getElementById(`filter-start-${e}`),n=document.getElementById(`filter-end-${e}`),o=document.getElementById(`filter-sub-topic-${e}`),a=t&&t.value?new Date(t.value).getTime():null,s=n&&n.value?new Date(n.value).getTime():null,i=o?o.value:"";J[e]={start:a,end:s,subTopic:i},tt()}function Et(e){const t=document.getElementById(`filter-start-${e}`),n=document.getElementById(`filter-end-${e}`),o=document.getElementById(`filter-sub-topic-${e}`);t&&(t.value=""),n&&(n.value=""),o&&(o.value=""),J[e]=null,tt()}function $t(e){const t=document.getElementById(`filters-panel-${e}`),n=document.getElementById(`btn-toggle-${e}`);t&&n&&(t.classList.toggle("hidden"),n.classList.toggle("active"))}function pt(e){const t=document.getElementById(e);t&&t.classList.toggle("active")}function tt(){const e=document.getElementById("timeline-tracks-container");if(!e)return;const t=Array.from(e.querySelectorAll(".timeline-container")).map(a=>a.dataset.topicId),n=j.map(a=>a.id);if(t.length!==n.length||!n.every(a=>t.includes(a))){e.innerHTML="";const a=window.auth&&window.auth.isAdmin(),s=a?'style="cursor: grab;"':"";j.forEach(i=>{const r=`
            ...
            `;e.insertAdjacentHTML("beforeend",r);const c=document.getElementById(`filter-sub-topic-${i.id}`);c&&Q[i.id]&&Q[i.id].forEach(u=>{const d=document.createElement("option");d.value=u.toLowerCase(),d.textContent=u,c.appendChild(d)})})}j.forEach(a=>{const s=document.getElementById(`track-${a.id}`),i=document.getElementById(`min-date-${a.id}`),r=document.getElementById(`max-date-${a.id}`);s&&(s.innerHTML=""),i&&(i.textContent=""),r&&(r.textContent="")}),le.length!==0&&j.forEach(a=>{const s=a.id,i=le.filter(y=>Ie(y.topico)===s);let r=i;J[s]&&J[s].subTopic&&(r=i.filter(y=>(y.sub_topico?y.sub_topico.toLowerCase():"")===J[s].subTopic.toLowerCase()));const c=J[s]&&J[s].start?J[s].start:new Date("2026-01-01T00:00:00").getTime(),u=J[s]&&J[s].end?J[s].end:Date.now();Tt(s,r,c,u);const d=c,m=u,p=m-d,f=document.getElementById(`min-date-${s}`),g=document.getElementById(`max-date-${s}`);f&&(f.textContent=new Date(d).toLocaleDateString()+" "+new Date(d).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})),g&&(g.textContent=new Date(m).toLocaleDateString()+" "+new Date(m).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}));const h=document.getElementById(`track-${s}`);h&&r.forEach(y=>{const I=new Date(y.inicio).getTime(),$=y.fim?new Date(y.fim).getTime():Date.now();if($<d||I>m)return;const S=Math.max(I,d),q=Math.min($,m),V=(S-d)/p*100,b=(q-S)/p*100;if(b<=0)return;const k=document.createElement("div");k.className="timeline-bar",k.style.left=`${V}%`,k.style.width=`${b}%`,k.style.color=y.cor&&y.cor!=="#000000"?y.cor:We[s]||"#6b7280";const v=document.createElement("div");v.className="timeline-bar-visual",k.appendChild(v);const A=document.createElement("div");A.className="timeline-identifier-point";const z=new Date(y.inicio).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),U=y.fim?new Date(y.fim).toLocaleString([],{dateStyle:"short",timeStyle:"short"}):"Em andamento",G=a.name,D=y.sub_topico?y.sub_topico.charAt(0).toUpperCase()+y.sub_topico.slice(1):"-";A.setAttribute("data-tooltip",`Tópico: ${G}
Eventos: ${D}
Início: ${z} - Fim: ${U}
Descrição: ${y.descricao||"-"}`),!y.fim&&A.classList.add("pulsing"),window.auth&&window.auth.isAdmin()?(A.style.cursor="pointer",A.onclick=de=>{de.stopPropagation(),Ze(y.id)}):A.style.cursor="default",k.appendChild(A),h.appendChild(k)})})}function Ie(e){return e?e.toLowerCase().trim():""}function Tt(e,t,n,o){const a=document.getElementById(`sla-${e}`);if(!a)return;const s=o-n;if(s<=0){a.textContent="N/A";return}const r=t.filter(p=>{const f=new Date(p.inicio).getTime();return(p.fim?new Date(p.fim).getTime():Date.now())>n&&f<o}).map(p=>({start:Math.max(new Date(p.inicio).getTime(),n),end:Math.min(p.fim?new Date(p.fim).getTime():Date.now(),o)}));r.sort((p,f)=>p.start-f.start);const c=[];if(r.length>0){let p=r[0];for(let f=1;f<r.length;f++){const g=r[f];g.start<p.end?p.end=Math.max(p.end,g.end):(c.push(p),p=g)}c.push(p)}let u=0;c.forEach(p=>{u+=p.end-p.start});const d=(s-u)/s*100;let m="#10b981";d<50?m="#ef4444":d<90&&(m="#f97316"),a.style.color=m,a.textContent=d.toFixed(4)+"%"}function gt(){const e=document.getElementById("attention-topics-container");if(!e)return;e.innerHTML="";const t=le.filter(n=>!n.fim);j.forEach(n=>{const o=n.id,a=t.filter(h=>Ie(h.topico)===o),s=document.createElement("div");s.className=a.length>0?"accordion-item active":"accordion-item",s.id=`attn-acc-${o}`;const i=document.createElement("div");i.className="accordion-header",i.onclick=()=>pt(`attn-acc-${o}`);const r=document.createElement("div");r.className="accordion-title-group";const c=document.createElement("div");c.className="topic-indicator",c.style.backgroundColor=n.color;const u=document.createElement("h3");u.textContent=n.name;const d=document.createElement("span");d.style.cssText="background: #f1f5f9; padding: 2px 8px; border-radius: 12px; font-size: 0.95rem; font-weight: 900; color: #0f172a; margin-left: 0.5rem; border: 1px solid #cbd5e1;",d.textContent=`${a.length}`,r.appendChild(c),r.appendChild(u),r.appendChild(d);const m=document.createElement("span");m.className="accordion-chevron",m.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>',i.appendChild(r),i.appendChild(m);const p=document.createElement("div");p.className="accordion-content";const f=document.createElement("div");f.className="accordion-body";const g=document.createElement("div");if(g.className="attention-carousel",a.length===0){const h=document.createElement("div");h.className="empty-state",h.textContent="Nenhum evento em andamento.",g.appendChild(h)}else a.forEach(h=>{const y=document.createElement("div");y.className="attention-card",y.style.borderLeftColor=h.cor&&h.cor!=="#000000"?h.cor:n.color;const I=document.createElement("h3");I.textContent=h.nome;const $=document.createElement("div");$.className="sub-topic",$.textContent=h.sub_topico||"-";const S=document.createElement("div");S.className="card-detail",S.innerHTML=`<strong>Início:</strong> ${new Date(h.inicio).toLocaleString()}`;const q=Date.now()-new Date(h.inicio).getTime(),V=document.createElement("div");V.className="card-duration",V.innerHTML=`<strong>Tempo:</strong> <span>${It(q)}</span>`;const b=document.createElement("div");b.className="card-description",b.textContent=h.descricao||"-",y.appendChild(I),y.appendChild($),y.appendChild(S),y.appendChild(V),y.appendChild(b),window.auth&&window.auth.isAdmin()?(y.style.cursor="pointer",y.onclick=()=>Ze(h.id)):y.style.cursor="default",g.appendChild(y)});f.appendChild(g),p.appendChild(f),s.appendChild(i),s.appendChild(p),e.appendChild(s)})}function It(e){if(e<0)return"0s";const t=Math.floor(e/1e3),n=Math.floor(t/60),o=Math.floor(n/60),a=Math.floor(o/24),s=[];return a>0&&s.push(`${a}d`),(o%24>0||a>0)&&s.push(`${o%24}h`),(n%60>0||o>0)&&s.push(`${n%60}m`),s.push(`${t%60}s`),s.join(" ")}function Lt(e){const t=document.getElementById("rep-filter-subtopic");if(!t)return;t.innerHTML='<option value="Todos">Todos</option>';const n=e?e.toLowerCase().trim():"";n&&Q[n]&&Q[n].forEach(o=>{const a=document.createElement("option");a.value=o.toLowerCase(),a.textContent=o,t.appendChild(a)})}function Te(){let e=le;const t=document.getElementById("rep-filter-start")?.value,n=document.getElementById("rep-filter-end")?.value,o=document.getElementById("rep-filter-topic")?.value,a=document.getElementById("rep-filter-subtopic")?.value;if(t){const b=new Date(t+"T00:00:00").getTime();e=e.filter(k=>new Date(k.inicio).getTime()>=b)}if(n){const b=new Date(n+"T23:59:59").getTime();e=e.filter(k=>new Date(k.inicio).getTime()<=b)}o&&o!=="Todos"&&(e=e.filter(b=>Ie(b.topico)===o.toLowerCase())),a&&a!=="Todos"&&(e=e.filter(b=>b.sub_topico&&b.sub_topico.toLowerCase()===a.toLowerCase()));const s=document.getElementById("rep-kpi-total"),i=document.getElementById("rep-kpi-active"),r=document.getElementById("rep-kpi-avg-time");s&&(s.textContent=e.length);const c=e.filter(b=>b.em_ocorrencia==1||b.em_ocorrencia==="true"||!b.fim);i&&(i.textContent=c.length);const u=e.filter(b=>b.fim);let d="0h 0m";if(u.length>0){const k=u.reduce((U,G)=>U+(new Date(G.fim).getTime()-new Date(G.inicio).getTime()),0)/u.length,v=Math.floor(k/6e4),A=Math.floor(v/60),z=v%60;d=`${A}h ${z}m`}if(r&&(r.textContent=d),!window.Chart){console.warn("Chart.js is not loaded.");return}const m=j,p=t?new Date(t+"T00:00:00").getTime():new Date(new Date().getFullYear()+"-01-01T00:00:00").getTime(),f=n?new Date(n+"T23:59:59").getTime():Date.now(),g=m.map(b=>b.name),h=m.map(b=>{const k=b.id,v=le.filter(B=>Ie(B.topico)===k),A=f-p;if(A<=0)return 100;const U=v.filter(B=>{const W=new Date(B.inicio).getTime();return(B.fim?new Date(B.fim).getTime():Date.now())>p&&W<f}).map(B=>({start:Math.max(new Date(B.inicio).getTime(),p),end:Math.min(B.fim?new Date(B.fim).getTime():Date.now(),f)}));U.sort((B,W)=>B.start-W.start);const G=[];if(U.length>0){let B=U[0];for(let W=1;W<U.length;W++){const oe=U[W];oe.start<B.end?B.end=Math.max(B.end,oe.end):(G.push(B),B=oe)}G.push(B)}const Y=(B=>{let W=0;return B.forEach(oe=>{W+=oe.end-oe.start}),W})(G),de=(A-Y)/A*100;return parseFloat(de.toFixed(4))}),y=m.map(b=>b.color||"#6b7280"),I=document.getElementById("chart-rep-sla");I&&(Re&&Re.destroy(),Re=new window.Chart(I,{type:"bar",data:{labels:g,datasets:[{label:"Disponibilidade %",data:h,backgroundColor:y,borderRadius:6}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{y:{min:Math.max(0,Math.min(...h)-5),max:100,ticks:{callback:b=>b+"%"}}}}}));const $={};e.forEach(b=>{const k=b.sub_topico?b.sub_topico.charAt(0).toUpperCase()+b.sub_topico.slice(1).toLowerCase():"Não especificado";$[k]=($[k]||0)+1});const S=Object.keys($),q=Object.values($),V=document.getElementById("chart-rep-qty");V&&(Ne&&Ne.destroy(),Ne=new window.Chart(V,{type:"doughnut",data:{labels:S.length>0?S:["Nenhum evento"],datasets:[{data:q.length>0?q:[0],backgroundColor:["#3b82f6","#10b981","#f59e0b","#8b5cf6","#ec4899","#6366f1","#14b8a6","#f43f5e","#a855f7","#06b6d4"]}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{position:"right",labels:{boxWidth:12}}}}}))}function Ct(e){e.preventDefault();const t=document.getElementById("topic-id"),n=document.getElementById("topic-name"),o=document.getElementById("topic-color");if(!t||!n||!o)return;const a={id:t.value.trim().toLowerCase(),name:n.value.trim(),color:o.value};if(!a.id){alert("Por favor, defina um ID para o tópico.");return}fetch("/api/timeline/config/topics",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)}).then(s=>{if(!s.ok)throw new Error("Erro ao salvar tópico");return s.json()}).then(()=>{alert("Tópico salvo com sucesso!"),t.value="",n.value="",o.value="#3b82f6",ce().then(()=>{Z()})}).catch(s=>{console.error(s),alert("Erro: "+s.message)})}function Dt(e){e.preventDefault();const t=document.getElementById("subtopic-topic-id"),n=document.getElementById("subtopic-name");if(!t||!n)return;const o={topic_id:t.value,name:n.value.trim()};if(!o.topic_id||!o.name){alert("Preencha todos os campos do evento.");return}fetch("/api/timeline/config/subtopics",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)}).then(a=>{if(!a.ok)throw new Error("Erro ao adicionar evento");return a.json()}).then(()=>{alert("Evento adicionado!"),n.value="",ce()}).catch(a=>{console.error(a),alert("Erro: "+a.message)})}function kt(e){confirm("Excluir este tópico também removerá todos os seus eventos associados. Deseja continuar?")&&fetch(`/api/timeline/config/topics/${e}`,{method:"DELETE"}).then(t=>{if(!t.ok)throw new Error("Erro ao excluir tópico");return t.json()}).then(()=>{alert("Tópico excluído!"),ce().then(()=>{Z()})}).catch(t=>{console.error(t),alert("Erro: "+t.message)})}function Bt(e){confirm("Deseja realmente excluir este evento?")&&fetch(`/api/timeline/config/subtopics/${e}`,{method:"DELETE"}).then(t=>{if(!t.ok)throw new Error("Erro ao excluir evento");return t.json()}).then(()=>{alert("Evento excluído!"),ce()}).catch(t=>{console.error(t),alert("Erro: "+t.message)})}function ft(){const e=document.getElementById("config-topics-list");e&&(e.innerHTML="",j.length===0?e.innerHTML='<div style="color: var(--text-muted); font-size: 0.9rem;">Nenhum tópico cadastrado.</div>':j.forEach(n=>{const o=document.createElement("div");o.style.cssText="display: flex; align-items: center; justify-content: space-between; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 8px; border: 1px solid var(--glass-border); margin-bottom: 6px;",o.innerHTML=`
                ...
                `,e.appendChild(o)}));const t=document.getElementById("config-subtopics-list");t&&(t.innerHTML="",Pe.length===0?t.innerHTML='<div style="color: var(--text-muted); font-size: 0.9rem;">Nenhum evento cadastrado.</div>':Pe.forEach(n=>{const o=j.find(r=>r.id===n.topic_id),a=o?o.name:n.topic_id,s=o?o.color:"#6b7280",i=document.createElement("div");i.style.cssText="display: flex; align-items: center; justify-content: space-between; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 8px; border: 1px solid var(--glass-border); margin-bottom: 6px;",i.innerHTML=`
                    <div>
                ...
                `,t.appendChild(i)}))}function St(e,t){e.currentTarget.classList.add("dragging"),e.dataTransfer.effectAllowed="move"}function Mt(e){e.preventDefault();const t=document.querySelector(".timeline-container.dragging");if(!t)return;const n=document.getElementById("timeline-tracks-container");if(!n)return;const a=[...n.querySelectorAll(".timeline-container:not(.dragging)")].find(s=>{const i=s.getBoundingClientRect();return e.clientY<=i.top+i.height/2});a?n.insertBefore(t,a):n.appendChild(t)}function At(e){const t=document.querySelector(".timeline-container.dragging");t&&t.classList.remove("dragging"),document.querySelectorAll(".timeline-container").forEach(a=>{a.setAttribute("draggable","false")});const n=document.getElementById("timeline-tracks-container");if(!n)return;const o=Array.from(n.querySelectorAll(".timeline-container")).map(a=>a.dataset.topicId);Ht(o)}function Ht(e){fetch("/api/timeline/config/topics/reorder",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({order:e})}).then(t=>{if(!t.ok)throw new Error("Erro ao salvar nova ordenação");return t.json()}).then(()=>{console.log("Ordem dos tópicos atualizada com sucesso."),ce().then(()=>{Z()})}).catch(t=>{console.error(t),alert("Erro ao salvar ordenação: "+t.message)})}let pe=[],ze=[],je=[],qe=[],L="extensions",M=1,ge=100,Ue=[];const $e={setActiveTab(e){L=e,M=1;const t=document.getElementById("telephony-search");t&&(t.value="",e==="extensions"?t.placeholder="Pesquisar ramais por número, nome ou usuário...":e==="queues"?t.placeholder="Pesquisar filas por número ou nome...":e==="blf"?t.placeholder="Pesquisar BLF por nome...":e==="users"&&(t.placeholder="Pesquisar usuários por nome ou perfil...")),document.querySelectorAll(".telephony-tabs-nav .acc-tab-btn").forEach(s=>{s.id===`tab-telephony-${e}`?s.classList.add("active"):s.classList.remove("active")}),document.querySelectorAll(".telephony-tab-content").forEach(s=>{s.id===`telephony-view-${e==="users"?"users":e==="queues"?"queues":e}`?s.classList.remove("hidden"):s.classList.add("hidden")});const a=this.getActiveDataList();this.render(a)},getActiveDataList(){return L==="extensions"?pe:L==="queues"?ze:L==="blf"?je:L==="users"?qe:[]},async fetch(){const e=this.getActiveTableBody();e&&(e.innerHTML='<tr><td colspan="10" style="text-align: center; padding: 2rem; color: var(--text-muted);">Carregando dados...</td></tr>');try{if(M=1,L==="extensions")pe=await E.get("/telephony/extensions"),this.render(pe);else if(L==="queues")ze=await E.get("/telephony/queues"),this.render(ze);else if(L==="blf"){if(pe.length===0)try{pe=await E.get("/telephony/extensions")}catch(t){console.warn("Could not pre-fetch extensions for BLF mapping:",t)}je=await E.get("/telephony/blfs"),this.render(je)}else L==="users"&&(qe=await E.get("/telephony/users"),this.render(qe))}catch(t){console.error(`Error fetching telephony ${L}:`,t),e&&(e.innerHTML=`<tr><td colspan="10" style="text-align: center; padding: 2rem; color: #ef4444;">Erro ao carregar dados: ${t.message||"Erro de rede"}</td></tr>`)}},getActiveTableBody(){return L==="extensions"?document.getElementById("telephony-table-body"):L==="queues"?document.getElementById("telephony-queues-table-body"):L==="blf"?document.getElementById("telephony-blf-table-body"):L==="users"?document.getElementById("telephony-users-table-body"):null},render(e){const t=this.getActiveTableBody();if(!t)return;Ue=e;const n=e.length,o=Math.ceil(n/ge);M>o&&(M=Math.max(1,o)),M<1&&(M=1);const a=(M-1)*ge,s=e.slice(a,a+ge);if(s.length===0){const i=L==="extensions"?7:L==="queues"?6:L==="blf"?4:5;t.innerHTML=`
                <tr>
        ...
        `;let s=0;for(let c=1;c<=t;c++)(c===1||c===t||c>=M-1&&c<=M+1)&&(s&&c-s>1&&(a+='<span style="color: var(--text-muted); padding: 0 4px;">...</span>'),a+=`
        ...
        `;const i=(M-1)*ge+1,r=Math.min(M*ge,n);a+=`
            <span class="pagination-info">
                Exibindo ${i}-${r} de ${n}
            </span>
        `,o.innerHTML=a}};let Se=[],Ge="",Ye="",Me=null,Ae="alerts",X=[],T=null;const nt={init(){try{const m=localStorage.getItem("monitoring_disabled_services");X=m?JSON.parse(m):[]}catch(m){console.error("Erro ao ler localStorage:",m),X=[]}const e=document.getElementById("tab-monitoring-alerts");e&&e.addEventListener("click",()=>this.setActiveTab("alerts"));const t=document.getElementById("tab-monitoring-disabled");t&&t.addEventListener("click",()=>this.setActiveTab("disabled"));const n=document.getElementById("tab-monitoring-events");n&&n.addEventListener("click",()=>this.setActiveTab("events"));const o=document.getElementById("tab-monitoring-gnew");o&&o.addEventListener("click",()=>this.setActiveTab("gnew"));const a=document.getElementById("monitoring-search-input");a&&a.addEventListener("input",m=>{Ge=m.target.value.toLowerCase(),this.render()});const s=document.getElementById("monitoring-events-search-input");s&&s.addEventListener("input",m=>{Ye=m.target.value.toLowerCase(),this.render()});const i=document.getElementById("btn-refresh-monitoring");i&&i.addEventListener("click",()=>{this.fetch()});const r=document.getElementById("monitoring-auto-refresh");r&&(r.addEventListener("change",m=>{this.toggleAutoRefresh(m.target.checked)}),this.toggleAutoRefresh(r.checked));const c=document.getElementById("gnew-disk-accordion-header");c&&c.addEventListener("click",()=>{const m=document.getElementById("gnew-disk-accordion-content"),p=document.getElementById("gnew-disk-chevron");m&&p&&(m.style.maxHeight==="0px"?(m.style.maxHeight="1000px",p.style.transform="rotate(0deg)"):(m.style.maxHeight="0px",p.style.transform="rotate(-90deg)"))});const u=document.getElementById("btn-refresh-gnew-disk");u&&u.addEventListener("click",m=>{m.stopPropagation(),this.fetchDiagnostics()});const d=document.getElementById("btn-refresh-gnew-services");d&&d.addEventListener("click",async()=>{const m=d,p=m.querySelector("svg");if(!m.disabled){m.disabled=!0,m.style.opacity="0.6",m.style.cursor="not-allowed",p&&(p.style.animation="spin 0.8s linear infinite");try{await this.fetchDiagnostics()}finally{m.disabled=!1,m.style.opacity="",m.style.cursor="pointer",p&&(p.style.animation="")}}}),window.monitoringHandler=this},setActiveTab(e){Ae=e;const t=document.getElementById("tab-monitoring-alerts"),n=document.getElementById("tab-monitoring-disabled"),o=document.getElementById("tab-monitoring-events"),a=document.getElementById("tab-monitoring-gnew");t&&n&&o&&(t.classList.toggle("active",e==="alerts"),n.classList.toggle("active",e==="disabled"),o.classList.toggle("active",e==="events"),a&&a.classList.toggle("active",e==="gnew"));const s=document.getElementById("monitoring-tab-content-alerts"),i=document.getElementById("monitoring-tab-content-disabled"),r=document.getElementById("monitoring-tab-content-events"),c=document.getElementById("monitoring-tab-content-gnew");s&&i&&r&&(s.classList.toggle("hidden",e!=="alerts"),s.classList.toggle("active",e==="alerts"),i.classList.toggle("hidden",e!=="disabled"),i.classList.toggle("active",e==="disabled"),r.classList.toggle("hidden",e!=="events"),r.classList.toggle("active",e==="events"),c&&(c.classList.toggle("hidden",e!=="gnew"),c.classList.toggle("active",e==="gnew"))),e==="gnew"?this.fetchDiagnostics():this.render()},toggleService(e,t){t?X=X.filter(n=>n!==e):X.includes(e)||X.push(e),localStorage.setItem("monitoring_disabled_services",JSON.stringify(X)),this.render()},toggleAutoRefresh(e){Me&&(clearInterval(Me),Me=null),e&&(Me=setInterval(()=>{const t=document.querySelector(".nav-btn.active");t&&t.dataset.section==="monitoring"&&this.fetch()},3e4))},async fetch(){if(Ae==="gnew"){await this.fetchDiagnostics();return}try{const e=await E.get("/monitoring/notifications");let t=[],n=!1;Array.isArray(e)?(t=e,this.updateApiStatus(!0,"API Online (OLIJUS)")):e&&Array.isArray(e.notifications)?(t=e.notifications,n=e.status==="offline",this.updateApiStatus(!n,n?"API Offline (Contingência)":"API Online (OLIJUS)")):e&&Array.isArray(e.data)?(t=e.data,this.updateApiStatus(!0,"API Online (OLIJUS)")):this.updateApiStatus(!1,"Resposta Inválida");const o=new Date;o.setDate(o.getDate()-30),Se=t.map(a=>this.parseNotification(a)).filter(a=>new Date(a.created_at)>=o),this.render()}catch(e){console.error("Erro ao buscar notificações de monitoramento:",e),this.updateApiStatus(!1,"Erro de Conexão");const t=document.getElementById("monitoring-alerts-grid");t&&Se.length===0&&(t.innerHTML=`
...
`):(i=r[0],s.length>45&&(s=e.type==="new_device"?"Novo Dispositivo":"Alerta de Monitoramento",i=r[0])))}return{id:e.id||Math.random().toString(),title:s,description:i,severity:t,source:"OLIJUS",ip:o,created_at:e.timestamp||e.created_at||new Date().toISOString()}},updateApiStatus(e,t){const n=document.getElementById("monitoring-api-status");if(n){n.className=`api-status-badge ${e?"online":"offline"}`;const o=n.querySelector(".status-text");o&&(o.textContent=t)}},getProcessedServices(){const e={};return Se.forEach(t=>{let n="",o="";const a=(t.description||"").match(/O serviço ([^\(]+)\s*(?:\(([^)]+)\))?/i)||(t.title||"").match(/O serviço ([^\(]+)\s*(?:\(([^)]+)\))?/i),s=(t.description||"").match(/dispositivo ([^\(]+)\s*(?:\(([^)]+)\))?/i);if(a?(n=a[1].trim(),o=a[2]?a[2].trim():""):s?(n=s[1].trim(),o=s[2]?s[2].trim():""):n=t.title,n=n.replace(/[🚀🛰️🛸🌟🛸]/g,"").trim(),!n)return;const i=n.toLowerCase(),r=t.severity==="critical"||t.severity==="warning";e[i]?(new Date(t.created_at)>new Date(e[i].lastUpdated)&&(e[i].status=r?"offline":"online",e[i].lastUpdated=t.created_at),o&&!e[i].ip&&(e[i].ip=o)):e[i]={name:n,ip:o,status:r?"offline":"online",lastUpdated:t.created_at}}),Object.values(e)},render(){const e=this.getProcessedServices();let t=e.filter(a=>!X.includes(a.name));const n=e.filter(a=>X.includes(a.name)),o=JSON.parse(localStorage.getItem("monitoring_services_order")||"[]");o.length>0&&t.sort((a,s)=>{let i=o.indexOf(a.name),r=o.indexOf(s.name);return i===-1&&(i=999),r===-1&&(r=999),i-r}),this.updateKPIs(t),Ae==="alerts"?this.renderActiveServices(t):Ae==="disabled"?this.renderDisabledServices(n):this.renderEvents()},renderActiveServices(e){const t=document.getElementById("monitoring-alerts-grid");if(!t)return;const n=e.filter(o=>!Ge||o.name.toLowerCase().includes(Ge));if(n.length===0){t.innerHTML=`
            ...
            `).join("")}},renderEvents(){const e=document.getElementById("monitoring-events-grid");if(!e)return;const t=Se.filter(n=>!Ye||(n.title||"").toLowerCase().includes(Ye));if(t.length===0){e.innerHTML=`
                <div style="grid-column: 1/-1; text-align: center; padding: 4rem; color: var(--text-muted);">
            ...
            `}).join("")},updateKPIs(e){let t=e.length,n=e.filter(r=>r.status==="offline").length,o=e.filter(r=>r.status==="online").length;const a=document.getElementById("monitor-kpi-total"),s=document.getElementById("monitor-kpi-warning"),i=document.getElementById("monitor-kpi-info");a&&(a.textContent=t),s&&(s.textContent=n),i&&(i.textContent=o)},async fetchDiagnostics(){try{const e=await E.get("/monitoring/diagnostico?t="+Date.now()),t=e&&e.status==="online";if(this.updateGnewApiStatus(t,t?"Gnew Online":"Gnew Offline (Contingência)",e?e.message:""),e&&e.data)T=e.data,this.renderGnewDiagnostics();else throw new Error("Dados inválidos na resposta da API.")}catch(e){console.error("Erro ao buscar diagnósticos da Gnew:",e),this.updateGnewApiStatus(!1,"Erro de Conexão",e.message)}},updateGnewApiStatus(e,t,n){const o=document.getElementById("gnew-api-status-badge"),a=document.getElementById("gnew-api-message");if(o){o.className=`api-status-badge ${e?"online":"offline"}`,o.style.background=e?"rgba(16, 185, 129, 0.1)":"rgba(239, 68, 68, 0.1)",o.style.color=e?"#6ee7b7":"#fca5a5",o.style.borderColor=e?"#10b981":"#ef4444";const s=o.querySelector(".status-text");s&&(s.textContent=t)}a&&n&&(a.textContent=n)},parseMemoryOutput(e){try{const n=e.split(`
`).find(o=>o.trim().startsWith("Mem:"));if(n){const o=n.trim().split(/\s+/);if(o.length>=3){const a=o[1],s=o[2],i=u=>{const d=parseFloat(u);return u.toLowerCase().includes("g")?d*1024:u.toLowerCase().includes("m")?d:u.toLowerCase().includes("k")?d/1024:d},r=i(a),c=i(s);if(!isNaN(r)&&!isNaN(c)&&r>0)return{percentage:Math.round(c/r*100),detail:`${s} em uso de ${a} total`}}}}catch(t){console.warn("Erro ao fazer parse da memória:",t)}return{percentage:0,detail:"Erro no parse"}},parseDiskOutput(e){try{const n=e.split(`
`).find(o=>o.trim().endsWith(" /"));if(n){const o=n.trim().split(/\s+/);if(o.length>=5){const a=o[1],s=o[2],i=o[4].replace("%",""),r=parseInt(i,10);if(!isNaN(r))return{percentage:r,detail:`${s} em uso de ${a} (Montagem em /)`}}}}catch(t){console.warn("Erro ao fazer parse do disco:",t)}return{percentage:0,detail:"Erro no parse"}},renderGnewDiagnostics(){if(!T)return;if(T.memoria){let n={percentage:0,detail:"Dados de memória indisponíveis"};if(T.memoria.output)n=this.parseMemoryOutput(T.memoria.output);else if(typeof T.memoria.percent<"u"){const i=(T.memoria.total_mb/1024).toFixed(1),r=(T.memoria.used_mb/1024).toFixed(1);n={percentage:Math.round(T.memoria.percent),detail:`${r}GB em uso de ${i}GB total`}}const o=document.getElementById("gnew-kpi-mem-text"),a=document.getElementById("gnew-kpi-mem-bar"),s=document.getElementById("gnew-kpi-mem-detail");o&&(o.textContent=`${n.percentage}%`),a&&(a.style.width=`${n.percentage}%`),s&&(s.textContent=n.detail)}if(T.disco){let n={percentage:0,detail:"Dados de disco indisponíveis"};if(T.disco.output)n=this.parseDiskOutput(T.disco.output);else if(Array.isArray(T.disco)){const i=T.disco.find(r=>r.mountpoint==="/");i&&(n={percentage:Math.round(i.percent),detail:`${i.used_gb.toFixed(1)}GB em uso de ${i.total_gb.toFixed(1)}GB (Montagem em /)`})}const o=document.getElementById("gnew-kpi-disk-text"),a=document.getElementById("gnew-kpi-disk-bar"),s=document.getElementById("gnew-kpi-disk-detail");o&&(o.textContent=`${n.percentage}%`),a&&(a.style.width=`${n.percentage}%`),s&&(s.textContent=n.detail)}const e=document.getElementById("gnew-disk-table-body");if(e){let n=[];if(T.disco)if(T.disco.output)try{const o=T.disco.output.trim().split(`
`);for(let a=1;a<o.length;a++){const s=o[a].trim().split(/\s+/);s.length>=6&&n.push({mountpoint:s[5],total:s[1],used:s[2],free:s[3],percent:parseInt(s[4].replace("%",""),10)||0})}}catch(o){console.warn("Erro ao fazer parse da tabela de disco offline:",o)}else Array.isArray(T.disco)&&(n=T.disco.map(o=>({mountpoint:o.mountpoint,total:typeof o.total_gb=="number"?`${o.total_gb.toFixed(2)} GB`:o.total_gb||"0 GB",used:typeof o.used_gb=="number"?`${o.used_gb.toFixed(2)} GB`:o.used_gb||"0 GB",free:typeof o.free_gb=="number"?`${o.free_gb.toFixed(2)} GB`:o.free_gb||"0 GB",percent:typeof o.percent=="number"?Math.round(o.percent):parseInt(o.percent,10)||0})));n.length>0?e.innerHTML=n.map(o=>{const a=o.percent;return`
                ...
                `}if(T.servicos&&T.servicos.timestamp)try{const o=new Date(T.servicos.timestamp).toLocaleString("pt-BR"),a=document.getElementById("gnew-services-timestamp");a&&(a.textContent=`Última verificação: ${o}`)}catch(n){console.warn("Erro ao formatar timestamp dos serviços:",n)}const t=document.getElementById("gnew-services-list");if(t){let n=[];T.servicos&&Array.isArray(T.servicos.servicos)&&(n=T.servicos.servicos),n.length>0?(t.innerHTML=n.map(a=>{const s=a.status==="active"||a.status_label==="ativo",i=s?"rgba(16, 185, 129, 0.1)":"rgba(239, 68, 68, 0.1)",r=s?"#6ee7b7":"#fca5a5",c=s?"#10b981":"#ef4444",u=s?"#10b981":"#ef4444";return`
                ...
                `}if(T.ipExterno){const n=document.getElementById("gnew-kpi-ip-text");n&&(n.textContent=T.ipExterno.ip||"Não detectado")}}};function Pt(e,t,n){return[...e.querySelectorAll('.notification-card[draggable="true"]:not(.dragging)')].reduce((a,s)=>{const i=s.getBoundingClientRect(),r=i.left+i.width/2,c=i.top+i.height/2,u=Math.pow(n-r,2)+Math.pow(t-c,2);return u<a.distance?{distance:u,element:s}:a},{distance:Number.POSITIVE_INFINITY}).element}let se="list";document.addEventListener("DOMContentLoaded",async()=>{console.log("%c 🚀 SISTEMA TI: INICIALIZANDO (MODULAR)... ","background: #4f46e5; color: white; font-weight: bold;"),window.auth=F,_t(),Ft(),Vt(),ut.init(),nt.init(),F.init()?(console.log("Sessão restaurada:",F.getUser().email),yt()):ht()});let Qe,re,Le,Ce;function _t(){Qe=document.querySelectorAll(".nav-btn"),re=document.getElementById("btn-new-item"),Le=document.getElementById("login-section"),Ce=document.getElementById("app-container")}function ht(){Le&&Le.classList.remove("hidden"),Ce&&Ce.classList.add("hidden"),document.body.style.overflow="hidden"}function Ft(){const e=new Date().getFullYear();[document.getElementById("filter-cal-year")].forEach(n=>{if(n&&n.options.length<=1)for(let o=e-5;o<=e+5;o++){const a=document.createElement("option");a.value=o,a.textContent=o,o===e&&(a.selected=!0),n.appendChild(a)}})}function yt(){if(Le&&Le.classList.add("hidden"),Ce&&Ce.classList.remove("hidden"),document.body.style.overflow="",se="list",He(),R.fetch(),te.fetch(),Je.fetch(),_.fetch(),window.auth){const e=document.getElementById("timeline-tab-anexo");e&&(window.auth.isAdmin()?e.classList.remove("role-hidden"):e.classList.add("role-hidden"));const t=document.getElementById("timeline-tab-config");t&&(window.auth.isAdmin()?t.classList.remove("role-hidden"):t.classList.add("role-hidden"))}}function He(){switch(["account-section","docs-section","list-section","detail-section","users-section","accounts-section","timeline-section","dedicated-account-page","telephony-section","monitoring-section"].forEach(e=>{l.hide(e)}),re&&re.classList.add("hidden"),ot.stop(),se){case"account":case"profile":l.show("account-section"),l.setText("section-title","Minha Conta"),setTimeout(()=>ot.start(),100);break;case"list":l.show("list-section"),l.setText("section-title","Listagem Geral"),F.isAdmin()&&re&&re.classList.remove("hidden");break;case"docs":l.show("docs-section"),l.setText("section-title","Documentação");break;case"detail":l.show("detail-section"),l.setText("section-title","Procedimento");break;case"users":l.show("users-section"),l.setText("section-title","Gestão de Usuários");break;case"accounts":l.show("accounts-section"),l.setText("section-title","Gestão de Contas"),_.handleSearch();break;case"timeline":l.show("timeline-section"),l.setText("section-title","Timeline");break;case"telephony":l.show("telephony-section"),l.setText("section-title","Telefonia");break;case"monitoring":l.show("monitoring-section"),l.setText("section-title","Monitoramento"),nt.fetch();break}vt()}function vt(){const e=F.isAdmin();l.toggle("nav-users",!e),l.toggle("nav-accounts",!e),re&&re.classList.toggle("role-hidden",!e);const t=document.getElementById("btn-floating-edit");t&&t.classList.toggle("role-hidden",!e),document.querySelectorAll(".btn-actions-container").forEach(i=>{i.classList.toggle("role-hidden",!e)}),["th-proc-actions","th-user-actions","th-account-actions","th-doc-actions"].forEach(i=>{const r=document.getElementById(i);r&&r.classList.toggle("role-hidden",!e)});const n=document.getElementById("btn-new-user");n&&n.classList.toggle("role-hidden",!e);const o=document.getElementById("btn-new-account");o&&o.classList.toggle("role-hidden",!e);const a=document.getElementById("btn-new-doc");a&&a.classList.toggle("role-hidden",!e);const s=F.getUser();if(s){let i=s.name;(i.toLowerCase().startsWith("usuário ")||i.toLowerCase().startsWith("usuario "))&&(i=i.substring(8)),l.setText("profile-name-display",i),l.setText("profile-role-display",s.role);let r=i.substring(0,2).toUpperCase();const c=i.trim().split(/\s+/);c.length>1&&(r=(c[0][0]+c[c.length-1][0]).toUpperCase()),l.setText("profile-avatar-initials",r)}}function Vt(){const e=document.getElementById("sidebar"),t=document.getElementById("sidebar-toggle");t&&e&&t.addEventListener("click",()=>{e.classList.toggle("collapsed")}),Qe.forEach(i=>{i.addEventListener("click",()=>{if(Qe.forEach(r=>r.classList.remove("active")),i.classList.add("active"),se=i.dataset.section,He(),window.innerWidth<=768){e.classList.remove("open");const r=document.getElementById("sidebar-overlay");r&&r.classList.remove("active")}})}),window.addEventListener("SectionChange",i=>{se=i.detail.section,He()}),l.on("login-form","submit",async i=>{i.preventDefault();const r=document.getElementById("login-btn"),c=document.getElementById("login-error");r&&(r.disabled=!0);const u=await F.login(l.getValue("login-email"),l.getValue("login-password"));r&&(r.disabled=!1),u.success?yt():c&&(c.innerText=u.error,c.classList.remove("hidden"))}),l.on("btn-logout","click",()=>{const i=document.getElementById("auto-refresh-toggle");i&&i.checked&&(i.checked=!1,i.dispatchEvent(new Event("change"))),F.logout(),ht()}),document.querySelectorAll(".close-modal").forEach(i=>{i.addEventListener("click",()=>{const r=i.closest(".modal");r&&r.classList.add("hidden")})}),window.UsersHandler=Je,window.DocsHandler=te,window.ProceduresHandler=R,window.AccountsHandler=_,window.TelephonyHandler=$e,window.monitoringHandler=nt,["extensions","queues","blf","users"].forEach(i=>{l.on(`tab-telephony-${i}`,"click",()=>$e.setActiveTab(i))}),l.on("telephony-search","input",i=>$e.search(i.target.value.toLowerCase())),l.on("telephony-page-size","change",i=>$e.setPageSize(i.target.value)),l.on("telephony-reload-btn","click",()=>{const i=document.getElementById("telephony-search");i&&(i.value=""),$e.fetch()}),l.on("accounts-search","input",()=>_.handleSearch()),l.on("filter-status","change",()=>_.handleSearch()),l.on("filter-date-toggle","change",i=>{const r=document.getElementById("sidebar-mini-calendar-list");r&&(r.style.opacity=i.target.checked?"1":"0.4",r.style.pointerEvents=i.target.checked?"auto":"none"),_.handleSearch()}),l.on("filter-cal-month","change",()=>_.handleFilterChange(!0)),l.on("filter-cal-year","change",()=>_.handleFilterChange(!0)),["dash-filter-start","dash-filter-end","dash-filter-type","dash-filter-status","dash-filter-payment","dash-sort-empresas","dash-sort-categorias"].forEach(i=>{l.on(i,"change",()=>{se==="accounts"&&_.renderDashboard()})}),l.on("btn-dash-clear-dates","click",()=>{l.setValue("dash-filter-start",""),l.setValue("dash-filter-end",""),l.setValue("dash-filter-type","Todos"),l.setValue("dash-filter-status","Todos"),l.setValue("dash-filter-payment","Todos"),_.resetMultiselects(),l.setValue("dash-sort-empresas","desc"),l.setValue("dash-sort-categorias","desc"),se==="accounts"&&_.renderDashboard()}),l.on("user-form","submit",i=>Je.save(i)),l.on("doc-form","submit",i=>te.handleUpload(i)),l.on("account-form","submit",i=>_.save(i)),l.on("faq-form","submit",i=>R.saveMeta(i));const n=document.getElementById("proc-color-palette"),o=document.getElementById("proc-color");n&&o&&(n.addEventListener("click",i=>{const r=i.target.closest(".color-swatch");if(r)if(r.id==="color-custom-swatch")o.click();else{const c=r.dataset.color;c&&(o.value=c,n.querySelectorAll(".color-swatch").forEach(u=>u.classList.remove("active")),r.classList.add("active"))}}),o.addEventListener("input",i=>{const r=document.getElementById("color-custom-swatch");r&&(r.style.background=i.target.value,n.querySelectorAll(".color-swatch").forEach(c=>c.classList.remove("active")),r.classList.add("active"))})),l.on("btn-new-item","click",()=>{if(l.setText("modal-form-title","Novo Procedimento"),l.setValue("proc-id",""),l.setValue("proc-content","[]"),n){n.querySelectorAll(".color-swatch").forEach(r=>r.classList.remove("active"));const i=n.querySelector('[data-color="#4F46E5"]');i&&i.classList.add("active")}o&&(o.value="#4F46E5"),l.show("modal-form")}),l.on("btn-new-account","click",()=>_.openAccountModal()),l.on("btn-new-account-cal","click",()=>_.openAccountModal()),l.on("btn-new-user","click",()=>{document.getElementById("user-form").reset(),l.setValue("user-id-form",""),l.show("modal-user")}),l.on("list-search","input",i=>{R.search(i.target.value.toLowerCase())}),l.on("doc-search","input",i=>{te.search(i.target.value.toLowerCase())}),l.on("doc-dash-search","input",()=>{te.renderDashboard()}),l.on("doc-dash-filter-category","change",()=>{te.renderDashboard()}),l.on("doc-dash-filter-status","change",()=>{te.renderDashboard()}),l.on("btn-new-doc","click",()=>{l.show("modal-upload")}),["geral","contratos","termo-de-uso","dashboard"].forEach(i=>{l.on(`tab-doc-${i}`,"click",()=>{let r;i==="termo-de-uso"?r="Termo de Uso":i==="dashboard"?r="dashboard":r=i,te.setActiveTab(r)})}),l.on("doc-category","change",i=>{const r=i.target.value.toLowerCase(),c=document.getElementById("doc-dates-container");c&&(c.style.display=r==="contratos"||r==="termo de uso"?"grid":"none")}),l.on("doc-indefinite","change",i=>{const r=document.getElementById("doc-end-date");r&&(r.disabled=i.target.checked,i.target.checked&&(r.value=""))});const a=document.getElementById("drop-zone"),s=document.getElementById("doc-file");a&&s&&(a.addEventListener("click",i=>{i.target!==s&&s.click()}),s.addEventListener("click",i=>{i.stopPropagation()}),s.addEventListener("change",i=>{i.target.files.length>0&&l.setText("file-name-display",i.target.files[0].name)}),a.addEventListener("dragover",i=>{i.preventDefault(),a.classList.add("dragover")}),a.addEventListener("dragleave",()=>{a.classList.remove("dragover")}),a.addEventListener("drop",i=>{i.preventDefault(),a.classList.remove("dragover"),i.dataTransfer.files.length>0&&(s.files=i.dataTransfer.files,l.setText("file-name-display",i.dataTransfer.files[0].name))})),l.on("toggle-list","click",i=>{i.currentTarget.classList.add("active"),document.getElementById("toggle-cards").classList.remove("active"),R.setListingMode("list")}),l.on("toggle-cards","click",i=>{i.currentTarget.classList.add("active"),document.getElementById("toggle-list").classList.remove("active"),R.setListingMode("cards")}),["lista","calendario","dashboard","notificacoes"].forEach(i=>{l.on(`tab-acc-${i}`,"click",r=>{document.querySelectorAll(".acc-tab-btn").forEach(p=>p.classList.remove("active")),r.currentTarget.classList.add("active"),document.querySelectorAll(".acc-tab-content").forEach(p=>{p.classList.add("hidden"),p.classList.remove("active")});const c=document.getElementById("accounts-dashboard-view");c&&(c.classList.add("hidden"),c.classList.remove("active"));const u=i==="dashboard"?"accounts-dashboard-view":`acc-tab-content-${i}`,d=document.getElementById(u);d&&(d.classList.remove("hidden"),d.classList.add("active"));const m=document.getElementById("calendar-view-toggle-container");m&&(i==="calendario"?(m.classList.remove("hidden"),m.style.display="flex"):(m.classList.add("hidden"),m.style.display="none")),_.setAccountsViewMode(i==="calendario"?"calendar":i==="dashboard"?"dashboard":i==="notificacoes"?"notificacoes":"list")})}),["day","month","year"].forEach(i=>{l.on(`toggle-accounts-cal-${i}`,"click",r=>{document.querySelectorAll("#calendar-view-toggle-container .toggle-btn").forEach(c=>c.classList.remove("active")),r.currentTarget.classList.add("active"),["day","month","year"].forEach(c=>{document.getElementById(`cal-${c}-view-container`).classList.toggle("hidden-cal-view",c!==i)}),_.setCalendarSubView(i)})}),l.on("btn-prev-date-nav","click",()=>_.shiftCalendarDate(-1)),l.on("btn-next-date-nav","click",()=>_.shiftCalendarDate(1)),l.on("btn-back-to-accounts","click",()=>{l.hide("dedicated-account-page"),l.show("accounts-section"),vt()}),l.on("btn-back-to-list","click",()=>{const i=document.getElementById("procedure-edit-wrapper");i&&!i.classList.contains("hidden")?R.toggleEditMode(!1):(se="list",He())}),l.on("btn-floating-edit","click",()=>R.toggleEditMode(!0)),l.on("btn-cancel-edit","click",()=>R.toggleEditMode(!1)),l.on("btn-save-procedure","click",()=>R.handleSaveProcedure()),l.on("confirm-yes","click",()=>{l.hide("modal-confirm"),R.openDetail(R.getPendingProcId())}),l.on("confirm-no","click",()=>{l.hide("modal-confirm")}),l.on("procedure-search","input",i=>{R.filterProcedureContent(i.target.value)}),l.on("btn-add-block","click",()=>{const i=document.getElementById("section-title-input"),r=document.getElementById("section-type-input");i&&(i.value=""),r&&(r.value="TEXTO"),l.show("modal-add-section")}),l.on("btn-confirm-add-section","click",()=>{const i=l.getValue("section-title-input"),r=l.getValue("section-type-input");if(!i)return alert("Por favor, informe o título da seção.");R.addSection(i,r),l.hide("modal-add-section")})}
