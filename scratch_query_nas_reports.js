...
function fetchLansweeperCustomReport(url, cookies, reportName, queryParams) {
    return new Promise((resolve, reject) => {
        ...
        const options = {
            hostname: parsedUrl.hostname,
            port: parsedUrl.port || (parsedUrl.protocol === 'https:' ? 443 : 80),
            path: `/ReportJson.aspx?det=${reportName}&${queryParams}&top=5000&page=1&cache=0`,
            method: 'POST',
            agent: lansweeperAgent,
            headers: {
...
async function run() {
    try {
        ...
        const reports = [
            'web40repdisks',
            'web40repPartitions',
            'web50replinuxpartitionlessthen10free',
            'web40repSharedFolder'
        ];

        for (const name of reports) {
            console.log(`\nQuerying report "${name}"...`);
            try {
                const data = await fetchLansweeperCustomReport(env.LANSWEEPER_URL, cookies, name, '');
                if (data && data.AddedRows) {
                    console.log(`Report columns:`, data.Columns);
                    // Filter rows by AssetID
                    const nasRows = data.AddedRows.filter(row => {
                        return row.some(val => String(val) === '42');
                    });
                    console.log(`Found ${nasRows.length} rows matching AssetID 42.`);
                    if (nasRows.length > 0) {
                        console.log(`Matching rows:`, nasRows);
                    }
                }
            } catch (err) {
